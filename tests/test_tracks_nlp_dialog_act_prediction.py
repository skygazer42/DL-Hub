...
def test_dialog_act_prediction_batch_contract() -> None:
    ...
    assert tuple(batch["input_ids"].shape) == (8, 24)
    assert tuple(batch["attention_mask"].shape) == (8, 24)
    assert tuple(batch["labels"].shape) == (8,)
    assert batch["labels"].dtype == torch.long
    assert torch.all(batch["labels"] >= 0)
    assert torch.all(batch["labels"] < 6)
    ...
    try:
        exit_code = run_training(
            TrainConfig(
                ...
                max_train_batches=2,
                max_eval_batches=1,
                run_name="pytest_dialog_act_prediction_smoke",
                embed_dim=32,
                dropout=0.0,
            ),
            DataConfig(
