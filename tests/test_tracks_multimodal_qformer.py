...
def test_multimodal_qformer_batch_shapes() -> None:
    from tracks.multimodal.lesson_07_qformer_compact_bridge_vlm.data import (
        DataConfig,
        get_dataloaders,
    )
...
def test_multimodal_qformer_model_outputs() -> None:
    from tracks.multimodal.lesson_07_qformer_compact_bridge_vlm.data import (
        DataConfig,
        get_dataloaders,
    )
    from tracks.multimodal.lesson_07_qformer_compact_bridge_vlm.model import (
        QFormerModelConfig,
        CompactQFormerModel,
        qa_loss,
    )
    ...
    assert set(outputs) >= {"logits", "query_states"}
    assert outputs["logits"].shape == (8, 12, vocab.size)
    assert outputs["query_states"].shape == (8, 4, 48)
...
def test_multimodal_qformer_training_smoke() -> None:
    run_dir = (
        _repo_root()
        / "outputs"
        / "multimodal"
        / "lesson_07_qformer_compact_bridge_vlm"
        / "pytest_qformer_smoke"
    )
    if run_dir.exists():
    ...
    proc = subprocess.run(
        [
            sys.executable,
            "-m",
            "tracks.multimodal.lesson_07_qformer_compact_bridge_vlm.train",
            "--epochs",
            "1",
            "--num-samples",
            "64",
