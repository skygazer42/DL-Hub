...
def test_multimodal_multiscale_2dtan_batch_shapes() -> None:
    ...
    assert batch["map_labels_s1"].shape == (8, 8, 8)
    assert batch["map_mask_s1"].shape == (8, 8, 8)
    assert batch["map_labels_s2"].shape == (8, 4, 4)
    assert batch["map_mask_s2"].shape == (8, 4, 4)
    assert batch["map_labels_s3"].shape == (8, 2, 2)
    assert batch["map_mask_s3"].shape == (8, 2, 2)
...
def test_multimodal_multiscale_2dtan_model_outputs() -> None:
    ...
    assert outputs["score_map_s1"].shape == (8, 8, 8)
    assert outputs["score_map_s2"].shape == (8, 4, 4)
    assert outputs["score_map_s3"].shape == (8, 2, 2)
    assert outputs["fused_score_map"].shape == (8, 8, 8)
    assert outputs["pred_segment_s1"].shape == (8, 2)
    assert outputs["pred_segment_s2"].shape == (8, 2)
    assert outputs["pred_segment_s3"].shape == (8, 2)
    ...
    assert set(losses) >= {"loss", "fused_loss", "aux_loss_s1", "aux_loss_s2", "aux_loss_s3"}
    assert losses["loss"].ndim == 0
    assert torch.isfinite(losses["loss"])


def test_multimodal_multiscale_2dtan_training_smoke() -> None:
    run_dir = (
        _repo_root()
        / "outputs"
        / "multimodal"
