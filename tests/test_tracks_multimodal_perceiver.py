...
def test_multimodal_perceiver_batch_shapes() -> None:
    ...
    assert batch["images"].shape == (8, 5, 3, 16, 16)
    assert batch["question_ids"].shape == (8, 14)
    assert batch["input_ids"].shape == (8, 14)
    assert batch["labels"].shape == (8, 14)
    assert batch["attention_mask"].shape == (8, 14)
...
def test_multimodal_perceiver_model_outputs() -> None:
    ...
    assert set(outputs) >= {"logits", "resampled_tokens"}
    assert outputs["logits"].shape == (8, 14, vocab.size)
    assert outputs["resampled_tokens"].shape == (8, 6, 48)
...
def test_multimodal_perceiver_training_smoke() -> None:
    run_dir = (
        _repo_root()
        / "outputs"
        / "multimodal"
