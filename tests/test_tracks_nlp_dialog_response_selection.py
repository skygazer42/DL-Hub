...
def test_dialog_response_selection_batch_contract() -> None:
    ...
    assert tuple(batch["context_ids"].shape) == (6, 20)
    assert tuple(batch["context_attention_mask"].shape) == (6, 20)
    assert tuple(batch["candidate_ids"].shape) == (6, 4, 20)
    assert tuple(batch["candidate_attention_mask"].shape) == (6, 4, 20)
    assert tuple(batch["labels"].shape) == (6,)
    assert batch["labels"].dtype == torch.long
    assert torch.all(batch["labels"] >= 0)
    assert torch.all(batch["labels"] < 4)
    assert "user" in vocab.token_to_id
    assert "system" in vocab.token_to_id
    ...
    try:
        exit_code = run_training(
            TrainConfig(
                ...
                max_train_batches=2,
                max_eval_batches=1,
                run_name="pytest_dialog_response_selection_smoke",
                embed_dim=32,
                dropout=0.0,
            ),
            DataConfig(
