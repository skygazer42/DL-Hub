...
def test_llm_constraint_repair_prompting_batch_mask_and_loss_smoke() -> None:
    ...
    repair_positions = (inputs["input_ids"] == int(vocab.repair_token_id)).to(torch.int64).argmax(dim=1)
    for row_idx, repair_pos in enumerate(repair_positions.tolist()):
        assert torch.all(labels[row_idx, : int(repair_pos)] == int(vocab.ignore_index))
    ...
    try:
        exit_code = run_training(
            TrainConfig(
                ...
                max_train_batches=2,
                max_eval_batches=1,
                run_name="pytest_constraint_repair_prompting_smoke",
                generation_tokens=8,
                embed_dim=48,
                num_heads=4,
                num_layers=2,
