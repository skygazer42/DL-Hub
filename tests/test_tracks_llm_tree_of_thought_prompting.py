...
def test_llm_tree_of_thought_prompting_batch_mask_and_loss_smoke() -> None:
    ...
    assert tuple(inputs["input_ids"].shape) == (8, 30)
    assert tuple(inputs["attention_mask"].shape) == (8, 30)
    assert tuple(labels.shape) == (8, 30)

    choose_positions = (inputs["input_ids"] == int(vocab.choose_token_id)).to(torch.int64).argmax(dim=1)
    for row_idx, choose_pos in enumerate(choose_positions.tolist()):
        assert torch.all(labels[row_idx, : int(choose_pos)] == int(vocab.ignore_index))
    assert (labels != int(vocab.ignore_index)).any()

    model = TreeOfThoughtTransformerLM(
        ModelConfig(
    ...
    logits = model(inputs)
    assert tuple(logits.shape) == (8, 30, vocab.size)
    ...
    try:
        exit_code = run_training(
            TrainConfig(
                ...
                max_train_batches=2,
                max_eval_batches=1,
                run_name="pytest_tree_of_thought_prompting_smoke",
                generation_tokens=8,
                embed_dim=48,
                num_heads=4,
                num_layers=2,
