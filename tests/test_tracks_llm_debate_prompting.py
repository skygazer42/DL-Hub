...
def test_llm_debate_prompting_batch_mask_and_loss_smoke() -> None:
    ...
    judge_positions = (inputs["input_ids"] == int(vocab.judge_token_id)).to(torch.int64).argmax(dim=1)
    for row_idx, judge_pos in enumerate(judge_positions.tolist()):
    ...
    try:
        exit_code = run_training(
            TrainConfig(
                ...
                max_train_batches=2,
                max_eval_batches=1,
                run_name="pytest_debate_prompting_smoke",
                generation_tokens=8,
                embed_dim=48,
                num_heads=4,
                num_layers=2,
