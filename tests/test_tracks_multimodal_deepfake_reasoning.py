...
def test_multimodal_deepfake_reasoning_batch_shapes() -> None:
    ...
    assert len(batch["cue_text"]) == 8
    assert len(batch["authenticity_text"]) == 8
    assert "authentic" in vocab.token_to_id
    assert "spoof" in vocab.token_to_id
    assert "moire" in vocab.token_to_id


def test_multimodal_deepfake_reasoning_model_outputs() -> None:
    from tracks.multimodal.lesson_36_face_anti_spoof_vlm_reasoning.data import DataConfig, get_dataloaders
    ...
    assert 0.0 <= reasoning_accuracy(outputs["logits"], batch["labels"]) <= 1.0


def test_multimodal_deepfake_reasoning_training_smoke(tmp_path, monkeypatch: pytest.MonkeyPatch) -> None:
    from tracks.multimodal.lesson_36_face_anti_spoof_vlm_reasoning.data import DataConfig
    from tracks.multimodal.lesson_36_face_anti_spoof_vlm_reasoning.train import TrainConfig, run_training
    ...
    exit_code = run_training(
        TrainConfig(
            ...
            max_train_batches=2,
            max_eval_batches=1,
            run_name="pytest_deepfake_reasoning_smoke",
            hidden_dim=48,
            text_dim=32,
            vision_width=32,
        ),
    ...
    run_dir = (
        ...
        / "pytest_deepfake_reasoning_smoke"
    )
