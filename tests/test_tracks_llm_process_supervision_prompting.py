...
def test_llm_process_supervision_prompting_batch_mask_and_loss_smoke() -> None:
    ...
    process_positions = (inputs["input_ids"] == int(vocab.process_token_id)).to(torch.int64).argmax(dim=1)
    for row_idx, process_pos in enumerate(process_positions.tolist()):
        assert torch.all(labels[row_idx, : int(process_pos)] == int(vocab.ignore_index))
    ...
    try:
        exit_code = run_training(
            TrainConfig(
                ...
                max_train_batches=2,
                max_eval_batches=1,
                run_name="pytest_process_supervision_prompting_smoke",
                generation_tokens=8,
                embed_dim=48,
                num_heads=4,
                num_layers=2,
