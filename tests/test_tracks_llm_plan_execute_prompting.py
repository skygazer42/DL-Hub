...
def test_llm_plan_execute_prompting_batch_mask_and_loss_smoke() -> None:
    ...
    assert tuple(inputs["input_ids"].shape) == (8, 24)
    assert tuple(inputs["attention_mask"].shape) == (8, 24)
    assert tuple(labels.shape) == (8, 24)

    execute_positions = (inputs["input_ids"] == int(vocab.execute_token_id)).to(torch.int64).argmax(dim=1)
    for row_idx, execute_pos in enumerate(execute_positions.tolist()):
        assert torch.all(labels[row_idx, : int(execute_pos)] == int(vocab.ignore_index))
    assert (labels != int(vocab.ignore_index)).any()

    model = PlanExecuteTransformerLM(
        ModelConfig(
    ...
    logits = model(inputs)
    assert tuple(logits.shape) == (8, 24, vocab.size)
    ...
    try:
        exit_code = run_training(
            TrainConfig(
                ...
                max_train_batches=2,
                max_eval_batches=1,
                run_name="pytest_plan_execute_prompting_smoke",
                generation_tokens=8,
                embed_dim=48,
                num_heads=4,
                num_layers=2,
