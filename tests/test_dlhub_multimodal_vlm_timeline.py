...
def test_vlm_timeline_metadata_covers_20_families() -> None:
    ...
    assert mapping["simvlm"].year == 2021
    assert mapping["lit"].group == "dual_encoder"
    assert mapping["pali"].year == 2022
    assert mapping["qwen_vl"].group == "multimodal_llm"
    assert mapping["cogvlm"].year == 2023
...
def test_vlm_zoo_script_timeline() -> None:
    ...
    assert "simvlm [fusion_encoder_decoder]" in proc.stdout
    assert "qwen_vl [multimodal_llm]" in proc.stdout
