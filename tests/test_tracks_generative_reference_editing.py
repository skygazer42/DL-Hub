...
def test_compact_diffusion_reference_editing_data_and_model_contract() -> None:
    from tracks.generative.lesson_31_compact_diffusion_reference_editing.data import DataConfig, get_dataloaders
    from tracks.generative.lesson_31_compact_diffusion_reference_editing.model import (
        DiffusionSchedule,
        ModelConfig,
    ...
    source, reference, target = next(iter(train_loader))
    ...
    model = CompactReferenceEditingDiffusionModel(cfg)
...
def test_compact_diffusion_reference_editing_training_smoke(tmp_path, monkeypatch: pytest.MonkeyPatch) -> None:
    from tracks.generative.lesson_31_compact_diffusion_reference_editing.data import DataConfig
    from tracks.generative.lesson_31_compact_diffusion_reference_editing.model import DiffusionSchedule, ModelConfig
    from tracks.generative.lesson_31_compact_diffusion_reference_editing.train import TrainConfig, run_training
    ...
    exit_code = run_training(
        TrainConfig(
            ...
            max_train_batches=2,
            max_eval_batches=1,
            run_name="pytest_reference_editing_smoke",
            num_sample_steps=6,
        ),
    ...
    assert exit_code == 0
    run_dir = tmp_path / "generative" / "lesson_31_compact_diffusion_reference_editing" / "pytest_reference_editing_smoke"
    assert (run_dir / "config.json").is_file()
    assert (run_dir / "metrics.jsonl").is_file()
    assert (run_dir / "samples.pt").is_file()
    assert (run_dir / "reference_edit_triplets.pt").is_file()
