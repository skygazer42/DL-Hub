...
def test_llm_lesson_01_shapes_smoke() -> None:
    ...
    from tracks.llm.lesson_01_toy_causal_lm_transformer.model import CausalTransformerLM, ModelConfig

    train_loader, _, vocab = get_dataloaders(
        DataConfig(num_samples=64, batch_size=8, seq_length=16, base_vocab_size=32, val_fraction=0.2, seed=0, num_workers=0)
    )
    ...
    loss = torch.nn.CrossEntropyLoss(ignore_index=vocab.pad_id)(logits.reshape(-1, vocab.size), labels.reshape(-1))
