...
def test_dialog_transfer_prediction_batch_contract() -> None:
    from tracks.nlp.lesson_41_compact_dialog_transfer_prediction.data import DataConfig, get_dataloaders
    from tracks.nlp.lesson_41_compact_dialog_transfer_prediction.model import (
    ...
    assert "transfer" in vocab.token_to_id
    assert "specialist" in vocab.token_to_id
    assert "agent" in vocab.token_to_id

    model = DialogTransferClassifier(
        ModelConfig(
            vocab_size=vocab.size,
            pad_id=vocab.pad_id,
            num_transfer_levels=3,
            embed_dim=32,
            dropout=0.0,
        )
    )
    ...
    loss = dialog_transfer_loss(logits, batch["labels"])
    assert torch.isfinite(loss)
    accuracy = compute_accuracy(logits, batch["labels"])
    assert 0.0 <= accuracy <= 1.0


def test_dialog_transfer_prediction_training_smoke(tmp_path) -> None:
    from tracks.nlp.lesson_41_compact_dialog_transfer_prediction.data import DataConfig
    from tracks.nlp.lesson_41_compact_dialog_transfer_prediction.train import TrainConfig, run_training
    ...
    try:
        exit_code = run_training(
            TrainConfig(
                ...
                max_train_batches=2,
                max_eval_batches=1,
                run_name="pytest_dialog_transfer_prediction_smoke",
                embed_dim=32,
                dropout=0.0,
            ),
            DataConfig(
    ...
    run_dir = (
        ...
        / "pytest_dialog_transfer_prediction_smoke"
    )
