...
def test_few_shot_text_classification_episode_shapes() -> None:
    ...
    assert batch["support_input_ids"].shape == (3, 8, 10)
    assert batch["support_attention_mask"].shape == (3, 8, 10)
    assert batch["support_labels"].shape == (3, 8)
    assert batch["query_input_ids"].shape == (3, 12, 10)
    assert batch["query_attention_mask"].shape == (3, 12, 10)
    ...
    try:
        exit_code = run_training(
            TrainConfig(
                ...
                max_train_batches=2,
                max_eval_batches=1,
                run_name="pytest_few_shot_text_smoke",
                embed_dim=32,
                proj_dim=24,
                dropout=0.0,
            ),
