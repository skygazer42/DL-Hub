...
def test_toy_diffusion_deraining_data_and_model_contract() -> None:
    from tracks.generative.lesson_18_toy_diffusion_deraining.data import DataConfig, get_dataloaders
    from tracks.generative.lesson_18_toy_diffusion_deraining.model import (
        DiffusionSchedule,
        ModelConfig,
        ToyDerainingDiffusionModel,
        q_sample,
    )
    ...
    model = ToyDerainingDiffusionModel(cfg)
...
def test_toy_diffusion_deraining_training_and_dry_run(tmp_path, monkeypatch: pytest.MonkeyPatch) -> None:
    from tracks.generative.lesson_18_toy_diffusion_deraining.data import DataConfig
    from tracks.generative.lesson_18_toy_diffusion_deraining.model import DiffusionSchedule, ModelConfig
    from tracks.generative.lesson_18_toy_diffusion_deraining.train import TrainConfig, run_training
    ...
    assert exit_code == 0
    run_dir = tmp_path / "generative" / "lesson_18_toy_diffusion_deraining" / "pytest_deraining_smoke"
