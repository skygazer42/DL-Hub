...
def test_compact_diffusion_path_mask_editing_data_and_model_contract() -> None:
    ...
    source, path_mask, target = next(iter(train_loader))
    assert tuple(source.shape) == (6, 1, 28, 28)
    assert tuple(path_mask.shape) == (6, 1, 28, 28)
    assert tuple(target.shape) == (6, 1, 28, 28)
    assert torch.all(path_mask >= 0.0)
    assert torch.all(path_mask <= 1.0)
    assert float(path_mask.mean()) > 0.0
...
def test_compact_diffusion_path_mask_editing_training_smoke(tmp_path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    exit_code = run_training(
        TrainConfig(
            ...
            max_train_batches=2,
            max_eval_batches=1,
            run_name="pytest_path_mask_editing_smoke",
            num_sample_steps=6,
        ),
    ...
    assert (run_dir / "path_mask_triplets.pt").is_file()
