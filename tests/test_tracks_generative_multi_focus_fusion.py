...
def test_compact_diffusion_multi_focus_fusion_data_and_model_contract() -> None:
    ...
    focus_a, focus_b, target = next(iter(train_loader))
    assert tuple(focus_a.shape) == (6, 1, 28, 28)
    assert tuple(focus_b.shape) == (6, 1, 28, 28)
    assert tuple(target.shape) == (6, 1, 28, 28)
    assert torch.all(focus_a >= 0.0)
    assert torch.all(focus_a <= 1.0)
    assert torch.all(focus_b >= 0.0)
    assert torch.all(focus_b <= 1.0)
    assert torch.all(target >= 0.0)
    assert torch.all(target <= 1.0)
    assert not torch.allclose(focus_a, focus_b)
    assert not torch.allclose(focus_a, target)
    assert not torch.allclose(focus_b, target)
...
def test_compact_diffusion_multi_focus_fusion_training_and_dry_run(
    ...
    exit_code = run_training(
        TrainConfig(
            ...
            max_train_batches=2,
            max_eval_batches=1,
            run_name="pytest_multi_focus_fusion_smoke",
            num_sample_steps=6,
        ),
    ...
    assert (run_dir / "focus_trajectory.pt").is_file()
