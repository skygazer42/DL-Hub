...
def test_image_matching_batch_contract_and_loss_smoke() -> None:
    from tracks.vision.lesson_62_synthetic_image_matching.data import (
        DataConfig,
        SyntheticImageMatchingDataset,
        get_dataloaders,
    )
    from tracks.vision.lesson_62_synthetic_image_matching.model import (
        ImageMatchingModel,
        ModelConfig,
    ...
    assert 0.0 <= image_matching_accuracy(logits.detach(), labels) <= 1.0
    loss.backward()


def test_image_matching_training_smoke(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    from tracks.vision.lesson_62_synthetic_image_matching.data import DataConfig
    from tracks.vision.lesson_62_synthetic_image_matching.model import ModelConfig
    from tracks.vision.lesson_62_synthetic_image_matching.train import TrainConfig, run_training
    ...
    exit_code = run_training(
        TrainConfig(
            ...
            max_train_batches=2,
            max_eval_batches=1,
            run_name="pytest_image_matching_smoke",
        ),
        DataConfig(
            num_samples=48,
            batch_size=6,
    ...
    run_dir = tmp_path / "vision" / "lesson_62_synthetic_image_matching" / "pytest_image_matching_smoke"
