...
def test_compact_diffusion_masked_reference_editing_data_and_model_contract() -> None:
    ...
    source, reference, mask, target = next(iter(train_loader))
...
def test_compact_diffusion_masked_reference_editing_training_smoke(
    ...
    exit_code = run_training(
        TrainConfig(
            ...
            max_train_batches=2,
            max_eval_batches=1,
            run_name="pytest_masked_reference_editing_smoke",
            num_sample_steps=6,
        ),
    ...
    assert (run_dir / "masked_reference_triplets.pt").is_file()
