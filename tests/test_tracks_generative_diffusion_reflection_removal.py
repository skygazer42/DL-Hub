...
def test_toy_diffusion_reflection_removal_data_and_model_contract() -> None:
    from tracks.generative.lesson_20_toy_diffusion_reflection_removal.data import DataConfig, get_dataloaders
    from tracks.generative.lesson_20_toy_diffusion_reflection_removal.model import (
        DiffusionSchedule,
        ModelConfig,
        ToyReflectionRemovalDiffusionModel,
        q_sample,
    )
    ...
    model = ToyReflectionRemovalDiffusionModel(cfg)
...
def test_toy_diffusion_reflection_removal_training_and_dry_run(
    tmp_path, monkeypatch: pytest.MonkeyPatch
) -> None:
    from tracks.generative.lesson_20_toy_diffusion_reflection_removal.data import DataConfig
    from tracks.generative.lesson_20_toy_diffusion_reflection_removal.model import DiffusionSchedule, ModelConfig
    from tracks.generative.lesson_20_toy_diffusion_reflection_removal.train import TrainConfig, run_training
    ...
    assert exit_code == 0
    run_dir = tmp_path / "generative" / "lesson_20_toy_diffusion_reflection_removal" / "pytest_reflection_removal_smoke"
