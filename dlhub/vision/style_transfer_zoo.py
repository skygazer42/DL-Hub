...
def _split_arch_id(arch_id: str) -> tuple[str, str]:
    ...
    return split_arch_id(arch_id, default_prefix="dlst")
...
def _extract_variants_from_source(src: str) -> list[str] | None:
    ...
    for node in tree.body:
        ...
        if (
            ...
            and value.func.id == "_default_variants"
...
def _extract_builder_name_from_source(src: str) -> str | None:
    ...
    for node in tree.body:
        if isinstance(node, ast.FunctionDef):
            name = str(node.name)
            if name.startswith("build_") and name.endswith("_style_transfer"):
                return name
    return None
...
def _make_lazy_builder(module_name: str, *, builder_name: str, variant: str) -> Builder:
    ...
    def _builder(cfg: BuildConfig):
        ...
        mod = importlib.import_module(f"dlhub.vision.style_transfer.{module_name}")
        fn = getattr(mod, builder_name, None)
        if fn is None:
            raise RuntimeError(f"Style transfer module {module_name!r} missing {builder_name}()")
...
def _extend_registry(registry: dict[str, Builder]) -> None:
    ...
    module_dir = here / "style_transfer"
...
def list_local_arches() -> list[str]:
    ...
    return list_arch_ids(_REGISTRY, prefix="dlst")
...
def build_local_model(
    ...
    **builder_kwargs: object,
    ...
    if prefix in {"st", "style", "style_transfer"}:
        prefix = "dlst"
    if prefix not in {"dlst", "local"}:
        raise ValueError(f"Unsupported style transfer prefix: {prefix!r} (arch_id={arch_id!r})")

    builder = _REGISTRY.get(str(name).lower().strip())
    if builder is None:
        raise UnknownLocalArch(
            f"Unknown style transfer arch: {arch_id!r}. Tip: run `python scripts/style_transfer_zoo.py --list`."
        )
