...
class ToyModel(nn.Module):
    def __init__(self, *, family:str, in_channels:int, width:int, depth:int):
        super().__init__(); self.family=str(family); self.enc=TinyEncoder(in_channels,width,depth); c=self.enc.out_channels; self.color=nn.Conv2d(c,3,1); self.mask=nn.Conv2d(c,1,1)
    def forward(self, image): x=check_nchw(image); feat=self.enc(x); mask=torch.sigmoid(self.mask(feat)); color=torch.tanh(self.color(feat)); harmonized=x + 0.2*mask*color; return {'harmonized': harmonized, 'mask': mask}
