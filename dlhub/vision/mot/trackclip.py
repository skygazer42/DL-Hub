...
_VARIANTS: dict[str, dict[str, int]] = {
    "trackclip_tiny": {"width": 64, "num_tracks": 32},
    "trackclip_small": {"width": 96, "num_tracks": 48},
    "trackclip_base": {"width": 128, "num_tracks": 64},
}


def build_trackclip_tracker(
    *,
    in_channels: int,
    num_classes: int,
    seq_len: int = 4,
    image_size: int = 64,
    variant: str = "trackclip_tiny",
    ...
    if cfg is None:
        raise ValueError(
            f"Unknown variant for trackclip: {variant!r}. Available: {sorted(_VARIANTS)}"
        )
