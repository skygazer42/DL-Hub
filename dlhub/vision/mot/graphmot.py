...
_VARIANTS: dict[str, dict[str, int]] = {
    "graphmot_tiny": {"width": 64, "num_tracks": 32},
    "graphmot_small": {"width": 96, "num_tracks": 48},
    "graphmot_base": {"width": 128, "num_tracks": 64},
}


def build_graphmot_tracker(
    *,
    in_channels: int,
    num_classes: int,
    seq_len: int = 4,
    image_size: int = 64,
    variant: str = "graphmot_tiny",
    ...
    if cfg is None:
        raise ValueError(
            f"Unknown variant for graphmot: {variant!r}. Available: {sorted(_VARIANTS)}"
        )
