...
_VARIANTS: dict[str, dict[str, int]] = {
    "unicorn_tiny": {"width": 64, "num_tracks": 32},
    "unicorn_small": {"width": 96, "num_tracks": 48},
    "unicorn_base": {"width": 128, "num_tracks": 64},
}


def build_unicorn_tracker(
    *,
    in_channels: int,
    num_classes: int,
    seq_len: int = 4,
    image_size: int = 64,
    variant: str = "unicorn_tiny",
    ...
    if cfg is None:
        raise ValueError(
            f"Unknown variant for unicorn: {variant!r}. Available: {sorted(_VARIANTS)}"
        )
...
if __name__ == "__main__":
    smoke_test_tracker(build_unicorn_tracker, "unicorn_tiny")
