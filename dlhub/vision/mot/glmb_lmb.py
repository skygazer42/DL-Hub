...
_VARIANTS: dict[str, dict[str, int]] = {
    "glmb_lmb_tiny": {"width": 64, "num_tracks": 32},
    "glmb_lmb_small": {"width": 96, "num_tracks": 48},
    "glmb_lmb_base": {"width": 128, "num_tracks": 64},
}


def build_glmb_lmb_tracker(
    *,
    in_channels: int,
    num_classes: int,
    seq_len: int = 4,
    image_size: int = 64,
    variant: str = "glmb_lmb_tiny",
    ...
    if cfg is None:
        raise ValueError(
            f"Unknown variant for glmb_lmb: {variant!r}. Available: {sorted(_VARIANTS)}"
        )
...
if __name__ == "__main__":
    smoke_test_tracker(build_glmb_lmb_tracker, "glmb_lmb_tiny")
