...
_VARIANTS: dict[str, dict[str, int]] = {
    "masksort_tiny": {"width": 64, "num_tracks": 32},
    "masksort_small": {"width": 96, "num_tracks": 48},
    "masksort_base": {"width": 128, "num_tracks": 64},
}


def build_masksort_tracker(
    *,
    in_channels: int,
    num_classes: int,
    seq_len: int = 4,
    image_size: int = 64,
    variant: str = "masksort_tiny",
    ...
    if cfg is None:
        raise ValueError(
            f"Unknown variant for masksort: {variant!r}. Available: {sorted(_VARIANTS)}"
        )
