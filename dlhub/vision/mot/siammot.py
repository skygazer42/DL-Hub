...
_VARIANTS: dict[str, dict[str, int]] = {
    "siammot_tiny": {"width": 64, "num_tracks": 32},
    "siammot_small": {"width": 96, "num_tracks": 48},
    "siammot_base": {"width": 128, "num_tracks": 64},
}


def build_siammot_tracker(
    *,
    in_channels: int,
    num_classes: int,
    seq_len: int = 4,
    image_size: int = 64,
    variant: str = "siammot_tiny",
    ...
    if cfg is None:
        raise ValueError(
            f"Unknown variant for siammot: {variant!r}. Available: {sorted(_VARIANTS)}"
        )
...
if __name__ == "__main__":
    smoke_test_tracker(build_siammot_tracker, "siammot_tiny")
