...
_VARIANTS: dict[str, dict[str, int]] = {
    "bernoulli_mixture_track_tiny": {"width": 64, "num_tracks": 32},
    "bernoulli_mixture_track_small": {"width": 96, "num_tracks": 48},
    "bernoulli_mixture_track_base": {"width": 128, "num_tracks": 64},
}


def build_bernoulli_mixture_track_tracker(
    *,
    in_channels: int,
    num_classes: int,
    seq_len: int = 4,
    image_size: int = 64,
    variant: str = "bernoulli_mixture_track_tiny",
    ...
    if cfg is None:
        raise ValueError(
            "Unknown variant for bernoulli_mixture_track: "
    ...
    return MOTTracker2D(
        family="bernoulli_mixture_track",
