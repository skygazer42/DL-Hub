...
_VARIANTS: dict[str, dict[str, int]] = {
    "strongsort_tiny": {"width": 64, "num_tracks": 32},
    "strongsort_small": {"width": 96, "num_tracks": 48},
    "strongsort_base": {"width": 128, "num_tracks": 64},
}
...
def build_strongsort_tracker(
    *,
    ...
    if cfg is None:
        raise ValueError(
            f"Unknown variant for strongsort: {variant!r}. Available: {sorted(_VARIANTS)}"
        )
...
if __name__ == "__main__":
    smoke_test_tracker(build_strongsort_tracker, "strongsort_tiny")
