...
_VARIANTS: dict[str, dict[str, int]] = {
    "fairmot_tiny": {"width": 64, "num_tracks": 32},
    "fairmot_small": {"width": 96, "num_tracks": 48},
    "fairmot_base": {"width": 128, "num_tracks": 64},
}


def build_fairmot_tracker(
    *,
    in_channels: int,
    num_classes: int,
    seq_len: int = 4,
    image_size: int = 64,
    variant: str = "fairmot_tiny",
    ...
    if cfg is None:
        raise ValueError(
            f"Unknown variant for fairmot: {variant!r}. Available: {sorted(_VARIANTS)}"
        )
...
if __name__ == "__main__":
    smoke_test_tracker(build_fairmot_tracker, "fairmot_tiny")
