...
_VARIANTS: dict[str, dict[str, int]] = {
    "motr_tiny": {"width": 64, "num_tracks": 32},
    "motr_small": {"width": 96, "num_tracks": 48},
    "motr_base": {"width": 128, "num_tracks": 64},
}


def build_motr_tracker(
    *,
    in_channels: int,
    num_classes: int,
    seq_len: int = 4,
    image_size: int = 64,
    variant: str = "motr_tiny",
    width_mult: float = 1.0,
    dropout: float = 0.0,
):
    cfg = _VARIANTS.get(str(variant).lower().strip())
    if cfg is None:
        raise ValueError(f"Unknown variant for motr: {variant!r}. Available: {sorted(_VARIANTS)}")
...
if __name__ == "__main__":
    smoke_test_tracker(build_motr_tracker, "motr_tiny")
