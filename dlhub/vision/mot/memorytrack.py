...
_VARIANTS: dict[str, dict[str, int]] = {
    "memorytrack_tiny": {"width": 64, "num_tracks": 32},
    "memorytrack_small": {"width": 96, "num_tracks": 48},
    "memorytrack_base": {"width": 128, "num_tracks": 64},
}


def build_memorytrack_tracker(
    *,
    in_channels: int,
    num_classes: int,
    seq_len: int = 4,
    image_size: int = 64,
    variant: str = "memorytrack_tiny",
    ...
    if cfg is None:
        raise ValueError(
            f"Unknown variant for memorytrack: {variant!r}. Available: {sorted(_VARIANTS)}"
        )
