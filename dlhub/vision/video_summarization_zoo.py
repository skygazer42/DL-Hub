...
def _split_arch_id(arch_id: str) -> tuple[str, str]:
    ...
    return split_arch_id(arch_id, default_prefix="vsum")
...
def _extract_builder_name_from_source(src: str) -> str | None:
    ...
    for node in tree.body:
        if isinstance(node, ast.FunctionDef):
            name = str(node.name)
            if name.startswith("build_") and name.endswith("_video_summarizer"):
                return name
    return None
...
def _make_lazy_builder(module_name: str, *, builder_name: str, variant: str) -> Builder:
    ...
    def _builder(cfg: BuildConfig):
        ...
        mod = importlib.import_module(f"dlhub.vision.video_summarization.{module_name}")
        fn = getattr(mod, builder_name, None)
        if fn is None:
            raise RuntimeError(
                f"Video summarization module {module_name!r} missing {builder_name}()"
            )
...
def _extend_registry(registry: dict[str, Builder]) -> None:
    ...
    module_dir = here / "video_summarization"
...
def list_local_arches() -> list[str]:
    ...
    return list_arch_ids(_REGISTRY, prefix="vsum")
...
def build_local_model(
    ...
    *,
    ...
    if prefix in {"video_sum", "videosum", "summary"}:
        prefix = "vsum"
    if prefix not in {"vsum", "local"}:
        raise ValueError(
            f"Unsupported video summarization prefix: {prefix!r} (arch_id={arch_id!r})"
        )

    builder = _REGISTRY.get(str(name).lower().strip())
    if builder is None:
        raise UnknownLocalArch(
            "Unknown video summarization arch: "
            f"{arch_id!r}. Tip: run `python scripts/video_summarization_zoo.py --list`."
        )
