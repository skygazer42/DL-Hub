...
class TinyPupilBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.mode in {"direct", "heatmap", "ellipse", "contour", "starburst"}:
...
class TinyPupilDetector(nn.Module):
    def __init__(self, *, family: str, mode: str, in_channels: int, width: int, depth: int) -> None:
        ...
        self.blocks = nn.ModuleList(
            [TinyPupilBlock(channels=int(width), mode=str(mode)) for _ in range(max(1, int(depth)))]
        )
        self.coord_head = nn.Linear(int(width), 2)
        self.conf_head = nn.Linear(int(width), 1)
        self.heatmap_head = nn.Conv2d(int(width), 1, 1)
        self.ellipse_head = nn.Linear(int(width), 5)
...
def smoke_test_pupil_detector(builder, variant: str) -> None:
