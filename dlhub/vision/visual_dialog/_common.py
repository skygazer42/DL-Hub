...
class ToyModel(nn.Module):
    def __init__(self, *, family:str, in_channels:int, width:int, depth:int, vocab_size:int=32):
        super().__init__(); self.family=str(family); self.enc=TinyEncoder(in_channels,width,depth); self.text=nn.Linear(32,self.enc.out_channels); self.head=nn.Linear(self.enc.out_channels,int(vocab_size))
    def forward(self,image, history=None): feat=self.enc(image); history = torch.randn(image.shape[0],32,device=image.device) if history is None else history.to(feat.dtype); logits=self.head(feat+self.text(history)); return {'answer_logits': logits}
...
def smoke_test_model(builder, variant:str): out=builder(in_channels=3, variant=variant, width_mult=0.5, vocab_size=32)(torch.randn(2,3,64,64)); print(variant, tuple(out['answer_logits'].shape))
