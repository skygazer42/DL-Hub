...
class TinyHomographyBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.mode in {"direct", "correlation", "pyramid", "patch"}:
...
class TinyHomographyEstimator(nn.Module):
    ...
    def forward(
        ...
        homo[:, 0, 2] = offsets[:, 2, 0]
        homo[:, 1, 0] = offsets[:, 0, 1]
        homo[:, 1, 1] = 1.0 + offsets[:, 1, 1]
        homo[:, 1, 2] = offsets[:, 2, 1]
        homo[:, 2, 0] = 0.1 * offsets[:, 3, 0]
        homo[:, 2, 1] = 0.1 * offsets[:, 3, 1]
...
def smoke_test_homography_estimator(builder, variant: str) -> None:
    ...
    print(variant, tuple(out["offsets"].shape), tuple(out["homography"].shape))
    print("ok")
