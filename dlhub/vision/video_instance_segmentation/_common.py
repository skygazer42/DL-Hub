...
class CompactVISModel(nn.Module):
    def __init__(
        ...
        self.head = nn.Conv2d(self.enc.out_channels, int(num_instances), 1)
    ...
    def forward(self, video):
        ...
        feat = self.enc(video.view(b * t, c, h, w))
        logits = self.head(feat).view(b, t, self.num_instances, h, w)
...
def smoke_test_vis(builder, variant: str):
    model = builder(in_channels=3, variant=variant, width_mult=0.5, num_instances=6)
