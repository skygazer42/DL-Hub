...
_FAMILIES = ['u2net_salient', 'poolnet_salient', 'bassnet_salient', 'boundary_salient', 'transformer_salient', 'focal_salient', 'cascade_salient', 'prompt_salient', 'diffusion_salient', 'mamba_salient']
    ...
    for family in _FAMILIES:
        for size in _SIZES:
            ...
            def _builder(cfg: BuildConfig, family: str = family, variant: str = variant):
                module = importlib.import_module(f"dlhub.vision.salient_object_detection.{family}")
                fn = getattr(module, f"build_{family}_salient_detector")
                return fn(
...
def list_local_arches() -> list[str]:
    return [f"salobj:{name}" for name in sorted(_REGISTRY)]
