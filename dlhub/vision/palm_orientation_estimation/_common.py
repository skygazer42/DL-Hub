...
class TinyHandRegressor(nn.Module):
    def __init__(self, *, family: str, mode: str, in_channels: int, width: int, depth: int) -> None:
        ...
        self.blocks = nn.ModuleList([TinyHandRegressorBlock(channels=int(width), mode=str(mode)) for _ in range(max(1, int(depth)))])
    ...
    def forward(self, image: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        return {"value": torch.tanh(self.value_head(pooled)), "confidence": torch.sigmoid(self.conf_head(pooled)), "map": self.map_head(feat)}


def build_toy_hand_regressor(*, family: str, mode: str, variants: dict[str, dict[str, int]], in_channels: int, variant: str, width_mult: float = 1.0) -> nn.Module:
    ...
    return TinyHandRegressor(family=str(family), mode=str(mode), in_channels=int(in_channels), width=width, depth=depth)
