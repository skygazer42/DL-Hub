...
def _split_arch_id(arch_id: str) -> tuple[str, str]:
    ...
    return split_arch_id(arch_id, default_prefix="roadscene")
...
def _registry() -> dict[str, Builder]:
    ...
    return make_lazy_family_registry(
        _FAMILIES,
        _SIZES,
        module_template="dlhub.vision.road_scene_understanding.{family}",
        builder_template="build_{family}_road_scene_model",
...
def list_local_arches() -> list[str]:
    ...
    return list_arch_ids(_REGISTRY, prefix="roadscene")
...
def build_local_model(arch_id: str, *, in_channels: int, width_mult: float = 1.0):
    ...
    if builder is None:
        raise UnknownLocalArch(
            f"Unknown road scene understanding arch: {arch_id!r}. Tip: import `dlhub.vision.road_scene_understanding_zoo` and call `list_local_arches()`."
        )
