    ...
    for family in _FAMILIES:
        for size in _SIZES:
            ...
            def _builder(cfg: BuildConfig, family: str = family, variant: str = variant):
                module = importlib.import_module(f"dlhub.vision.road_scene_understanding.{family}")
                fn = getattr(module, f"build_{family}_road_scene_model")
                return fn(
...
def list_local_arches() -> list[str]:
    return [f"roadscene:{name}" for name in sorted(_REGISTRY)]
