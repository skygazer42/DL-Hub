...
class ToyModel(nn.Module):
    def __init__(self, *, family:str, in_channels:int, width:int, depth:int):
        super().__init__(); self.family=str(family); self.enc=TinyEncoder(in_channels,width,depth); self.text=nn.Linear(32,self.enc.out_channels); self.head=nn.Linear(self.enc.out_channels,3)
    def forward(self,image, text_feat=None): feat=self.enc(image); text_feat=torch.randn(image.shape[0],32,device=image.device) if text_feat is None else text_feat.to(feat.dtype); logits=self.head(feat+self.text(text_feat)); return {'logits': logits}
...
def smoke_test_model(builder, variant:str): out=builder(in_channels=3, variant=variant, width_mult=0.5)(torch.randn(2,3,64,64)); print(variant, tuple(out['logits'].shape))
