    ...
    for family in _FAMILIES:
        for size in _SIZES:
            ...
            def _builder(cfg: BuildConfig, family: str = family, variant: str = variant):
                module = importlib.import_module(f"dlhub.vision.finger_count_estimation.{family}")
                fn = getattr(module, f"build_{family}_finger_count_estimator")
                return fn(
...
def list_local_arches() -> list[str]:
    return [f"fingercount:{name}" for name in sorted(_REGISTRY)]
