...
_FAMILIES = ['plain_unet_seg', 'res_unet_seg', 'attn_unet_seg', 'nested_unet_seg', 'light_unet_seg', 'transformer_unet_seg', 'prompt_unet_seg', 'pyramid_unet_seg', 'dual_unet_seg', 'mamba_unet_seg']
    ...
    for family in _FAMILIES:
        for size in _SIZES:
            ...
            def _builder(cfg: BuildConfig, family: str = family, variant: str = variant):
                module = importlib.import_module(f"dlhub.vision.unet_segmentation.{family}")
                fn = getattr(module, f"build_{family}_segmentor")
                return fn(
...
def list_local_arches() -> list[str]:
    return [f"unetseg:{name}" for name in sorted(_REGISTRY)]
...
def build_local_model(arch_id: str, *, in_channels: int, width_mult: float = 1.0):
    ...
    if prefix not in {'unetseg', "local"}:
        raise ValueError(
            f"Unsupported unet segmentation prefix: {prefix!r} (arch_id={arch_id!r})"
        )
