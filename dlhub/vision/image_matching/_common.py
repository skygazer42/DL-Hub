...
class ToyImageMatcher(nn.Module):
    ...
    def forward(self,image1,image2): f1=self.enc(image1).mean(dim=(2,3)); f2=self.enc(image2).mean(dim=(2,3)); e1=F.normalize(self.proj(f1),dim=1); e2=F.normalize(self.proj(f2),dim=1); return {'similarity': (e1*e2).sum(dim=1), 'embedding1': e1, 'embedding2': e2}

def build_toy_model(*, family:str, variants:dict[str,dict[str,int]], in_channels:int, variant:str, width_mult:float=1.0, **kwargs):
    spec=variants[str(variant)]; width=max(16,int(int(spec['width'])*float(width_mult))); embed=max(64,int(int(spec['embed'])*float(width_mult))); return ToyImageMatcher(family=str(family), in_channels=int(in_channels), width=width, depth=int(spec['depth']), embed_dim=embed)

def smoke_test_model(builder, variant:str): out=builder(in_channels=3, variant=variant, width_mult=0.5)(torch.randn(2,3,64,64),torch.randn(2,3,64,64)); print(variant,{k:tuple(v.shape) if hasattr(v,'shape') else type(v).__name__ for k,v in out.items()})
