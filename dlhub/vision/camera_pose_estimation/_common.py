...
class TinyCameraPoseEstimator(nn.Module):
    def __init__(self, *, family: str, mode: str, in_channels: int, width: int, depth: int) -> None:
        ...
        self.blocks = nn.ModuleList([TinyCameraPoseBlock(channels=int(width), mode=str(mode)) for _ in range(max(1, int(depth)))])
    ...
    def forward(self, image: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        pose = torch.eye(4, device=image.device, dtype=image.dtype).unsqueeze(0).repeat(image.shape[0], 1, 1)
        pose[:, :3, 3] = translation
        pose[:, 0, 0] = rotation[:, 0]
        pose[:, 1, 1] = rotation[:, 1]
        pose[:, 2, 2] = rotation[:, 2]
        pose[:, 0, 1] = rotation[:, 3]
        return {"translation": translation, "rotation": rotation, "pose": pose, "confidence": torch.sigmoid(self.conf_head(pooled))}


def build_toy_camera_pose_estimator(*, family: str, mode: str, variants: dict[str, dict[str, int]], in_channels: int, variant: str, width_mult: float = 1.0) -> nn.Module:
    ...
    return TinyCameraPoseEstimator(family=str(family), mode=str(mode), in_channels=int(in_channels), width=width, depth=depth)
