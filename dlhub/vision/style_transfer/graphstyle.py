...
_VARIANTS: dict[str, dict[str, int]] = {
    "graphstyle_tiny": {"width": 24, "depth": 2},
    "graphstyle_small": {"width": 32, "depth": 3},
    "graphstyle_base": {"width": 48, "depth": 4},
}


class GraphstyleStyleTransfer(nn.Module):
...
def build_graphstyle_style_transfer(
    *,
    in_channels: int,
    image_size: int = 64,  # kept for zoo signature consistency
    variant: str = "graphstyle_small",
    ...
    return GraphstyleStyleTransfer(
        in_channels=int(in_channels),
        width=width,
        depth=int(cfg["depth"]),
        dropout=float(dropout),
...
if __name__ == "__main__":
    ...
    m = build_graphstyle_style_transfer(in_channels=3, variant="graphstyle_tiny", width_mult=0.5)
    out = m(x, s)
    print("graphstyle_tiny", tuple(out["stylized"].shape))
