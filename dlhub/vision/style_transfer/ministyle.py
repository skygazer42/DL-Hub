...
_VARIANTS: dict[str, dict[str, int]] = {
    "ministyle_tiny": {"width": 24, "depth": 2},
    "ministyle_small": {"width": 32, "depth": 3},
    "ministyle_base": {"width": 48, "depth": 4},
}


class MinistyleStyleTransfer(nn.Module):
...
def build_ministyle_style_transfer(
    *,
    in_channels: int,
    image_size: int = 64,  # kept for zoo signature consistency
    variant: str = "ministyle_small",
...
if __name__ == "__main__":
    ...
    m = build_ministyle_style_transfer(in_channels=3, variant="ministyle_tiny", width_mult=0.5)
    out = m(x, s)
    print("ministyle_tiny", tuple(out["stylized"].shape))
