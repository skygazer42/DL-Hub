...
_VARIANTS: dict[str, dict[str, int]] = {
    "brushstyle_tiny": {"width": 24, "depth": 2},
    "brushstyle_small": {"width": 32, "depth": 3},
    "brushstyle_base": {"width": 48, "depth": 4},
}
...
class BrushstyleStyleTransfer(nn.Module):
...
def build_brushstyle_style_transfer(
    *,
    in_channels: int,
    image_size: int = 64,
    variant: str = "brushstyle_small",
    ...
    return BrushstyleStyleTransfer(
...
if __name__ == "__main__":
    ...
    m = build_brushstyle_style_transfer(in_channels=3, variant="brushstyle_tiny", width_mult=0.5, num_layers=1)
    out = m(x, s)
    print("brushstyle_tiny", tuple(out["stylized"].shape))
