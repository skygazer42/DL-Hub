...
_VARIANTS: dict[str, dict[str, int]] = {
    "domainstyle_tiny": {"width": 24, "depth": 2},
    "domainstyle_small": {"width": 32, "depth": 3},
    "domainstyle_base": {"width": 48, "depth": 4},
}


class DomainstyleStyleTransfer(nn.Module):
...
def build_domainstyle_style_transfer(
    *,
    in_channels: int,
    image_size: int = 64,  # kept for zoo signature consistency
    variant: str = "domainstyle_small",
    ...
    return DomainstyleStyleTransfer(
        in_channels=int(in_channels),
        width=width,
        depth=int(cfg["depth"]),
        dropout=float(dropout),
...
if __name__ == "__main__":
    ...
    m = build_domainstyle_style_transfer(in_channels=3, variant="domainstyle_tiny", width_mult=0.5)
    out = m(x, s)
    print("domainstyle_tiny", tuple(out["stylized"].shape))
