...
_VARIANTS: dict[str, dict[str, int]] = {
    "purestyle_tiny": {"width": 24, "depth": 2},
    "purestyle_small": {"width": 32, "depth": 3},
    "purestyle_base": {"width": 48, "depth": 4},
}
...
class PurestyleStyleTransfer(nn.Module):
...
def build_purestyle_style_transfer(
    *,
    in_channels: int,
    image_size: int = 64,
    variant: str = "purestyle_small",
...
if __name__ == "__main__":
    ...
    m = build_purestyle_style_transfer(in_channels=3, variant="purestyle_tiny", width_mult=0.5, num_layers=1)
    out = m(x, s)
    print("purestyle_tiny", tuple(out["stylized"].shape))
