...
_VARIANTS: dict[str, dict[str, int]] = {
    "clipstyler_tiny": {"width": 24, "depth": 2},
    "clipstyler_small": {"width": 32, "depth": 3},
    "clipstyler_base": {"width": 48, "depth": 4},
}


class ClipstylerStyleTransfer(nn.Module):
...
def build_clipstyler_style_transfer(
    *,
    in_channels: int,
    image_size: int = 64,  # kept for zoo signature consistency
    variant: str = "clipstyler_small",
    ...
    return ClipstylerStyleTransfer(
        in_channels=int(in_channels),
        width=width,
        depth=int(cfg["depth"]),
        dropout=float(dropout),
...
if __name__ == "__main__":
    ...
    m = build_clipstyler_style_transfer(in_channels=3, variant="clipstyler_tiny", width_mult=0.5)
    out = m(x, s)
    print("clipstyler_tiny", tuple(out["stylized"].shape))
