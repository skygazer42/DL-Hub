...
_VARIANTS: dict[str, dict[str, int]] = {
    "lineart_style_tiny": {"width": 24, "depth": 2},
    "lineart_style_small": {"width": 32, "depth": 3},
    "lineart_style_base": {"width": 48, "depth": 4},
}


class LineartStyleStyleTransfer(nn.Module):
...
def build_lineart_style_style_transfer(
    *,
    in_channels: int,
    image_size: int = 64,  # kept for zoo signature consistency
    variant: str = "lineart_style_small",
    ...
    return LineartStyleStyleTransfer(
        in_channels=int(in_channels),
        width=width,
        depth=int(cfg["depth"]),
        dropout=float(dropout),
