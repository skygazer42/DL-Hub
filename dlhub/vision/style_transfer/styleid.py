...
_VARIANTS: dict[str, dict[str, int]] = {
    "styleid_tiny": {"width": 24, "depth": 2},
    "styleid_small": {"width": 32, "depth": 3},
    "styleid_base": {"width": 48, "depth": 4},
}


class StyleidStyleTransfer(nn.Module):
...
def build_styleid_style_transfer(
    *,
    in_channels: int,
    image_size: int = 64,  # kept for zoo signature consistency
    variant: str = "styleid_small",
...
if __name__ == "__main__":
    ...
    m = build_styleid_style_transfer(in_channels=3, variant="styleid_tiny", width_mult=0.5)
    out = m(x, s)
    print("styleid_tiny", tuple(out["stylized"].shape))
