...
_VARIANTS: dict[str, dict[str, int]] = {
    "resstyle_tiny": {"width": 24, "depth": 2},
    "resstyle_small": {"width": 32, "depth": 3},
    "resstyle_base": {"width": 48, "depth": 4},
}
...
class ResstyleStyleTransfer(nn.Module):
...
def build_resstyle_style_transfer(
    *,
    in_channels: int,
    image_size: int = 64,
    variant: str = "resstyle_small",
...
if __name__ == "__main__":
    ...
    m = build_resstyle_style_transfer(in_channels=3, variant="resstyle_tiny", width_mult=0.5, num_layers=1)
    out = m(x, s)
    print("resstyle_tiny", tuple(out["stylized"].shape))
