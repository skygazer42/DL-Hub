...
_VARIANTS: dict[str, dict[str, int]] = {
    "muse_style_tiny": {"width": 24, "depth": 2},
    "muse_style_small": {"width": 32, "depth": 3},
    "muse_style_base": {"width": 48, "depth": 4},
}
...
class MuseStyleTransfer(nn.Module):
...
def build_muse_style_style_transfer(
    *,
    in_channels: int,
    image_size: int = 64,
    variant: str = "muse_style_small",
...
if __name__ == "__main__":
    ...
    m = build_muse_style_style_transfer(in_channels=3, variant="muse_style_tiny", width_mult=0.5, num_layers=1)
    out = m(x, s)
    print("muse_style_tiny", tuple(out["stylized"].shape))
