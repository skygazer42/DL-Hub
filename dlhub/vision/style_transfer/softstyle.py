...
_VARIANTS: dict[str, dict[str, int]] = {
    "softstyle_tiny": {"width": 24, "depth": 2},
    "softstyle_small": {"width": 32, "depth": 3},
    "softstyle_base": {"width": 48, "depth": 4},
}


class SoftstyleStyleTransfer(nn.Module):
...
def build_softstyle_style_transfer(
    *,
    in_channels: int,
    image_size: int = 64,  # kept for zoo signature consistency
    variant: str = "softstyle_small",
...
if __name__ == "__main__":
    ...
    m = build_softstyle_style_transfer(in_channels=3, variant="softstyle_tiny", width_mult=0.5)
    out = m(x, s)
    print("softstyle_tiny", tuple(out["stylized"].shape))
