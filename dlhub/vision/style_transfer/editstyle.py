...
_VARIANTS: dict[str, dict[str, int]] = {
    "editstyle_tiny": {"width": 24, "depth": 2},
    "editstyle_small": {"width": 32, "depth": 3},
    "editstyle_base": {"width": 48, "depth": 4},
}


class EditstyleStyleTransfer(nn.Module):
...
def build_editstyle_style_transfer(
    *,
    in_channels: int,
    image_size: int = 64,  # kept for zoo signature consistency
    variant: str = "editstyle_small",
...
if __name__ == "__main__":
    ...
    m = build_editstyle_style_transfer(in_channels=3, variant="editstyle_tiny", width_mult=0.5)
    out = m(x, s)
    print("editstyle_tiny", tuple(out["stylized"].shape))
