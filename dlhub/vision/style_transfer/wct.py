...
_VARIANTS: dict[str, dict[str, int]] = {
    "wct_tiny": {"width": 24, "depth": 2},
    "wct_small": {"width": 32, "depth": 3},
    "wct_base": {"width": 48, "depth": 4},
}
...
def build_wct_style_transfer(
    *,
    in_channels: int,
    image_size: int = 64,
    variant: str = "wct_small",
    ...
    if name not in _VARIANTS:
        raise ValueError(f"Unknown WCT variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    cfg = _VARIANTS[name]
    width = max(8, int(int(cfg["width"]) * float(width_mult)))
    return WCTStyleTransfer(
        in_channels=int(in_channels),
        width=width,
        depth=int(cfg["depth"]),
        dropout=float(dropout),
...
if __name__ == "__main__":
    ...
    m = build_wct_style_transfer(in_channels=3, variant="wct_tiny", width_mult=0.5)
    out = m(x, s)
    print("wct_tiny", tuple(out["stylized"].shape))
