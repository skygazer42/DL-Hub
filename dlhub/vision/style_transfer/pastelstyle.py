...
_VARIANTS: dict[str, dict[str, int]] = {
    "pastelstyle_tiny": {"width": 24, "depth": 2},
    "pastelstyle_small": {"width": 32, "depth": 3},
    "pastelstyle_base": {"width": 48, "depth": 4},
}


class PastelstyleStyleTransfer(nn.Module):
...
def build_pastelstyle_style_transfer(
    *,
    in_channels: int,
    image_size: int = 64,  # kept for zoo signature consistency
    variant: str = "pastelstyle_small",
    ...
    return PastelstyleStyleTransfer(
        in_channels=int(in_channels),
        width=width,
        depth=int(cfg["depth"]),
        dropout=float(dropout),
...
if __name__ == "__main__":
    ...
    m = build_pastelstyle_style_transfer(in_channels=3, variant="pastelstyle_tiny", width_mult=0.5)
    out = m(x, s)
    print("pastelstyle_tiny", tuple(out["stylized"].shape))
