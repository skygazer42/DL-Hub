...
_VARIANTS: dict[str, dict[str, int]] = {
    "patchstyle_tiny": {"width": 24, "depth": 2},
    "patchstyle_small": {"width": 32, "depth": 3},
    "patchstyle_base": {"width": 48, "depth": 4},
}
...
class PatchstyleStyleTransfer(nn.Module):
...
def build_patchstyle_style_transfer(
    *,
    in_channels: int,
    image_size: int = 64,
    variant: str = "patchstyle_small",
    ...
    return PatchstyleStyleTransfer(
...
if __name__ == "__main__":
    ...
    m = build_patchstyle_style_transfer(in_channels=3, variant="patchstyle_tiny", width_mult=0.5, num_layers=1)
    out = m(x, s)
    print("patchstyle_tiny", tuple(out["stylized"].shape))
