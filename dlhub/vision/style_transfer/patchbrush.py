...
_VARIANTS: dict[str, dict[str, int]] = {
    "patchbrush_tiny": {"width": 24, "depth": 2},
    "patchbrush_small": {"width": 32, "depth": 3},
    "patchbrush_base": {"width": 48, "depth": 4},
}
...
class PatchbrushStyleTransfer(nn.Module):
...
def build_patchbrush_style_transfer(
    *,
    in_channels: int,
    image_size: int = 64,
    variant: str = "patchbrush_small",
    ...
    return PatchbrushStyleTransfer(
...
if __name__ == "__main__":
    ...
    m = build_patchbrush_style_transfer(in_channels=3, variant="patchbrush_tiny", width_mult=0.5, num_layers=1)
    out = m(x, s)
    print("patchbrush_tiny", tuple(out["stylized"].shape))
