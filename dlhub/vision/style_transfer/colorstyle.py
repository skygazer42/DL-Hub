...
_VARIANTS: dict[str, dict[str, int]] = {
    "colorstyle_tiny": {"width": 24, "depth": 2},
    "colorstyle_small": {"width": 32, "depth": 3},
    "colorstyle_base": {"width": 48, "depth": 4},
}


class ColorstyleStyleTransfer(nn.Module):
...
def build_colorstyle_style_transfer(
    *,
    in_channels: int,
    image_size: int = 64,  # kept for zoo signature consistency
    variant: str = "colorstyle_small",
    ...
    return ColorstyleStyleTransfer(
        in_channels=int(in_channels),
        width=width,
        depth=int(cfg["depth"]),
        dropout=float(dropout),
...
if __name__ == "__main__":
    ...
    m = build_colorstyle_style_transfer(in_channels=3, variant="colorstyle_tiny", width_mult=0.5)
    out = m(x, s)
    print("colorstyle_tiny", tuple(out["stylized"].shape))
