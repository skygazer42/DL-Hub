...
_VARIANTS: dict[str, dict[str, int]] = {
    "objectstyle_tiny": {"width": 24, "depth": 2},
    "objectstyle_small": {"width": 32, "depth": 3},
    "objectstyle_base": {"width": 48, "depth": 4},
}
...
class ObjectstyleStyleTransfer(nn.Module):
...
def build_objectstyle_style_transfer(
    *,
    in_channels: int,
    image_size: int = 64,
    variant: str = "objectstyle_small",
    ...
    return ObjectstyleStyleTransfer(
...
if __name__ == "__main__":
    ...
    m = build_objectstyle_style_transfer(in_channels=3, variant="objectstyle_tiny", width_mult=0.5, num_layers=1)
    out = m(x, s)
    print("objectstyle_tiny", tuple(out["stylized"].shape))
