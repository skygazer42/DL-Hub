...
_VARIANTS: dict[str, dict[str, int]] = {
    "textstyle_tiny": {"width": 24, "depth": 2},
    "textstyle_small": {"width": 32, "depth": 3},
    "textstyle_base": {"width": 48, "depth": 4},
}


class TextstyleStyleTransfer(nn.Module):
...
def build_textstyle_style_transfer(
    *,
    in_channels: int,
    image_size: int = 64,  # kept for zoo signature consistency
    variant: str = "textstyle_small",
...
if __name__ == "__main__":
    ...
    m = build_textstyle_style_transfer(in_channels=3, variant="textstyle_tiny", width_mult=0.5)
    out = m(x, s)
    print("textstyle_tiny", tuple(out["stylized"].shape))
