...
_VARIANTS: dict[str, dict[str, int]] = {
    "patternstyle_tiny": {"width": 24, "depth": 2},
    "patternstyle_small": {"width": 32, "depth": 3},
    "patternstyle_base": {"width": 48, "depth": 4},
}


class PatternstyleStyleTransfer(nn.Module):
...
def build_patternstyle_style_transfer(
    *,
    in_channels: int,
    image_size: int = 64,  # kept for zoo signature consistency
    variant: str = "patternstyle_small",
    ...
    return PatternstyleStyleTransfer(
        in_channels=int(in_channels),
        width=width,
        depth=int(cfg["depth"]),
        dropout=float(dropout),
...
if __name__ == "__main__":
    ...
    m = build_patternstyle_style_transfer(in_channels=3, variant="patternstyle_tiny", width_mult=0.5)
    out = m(x, s)
    print("patternstyle_tiny", tuple(out["stylized"].shape))
