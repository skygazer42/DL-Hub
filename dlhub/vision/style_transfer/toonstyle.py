...
_VARIANTS: dict[str, dict[str, int]] = {
    "toonstyle_tiny": {"width": 24, "depth": 2},
    "toonstyle_small": {"width": 32, "depth": 3},
    "toonstyle_base": {"width": 48, "depth": 4},
}


class ToonstyleStyleTransfer(nn.Module):
...
def build_toonstyle_style_transfer(
    *,
    in_channels: int,
    image_size: int = 64,  # kept for zoo signature consistency
    variant: str = "toonstyle_small",
...
if __name__ == "__main__":
    ...
    m = build_toonstyle_style_transfer(in_channels=3, variant="toonstyle_tiny", width_mult=0.5)
    out = m(x, s)
    print("toonstyle_tiny", tuple(out["stylized"].shape))
