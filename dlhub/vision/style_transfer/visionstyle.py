...
_VARIANTS: dict[str, dict[str, int]] = {
    "visionstyle_tiny": {"width": 24, "depth": 2},
    "visionstyle_small": {"width": 32, "depth": 3},
    "visionstyle_base": {"width": 48, "depth": 4},
}


class VisionstyleStyleTransfer(nn.Module):
...
def build_visionstyle_style_transfer(
    *,
    in_channels: int,
    image_size: int = 64,  # kept for zoo signature consistency
    variant: str = "visionstyle_small",
    ...
    return VisionstyleStyleTransfer(
        in_channels=int(in_channels),
        width=width,
        depth=int(cfg["depth"]),
        dropout=float(dropout),
...
if __name__ == "__main__":
    ...
    m = build_visionstyle_style_transfer(in_channels=3, variant="visionstyle_tiny", width_mult=0.5)
    out = m(x, s)
    print("visionstyle_tiny", tuple(out["stylized"].shape))
