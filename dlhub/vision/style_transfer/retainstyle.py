...
_VARIANTS: dict[str, dict[str, int]] = {
    "retainstyle_tiny": {"width": 24, "depth": 2},
    "retainstyle_small": {"width": 32, "depth": 3},
    "retainstyle_base": {"width": 48, "depth": 4},
}


class RetainstyleStyleTransfer(nn.Module):
...
def build_retainstyle_style_transfer(
    *,
    in_channels: int,
    image_size: int = 64,  # kept for zoo signature consistency
    variant: str = "retainstyle_small",
    ...
    return RetainstyleStyleTransfer(
        in_channels=int(in_channels),
        width=width,
        depth=int(cfg["depth"]),
        dropout=float(dropout),
...
if __name__ == "__main__":
    ...
    m = build_retainstyle_style_transfer(in_channels=3, variant="retainstyle_tiny", width_mult=0.5)
    out = m(x, s)
    print("retainstyle_tiny", tuple(out["stylized"].shape))
