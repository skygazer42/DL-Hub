...
_VARIANTS: dict[str, dict[str, int]] = {
    "contourstyle_tiny": {"width": 24, "depth": 2},
    "contourstyle_small": {"width": 32, "depth": 3},
    "contourstyle_base": {"width": 48, "depth": 4},
}
...
class ContourstyleStyleTransfer(nn.Module):
...
def build_contourstyle_style_transfer(
    *,
    in_channels: int,
    image_size: int = 64,
    variant: str = "contourstyle_small",
    ...
    return ContourstyleStyleTransfer(
...
if __name__ == "__main__":
    ...
    m = build_contourstyle_style_transfer(in_channels=3, variant="contourstyle_tiny", width_mult=0.5, num_layers=1)
    out = m(x, s)
    print("contourstyle_tiny", tuple(out["stylized"].shape))
