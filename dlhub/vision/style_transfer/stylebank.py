...
_VARIANTS: dict[str, dict[str, int]] = {
    "stylebank_tiny": {"width": 24, "depth": 2},
    "stylebank_small": {"width": 32, "depth": 3},
    "stylebank_base": {"width": 48, "depth": 4},
}


class StylebankStyleTransfer(nn.Module):
...
def build_stylebank_style_transfer(
    *,
    in_channels: int,
    image_size: int = 64,  # kept for zoo signature consistency
    variant: str = "stylebank_small",
...
if __name__ == "__main__":
    ...
    m = build_stylebank_style_transfer(in_channels=3, variant="stylebank_tiny", width_mult=0.5)
    out = m(x, s)
    print("stylebank_tiny", tuple(out["stylized"].shape))
