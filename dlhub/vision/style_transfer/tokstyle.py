...
_VARIANTS: dict[str, dict[str, int]] = {
    "tokstyle_tiny": {"width": 24, "depth": 2},
    "tokstyle_small": {"width": 32, "depth": 3},
    "tokstyle_base": {"width": 48, "depth": 4},
}
...
class TokstyleStyleTransfer(nn.Module):
...
def build_tokstyle_style_transfer(
    *,
    in_channels: int,
    image_size: int = 64,
    variant: str = "tokstyle_small",
...
if __name__ == "__main__":
    ...
    m = build_tokstyle_style_transfer(in_channels=3, variant="tokstyle_tiny", width_mult=0.5, num_layers=1)
    out = m(x, s)
    print("tokstyle_tiny", tuple(out["stylized"].shape))
