...
class TinyMirrorSegBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.mode in {"direct", "edge", "context", "boundary", "coarse"}:
...
class TinyMirrorSegmentor(nn.Module):
    def __init__(self, *, family: str, mode: str, in_channels: int, width: int, depth: int, classes: int = 2) -> None:
        ...
        self.blocks = nn.ModuleList(
            [TinyMirrorSegBlock(channels=int(width), mode=str(mode)) for _ in range(max(1, int(depth)))]
        )
        self.logits = nn.Conv2d(int(width), int(classes), 1)
        self.boundary = nn.Conv2d(int(width), 1, 1)
        self.confidence = nn.Conv2d(int(width), 1, 1)
    ...
    def forward(self, image: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        return {
            ...
            "boundary": self.boundary(feat),
            "confidence": torch.sigmoid(self.confidence(feat)),
        }


def build_toy_mirror_segmentor(*, family: str, mode: str, variants: dict[str, dict[str, int]], in_channels: int, variant: str, width_mult: float = 1.0) -> nn.Module:
...
def smoke_test_mirror_segmentor(builder, variant: str) -> None:
