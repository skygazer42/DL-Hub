...
class CompactCrossView(nn.Module):
    def __init__(self, *, family: str, in_channels: int, width: int, depth: int, embed_dim: int):
        ...
        self.encoder = TinyEncoder(in_channels, width, depth)
        self.proj = nn.Linear(self.encoder.out_channels, int(embed_dim))

    def forward(self, aerial, ground):
        a = F.normalize(self.proj(self.encoder(aerial)), dim=1)
        g = F.normalize(self.proj(self.encoder(ground)), dim=1)
...
def smoke_test_cv(builder, variant: str):
    out = builder(in_channels=3, variant=variant, width_mult=0.5)(
        torch.randn(2, 3, 128, 128), torch.randn(2, 3, 128, 128)
    )
