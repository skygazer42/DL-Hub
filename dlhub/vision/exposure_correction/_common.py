...
class TinyExposureBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, guide: torch.Tensor) -> torch.Tensor:
        ...
        if self.mode in {"gamma", "tone", "histogram", "curve", "contrast", "retinex"}:
...
def smoke_test_exposure_corrector(builder, variant: str) -> None:
    ...
    print(variant, tuple(out["corrected"].shape), tuple(out["exposure_map"].shape))
    print("ok")
