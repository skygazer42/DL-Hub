...
def _split_arch_id(arch_id: str) -> tuple[str, str]:
    ...
    return split_arch_id(arch_id, default_prefix="handedness")
...
def _registry() -> dict[str, Builder]:
    ...
    return make_lazy_family_registry(
        _FAMILIES,
        _SIZES,
        module_template="dlhub.vision.handedness_classification.{family}",
        builder_template="build_{family}_handedness_classifier",
...
def list_local_arches() -> list[str]:
    ...
    return list_arch_ids(_REGISTRY, prefix="handedness")
...
def build_local_model(arch_id: str, *, in_channels: int, width_mult: float = 1.0):
    ...
    if builder is None:
        raise UnknownLocalArch(
            f"Unknown handedness classification arch: {arch_id!r}. Tip: import `dlhub.vision.handedness_classification_zoo` and call `list_local_arches()`."
        )
