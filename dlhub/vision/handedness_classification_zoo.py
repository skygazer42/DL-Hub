    ...
    for family in _FAMILIES:
        for size in _SIZES:
            ...
            def _builder(cfg: BuildConfig, family: str = family, variant: str = variant):
                module = importlib.import_module(f"dlhub.vision.handedness_classification.{family}")
                fn = getattr(module, f"build_{family}_handedness_classifier")
                return fn(
...
def list_local_arches() -> list[str]:
    return [f"handedness:{name}" for name in sorted(_REGISTRY)]
