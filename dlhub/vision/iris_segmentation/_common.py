...
class TinyIrisBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.mode in {"unet", "deeplab", "boundary", "contour", "pupil"}:
...
class TinyIrisSegmentor(nn.Module):
    def __init__(self, *, family: str, mode: str, in_channels: int, width: int, depth: int) -> None:
        ...
        self.blocks = nn.ModuleList([TinyIrisBlock(channels=int(width), mode=str(mode)) for _ in range(max(1, int(depth)))])
        self.iris_head = nn.Conv2d(int(width), 2, 1)
        self.pupil_head = nn.Conv2d(int(width), 1, 1)
    ...
    def forward(self, image: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        for block in self.blocks:
            feat = block(feat)
        iris_logits = self.iris_head(feat)
        pupil_logits = self.pupil_head(feat)
        return {
            "iris_logits": iris_logits,
            "iris_mask": torch.softmax(iris_logits, dim=1),
            "pupil_logits": pupil_logits,
            "pupil_mask": torch.sigmoid(pupil_logits),
            "boundary_map": self.boundary_head(feat),
        }
...
def smoke_test_iris_segmentor(builder, variant: str) -> None:
    model = builder(in_channels=3, variant=variant, width_mult=0.5)
    out = model(torch.randn(2, 3, 64, 64))
    print(variant, tuple(out["iris_logits"].shape), tuple(out["pupil_mask"].shape))
    print("ok")
