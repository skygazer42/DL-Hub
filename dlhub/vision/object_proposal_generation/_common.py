...
class TinyProposalBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.mode in {"rpn", "fcos", "center", "corner", "query"}:
...
class TinyProposalModel(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.objectness = nn.Linear(int(width), 1)
    ...
    def forward(self, image: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        return {
            "proposal_boxes": torch.sigmoid(self.box(q)),
            "proposal_scores": torch.sigmoid(self.score(q)).squeeze(-1),
            "objectness_logits": self.objectness(q).squeeze(-1),
        }
...
def build_baseline_proposer(
    *,
    ...
    num_props = int(spec.get("proposals", 24))
    return TinyProposalModel(
        family=str(family),
        mode=str(mode),
...
def smoke_test_proposer(builder, variant: str) -> None:
    model = builder(in_channels=3, variant=variant, width_mult=0.5)
    out = model(torch.randn(2, 3, 64, 64))
    print(variant, tuple(out["proposal_boxes"].shape), tuple(out["proposal_scores"].shape))
    print("ok")
