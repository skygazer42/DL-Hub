    ...
    for family in _FAMILIES:
        for size in _SIZES:
            ...
            def _builder(cfg: BuildConfig, family: str = family, variant: str = variant):
                module = importlib.import_module(f"dlhub.vision.sign_digit_classification.{family}")
                fn = getattr(module, f"build_{family}_sign_digit_classifier")
                return fn(
...
def list_local_arches() -> list[str]:
    return [f"signdigit:{name}" for name in sorted(_REGISTRY)]
