...
class TinyFacePoseBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.mode in {"direct", "heatmap", "axis_angle", "landmark"}:
...
class TinyFacePoseEstimator(nn.Module):
    def __init__(self, *, family: str, mode: str, in_channels: int, width: int, depth: int) -> None:
        ...
        self.blocks = nn.ModuleList([TinyFacePoseBlock(channels=int(width), mode=str(mode)) for _ in range(max(1, int(depth)))])
        self.angle_head = nn.Linear(int(width), 3)
        self.conf_head = nn.Linear(int(width), 3)
        self.map_head = nn.Conv2d(int(width), 3, kernel_size=1)
    ...
    def forward(self, image: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        return {
            "angles": self.angle_head(pooled),
            "confidence": torch.sigmoid(self.conf_head(pooled)),
            "pose_map": self.map_head(feat),
        }


def build_toy_face_pose_estimator(*, family: str, mode: str, variants: dict[str, dict[str, int]], in_channels: int, variant: str, width_mult: float = 1.0) -> nn.Module:
    ...
    return TinyFacePoseEstimator(family=str(family), mode=str(mode), in_channels=int(in_channels), width=width, depth=depth)


def smoke_test_face_pose_estimator(builder, variant: str) -> None:
    model = builder(in_channels=3, variant=variant, width_mult=0.5)
    out = model(torch.randn(2, 3, 64, 64))
    print(variant, tuple(out["angles"].shape), tuple(out["pose_map"].shape))
    print("ok")
