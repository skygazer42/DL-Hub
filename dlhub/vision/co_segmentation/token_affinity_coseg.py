...
class TokenAffinityCosegCoSegmentor(nn.Module):
    """Toy co-segmentation model for the token_affinity_coseg family."""
...
_VARIANTS: dict[str, dict[str, int | float]] = {
    "token_affinity_coseg_tiny": {"width": 16, "depth": 1, "dropout": 0.0},
    "token_affinity_coseg_small": {"width": 24, "depth": 2, "dropout": 0.0},
    "token_affinity_coseg_base": {"width": 32, "depth": 3, "dropout": 0.1},
}


def build_token_affinity_coseg_co_segmentor(
    *,
    in_channels: int,
    num_classes: int,
    set_size: int = 3,
    image_size: int = 64,
    variant: str = "token_affinity_coseg_small",
    ...
    if name not in _VARIANTS:
        raise ValueError(f"Unknown token_affinity_coseg variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    cfg = _VARIANTS[name]
    width = max(8, int(int(cfg["width"]) * float(width_mult)))
    return TokenAffinityCosegCoSegmentor(
        in_channels=int(in_channels),
        num_classes=int(num_classes),
        width=width,
        depth=int(cfg["depth"]),
...
if __name__ == "__main__":
    ...
    m = build_token_affinity_coseg_co_segmentor(
        in_channels=3,
        num_classes=2,
        variant="token_affinity_coseg_tiny",
        width_mult=0.5,
    )
    out = m(x)
    print("token_affinity_coseg_tiny", {k: tuple(v.shape) for k, v in out.items() if isinstance(v, torch.Tensor)})
