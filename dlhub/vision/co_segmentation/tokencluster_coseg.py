...
_VARIANTS: dict[str, dict[str, int]] = {
    "tokencluster_coseg_tiny": {"embed_dim": 48, "depth": 2, "heads": 4},
    "tokencluster_coseg_small": {"embed_dim": 64, "depth": 3, "heads": 4},
    "tokencluster_coseg_base": {"embed_dim": 96, "depth": 4, "heads": 8},
}


class TokenclusterCoSeg(nn.Module):
...
def build_tokencluster_coseg_co_segmentor(
    *,
    in_channels: int,
    num_classes: int,
    set_size: int = 3,
    image_size: int = 64,
    variant: str = "tokencluster_coseg_small",
...
if __name__ == "__main__":
    ...
    m = build_tokencluster_coseg_co_segmentor(
        in_channels=3,
        num_classes=2,
        variant="tokencluster_coseg_tiny",
        width_mult=0.5,
    )
    out = m(x)
    print("tokencluster_coseg_tiny", tuple(out["logits"].shape), tuple(out["group_tokens"].shape))
