...
_VARIANTS: dict[str, dict[str, int]] = {
    "relation_coseg_tiny": {"embed_dim": 48, "depth": 2, "heads": 4},
    "relation_coseg_small": {"embed_dim": 64, "depth": 3, "heads": 4},
    "relation_coseg_base": {"embed_dim": 96, "depth": 4, "heads": 8},
}


class RelationCoSeg(nn.Module):
...
def build_relation_coseg_co_segmentor(
    *,
    in_channels: int,
    num_classes: int,
    set_size: int = 3,
    image_size: int = 64,
    variant: str = "relation_coseg_small",
...
if __name__ == "__main__":
    ...
    m = build_relation_coseg_co_segmentor(
        in_channels=3,
        num_classes=2,
        variant="relation_coseg_tiny",
        width_mult=0.5,
    )
    out = m(x)
    print("relation_coseg_tiny", tuple(out["logits"].shape), tuple(out["group_tokens"].shape))
