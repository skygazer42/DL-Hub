...
_VARIANTS: dict[str, dict[str, int]] = {
    "graphproto_coseg_tiny": {"embed_dim": 48, "depth": 2, "heads": 4},
    "graphproto_coseg_small": {"embed_dim": 64, "depth": 3, "heads": 4},
    "graphproto_coseg_base": {"embed_dim": 96, "depth": 4, "heads": 8},
}


class GraphprotoCoSeg(nn.Module):
...
def build_graphproto_coseg_co_segmentor(
    *,
    in_channels: int,
    num_classes: int,
    set_size: int = 3,
    image_size: int = 64,
    variant: str = "graphproto_coseg_small",
...
if __name__ == "__main__":
    ...
    m = build_graphproto_coseg_co_segmentor(
        in_channels=3,
        num_classes=2,
        variant="graphproto_coseg_tiny",
        width_mult=0.5,
    )
    out = m(x)
    print("graphproto_coseg_tiny", tuple(out["logits"].shape), tuple(out["group_tokens"].shape))
