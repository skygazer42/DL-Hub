...
_VARIANTS: dict[str, dict[str, int]] = {
    "dense_corr_coseg_tiny": {"width": 16, "depth": 1},
    "dense_corr_coseg_small": {"width": 24, "depth": 2},
    "dense_corr_coseg_base": {"width": 32, "depth": 3},
}


class DenseCorrCoseg(nn.Module):
    def __init__(self, *, in_channels: int, num_classes: int, width: int, depth: int, dropout: float = 0.0) -> None:
        ...
        self.fuser = GroupFusionBlock(c3, mode="attention", num_prototypes=4)
        self.prompt = None
        self.head = CoSegHead(
            in_channels=c3,
...
if __name__ == "__main__":
    ...
    m = build_dense_corr_coseg_co_segmentor(in_channels=3, num_classes=2, variant="dense_corr_coseg_tiny", width_mult=0.5)
    out = m(x)
