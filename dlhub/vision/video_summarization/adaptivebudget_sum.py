...
_VARIANTS: dict[str, dict[str, int]] = {
    "adaptivebudget_sum_tiny": {"width": 24, "depth": 2},
    "adaptivebudget_sum_small": {"width": 32, "depth": 3},
    "adaptivebudget_sum_base": {"width": 48, "depth": 4},
}


class AdaptivebudgetSumVideoSummarizer(nn.Module):
    """VASNet-style self-attention frame scorer (compact)."""
...
def build_adaptivebudget_sum_video_summarizer(
    *,
    in_channels: int,
    seq_len: int = 8,
    image_size: int = 64,
    variant: str = "adaptivebudget_sum_small",
    ...
    return AdaptivebudgetSumVideoSummarizer(
        in_channels=int(in_channels),
        width=width,
        depth=int(cfg["depth"]),
        dropout=float(dropout),
...
if __name__ == "__main__":
    ...
    m = build_adaptivebudget_sum_video_summarizer(
        in_channels=3, variant="adaptivebudget_sum_tiny", width_mult=0.5
    )
    out = m(x)
    print("adaptivebudget_sum_tiny", tuple(out["scores"].shape), tuple(out["summary_mask"].shape))
