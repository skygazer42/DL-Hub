...
_VARIANTS: dict[str, dict[str, int]] = {
    "patchsum_tiny": {"width": 24, "depth": 2},
    "patchsum_small": {"width": 32, "depth": 3},
    "patchsum_base": {"width": 48, "depth": 4},
}


class PatchsumVideoSummarizer(nn.Module):
...
def build_patchsum_video_summarizer(
    *,
    in_channels: int,
    seq_len: int = 8,
    image_size: int = 64,
    variant: str = "patchsum_small",
    ...
    return PatchsumVideoSummarizer(
        in_channels=int(in_channels),
        width=width,
        depth=int(cfg["depth"]),
        dropout=float(dropout),
...
if __name__ == "__main__":
    ...
    m = build_patchsum_video_summarizer(in_channels=3, variant="patchsum_tiny", width_mult=0.5)
    out = m(x)
    print("patchsum_tiny", tuple(out["scores"].shape), tuple(out["summary_mask"].shape))
