...
_VARIANTS: dict[str, dict[str, int]] = {
    "eventrank_tiny": {"width": 24, "depth": 2},
    "eventrank_small": {"width": 32, "depth": 3},
    "eventrank_base": {"width": 48, "depth": 4},
}


class EventrankVideoSummarizer(nn.Module):
...
def build_eventrank_video_summarizer(
    *,
    in_channels: int,
    seq_len: int = 8,
    image_size: int = 64,
    variant: str = "eventrank_small",
    ...
    return EventrankVideoSummarizer(
        in_channels=int(in_channels),
        width=width,
        depth=int(cfg["depth"]),
        dropout=float(dropout),
...
if __name__ == "__main__":
    ...
    m = build_eventrank_video_summarizer(in_channels=3, variant="eventrank_tiny", width_mult=0.5)
    out = m(x)
    print("eventrank_tiny", {k: tuple(v.shape) for k, v in out.items() if isinstance(v, torch.Tensor)})
