...
_VARIANTS: dict[str, dict[str, int]] = {
    "sumformer_tiny": {"width": 24, "depth": 2},
    "sumformer_small": {"width": 32, "depth": 3},
    "sumformer_base": {"width": 48, "depth": 4},
}


class SumformerVideoSummarizer(nn.Module):
...
def build_sumformer_video_summarizer(
    *,
    in_channels: int,
    seq_len: int = 8,
    image_size: int = 64,
    variant: str = "sumformer_small",
    ...
    return SumformerVideoSummarizer(
        in_channels=int(in_channels),
        width=width,
        depth=int(cfg["depth"]),
        dropout=float(dropout),
