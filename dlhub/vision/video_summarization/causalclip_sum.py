...
_VARIANTS: dict[str, dict[str, int]] = {
    "causalclip_sum_tiny": {"width": 24, "depth": 2},
    "causalclip_sum_small": {"width": 32, "depth": 3},
    "causalclip_sum_base": {"width": 48, "depth": 4},
}


class CausalclipSumVideoSummarizer(nn.Module):
...
def build_causalclip_sum_video_summarizer(
    *,
    in_channels: int,
    seq_len: int = 8,
    image_size: int = 64,
    variant: str = "causalclip_sum_small",
    ...
    return CausalclipSumVideoSummarizer(
        in_channels=int(in_channels),
        width=width,
        depth=int(cfg["depth"]),
        dropout=float(dropout),
