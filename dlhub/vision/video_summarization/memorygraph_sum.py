...
_VARIANTS: dict[str, dict[str, int]] = {
    "memorygraph_sum_tiny": {"width": 24, "depth": 2},
    "memorygraph_sum_small": {"width": 32, "depth": 3},
    "memorygraph_sum_base": {"width": 48, "depth": 4},
}


class MemorygraphSumVideoSummarizer(nn.Module):
...
def build_memorygraph_sum_video_summarizer(
    *,
    in_channels: int,
    seq_len: int = 8,
    image_size: int = 64,
    variant: str = "memorygraph_sum_small",
    ...
    return MemorygraphSumVideoSummarizer(
        in_channels=int(in_channels),
        width=width,
        depth=int(cfg["depth"]),
        dropout=float(dropout),
