...
_VARIANTS: dict[str, dict[str, int]] = {
    "streamsum_tiny": {"width": 24, "depth": 2},
    "streamsum_small": {"width": 32, "depth": 3},
    "streamsum_base": {"width": 48, "depth": 4},
}


class StreamsumVideoSummarizer(nn.Module):
...
def build_streamsum_video_summarizer(
    *,
    in_channels: int,
    seq_len: int = 8,
    image_size: int = 64,
    variant: str = "streamsum_small",
    ...
    return StreamsumVideoSummarizer(
        in_channels=int(in_channels),
        width=width,
        depth=int(cfg["depth"]),
        dropout=float(dropout),
...
if __name__ == "__main__":
    ...
    m = build_streamsum_video_summarizer(in_channels=3, variant="streamsum_tiny", width_mult=0.5)
    out = m(x)
    print("streamsum_tiny", tuple(out["scores"].shape), tuple(out["summary_mask"].shape))
