...
_VARIANTS: dict[str, dict[str, int]] = {
    "segmentformer_sum_tiny": {"width": 24, "depth": 2},
    "segmentformer_sum_small": {"width": 32, "depth": 3},
    "segmentformer_sum_base": {"width": 48, "depth": 4},
}


class SegmentformerSumVideoSummarizer(nn.Module):
...
def build_segmentformer_sum_video_summarizer(
    *,
    ...
    seq_len: int = 8,
    image_size: int = 64,
    variant: str = "segmentformer_sum_small",
    ...
    return SegmentformerSumVideoSummarizer(
        in_channels=int(in_channels),
        width=width,
        depth=int(cfg["depth"]),
        dropout=float(dropout),
