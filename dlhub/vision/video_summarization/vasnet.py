...
_VARIANTS: dict[str, dict[str, int]] = {
    "vasnet_tiny": {"width": 24, "depth": 2},
    "vasnet_small": {"width": 32, "depth": 3},
    "vasnet_base": {"width": 48, "depth": 4},
}


class VASNetVideoSummarizer(nn.Module):
...
def build_vasnet_video_summarizer(
    *,
    in_channels: int,
    seq_len: int = 8,
    image_size: int = 64,
    variant: str = "vasnet_small",
...
if __name__ == "__main__":
    ...
    m = build_vasnet_video_summarizer(in_channels=3, variant="vasnet_tiny", width_mult=0.5)
    out = m(x)
    print("vasnet_tiny", tuple(out["scores"].shape), tuple(out["summary_mask"].shape))
