...
_VARIANTS: dict[str, dict[str, int]] = {
    "storysum_tiny": {"width": 24, "depth": 2},
    "storysum_small": {"width": 32, "depth": 3},
    "storysum_base": {"width": 48, "depth": 4},
}


class StorysumVideoSummarizer(nn.Module):
...
def build_storysum_video_summarizer(
    *,
    in_channels: int,
    seq_len: int = 8,
    image_size: int = 64,
    variant: str = "storysum_small",
    ...
    return StorysumVideoSummarizer(
        in_channels=int(in_channels),
        width=width,
        depth=int(cfg["depth"]),
        dropout=float(dropout),
...
if __name__ == "__main__":
    ...
    m = build_storysum_video_summarizer(in_channels=3, variant="storysum_tiny", width_mult=0.5)
    out = m(x)
    print("storysum_tiny", {k: tuple(v.shape) for k, v in out.items() if isinstance(v, torch.Tensor)})
