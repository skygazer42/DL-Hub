...
_VARIANTS: dict[str, dict[str, int]] = {
    "sparsegist_sum_tiny": {"width": 24, "depth": 2},
    "sparsegist_sum_small": {"width": 32, "depth": 3},
    "sparsegist_sum_base": {"width": 48, "depth": 4},
}


class SparsegistSumVideoSummarizer(nn.Module):
...
def build_sparsegist_sum_video_summarizer(
    *,
    ...
    seq_len: int = 8,
    image_size: int = 64,
    variant: str = "sparsegist_sum_small",
    ...
    return SparsegistSumVideoSummarizer(
        in_channels=int(in_channels),
        width=width,
        depth=int(cfg["depth"]),
        dropout=float(dropout),
...
if __name__ == "__main__":
    ...
    m = build_sparsegist_sum_video_summarizer(in_channels=3, variant="sparsegist_sum_tiny", width_mult=0.5)
    out = m(x)
    print("sparsegist_sum_tiny", {k: tuple(v.shape) for k, v in out.items() if isinstance(v, torch.Tensor)})
