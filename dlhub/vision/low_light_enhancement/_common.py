...
class ToyEnhancer(nn.Module):
    ...
    def forward(self, image):
        x = check_nchw(image)
        illum = torch.sigmoid(self.head(self.enc(x)))
        enhanced = torch.clamp(x + 0.5 * illum, 0.0, 1.0)
...
def smoke_test_model(builder, variant: str):
    out = builder(in_channels=3, variant=variant, width_mult=0.5)(torch.rand(2, 3, 64, 64))
