...
class ToyModel(nn.Module):
    ...
    def forward(self,image): logits=self.head(self.enc(image)); return {'logits': logits, 'defect_map': torch.sigmoid(logits)}
...
def smoke_test_model(builder, variant:str): out=builder(in_channels=3, variant=variant, width_mult=0.5)(torch.randn(2,3,64,64)); print(variant, tuple(out['defect_map'].shape))
