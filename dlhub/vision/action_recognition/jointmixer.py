...
class JointmixerSkeletonClassifier(nn.Module):
    def __init__(
        self,
        *,
        in_channels: int,
...
_VARIANTS: dict[str, dict] = {
    "jointmixer_tiny": {"width": 32, "depth": 2, "kt": 5},
    "jointmixer_small": {"width": 48, "depth": 3, "kt": 5},
    "jointmixer_base": {"width": 64, "depth": 4, "kt": 9},
}


def build_jointmixer_skeleton_classifier(
    *,
    in_channels: int,
    num_classes: int,
    num_joints: int = 17,
    seq_len: int = 32,
    variant: str = "jointmixer_small",
    ...
    return JointmixerSkeletonClassifier(
...
if __name__ == "__main__":
    ...
    m = build_jointmixer_skeleton_classifier(
        in_channels=3,
        num_classes=6,
        num_joints=17,
        seq_len=32,
        variant="jointmixer_tiny",
        width_mult=0.5,
        dropout=0.0,
    )
    y = m(x)
    print("jointmixer_tiny", tuple(y.shape))
    loss = y.mean()
    loss.backward()
    print("ok")
