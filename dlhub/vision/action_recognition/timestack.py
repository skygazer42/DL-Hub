...
class TimestackVideoClassifier(nn.Module):
    def __init__(
        self,
        *,
        in_channels: int,
...
_VARIANTS: dict[str, dict] = {
    "timestack_tiny": {"patch": 8, "embed": 96, "depth": 2, "heads": 4},
    "timestack_small": {"patch": 8, "embed": 128, "depth": 3, "heads": 4},
    "timestack_base": {"patch": 4, "embed": 160, "depth": 4, "heads": 8},
}


def build_timestack_video_classifier(
    *,
    in_channels: int,
    num_classes: int,
    variant: str = "timestack_small",
    image_size: int = 64,
    frames: int = 8,
    ...
    return TimestackVideoClassifier(
...
if __name__ == "__main__":
    ...
    m = build_timestack_video_classifier(
        in_channels=3, num_classes=6, variant="timestack_tiny", width_mult=0.5, dropout=0.0
    )
    y = m(x)
    print("timestack_tiny", tuple(y.shape))
    loss = y.mean()
    loss.backward()
    print("ok")
