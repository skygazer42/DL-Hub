...
class AagcnSkeletonClassifier(nn.Module):
    def __init__(
        self,
        *,
        in_channels: int,
...
_VARIANTS: dict[str, dict] = {
    "aagcn_tiny": {"width": 32, "depth": 2, "kt": 5},
    "aagcn_small": {"width": 48, "depth": 3, "kt": 5},
    "aagcn_base": {"width": 64, "depth": 4, "kt": 9},
}


def build_aagcn_skeleton_classifier(
    *,
    in_channels: int,
    num_classes: int,
    num_joints: int = 17,
    seq_len: int = 32,
    variant: str = "aagcn_small",
    ...
    return AagcnSkeletonClassifier(
...
if __name__ == "__main__":
    ...
    m = build_aagcn_skeleton_classifier(
        in_channels=3,
        num_classes=6,
        num_joints=17,
        seq_len=32,
    ...
    y = m(x)
    print("aagcn_tiny", tuple(y.shape))
    loss = y.mean()
    loss.backward()
    print("ok")
