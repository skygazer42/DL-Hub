...
class TinySalientBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        elif self.mode == "poolnet":
            local = local + self.mix(F.avg_pool2d(h, 3, 1, 1))
        elif self.mode == "bassnet":
...
class TinySalientDetector(nn.Module):
    ...
    def forward(self, image: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        return {
            ...
            "saliency": torch.sigmoid(mask_logits),
        }
...
def smoke_test_salient_detector(builder, variant: str) -> None:
    ...
    print(variant, tuple(out["mask_logits"].shape), tuple(out["boundary_logits"].shape))
    print("ok")
