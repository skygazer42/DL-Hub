    ...
    for family in _FAMILIES:
        for size in _SIZES:
            ...
            def _builder(cfg: BuildConfig, family: str = family, variant: str = variant):
                module = importlib.import_module(f"dlhub.vision.video_frame_interpolation.{family}")
                fn = getattr(module, f"build_{family}_interpolator")
                return fn(
...
def list_local_arches() -> list[str]:
    return [f"vfi:{name}" for name in sorted(_REGISTRY)]
...
def build_local_model(arch_id: str, *, in_channels: int, width_mult: float = 1.0):
    ...
    if prefix not in {"vfi", "local"}:
        raise ValueError(f"Unsupported video frame interpolation prefix: {prefix!r} (arch_id={arch_id!r})")
