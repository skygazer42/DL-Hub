...
_VARIANTS: dict[str, dict[str, int]] = {
    "hairclip_parse_tiny": {"width": 16, "depth": 1},
    "hairclip_parse_small": {"width": 24, "depth": 2},
    "hairclip_parse_base": {"width": 32, "depth": 3},
}


class HairclipParseFaceParser(nn.Module):
...
def build_hairclip_parse_face_parser(
    *,
    in_channels: int,
    num_classes: int,
    image_size: int = 64,
    variant: str = "hairclip_parse_small",
    ...
    return HairclipParseFaceParser(
        in_channels=int(in_channels),
        num_classes=int(num_classes),
        width=width,
        depth=int(cfg["depth"]),
...
if __name__ == "__main__":
    ...
    m = build_hairclip_parse_face_parser(
        in_channels=3,
        num_classes=11,
        variant="hairclip_parse_tiny",
        width_mult=0.5,
    )
    out = m(x)
    print("hairclip_parse_tiny", tuple(out["logits"].shape), tuple(out["boundary_map"].shape))
