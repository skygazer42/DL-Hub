...
_VARIANTS: dict[str, dict[str, int]] = {
    "styleparse_tiny": {"width": 16, "depth": 1},
    "styleparse_small": {"width": 24, "depth": 2},
    "styleparse_base": {"width": 32, "depth": 3},
}


class StyleparseFaceParser(nn.Module):
...
def build_styleparse_face_parser(
    *,
    in_channels: int,
    num_classes: int,
    image_size: int = 64,
    variant: str = "styleparse_small",
...
if __name__ == "__main__":
    ...
    m = build_styleparse_face_parser(
        in_channels=3,
        num_classes=11,
        variant="styleparse_tiny",
        width_mult=0.5,
    )
    out = m(x)
    print("styleparse_tiny", tuple(out["logits"].shape), tuple(out["boundary_map"].shape))
