...
_VARIANTS: dict[str, dict[str, int]] = {
    "structureparse_tiny": {"width": 16, "depth": 1},
    "structureparse_small": {"width": 24, "depth": 2},
    "structureparse_base": {"width": 32, "depth": 3},
}


class StructureparseFaceParser(nn.Module):
...
def build_structureparse_face_parser(
    *,
    in_channels: int,
    num_classes: int,
    image_size: int = 64,
    variant: str = "structureparse_small",
    ...
    return StructureparseFaceParser(
        in_channels=int(in_channels),
        num_classes=int(num_classes),
        width=width,
        depth=int(cfg["depth"]),
...
if __name__ == "__main__":
    ...
    m = build_structureparse_face_parser(
        in_channels=3,
        num_classes=11,
        variant="structureparse_tiny",
        width_mult=0.5,
    )
    out = m(x)
    print("structureparse_tiny", tuple(out["logits"].shape), tuple(out["boundary_map"].shape))
