...
_VARIANTS: dict[str, dict[str, int]] = {
    "bisenet_parse2_tiny": {"width": 16, "depth": 1},
    "bisenet_parse2_small": {"width": 24, "depth": 2},
    "bisenet_parse2_base": {"width": 32, "depth": 3},
}


class BisenetParse2FaceParser(nn.Module):
...
def build_bisenet_parse2_face_parser(
    *,
    in_channels: int,
    num_classes: int,
    image_size: int = 64,
    variant: str = "bisenet_parse2_small",
    ...
    return BisenetParse2FaceParser(
        in_channels=int(in_channels),
        num_classes=int(num_classes),
        width=width,
        depth=int(cfg["depth"]),
...
if __name__ == "__main__":
    ...
    m = build_bisenet_parse2_face_parser(
        in_channels=3,
        num_classes=11,
        variant="bisenet_parse2_tiny",
        width_mult=0.5,
    )
    out = m(x)
    print("bisenet_parse2_tiny", tuple(out["logits"].shape), tuple(out["boundary_map"].shape))
