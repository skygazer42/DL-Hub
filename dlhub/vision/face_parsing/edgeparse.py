...
_VARIANTS: dict[str, dict[str, int]] = {
    "edgeparse_tiny": {"width": 16, "depth": 1},
    "edgeparse_small": {"width": 24, "depth": 2},
    "edgeparse_base": {"width": 32, "depth": 3},
}


class EdgeparseFaceParser(nn.Module):
...
def build_edgeparse_face_parser(
    *,
    in_channels: int,
    num_classes: int,
    image_size: int = 64,
    variant: str = "edgeparse_small",
...
if __name__ == "__main__":
    ...
    m = build_edgeparse_face_parser(
        in_channels=3,
        num_classes=11,
        variant="edgeparse_tiny",
        width_mult=0.5,
    )
    out = m(x)
    print("edgeparse_tiny", tuple(out["logits"].shape), tuple(out["boundary_map"].shape))
