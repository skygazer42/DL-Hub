...
_VARIANTS: dict[str, dict[str, int]] = {
    "cheekparse_tiny": {"width": 16, "depth": 1},
    "cheekparse_small": {"width": 24, "depth": 2},
    "cheekparse_base": {"width": 32, "depth": 3},
}


class CheekparseFaceParser(nn.Module):
...
def build_cheekparse_face_parser(
    *,
    in_channels: int,
    num_classes: int,
    image_size: int = 64,
    variant: str = "cheekparse_small",
...
if __name__ == "__main__":
    ...
    m = build_cheekparse_face_parser(
        in_channels=3,
        num_classes=11,
        variant="cheekparse_tiny",
        width_mult=0.5,
    )
    out = m(x)
    print("cheekparse_tiny", tuple(out["logits"].shape), tuple(out["boundary_map"].shape))
