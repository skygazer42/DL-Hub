...
_VARIANTS: dict[str, dict[str, int]] = {
    "skinparse_tiny": {"width": 16, "depth": 1},
    "skinparse_small": {"width": 24, "depth": 2},
    "skinparse_base": {"width": 32, "depth": 3},
}


class SkinparseFaceParser(nn.Module):
...
def build_skinparse_face_parser(
    *,
    in_channels: int,
    num_classes: int,
    image_size: int = 64,
    variant: str = "skinparse_small",
...
if __name__ == "__main__":
    ...
    m = build_skinparse_face_parser(
        in_channels=3,
        num_classes=11,
        variant="skinparse_tiny",
        width_mult=0.5,
    )
    out = m(x)
    print("skinparse_tiny", tuple(out["logits"].shape), tuple(out["boundary_map"].shape))
