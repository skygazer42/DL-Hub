...
_VARIANTS: dict[str, dict[str, int]] = {
    "hparsenet_tiny": {"width": 16, "depth": 1},
    "hparsenet_small": {"width": 24, "depth": 2},
    "hparsenet_base": {"width": 32, "depth": 3},
}


class HparsenetFaceParser(nn.Module):
...
def build_hparsenet_face_parser(
    *,
    in_channels: int,
    num_classes: int,
    image_size: int = 64,
    variant: str = "hparsenet_small",
...
if __name__ == "__main__":
    ...
    m = build_hparsenet_face_parser(
        in_channels=3,
        num_classes=11,
        variant="hparsenet_tiny",
        width_mult=0.5,
    )
    out = m(x)
    print("hparsenet_tiny", tuple(out["logits"].shape), tuple(out["boundary_map"].shape))
