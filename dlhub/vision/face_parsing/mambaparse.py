...
_VARIANTS: dict[str, dict[str, int]] = {
    "mambaparse_tiny": {"width": 16, "depth": 1},
    "mambaparse_small": {"width": 24, "depth": 2},
    "mambaparse_base": {"width": 32, "depth": 3},
}


class MambaparseFaceParser(nn.Module):
...
def build_mambaparse_face_parser(
    *,
    in_channels: int,
    num_classes: int,
    image_size: int = 64,
    variant: str = "mambaparse_small",
...
if __name__ == "__main__":
    ...
    m = build_mambaparse_face_parser(
        in_channels=3,
        num_classes=11,
        variant="mambaparse_tiny",
        width_mult=0.5,
    )
    out = m(x)
    print("mambaparse_tiny", tuple(out["logits"].shape), tuple(out["boundary_map"].shape))
