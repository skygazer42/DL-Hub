...
_VARIANTS: dict[str, dict[str, int]] = {
    "face_bisenet_tiny": {"width": 16, "depth": 1},
    "face_bisenet_small": {"width": 24, "depth": 2},
    "face_bisenet_base": {"width": 32, "depth": 3},
}


class FaceBisenetFaceParser(nn.Module):
...
def build_face_bisenet_face_parser(
    *,
    in_channels: int,
    num_classes: int,
    image_size: int = 64,
    variant: str = "face_bisenet_small",
...
if __name__ == "__main__":
    ...
    m = build_face_bisenet_face_parser(
        in_channels=3,
        num_classes=11,
        variant="face_bisenet_tiny",
        width_mult=0.5,
    )
    out = m(x)
    print("face_bisenet_tiny", tuple(out["logits"].shape), tuple(out["boundary_map"].shape))
