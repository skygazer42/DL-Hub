...
_VARIANTS: dict[str, dict[str, int]] = {
    "mouthparse_tiny": {"width": 16, "depth": 1},
    "mouthparse_small": {"width": 24, "depth": 2},
    "mouthparse_base": {"width": 32, "depth": 3},
}


class MouthparseFaceParser(nn.Module):
...
def build_mouthparse_face_parser(
    *,
    in_channels: int,
    num_classes: int,
    image_size: int = 64,
    variant: str = "mouthparse_small",
...
if __name__ == "__main__":
    ...
    m = build_mouthparse_face_parser(
        in_channels=3,
        num_classes=11,
        variant="mouthparse_tiny",
        width_mult=0.5,
    )
    out = m(x)
    print("mouthparse_tiny", tuple(out["logits"].shape), tuple(out["boundary_map"].shape))
