...
_VARIANTS: dict[str, dict[str, int]] = {
    "hierparse_tiny": {"width": 16, "depth": 1},
    "hierparse_small": {"width": 24, "depth": 2},
    "hierparse_base": {"width": 32, "depth": 3},
}


class HierparseFaceParser(nn.Module):
...
def build_hierparse_face_parser(
    *,
    in_channels: int,
    num_classes: int,
    image_size: int = 64,
    variant: str = "hierparse_small",
...
if __name__ == "__main__":
    ...
    m = build_hierparse_face_parser(
        in_channels=3,
        num_classes=11,
        variant="hierparse_tiny",
        width_mult=0.5,
    )
    out = m(x)
    print("hierparse_tiny", tuple(out["logits"].shape), tuple(out["boundary_map"].shape))
