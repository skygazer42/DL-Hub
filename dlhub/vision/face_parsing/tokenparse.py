...
_VARIANTS: dict[str, dict[str, int]] = {
    "tokenparse_tiny": {"width": 16, "depth": 1},
    "tokenparse_small": {"width": 24, "depth": 2},
    "tokenparse_base": {"width": 32, "depth": 3},
}


class TokenparseFaceParser(nn.Module):
...
def build_tokenparse_face_parser(
    *,
    in_channels: int,
    num_classes: int,
    image_size: int = 64,
    variant: str = "tokenparse_small",
...
if __name__ == "__main__":
    ...
    m = build_tokenparse_face_parser(
        in_channels=3,
        num_classes=11,
        variant="tokenparse_tiny",
        width_mult=0.5,
    )
    out = m(x)
    print("tokenparse_tiny", tuple(out["logits"].shape), tuple(out["boundary_map"].shape))
