...
_VARIANTS: dict[str, dict[str, int]] = {
    "eyeparse_tiny": {"width": 16, "depth": 1},
    "eyeparse_small": {"width": 24, "depth": 2},
    "eyeparse_base": {"width": 32, "depth": 3},
}


class EyeparseFaceParser(nn.Module):
...
def build_eyeparse_face_parser(
    *,
    in_channels: int,
    num_classes: int,
    image_size: int = 64,
    variant: str = "eyeparse_small",
...
if __name__ == "__main__":
    ...
    m = build_eyeparse_face_parser(
        in_channels=3,
        num_classes=11,
        variant="eyeparse_tiny",
        width_mult=0.5,
    )
    out = m(x)
    print("eyeparse_tiny", tuple(out["logits"].shape), tuple(out["boundary_map"].shape))
