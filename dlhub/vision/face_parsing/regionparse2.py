...
_VARIANTS: dict[str, dict[str, int]] = {
    "regionparse2_tiny": {"width": 16, "depth": 1},
    "regionparse2_small": {"width": 24, "depth": 2},
    "regionparse2_base": {"width": 32, "depth": 3},
}


class Regionparse2FaceParser(nn.Module):
...
def build_regionparse2_face_parser(
    *,
    in_channels: int,
    num_classes: int,
    image_size: int = 64,
    variant: str = "regionparse2_small",
...
if __name__ == "__main__":
    ...
    m = build_regionparse2_face_parser(
        in_channels=3,
        num_classes=11,
        variant="regionparse2_tiny",
        width_mult=0.5,
    )
    out = m(x)
    print("regionparse2_tiny", tuple(out["logits"].shape), tuple(out["boundary_map"].shape))
