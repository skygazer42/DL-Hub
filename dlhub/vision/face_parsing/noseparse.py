...
_VARIANTS: dict[str, dict[str, int]] = {
    "noseparse_tiny": {"width": 16, "depth": 1},
    "noseparse_small": {"width": 24, "depth": 2},
    "noseparse_base": {"width": 32, "depth": 3},
}


class NoseparseFaceParser(nn.Module):
...
def build_noseparse_face_parser(
    *,
    in_channels: int,
    num_classes: int,
    image_size: int = 64,
    variant: str = "noseparse_small",
...
if __name__ == "__main__":
    ...
    m = build_noseparse_face_parser(
        in_channels=3,
        num_classes=11,
        variant="noseparse_tiny",
        width_mult=0.5,
    )
    out = m(x)
    print("noseparse_tiny", tuple(out["logits"].shape), tuple(out["boundary_map"].shape))
