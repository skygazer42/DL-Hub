...
_VARIANTS: dict[str, dict[str, int]] = {
    "browparse_tiny": {"width": 16, "depth": 1},
    "browparse_small": {"width": 24, "depth": 2},
    "browparse_base": {"width": 32, "depth": 3},
}


class BrowparseFaceParser(nn.Module):
...
def build_browparse_face_parser(
    *,
    in_channels: int,
    num_classes: int,
    image_size: int = 64,
    variant: str = "browparse_small",
...
if __name__ == "__main__":
    ...
    m = build_browparse_face_parser(
        in_channels=3,
        num_classes=11,
        variant="browparse_tiny",
        width_mult=0.5,
    )
    out = m(x)
    print("browparse_tiny", tuple(out["logits"].shape), tuple(out["boundary_map"].shape))
