...
_VARIANTS: dict[str, dict[str, int]] = {
    "lipparse_tiny": {"width": 16, "depth": 1},
    "lipparse_small": {"width": 24, "depth": 2},
    "lipparse_base": {"width": 32, "depth": 3},
}


class LipparseFaceParser(nn.Module):
...
def build_lipparse_face_parser(
    *,
    in_channels: int,
    num_classes: int,
    image_size: int = 64,
    variant: str = "lipparse_small",
...
if __name__ == "__main__":
    ...
    m = build_lipparse_face_parser(
        in_channels=3,
        num_classes=11,
        variant="lipparse_tiny",
        width_mult=0.5,
    )
    out = m(x)
    print("lipparse_tiny", tuple(out["logits"].shape), tuple(out["boundary_map"].shape))
