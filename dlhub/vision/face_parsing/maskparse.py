...
_VARIANTS: dict[str, dict[str, int]] = {
    "maskparse_tiny": {"width": 16, "depth": 1},
    "maskparse_small": {"width": 24, "depth": 2},
    "maskparse_base": {"width": 32, "depth": 3},
}


class MaskparseFaceParser(nn.Module):
...
def build_maskparse_face_parser(
    *,
    in_channels: int,
    num_classes: int,
    image_size: int = 64,
    variant: str = "maskparse_small",
...
if __name__ == "__main__":
    ...
    m = build_maskparse_face_parser(
        in_channels=3,
        num_classes=11,
        variant="maskparse_tiny",
        width_mult=0.5,
    )
    out = m(x)
    print("maskparse_tiny", tuple(out["logits"].shape), tuple(out["boundary_map"].shape))
