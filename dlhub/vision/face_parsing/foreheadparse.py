...
_VARIANTS: dict[str, dict[str, int]] = {
    "foreheadparse_tiny": {"width": 16, "depth": 1},
    "foreheadparse_small": {"width": 24, "depth": 2},
    "foreheadparse_base": {"width": 32, "depth": 3},
}


class ForeheadparseFaceParser(nn.Module):
...
def build_foreheadparse_face_parser(
    *,
    in_channels: int,
    num_classes: int,
    image_size: int = 64,
    variant: str = "foreheadparse_small",
    ...
    return ForeheadparseFaceParser(
        in_channels=int(in_channels),
        num_classes=int(num_classes),
        width=width,
        depth=int(cfg["depth"]),
...
if __name__ == "__main__":
    ...
    m = build_foreheadparse_face_parser(
        in_channels=3,
        num_classes=11,
        variant="foreheadparse_tiny",
        width_mult=0.5,
    )
    out = m(x)
    print("foreheadparse_tiny", tuple(out["logits"].shape), tuple(out["boundary_map"].shape))
