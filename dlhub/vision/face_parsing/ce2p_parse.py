...
_VARIANTS: dict[str, dict[str, int]] = {
    "ce2p_parse_tiny": {"width": 16, "depth": 1},
    "ce2p_parse_small": {"width": 24, "depth": 2},
    "ce2p_parse_base": {"width": 32, "depth": 3},
}


class Ce2pParseFaceParser(nn.Module):
...
def build_ce2p_parse_face_parser(
    *,
    in_channels: int,
    num_classes: int,
    image_size: int = 64,
    variant: str = "ce2p_parse_small",
...
if __name__ == "__main__":
    ...
    m = build_ce2p_parse_face_parser(
        in_channels=3,
        num_classes=11,
        variant="ce2p_parse_tiny",
        width_mult=0.5,
    )
    out = m(x)
    print("ce2p_parse_tiny", tuple(out["logits"].shape), tuple(out["boundary_map"].shape))
