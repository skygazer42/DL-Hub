...
_VARIANTS: dict[str, dict[str, int]] = {
    "promptparse_tiny": {"width": 16, "depth": 1},
    "promptparse_small": {"width": 24, "depth": 2},
    "promptparse_base": {"width": 32, "depth": 3},
}


class PromptparseFaceParser(nn.Module):
...
def build_promptparse_face_parser(
    *,
    in_channels: int,
    num_classes: int,
    image_size: int = 64,
    variant: str = "promptparse_small",
...
if __name__ == "__main__":
    ...
    m = build_promptparse_face_parser(
        in_channels=3,
        num_classes=11,
        variant="promptparse_tiny",
        width_mult=0.5,
    )
    out = m(x)
    print("promptparse_tiny", tuple(out["logits"].shape), tuple(out["boundary_map"].shape))
