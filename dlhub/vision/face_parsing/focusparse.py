...
_VARIANTS: dict[str, dict[str, int]] = {
    "focusparse_tiny": {"width": 16, "depth": 1},
    "focusparse_small": {"width": 24, "depth": 2},
    "focusparse_base": {"width": 32, "depth": 3},
}


class FocusparseFaceParser(nn.Module):
...
def build_focusparse_face_parser(
    *,
    in_channels: int,
    num_classes: int,
    image_size: int = 64,
    variant: str = "focusparse_small",
...
if __name__ == "__main__":
    ...
    m = build_focusparse_face_parser(
        in_channels=3,
        num_classes=11,
        variant="focusparse_tiny",
        width_mult=0.5,
    )
    out = m(x)
    print("focusparse_tiny", tuple(out["logits"].shape), tuple(out["boundary_map"].shape))
