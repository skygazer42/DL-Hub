...
_VARIANTS: dict[str, dict[str, int]] = {
    "segface_tiny": {"width": 16, "depth": 1},
    "segface_small": {"width": 24, "depth": 2},
    "segface_base": {"width": 32, "depth": 3},
}


class SegFaceParser(nn.Module):
...
def build_segface_face_parser(
    *,
    in_channels: int,
    num_classes: int,
    image_size: int = 64,
    variant: str = "segface_small",
...
if __name__ == "__main__":
    ...
    m = build_segface_face_parser(
        in_channels=3,
        num_classes=11,
        variant="segface_tiny",
        width_mult=0.5,
    )
    out = m(x)
    print("segface_tiny", tuple(out["logits"].shape), tuple(out["boundary_map"].shape))
