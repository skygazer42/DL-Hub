...
_VARIANTS: dict[str, dict[str, int]] = {
    "decoupleparse_tiny": {"width": 16, "depth": 1},
    "decoupleparse_small": {"width": 24, "depth": 2},
    "decoupleparse_base": {"width": 32, "depth": 3},
}


class DecoupleparseFaceParser(nn.Module):
...
def build_decoupleparse_face_parser(
    *,
    in_channels: int,
    num_classes: int,
    image_size: int = 64,
    variant: str = "decoupleparse_small",
    ...
    return DecoupleparseFaceParser(
        in_channels=int(in_channels),
        num_classes=int(num_classes),
        width=width,
        depth=int(cfg["depth"]),
...
if __name__ == "__main__":
    ...
    m = build_decoupleparse_face_parser(
        in_channels=3,
        num_classes=11,
        variant="decoupleparse_tiny",
        width_mult=0.5,
    )
    out = m(x)
    print("decoupleparse_tiny", tuple(out["logits"].shape), tuple(out["boundary_map"].shape))
