...
_VARIANTS: dict[str, dict[str, int]] = {
    "contourparse_tiny": {"width": 16, "depth": 1},
    "contourparse_small": {"width": 24, "depth": 2},
    "contourparse_base": {"width": 32, "depth": 3},
}


class ContourparseFaceParser(nn.Module):
...
def build_contourparse_face_parser(
    *,
    in_channels: int,
    num_classes: int,
    image_size: int = 64,
    variant: str = "contourparse_small",
...
if __name__ == "__main__":
    ...
    m = build_contourparse_face_parser(
        in_channels=3,
        num_classes=11,
        variant="contourparse_tiny",
        width_mult=0.5,
    )
    out = m(x)
    print("contourparse_tiny", tuple(out["logits"].shape), tuple(out["boundary_map"].shape))
