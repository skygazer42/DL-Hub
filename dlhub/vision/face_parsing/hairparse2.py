...
_VARIANTS: dict[str, dict[str, int]] = {
    "hairparse2_tiny": {"width": 16, "depth": 1},
    "hairparse2_small": {"width": 24, "depth": 2},
    "hairparse2_base": {"width": 32, "depth": 3},
}


class Hairparse2FaceParser(nn.Module):
...
def build_hairparse2_face_parser(
    *,
    in_channels: int,
    num_classes: int,
    image_size: int = 64,
    variant: str = "hairparse2_small",
...
if __name__ == "__main__":
    ...
    m = build_hairparse2_face_parser(
        in_channels=3,
        num_classes=11,
        variant="hairparse2_tiny",
        width_mult=0.5,
    )
    out = m(x)
    print("hairparse2_tiny", tuple(out["logits"].shape), tuple(out["boundary_map"].shape))
