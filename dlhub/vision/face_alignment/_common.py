...
class CompactFaceAligner(nn.Module):
    def __init__(self, *, family: str, in_channels: int, width: int, depth: int, num_points: int):
        ...
        self.head = nn.Linear(c, int(num_points) * 2)

    def forward(self, image):
        feat = self.net(check_nchw(image))
        pooled = F.adaptive_avg_pool2d(feat, (1, 1)).flatten(1)
        landmarks = self.head(pooled).view(image.shape[0], self.num_points, 2)
...
def smoke_test_align(builder, variant: str):
    model = builder(in_channels=3, variant=variant, width_mult=0.5, num_points=68)
    out = model(torch.randn(2, 3, 128, 128))
    print(variant, tuple(out["landmarks"].shape))
