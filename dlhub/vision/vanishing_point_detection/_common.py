...
class TinyVPBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.mode in {"direct", "heatmap", "line", "horizon", "graph"}:
...
class TinyVPDetector(nn.Module):
    def __init__(
        ...
        self.blocks = nn.ModuleList(
            [TinyVPBlock(channels=int(width), mode=str(mode)) for _ in range(max(1, int(depth)))]
        )
        self.coords_head = nn.Linear(int(width), 2)
        self.horizon_head = nn.Linear(int(width), 2)
        self.conf_head = nn.Linear(int(width), 1)
        self.heatmap_head = nn.Conv2d(int(width), 1, 1)
    ...
    def forward(self, image: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        vp_coords = torch.tanh(self.coords_head(pooled))
        horizon_line = self.horizon_head(pooled)
        vp_heatmap = torch.sigmoid(self.heatmap_head(feat))
...
def smoke_test_vp_detector(builder, variant: str) -> None:
    model = builder(in_channels=3, variant=variant, width_mult=0.5)
    out = model(torch.randn(2, 3, 64, 64))
    print(
        variant,
        tuple(out["vp_coords"].shape),
        tuple(out["vp_heatmap"].shape),
        tuple(out["confidence"].shape),
    )
    print("ok")
