...
class ToyGeoLocalizer(nn.Module):
    ...
    def forward(self, query, gallery=None):
        q = F.normalize(self.proj(self.encoder(query)), dim=1)
        out = {"query_embedding": q}
        if gallery is not None:
            g = F.normalize(self.proj(self.encoder(gallery)), dim=1)
            out["gallery_embedding"] = g
            out["similarity"] = q @ g.t()
        return out
...
def smoke_test_geo(builder, variant: str):
    ...
    out = model(torch.randn(2, 3, 128, 128), torch.randn(3, 3, 128, 128))
