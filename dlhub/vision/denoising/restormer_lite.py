...
class NAFBlock(nn.Module):
    """RestormerLite block (Nonlinear Activation Free), toy-first implementation."""
...
class RestormerLite(nn.Module):
    """RestormerLite-style encoder/decoder denoiser (pure torch, toy-first)."""
...
_VARIANTS: dict[str, dict] = {
    "restormer_lite_tiny": {"width": 20, "depths": (1, 1, 1, 2), "dw": 2, "ffn": 2},
    "restormer_lite_small": {"width": 24, "depths": (1, 1, 2, 2), "dw": 2, "ffn": 2},
    "restormer_lite_base": {"width": 32, "depths": (2, 2, 3, 4), "dw": 2, "ffn": 2},
}


def build_restormer_lite_denoiser(
    *,
    in_channels: int,
    variant: str = "restormer_lite_small",
) -> nn.Module:
    name = str(variant).lower().strip()
    if name not in _VARIANTS:
        raise ValueError(f"Unknown RestormerLite variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
    return RestormerLite(
...
if __name__ == "__main__":
    ...
    m = build_restormer_lite_denoiser(in_channels=1, variant="restormer_lite_tiny")
    y = m(x)
    print("restormer_lite_tiny", tuple(y.shape))
    loss = (y - x).pow(2).mean()
    loss.backward()
    print("ok")
