...
class NAFBlock(nn.Module):
    """RestormerLite block (Nonlinear Activation Free), compact-first implementation."""
...
class RestormerLite(nn.Module):
    """RestormerLite-style encoder/decoder denoiser (pure torch, compact-first)."""
...
def build_restormer_lite_denoiser(
    *,
    ...
    if name not in _VARIANTS:
        raise ValueError(
            f"Unknown RestormerLite variant: {variant!r}. Supported: {sorted(_VARIANTS)}"
        )
    spec = _VARIANTS[name]
