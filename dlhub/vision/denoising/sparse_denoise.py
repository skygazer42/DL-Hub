...
class NAFBlock(nn.Module):
    """SparseDenoise block (Nonlinear Activation Free), toy-first implementation."""
...
class SparseDenoise(nn.Module):
    """SparseDenoise-style encoder/decoder denoiser (pure torch, toy-first)."""
...
_VARIANTS: dict[str, dict] = {
    "sparse_denoise_tiny": {"width": 20, "depths": (1, 1, 1, 2), "dw": 2, "ffn": 2},
    "sparse_denoise_small": {"width": 24, "depths": (1, 1, 2, 2), "dw": 2, "ffn": 2},
    "sparse_denoise_base": {"width": 32, "depths": (2, 2, 3, 4), "dw": 2, "ffn": 2},
}


def build_sparse_denoise_denoiser(
    *,
    in_channels: int,
    variant: str = "sparse_denoise_small",
) -> nn.Module:
    name = str(variant).lower().strip()
    if name not in _VARIANTS:
        raise ValueError(f"Unknown SparseDenoise variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
    return SparseDenoise(
...
if __name__ == "__main__":
    ...
    m = build_sparse_denoise_denoiser(in_channels=1, variant="sparse_denoise_tiny")
    y = m(x)
    print("sparse_denoise_tiny", tuple(y.shape))
    loss = (y - x).pow(2).mean()
    loss.backward()
    print("ok")
