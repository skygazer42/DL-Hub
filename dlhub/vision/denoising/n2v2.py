...
class N2V2(nn.Module):
    """N2V2 (Denoising CNN), implemented from scratch in torch.
    ...
    def __init__(
        ...
        *,
        ...
        d = int(depth)
        if d < 3:
            raise ValueError(f"N2V2 depth must be >= 3, got: {depth}")
...
class N2V2Denoiser(nn.Module):
    """N2V2 wrapper that returns the denoised image instead of the residual/noise."""

    def __init__(self, backbone: N2V2, *, residual: bool = True) -> None:
        super().__init__()
...
_VARIANTS: dict[str, dict] = {
    ...
    "n2v2_tiny": {"features": 32, "depth": 7, "use_bn": True},
}


def build_n2v2_denoiser(
    *,
    in_channels: int,
    variant: str = "n2v2_17",
    residual: bool = True,
    ...
    if name not in _VARIANTS:
        raise ValueError(f"Unknown N2V2 variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
    return N2V2Denoiser(
        N2V2(
...
if __name__ == "__main__":
    ...
    m = build_n2v2_denoiser(in_channels=3, variant="n2v2_9")
    y = m(x)
    print("n2v2_9", tuple(y.shape))
    loss = (y - x).pow(2).mean()
    loss.backward()
    print("ok")
