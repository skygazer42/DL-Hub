...
class NAFBlock(nn.Module):
    """RainliteDenoise block (Nonlinear Activation Free), compact-first implementation."""
...
class RainliteDenoise(nn.Module):
    """RainliteDenoise-style encoder/decoder denoiser (pure torch, compact-first)."""
...
def build_rainlite_denoise_denoiser(
    *,
    ...
    if name not in _VARIANTS:
        raise ValueError(
            f"Unknown RainliteDenoise variant: {variant!r}. Supported: {sorted(_VARIANTS)}"
        )
    spec = _VARIANTS[name]
