...
_VARIANTS: dict[str, dict] = {
    "patch_denoise_tiny": {"width": 20, "depths": (1, 1, 1, 2), "dw": 2, "ffn": 2},
    "patch_denoise_small": {"width": 24, "depths": (1, 1, 2, 2), "dw": 2, "ffn": 2},
    "patch_denoise_base": {"width": 32, "depths": (2, 2, 3, 4), "dw": 2, "ffn": 2},
}


def build_patch_denoise_denoiser(
    *,
    in_channels: int,
    variant: str = "patch_denoise_small",
) -> nn.Module:
    name = str(variant).lower().strip()
    if name not in _VARIANTS:
        raise ValueError(f"Unknown PatchDenoise variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
    return PatchDenoise(
...
if __name__ == "__main__":
    ...
    m = build_patch_denoise_denoiser(in_channels=1, variant="patch_denoise_tiny")
    y = m(x)
    print("patch_denoise_tiny", tuple(y.shape))
    loss = (y - x).pow(2).mean()
    loss.backward()
    print("ok")
