...
class CleanMamba(nn.Module):
    """CleanMamba (Denoising CNN), implemented from scratch in torch.
    ...
    def __init__(
        ...
        *,
        ...
        d = int(depth)
        if d < 3:
            raise ValueError(f"CleanMamba depth must be >= 3, got: {depth}")
...
class CleanMambaDenoiser(nn.Module):
    """CleanMamba wrapper that returns the denoised image instead of the residual/noise."""

    def __init__(self, backbone: CleanMamba, *, residual: bool = True) -> None:
        super().__init__()
...
_VARIANTS: dict[str, dict] = {
    ...
    "cleanmamba_tiny": {"features": 32, "depth": 7, "use_bn": True},
}


def build_cleanmamba_denoiser(
    *,
    in_channels: int,
    variant: str = "cleanmamba_17",
    residual: bool = True,
) -> nn.Module:
    name = str(variant).lower().strip()
    if name not in _VARIANTS:
        raise ValueError(f"Unknown CleanMamba variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
    return CleanMambaDenoiser(
        CleanMamba(
...
if __name__ == "__main__":
    ...
    m = build_cleanmamba_denoiser(in_channels=3, variant="cleanmamba_9")
    y = m(x)
    print("cleanmamba_9", tuple(y.shape))
    loss = (y - x).pow(2).mean()
    loss.backward()
    print("ok")
