...
class NAFBlock(nn.Module):
    """BlurfixDenoise block (Nonlinear Activation Free), toy-first implementation."""
...
class BlurfixDenoise(nn.Module):
    """BlurfixDenoise-style encoder/decoder denoiser (pure torch, toy-first)."""
...
_VARIANTS: dict[str, dict] = {
    "blurfix_denoise_tiny": {"width": 20, "depths": (1, 1, 1, 2), "dw": 2, "ffn": 2},
    "blurfix_denoise_small": {"width": 24, "depths": (1, 1, 2, 2), "dw": 2, "ffn": 2},
    "blurfix_denoise_base": {"width": 32, "depths": (2, 2, 3, 4), "dw": 2, "ffn": 2},
}


def build_blurfix_denoise_denoiser(
    *,
    in_channels: int,
    variant: str = "blurfix_denoise_small",
) -> nn.Module:
    name = str(variant).lower().strip()
    if name not in _VARIANTS:
        raise ValueError(f"Unknown BlurfixDenoise variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
    return BlurfixDenoise(
...
if __name__ == "__main__":
    ...
    m = build_blurfix_denoise_denoiser(in_channels=1, variant="blurfix_denoise_tiny")
    y = m(x)
    print("blurfix_denoise_tiny", tuple(y.shape))
    loss = (y - x).pow(2).mean()
    loss.backward()
    print("ok")
