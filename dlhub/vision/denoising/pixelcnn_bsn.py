...
class MaskedConv2d(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # Mask out all rows below the center row.
        mask[:, :, cy + 1 :, :] = 0
        # Mask out columns to the right on the center row.
        if mt == "A":
...
class MaskedResBlock(nn.Module):
    def __init__(self, channels: int) -> None:
        super().__init__()
        c = int(channels)
        self.conv1 = MaskedConv2d(c, c, kernel_size=3, mask_type="B")
        self.conv2 = MaskedConv2d(c, c, kernel_size=3, mask_type="B")
...
class PixelCNNBlindSpotBackbone(nn.Module):
    """Autoregressive masked-conv backbone used inside a rotation-based blind-spot denoiser."""
    ...
    def __init__(self, *, in_channels: int, width: int = 24, depth: int = 6) -> None:
        ...
        self.act = nn.ReLU(inplace=True)
        self.blocks = nn.Sequential(*[MaskedResBlock(w0) for _ in range(d)])
...
class PixelCNNBSN(nn.Module):
    ...
    - Apply it to 4 rotations of the input, rotate features back, fuse, and predict residual/noise.
    ...
    def __init__(
        ...
        *,
        ...
        self.backbone = PixelCNNBlindSpotBackbone(in_channels=c_in, width=w0, depth=d)
...
_VARIANTS: dict[str, dict] = {
    "pixelcnn_bsn_tiny": {"width": 16, "depth": 3},
    "pixelcnn_bsn_small": {"width": 24, "depth": 5},
    "pixelcnn_bsn_base": {"width": 32, "depth": 7},
}


def build_pixelcnn_bsn_denoiser(
    *,
    in_channels: int,
    variant: str = "pixelcnn_bsn_small",
) -> nn.Module:
...
if __name__ == "__main__":
    ...
    m = build_pixelcnn_bsn_denoiser(in_channels=1, variant="pixelcnn_bsn_tiny")
    y = m(noisy)
    print("pixelcnn_bsn_tiny", tuple(y.shape))
    loss = (y - x).pow(2).mean()
    loss.backward()
    print("ok")
