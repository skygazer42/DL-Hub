...
class NAFBlock(nn.Module):
    """FinegrainDenoise block (Nonlinear Activation Free), compact-first implementation."""
...
class FinegrainDenoise(nn.Module):
    """FinegrainDenoise-style encoder/decoder denoiser (pure torch, compact-first)."""
...
def build_finegrain_denoise_denoiser(
    *,
    ...
    if name not in _VARIANTS:
        raise ValueError(
            f"Unknown FinegrainDenoise variant: {variant!r}. Supported: {sorted(_VARIANTS)}"
        )
    spec = _VARIANTS[name]
