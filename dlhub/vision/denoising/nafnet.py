...
class NAFBlock(nn.Module):
    """NAFNet block (Nonlinear Activation Free), compact-first implementation."""
...
class NAFNet(nn.Module):
    """NAFNet-style encoder/decoder denoiser (pure torch, compact-first)."""
...
_VARIANTS: dict[str, dict] = {
    "nafnet_tiny": {"width": 20, "depths": (1, 1, 1, 2), "dw": 2, "ffn": 2},
    "nafnet_small": {"width": 24, "depths": (1, 1, 2, 2), "dw": 2, "ffn": 2},
    "nafnet_base": {"width": 32, "depths": (2, 2, 3, 4), "dw": 2, "ffn": 2},
}


def build_nafnet_denoiser(
    *,
    in_channels: int,
    variant: str = "nafnet_small",
) -> nn.Module:
    name = str(variant).lower().strip()
    if name not in _VARIANTS:
        raise ValueError(f"Unknown NAFNet variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
    return NAFNet(
...
if __name__ == "__main__":
    ...
    m = build_nafnet_denoiser(in_channels=1, variant="nafnet_tiny")
    y = m(x)
    print("nafnet_tiny", tuple(y.shape))
    loss = (y - x).pow(2).mean()
    loss.backward()
    print("ok")
