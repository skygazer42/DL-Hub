...
_VARIANTS: dict[str, dict] = {
    "grdn_tiny": {"width": 20, "depths": (1, 1, 1, 2), "dw": 2, "ffn": 2},
    "grdn_small": {"width": 24, "depths": (1, 1, 2, 2), "dw": 2, "ffn": 2},
    "grdn_base": {"width": 32, "depths": (2, 2, 3, 4), "dw": 2, "ffn": 2},
}


def build_grdn_denoiser(
    *,
    in_channels: int,
    variant: str = "grdn_small",
) -> nn.Module:
    name = str(variant).lower().strip()
    if name not in _VARIANTS:
        raise ValueError(f"Unknown GRDN variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
    return GRDN(
...
if __name__ == "__main__":
    ...
    m = build_grdn_denoiser(in_channels=1, variant="grdn_tiny")
    y = m(x)
    print("grdn_tiny", tuple(y.shape))
    loss = (y - x).pow(2).mean()
    loss.backward()
    print("ok")
