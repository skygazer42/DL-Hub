...
def build_gated_pixelcnn_bsn_denoiser(
    *,
    ...
    if name not in _VARIANTS:
        raise ValueError(f"Unknown gated PixelCNN-BSN variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
    return GatedPixelCNNBSN(in_channels=int(in_channels), width=int(spec["width"]), depth=int(spec["depth"]))
