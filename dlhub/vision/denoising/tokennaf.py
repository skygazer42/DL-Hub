...
class TokenNAF(nn.Module):
    """TokenNAF (Denoising CNN), implemented from scratch in torch.
    ...
    def __init__(
        ...
        *,
        ...
        d = int(depth)
        if d < 3:
            raise ValueError(f"TokenNAF depth must be >= 3, got: {depth}")
...
class TokenNAFDenoiser(nn.Module):
    """TokenNAF wrapper that returns the denoised image instead of the residual/noise."""

    def __init__(self, backbone: TokenNAF, *, residual: bool = True) -> None:
        super().__init__()
...
_VARIANTS: dict[str, dict] = {
    ...
    "tokennaf_tiny": {"features": 32, "depth": 7, "use_bn": True},
}


def build_tokennaf_denoiser(
    *,
    in_channels: int,
    variant: str = "tokennaf_17",
    residual: bool = True,
) -> nn.Module:
    name = str(variant).lower().strip()
    if name not in _VARIANTS:
        raise ValueError(f"Unknown TokenNAF variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
    return TokenNAFDenoiser(
        TokenNAF(
...
if __name__ == "__main__":
    ...
    m = build_tokennaf_denoiser(in_channels=3, variant="tokennaf_9")
    y = m(x)
    print("tokennaf_9", tuple(y.shape))
    loss = (y - x).pow(2).mean()
    loss.backward()
    print("ok")
