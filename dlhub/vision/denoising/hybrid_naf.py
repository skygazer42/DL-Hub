...
class NAFBlock(nn.Module):
    """HybridNAF block (Nonlinear Activation Free), compact-first implementation."""
...
class HybridNAF(nn.Module):
    """HybridNAF-style encoder/decoder denoiser (pure torch, compact-first)."""
...
_VARIANTS: dict[str, dict] = {
    "hybrid_naf_tiny": {"width": 20, "depths": (1, 1, 1, 2), "dw": 2, "ffn": 2},
    "hybrid_naf_small": {"width": 24, "depths": (1, 1, 2, 2), "dw": 2, "ffn": 2},
    "hybrid_naf_base": {"width": 32, "depths": (2, 2, 3, 4), "dw": 2, "ffn": 2},
}


def build_hybrid_naf_denoiser(
    *,
    in_channels: int,
    variant: str = "hybrid_naf_small",
) -> nn.Module:
    name = str(variant).lower().strip()
    if name not in _VARIANTS:
        raise ValueError(f"Unknown HybridNAF variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
    return HybridNAF(
...
if __name__ == "__main__":
    ...
    m = build_hybrid_naf_denoiser(in_channels=1, variant="hybrid_naf_tiny")
    y = m(x)
    print("hybrid_naf_tiny", tuple(y.shape))
    loss = (y - x).pow(2).mean()
    loss.backward()
    print("ok")
