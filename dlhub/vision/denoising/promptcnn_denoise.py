...
class PromptCNNDenoise(nn.Module):
    """PromptCNNDenoise (Denoising CNN), implemented from scratch in torch.
    ...
    def __init__(
        ...
        *,
        ...
        d = int(depth)
        if d < 3:
            raise ValueError(f"PromptCNNDenoise depth must be >= 3, got: {depth}")
...
class PromptCNNDenoiseDenoiser(nn.Module):
    """PromptCNNDenoise wrapper that returns the denoised image instead of the residual/noise."""

    def __init__(self, backbone: PromptCNNDenoise, *, residual: bool = True) -> None:
        super().__init__()
...
_VARIANTS: dict[str, dict] = {
    ...
    "promptcnn_denoise_tiny": {"features": 32, "depth": 7, "use_bn": True},
}


def build_promptcnn_denoise_denoiser(
    *,
    in_channels: int,
    variant: str = "promptcnn_denoise_17",
    residual: bool = True,
) -> nn.Module:
    name = str(variant).lower().strip()
    if name not in _VARIANTS:
        raise ValueError(f"Unknown PromptCNNDenoise variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
    return PromptCNNDenoiseDenoiser(
        PromptCNNDenoise(
...
if __name__ == "__main__":
    ...
    m = build_promptcnn_denoise_denoiser(in_channels=3, variant="promptcnn_denoise_9")
    y = m(x)
    print("promptcnn_denoise_9", tuple(y.shape))
    loss = (y - x).pow(2).mean()
    loss.backward()
    print("ok")
