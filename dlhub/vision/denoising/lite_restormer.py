...
_VARIANTS: dict[str, dict] = {
    "lite_restormer_tiny": {"width": 20, "depths": (1, 1, 1, 2), "dw": 2, "ffn": 2},
    "lite_restormer_small": {"width": 24, "depths": (1, 1, 2, 2), "dw": 2, "ffn": 2},
    "lite_restormer_base": {"width": 32, "depths": (2, 2, 3, 4), "dw": 2, "ffn": 2},
}


def build_lite_restormer_denoiser(
    *,
    in_channels: int,
    variant: str = "lite_restormer_small",
) -> nn.Module:
    name = str(variant).lower().strip()
    if name not in _VARIANTS:
        raise ValueError(f"Unknown LiteRestormer variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
    return LiteRestormer(
...
if __name__ == "__main__":
    ...
    m = build_lite_restormer_denoiser(in_channels=1, variant="lite_restormer_tiny")
    y = m(x)
    print("lite_restormer_tiny", tuple(y.shape))
    loss = (y - x).pow(2).mean()
    loss.backward()
    print("ok")
