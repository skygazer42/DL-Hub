...
class ToyTextRecognizer(nn.Module):
    def __init__(self, *, family:str, in_channels:int, vocab_size:int, seq_len:int, width:int, depth:int, decoder:str='gru'):
        super().__init__(); self.family=str(family); self.seq_len=int(seq_len); self.enc=TinyEncoder(in_channels,width,depth); c=self.enc.out_channels; self.decoder_type=str(decoder)
        if self.decoder_type=='transformer':
            layer=nn.TransformerEncoderLayer(d_model=c, nhead=4 if c%4==0 else 2, dim_feedforward=max(64,c*2), batch_first=True)
...
def smoke_test_rec(builder, variant:str):
    model=builder(in_channels=1, vocab_size=32, seq_len=16, variant=variant, width_mult=0.5); out=model(torch.randn(2,1,32,128)); print(variant,{k:tuple(v.shape) for k,v in out.items()})
