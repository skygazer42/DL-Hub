...
class TinyDeepfakeBlock(nn.Module):
...
class TinyDeepfakeDetector(nn.Module):
    def __init__(self, *, family: str, mode: str, in_channels: int, width: int, depth: int) -> None:
        ...
        self.blocks = nn.ModuleList([TinyDeepfakeBlock(channels=int(width), mode=str(mode)) for _ in range(max(1, int(depth)))])
        self.artifact_head = nn.Conv2d(int(width), 1, kernel_size=1)
        self.cls_head = nn.Linear(int(width) + 1, 2)
    ...
    def forward(self, image: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        for block in self.blocks:
            feat = block(feat)
        artifact_map = self.artifact_head(feat)
        pooled = F.adaptive_avg_pool2d(feat, 1).flatten(1)
        artifact_score = F.adaptive_avg_pool2d(artifact_map, 1).flatten(1)
        logits = self.cls_head(torch.cat([pooled, artifact_score], dim=1))
        return {"logits": logits, "artifact_map": artifact_map, "probabilities": torch.softmax(logits, dim=-1)}


def build_toy_deepfake_detector(*, family: str, mode: str, variants: dict[str, dict[str, int]], in_channels: int, variant: str, width_mult: float = 1.0) -> nn.Module:
    ...
    return TinyDeepfakeDetector(family=str(family), mode=str(mode), in_channels=int(in_channels), width=width, depth=depth)


def smoke_test_deepfake_detector(builder, variant: str) -> None:
    model = builder(in_channels=3, variant=variant, width_mult=0.5)
    out = model(torch.randn(2, 3, 64, 64))
    print(variant, tuple(out["logits"].shape), tuple(out["artifact_map"].shape))
    print("ok")
