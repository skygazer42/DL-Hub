...
class TinyStyleBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, cond: torch.Tensor) -> torch.Tensor:
        ...
        if self.mode in {"gatys", "adain", "wct", "cycle", "pix2pix"}:
...
class TinyImageTranslator(nn.Module):
    ...
    def forward(self, image: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        translated = torch.tanh(self.decoder(feat))
        return {"translated": translated, "residual": translated - image}


def build_toy_translator(*, family: str, mode: str, variants: dict[str, dict[str, int]], in_channels: int, variant: str, width_mult: float = 1.0) -> nn.Module:
    ...
    return TinyImageTranslator(family=str(family), mode=str(mode), in_channels=int(in_channels), width=width, depth=depth)


def smoke_test_translator(builder, variant: str) -> None:
    model = builder(in_channels=3, variant=variant, width_mult=0.5)
    out = model(torch.randn(2, 3, 64, 64))
    print(variant, tuple(out["translated"].shape))
    print("ok")
