...
class ToyModel(nn.Module):
    def __init__(
        self, *, family: str, in_channels: int, width: int, depth: int, num_classes: int = 10
    ):
        super().__init__()
        ...
        self.embed = nn.Linear(c, 128)
        self.cls = nn.Linear(128, int(num_classes))

    def forward(self, image):
        feat = self.enc(image)
        pooled = F.adaptive_avg_pool2d(feat, (1, 1)).flatten(1)
        emb = F.normalize(self.embed(pooled), dim=1)
        return {"score_map": self.det(feat), "embedding": emb, "logits": self.cls(emb)}
...
def smoke_test_model(builder, variant: str):
    out = builder(in_channels=3, variant=variant, width_mult=0.5, num_classes=6)(
        torch.randn(2, 3, 128, 64)
    )
