...
class ToyOCRModel(nn.Module):
    def __init__(self, *, family: str, in_channels: int, vocab_size: int, seq_len: int, width: int, depth: int, decoder_mode: str = 'gru') -> None:
        super().__init__()
        self.family = str(family)
        self.seq_len = int(seq_len)
        self.encoder = TinyOCREncoder(in_channels=int(in_channels), width=int(width), depth=int(depth))
        d = int(self.encoder.out_channels)
        self.decoder_mode = str(decoder_mode)
        if self.decoder_mode == 'transformer':
            layer = nn.TransformerEncoderLayer(d_model=d, nhead=4 if d % 4 == 0 else 2, dim_feedforward=max(64, d * 2), batch_first=True)
...
def build_toy_ocr_model(*, family: str, variants: dict[str, dict[str, int]], in_channels: int, vocab_size: int, seq_len: int, variant: str, width_mult: float = 1.0, decoder_mode: str = 'gru') -> nn.Module:
    spec = variants[str(variant)]
    width = max(16, int(int(spec['width']) * float(width_mult)))
    return ToyOCRModel(family=str(family), in_channels=int(in_channels), vocab_size=int(vocab_size), seq_len=int(seq_len), width=width, depth=int(spec['depth']), decoder_mode=str(decoder_mode))
