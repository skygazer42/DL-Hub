    ...
    for family in _FAMILIES:
        for size in _SIZES:
            ...
            def _builder(cfg: BuildConfig, family: str = family, variant: str = variant):
                module = importlib.import_module(f"dlhub.vision.image_moire_removal.{family}")
                fn = getattr(module, f"build_{family}_moire_remover")
                return fn(
...
def list_local_arches() -> list[str]:
    return [f"moire:{name}" for name in sorted(_REGISTRY)]
