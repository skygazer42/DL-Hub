...
_VARIANTS: dict[str, dict] = {
    "polarmask_panoptic_tiny": {"stem": 24, "low": 40, "det": 80, "depth": 1, "head": 80, "protos": 16, "rays": 24},
    "polarmask_panoptic_small": {"stem": 24, "low": 48, "det": 96, "depth": 2, "head": 96, "protos": 32, "rays": 36},
    "polarmask_panoptic_base": {"stem": 32, "low": 64, "det": 128, "depth": 3, "head": 128, "protos": 48, "rays": 36},
}


def build_polarmask_panoptic_segmenter(
    *,
    in_channels: int,
    num_thing_classes: int,
    num_stuff_classes: int,
    variant: str = "polarmask_panoptic_small",
    width_mult: float = 1.0,
) -> nn.Module:
    name = str(variant).lower().strip()
    if name not in _VARIANTS:
        raise ValueError(f"Unknown PolarMask-panoptic variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
...
if __name__ == "__main__":
    ...
    m = build_polarmask_panoptic_segmenter(
        in_channels=3, num_thing_classes=3, num_stuff_classes=2, variant="polarmask_panoptic_tiny", width_mult=0.5
    )
    out = m(x)
    print("polarmask_panoptic_tiny", {k: tuple(v.shape) for k, v in out.items()})
    loss = out["semantic_logits"].mean() + out["cls_logits"].mean() + out["mask_logits"].mean() + out["rays"].mean()
    loss.backward()
    print("ok")
