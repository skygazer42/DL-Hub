...
from dlhub.vision.panoptic_segmentation._common import BackboneC2C3C4C5, ProtoNet, check_nchw, fuse_panoptic, masks_from_prototypes
...
_VARIANTS: dict[str, dict] = {
    "upernet_panoptic_tiny": {"stem": 24, "c2": 24, "c3": 48, "c4": 64, "c5": 96, "depth": 1, "fpn": 64, "instances": 16, "protos": 16},
    "upernet_panoptic_small": {"stem": 24, "c2": 32, "c3": 64, "c4": 96, "c5": 128, "depth": 2, "fpn": 96, "instances": 32, "protos": 32},
    "upernet_panoptic_base": {"stem": 32, "c2": 40, "c3": 80, "c4": 128, "c5": 160, "depth": 2, "fpn": 128, "instances": 64, "protos": 48},
}
...
def build_upernet_panoptic_segmenter(
    *,
    ...
    if name not in _VARIANTS:
        raise ValueError(f"Unknown UPerNet-panoptic variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
...
if __name__ == "__main__":
    ...
    m = build_upernet_panoptic_segmenter(
        in_channels=3, num_thing_classes=3, num_stuff_classes=2, variant="upernet_panoptic_tiny", width_mult=0.5
    )
    out = m(x)
