...
class MaskFormerPanoptic(nn.Module):
    """MaskFormer-style panoptic segmentation (compact-first).
...
def build_maskformer_panoptic_segmenter(
    *,
    ...
    if name not in _VARIANTS:
        raise ValueError(
            f"Unknown MaskFormer-panoptic variant: {variant!r}. Supported: {sorted(_VARIANTS)}"
        )
    spec = _VARIANTS[name]
