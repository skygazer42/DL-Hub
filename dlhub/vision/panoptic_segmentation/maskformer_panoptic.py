...
class MaskFormerPanoptic(nn.Module):
    """MaskFormer-style panoptic segmentation (toy-first).
    ...
    def forward(self, x: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        query_cls_logits = self.cls(q)  # (B,Q,nt)
...
_VARIANTS: dict[str, dict] = {
    "maskformer_panoptic_tiny": {"stem": 24, "low": 40, "det": 80, "depth": 1, "queries": 16, "d_model": 80, "heads": 4, "layers": 1},
    "maskformer_panoptic_small": {"stem": 24, "low": 48, "det": 96, "depth": 2, "queries": 32, "d_model": 96, "heads": 4, "layers": 2},
    "maskformer_panoptic_base": {"stem": 32, "low": 64, "det": 128, "depth": 3, "queries": 64, "d_model": 128, "heads": 8, "layers": 3},
}


def build_maskformer_panoptic_segmenter(
    *,
    in_channels: int,
    num_thing_classes: int,
    num_stuff_classes: int,
    variant: str = "maskformer_panoptic_small",
    width_mult: float = 1.0,
) -> nn.Module:
    name = str(variant).lower().strip()
    if name not in _VARIANTS:
        raise ValueError(f"Unknown MaskFormer-panoptic variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
...
if __name__ == "__main__":
    ...
    m = build_maskformer_panoptic_segmenter(
        in_channels=3, num_thing_classes=3, num_stuff_classes=2, variant="maskformer_panoptic_tiny", width_mult=0.5
    )
    out = m(x)
    print("maskformer_panoptic_tiny", {k: tuple(v.shape) for k, v in out.items()})
