...
def build_sam2_panoptic_segmenter(
    *,
    in_channels: int,
    num_thing_classes: int,
    num_stuff_classes: int,
    variant: str = "sam2_panoptic_small",
...
if __name__ == "__main__":
    ...
    m = build_sam2_panoptic_segmenter(
        in_channels=3,
        num_thing_classes=3,
        num_stuff_classes=2,
        variant="sam2_panoptic_tiny",
        width_mult=0.5,
    )
    out = m(x)
    print("sam2_panoptic_tiny", {k: tuple(v.shape) for k, v in out.items()})
    loss = (
