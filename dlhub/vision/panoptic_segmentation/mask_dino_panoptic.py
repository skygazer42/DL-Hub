...
class MaskDinoPanoptic(nn.Module):
...
def build_mask_dino_panoptic_segmenter(
    *,
    in_channels: int,
    num_thing_classes: int,
    num_stuff_classes: int,
    variant: str = "mask_dino_panoptic_small",
...
if __name__ == "__main__":
    ...
    m = build_mask_dino_panoptic_segmenter(
        in_channels=3,
        num_thing_classes=3,
        num_stuff_classes=2,
        variant="mask_dino_panoptic_tiny",
        width_mult=0.5,
    )
    out = m(x)
    print("mask_dino_panoptic_tiny", {k: tuple(v.shape) for k, v in out.items()})
    loss = (
