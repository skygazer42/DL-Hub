...
_VARIANTS: dict[str, dict] = {
    "deformable_detr_panoptic_tiny": {"stem": 24, "c2": 40, "c3": 48, "c4": 64, "c5": 80, "depth": 1, "fpn": 96, "d_model": 96, "heads": 4, "q": 25, "enc": 1, "dec": 1},
    "deformable_detr_panoptic_small": {"stem": 32, "c2": 48, "c3": 64, "c4": 96, "c5": 128, "depth": 2, "fpn": 128, "d_model": 128, "heads": 4, "q": 50, "enc": 1, "dec": 2},
    "deformable_detr_panoptic_base": {"stem": 48, "c2": 64, "c3": 96, "c4": 144, "c5": 192, "depth": 3, "fpn": 192, "d_model": 192, "heads": 6, "q": 100, "enc": 2, "dec": 3},
}
...
def build_deformable_detr_panoptic_segmenter(
    *,
    ...
    if name not in _VARIANTS:
        raise ValueError(f"Unknown Deformable-DETR-panoptic variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
...
if __name__ == "__main__":
    ...
    m = build_deformable_detr_panoptic_segmenter(
        in_channels=3, num_thing_classes=3, num_stuff_classes=2, variant="deformable_detr_panoptic_tiny", width_mult=0.5
    )
    out = m(x)
