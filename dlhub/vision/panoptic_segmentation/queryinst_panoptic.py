...
_VARIANTS: dict[str, dict] = {
    "queryinst_panoptic_tiny": {"stem": 24, "low": 40, "det": 80, "depth": 1, "queries": 16, "protos": 16, "d_model": 80, "heads": 4},
    "queryinst_panoptic_small": {"stem": 24, "low": 48, "det": 96, "depth": 2, "queries": 32, "protos": 32, "d_model": 96, "heads": 4},
    "queryinst_panoptic_base": {"stem": 32, "low": 64, "det": 128, "depth": 3, "queries": 64, "protos": 48, "d_model": 128, "heads": 8},
}
...
def build_queryinst_panoptic_segmenter(
    *,
    ...
    if name not in _VARIANTS:
        raise ValueError(f"Unknown QueryInst-panoptic variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
...
if __name__ == "__main__":
    ...
    m = build_queryinst_panoptic_segmenter(
        in_channels=3, num_thing_classes=3, num_stuff_classes=2, variant="queryinst_panoptic_tiny", width_mult=0.5
    )
    out = m(x)
