...
class MobilePanoptic(nn.Module):
    """MobilePanoptic (Unified Panoptic Segmentation Network) (toy-first).
...
def build_mobile_panoptic_segmenter(
    *,
    in_channels: int,
    num_thing_classes: int,
    num_stuff_classes: int,
    variant: str = "mobile_panoptic_small",
    width_mult: float = 1.0,
) -> nn.Module:
    name = str(variant).lower().strip()
    if name not in _VARIANTS:
        raise ValueError(f"Unknown MobilePanoptic variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
...
if __name__ == "__main__":
    ...
    m = build_mobile_panoptic_segmenter(
        in_channels=3,
        num_thing_classes=3,
        num_stuff_classes=2,
        variant="mobile_panoptic_tiny",
        width_mult=0.5,
    )
    out = m(x)
    print("mobile_panoptic_tiny", {k: tuple(v.shape) for k, v in out.items()})
    loss = (
