...
class _CrossTaskGate(nn.Module):
    def __init__(self, channels: int) -> None:
        super().__init__()
        c = int(channels)
        self.net = nn.Sequential(
            nn.AdaptiveAvgPool2d((1, 1)),
            nn.Conv2d(c, max(4, c // 8), kernel_size=1, bias=True),
            nn.ReLU(inplace=True),
            nn.Conv2d(max(4, c // 8), c, kernel_size=1, bias=True),
            nn.Sigmoid(),
        )
...
class TASCNet(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.sem_gate = _CrossTaskGate(fpn)
        self.inst_gate = _CrossTaskGate(fpn)
    ...
    def forward(self, x: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        sem_feat = self.sem_gate(p2)
        inst_feat = self.inst_gate(p2)

        semantic_logits = self.semantic(sem_feat + inst_feat)
...
_VARIANTS: dict[str, dict] = {
    "tascnet_tiny": {"stem": 24, "c2": 24, "c3": 48, "c4": 64, "c5": 96, "depth": 1, "fpn": 64, "instances": 16, "protos": 16},
    "tascnet_small": {"stem": 24, "c2": 32, "c3": 64, "c4": 96, "c5": 128, "depth": 2, "fpn": 96, "instances": 32, "protos": 32},
    "tascnet_base": {"stem": 32, "c2": 40, "c3": 80, "c4": 128, "c5": 160, "depth": 2, "fpn": 128, "instances": 64, "protos": 48},
}


def build_tascnet_panoptic_segmenter(
    *,
    in_channels: int,
    num_thing_classes: int,
    num_stuff_classes: int,
    variant: str = "tascnet_small",
    width_mult: float = 1.0,
) -> nn.Module:
    name = str(variant).lower().strip()
    if name not in _VARIANTS:
        raise ValueError(f"Unknown TASCNet variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
...
if __name__ == "__main__":
    ...
    m = build_tascnet_panoptic_segmenter(in_channels=3, num_thing_classes=3, num_stuff_classes=2, variant="tascnet_tiny", width_mult=0.5)
    out = m(x)
    print("tascnet_tiny", {k: tuple(v.shape) for k, v in out.items()})
