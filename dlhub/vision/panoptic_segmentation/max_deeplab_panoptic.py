...
def build_max_deeplab_panoptic_segmenter(
    *,
    ...
    if name not in _VARIANTS:
        raise ValueError(
            f"Unknown MaX-DeepLab-panoptic variant: {variant!r}. Supported: {sorted(_VARIANTS)}"
        )
    spec = _VARIANTS[name]
