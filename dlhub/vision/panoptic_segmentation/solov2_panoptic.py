...
class SOLOv2Panoptic(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.cat = nn.Conv2d(int(head_channels), nt, kernel_size=3, padding=1)
    ...
    def forward(self, x: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        t = self.tower(det)
        cat_logits = self.cat(t)  # (B,nt,H/8,W/8)
        ...
        cls = cat_logits.permute(0, 2, 3, 1).reshape(b, -1, int(self.num_thing_classes))
...
_VARIANTS: dict[str, dict] = {
    "solov2_panoptic_tiny": {"stem": 24, "low": 40, "det": 80, "depth": 1, "head": 80, "mask": 24},
    "solov2_panoptic_small": {"stem": 24, "low": 48, "det": 96, "depth": 2, "head": 96, "mask": 32},
    "solov2_panoptic_base": {"stem": 32, "low": 64, "det": 128, "depth": 3, "head": 128, "mask": 48},
}


def build_solov2_panoptic_segmenter(
    *,
    in_channels: int,
    num_thing_classes: int,
    num_stuff_classes: int,
    variant: str = "solov2_panoptic_small",
    width_mult: float = 1.0,
) -> nn.Module:
    name = str(variant).lower().strip()
    if name not in _VARIANTS:
        raise ValueError(f"Unknown SOLOv2-panoptic variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
...
if __name__ == "__main__":
    ...
    m = build_solov2_panoptic_segmenter(
        in_channels=3, num_thing_classes=3, num_stuff_classes=2, variant="solov2_panoptic_tiny", width_mult=0.5
    )
    out = m(x)
    print("solov2_panoptic_tiny", {k: tuple(v.shape) for k, v in out.items()})
    loss = out["semantic_logits"].mean() + out["cat_logits"].mean() + out["mask_logits"].mean()
    loss.backward()
    print("ok")
