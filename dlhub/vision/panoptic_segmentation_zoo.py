...
def _split_arch_id(arch_id: str) -> tuple[str, str]:
    ...
    return split_arch_id(arch_id, default_prefix="dlpan")
...
def _extract_builder_name_from_source(src: str) -> str | None:
    ...
    for node in tree.body:
        if not isinstance(node, ast.FunctionDef):
            continue
        name = str(node.name)
        if name.startswith("build_") and name.endswith("_panoptic_segmenter"):
            return name
    return None
...
def _make_lazy_builder(module_name: str, *, builder_name: str, variant: str) -> Builder:
    def _builder(cfg: BuildConfig) -> nn.Module:
        ...
        mod = importlib.import_module(f"dlhub.vision.panoptic_segmentation.{module_name}")
        fn = getattr(mod, builder_name, None)
        if fn is None:
            raise RuntimeError(f"Panoptic module {module_name!r} missing {builder_name}()")
...
def _extend_registry(r: dict[str, Builder]) -> None:
    ...
    module_dir = here / "panoptic_segmentation"
...
def list_local_arches() -> list[str]:
    ...
    return list_arch_ids(_REGISTRY, prefix="dlpan")
...
def build_local_model(
    ...
    *,
    ...
    if prefix in {"pan", "pano", "panoptic"}:
        prefix = "dlpan"
    if prefix not in {"dlpan", "local"}:
        raise ValueError(f"Unsupported panoptic prefix: {prefix!r} (arch_id={arch_id!r})")

    builder = _REGISTRY.get(str(name).lower().strip())
    if builder is None:
        raise UnknownLocalArch(
            f"Unknown panoptic arch: {arch_id!r}. Tip: run `python scripts/panoptic_segmentation_zoo.py --list`."
        )
