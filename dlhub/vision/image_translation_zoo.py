    ...
    for family in _FAMILIES:
        for size in _SIZES:
            ...
            def _builder(cfg: BuildConfig, family: str = family, variant: str = variant):
                module = importlib.import_module(f"dlhub.vision.image_translation.{family}")
                fn = getattr(module, f"build_{family}_translator")
                return fn(
...
def list_local_arches() -> list[str]:
    return [f"imgtrans:{name}" for name in sorted(_REGISTRY)]
...
def build_local_model(arch_id: str, *, in_channels: int, width_mult: float = 1.0):
    ...
    if builder is None:
        raise UnknownLocalArch(
            f"Unknown image translation arch: {arch_id!r}. Tip: import `dlhub.vision.image_translation_zoo` and call `list_local_arches()`."
        )
