...
def _split_arch_id(arch_id: str) -> tuple[str, str]:
    ...
    return split_arch_id(arch_id, default_prefix="mot2d")
...
def _extract_builder_name_from_source(src: str) -> str | None:
    ...
    for node in tree.body:
        if isinstance(node, ast.FunctionDef):
            name = str(node.name)
            if name.startswith("build_") and name.endswith("_tracker"):
                return name
    return None
...
def _make_lazy_builder(module_name: str, *, builder_name: str, variant: str) -> Builder:
    def _builder(cfg: BuildConfig):
        ...
        mod = importlib.import_module(f"dlhub.vision.mot.{module_name}")
        fn = getattr(mod, builder_name, None)
        if fn is None:
            raise RuntimeError(f"MOT module {module_name!r} missing {builder_name}()")
...
def list_local_arches() -> list[str]:
    ...
    return list_arch_ids(_REGISTRY, prefix="mot2d")
...
def build_local_model(
    ...
    *,
    ...
    if prefix in {"mot", "trk2d"}:
        prefix = "mot2d"
    if prefix not in {"mot2d", "local"}:
        raise ValueError(f"Unsupported vision MOT prefix: {prefix!r} (arch_id={arch_id!r})")

    builder = _REGISTRY.get(str(name).lower().strip())
    if builder is None:
        raise UnknownLocalArch(
            f"Unknown vision MOT arch: {arch_id!r}. Tip: run `python scripts/mot_zoo.py --list`."
        )
    return builder(
        BuildConfig(
