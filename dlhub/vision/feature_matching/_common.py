...
class ToyFeatureMatcher(nn.Module):
    ...
    def forward(self,image1,image2): f1=self.enc(image1).flatten(2).mean(dim=-1); f2=self.enc(image2).flatten(2).mean(dim=-1); e1=F.normalize(self.proj(f1),dim=1); e2=F.normalize(self.proj(f2),dim=1); sim=e1 @ e2.t(); return {'similarity_matrix': sim}

def build_toy_model(*, family:str, variants:dict[str,dict[str,int]], in_channels:int, variant:str, width_mult:float=1.0, **kwargs):
    spec=variants[str(variant)]; width=max(16,int(int(spec['width'])*float(width_mult))); embed=max(64,int(int(spec['embed'])*float(width_mult))); return ToyFeatureMatcher(family=str(family), in_channels=int(in_channels), width=width, depth=int(spec['depth']), embed_dim=embed)

def smoke_test_model(builder, variant:str): out=builder(in_channels=1, variant=variant, width_mult=0.5)(torch.randn(2,1,64,64), torch.randn(3,1,64,64)); print(variant, tuple(out['similarity_matrix'].shape))
