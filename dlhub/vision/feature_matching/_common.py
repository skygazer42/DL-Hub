...
class CompactFeatureMatcher(nn.Module):
    ...
    def forward(self, image1, image2):
        f1 = self.enc(image1).flatten(2).mean(dim=-1)
        f2 = self.enc(image2).flatten(2).mean(dim=-1)
...
def smoke_test_model(builder, variant: str):
    out = builder(in_channels=1, variant=variant, width_mult=0.5)(
        torch.randn(2, 1, 64, 64), torch.randn(3, 1, 64, 64)
    )
    print(variant, tuple(out["similarity_matrix"].shape))
