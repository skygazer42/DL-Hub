...
class PANet(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        path = self.path_proj(p2) + F.interpolate(p3, size=p2.shape[-2:], mode="nearest").mean(
            dim=1, keepdim=True
        )
...
if __name__ == "__main__":
    ...
    m = build_panet_instance_segmenter(
        in_channels=3, num_classes=3, variant="panet_tiny", width_mult=0.5
    )
    out = m(x)
