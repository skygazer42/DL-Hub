...
class PANet(nn.Module):
    """PANet-style bottom-up path aggregation for instance segmentation."""
    ...
    def __init__(
        ...
        *,
        ...
        self.path_proj = ConvBNAct(int(p2_channels), int(hidden_channels), kernel_size=3, stride=1, act="relu")
    ...
    def forward(self, x: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        path = self.path_proj(p2) + F.interpolate(p3, size=p2.shape[-2:], mode="nearest").mean(dim=1, keepdim=True)
        ...
        mask_logits = mask_logits + path.mean(dim=1, keepdim=True).expand(-1, k, -1, -1)
        return {
...
_VARIANTS: dict[str, dict[str, int]] = {
    "panet_tiny": {"stem": 24, "p2": 40, "p3": 64, "p4": 96, "hidden": 96, "depth": 1, "instances": 16, "mask": 16},
    "panet_small": {"stem": 24, "p2": 48, "p3": 80, "p4": 128, "hidden": 128, "depth": 2, "instances": 24, "mask": 16},
    "panet_base": {"stem": 32, "p2": 64, "p3": 96, "p4": 160, "hidden": 160, "depth": 3, "instances": 32, "mask": 28},
}


def build_panet_instance_segmenter(
    *,
    in_channels: int,
    num_classes: int,
    variant: str = "panet_small",
    width_mult: float = 1.0,
    num_instances: int | None = None,
) -> nn.Module:
    name = str(variant).lower().strip()
    if name not in _VARIANTS:
        raise ValueError(f"Unknown PANet variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
...
if __name__ == "__main__":
    ...
    m = build_panet_instance_segmenter(in_channels=3, num_classes=3, variant="panet_tiny", width_mult=0.5)
    out = m(x)
    print("panet_tiny", {k: tuple(v.shape) for k, v in out.items()})
