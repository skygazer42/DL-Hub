...
class Box2Mask(nn.Module):
    """Box2Mask-style instance segmentation (compact-first).
...
def build_box2mask_instance_segmenter(
    *,
    in_channels: int,
    num_classes: int,
    variant: str = "box2mask_small",
    width_mult: float = 1.0,
) -> nn.Module:
    name = str(variant).lower().strip()
    if name not in _VARIANTS:
        raise ValueError(f"Unknown Box2Mask variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
...
if __name__ == "__main__":
    ...
    m = build_box2mask_instance_segmenter(
        in_channels=3, num_classes=3, variant="box2mask_tiny", width_mult=0.5
    )
    out = m(x)
    print("box2mask_tiny", {k: tuple(v.shape) for k, v in out.items()})
