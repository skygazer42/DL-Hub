...
class MaskDINO(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.tokens = InstanceTokenHead(
            int(p4_channels), int(hidden_channels), int(num_queries), depth=3
        )
    ...
    def forward(self, x: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        mask_logits = mask_logits + 0.5 * F.interpolate(
            p3.mean(dim=1, keepdim=True), size=p2.shape[-2:], mode="nearest"
        ).expand(-1, q, -1, -1)
...
if __name__ == "__main__":
    ...
    m = build_mask_dino_instance_segmenter(
        in_channels=3, num_classes=3, variant="mask_dino_tiny", width_mult=0.5
    )
    out = m(x)
