...
class CFM(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.tokens = InstanceTokenHead(int(p4_channels), int(hidden_channels), int(num_regions), depth=2)
    ...
    def forward(self, x: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        seg_context = (p2.mean(dim=1, keepdim=True) + F.interpolate(p3.mean(dim=1, keepdim=True), size=p2.shape[-2:], mode="nearest")) * 0.5
...
_VARIANTS: dict[str, dict[str, int]] = {
    "cfm_tiny": {"stem": 24, "p2": 40, "p3": 64, "p4": 96, "hidden": 96, "depth": 1, "regions": 16, "mask": 16},
    "cfm_small": {"stem": 24, "p2": 48, "p3": 80, "p4": 128, "hidden": 128, "depth": 2, "regions": 24, "mask": 16},
    "cfm_base": {"stem": 32, "p2": 64, "p3": 96, "p4": 160, "hidden": 160, "depth": 3, "regions": 32, "mask": 28},
}
...
if __name__ == "__main__":
    ...
    m = build_cfm_instance_segmenter(in_channels=3, num_classes=3, variant="cfm_tiny", width_mult=0.5)
    out = m(x)
