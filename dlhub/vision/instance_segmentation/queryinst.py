...
class QueryInst(nn.Module):
    """QueryInst-style instance segmentation (compact-first).
...
def build_queryinst_instance_segmenter(
    *,
    in_channels: int,
    num_classes: int,
    variant: str = "queryinst_small",
    width_mult: float = 1.0,
) -> nn.Module:
    name = str(variant).lower().strip()
    if name not in _VARIANTS:
        raise ValueError(f"Unknown QueryInst variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
