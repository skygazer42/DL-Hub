...
class CondInst(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        mask_flat = torch.bmm(ker_flat, mf_flat)
...
_VARIANTS: dict[str, dict] = {
    "condinst_tiny": {"stem": 24, "low": 40, "det": 80, "depth": 1, "head": 80, "mask": 24},
    "condinst_small": {"stem": 24, "low": 48, "det": 96, "depth": 2, "head": 96, "mask": 32},
    "condinst_base": {"stem": 32, "low": 64, "det": 128, "depth": 3, "head": 128, "mask": 48},
}


def build_condinst_instance_segmenter(
    *,
    in_channels: int,
    num_classes: int,
    variant: str = "condinst_small",
    width_mult: float = 1.0,
) -> nn.Module:
    name = str(variant).lower().strip()
    if name not in _VARIANTS:
        raise ValueError(f"Unknown CondInst variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
