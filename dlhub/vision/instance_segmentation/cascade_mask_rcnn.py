...
class _ROIStage(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        return h, self.cls(h), torch.sigmoid(self.box(h))
...
class CascadeMaskRCNN(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.stages = nn.ModuleList([_ROIStage(c, nc) for _ in range(s)])
    ...
    def forward(self, x: torch.Tensor) -> dict[str, torch.Tensor | list[torch.Tensor]]:
        ...
        h = pooled.unsqueeze(1) + self.roi_queries.unsqueeze(0).expand(b, -1, -1)  # (B,R,C)
...
def build_cascade_mask_rcnn_instance_segmenter(
    *,
    in_channels: int,
    num_classes: int,
    variant: str = "cascade_mask_rcnn_small",
...
if __name__ == "__main__":
    ...
    for v in out.values():
        if isinstance(v, list):
            loss_terms.extend([t.mean() for t in v])
        else:
