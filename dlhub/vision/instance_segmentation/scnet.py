...
_VARIANTS: dict[str, dict] = {
    "scnet_tiny": {"stem": 24, "feat": 64, "depth": 1, "rois": 16, "mask": 14},
    "scnet_small": {"stem": 32, "feat": 96, "depth": 2, "rois": 32, "mask": 14},
    "scnet_base": {"stem": 48, "feat": 128, "depth": 3, "rois": 64, "mask": 28},
}


def build_scnet_instance_segmenter(
    *,
    in_channels: int,
    num_classes: int,
    variant: str = "scnet_small",
    width_mult: float = 1.0,
) -> nn.Module:
    name = str(variant).lower().strip()
    if name not in _VARIANTS:
        raise ValueError(f"Unknown SCNet variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
