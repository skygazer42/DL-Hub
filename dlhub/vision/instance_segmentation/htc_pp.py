...
def build_htc_pp_instance_segmenter(
    *,
    in_channels: int,
    num_classes: int,
    variant: str = "htc_pp_small",
    width_mult: float = 1.0,
) -> nn.Module:
    name = str(variant).lower().strip()
    if name not in _VARIANTS:
        raise ValueError(f"Unknown HTCPP variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
...
if __name__ == "__main__":
    ...
    m = build_htc_pp_instance_segmenter(
        in_channels=3, num_classes=3, variant="htc_pp_tiny", width_mult=0.5
    )
    out = m(x)
    print("htc_pp_tiny", {k: tuple(v.shape) for k, v in out.items()})
