...
class DETRMask(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        return {"query_cls_logits": cls_logits, "query_boxes": boxes, "mask_logits": mask_logits}
...
def build_detr_mask_instance_segmenter(
    *,
    in_channels: int,
    num_classes: int,
    variant: str = "detr_mask_small",
    width_mult: float = 1.0,
) -> nn.Module:
    name = str(variant).lower().strip()
    if name not in _VARIANTS:
        raise ValueError(f"Unknown DETR-Mask variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
