...
class CompactModel(nn.Module):
    def __init__(self, *, family: str, in_channels: int, width: int, depth: int):
        ...
        self.text = nn.Linear(32, self.enc.out_channels)
        self.box = nn.Linear(self.enc.out_channels, 4)

    def forward(self, image, text_feat=None):
        feat = self.enc(image)
        text_feat = (
            torch.randn(image.shape[0], 32, device=image.device)
            if text_feat is None
            else text_feat.to(feat.dtype)
        )
        box = torch.sigmoid(self.box(feat + self.text(text_feat)))
        return {"boxes": box}
