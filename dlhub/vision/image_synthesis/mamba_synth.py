...
def build_mamba_synth_generator(*, in_channels:int, variant:str='mamba_synth_small', width_mult:float=1.0):
    return build_toy_generator(family='mamba_synth', variants=_VARIANTS, in_channels=int(in_channels), variant=str(variant), width_mult=float(width_mult))
if __name__ == '__main__': smoke_test_generator(build_mamba_synth_generator, 'mamba_synth_tiny')
