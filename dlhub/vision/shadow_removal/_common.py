...
class ToyModel(nn.Module):
    ...
    def forward(self,image): x=check_nchw(image); out=torch.clamp(x+0.2*torch.tanh(self.head(self.enc(x))),-1,1); return {'shadow_free': out}
...
def smoke_test_model(builder, variant:str): out=builder(in_channels=3, variant=variant, width_mult=0.5)(torch.randn(2,3,64,64)); print(variant, tuple(out['shadow_free'].shape))
