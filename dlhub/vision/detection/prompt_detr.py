...
from dlhub.vision.detection._prompt_detr_utils import (
    MLP,
    SimpleTransformer,
    flatten_hw,
...
class PromptDetrDetector(nn.Module):
...
if __name__ == "__main__":
    ...
    m = build_prompt_prompt_detr_detector(in_channels=3, num_classes=2, variant="prompt_detr_tiny", width_mult=0.5)
    out = m(x)
    print("prompt_detr_tiny", {k: tuple(v.shape) for k, v in out.items()})
