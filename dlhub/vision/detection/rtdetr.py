...
class RTDETRDetector(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        p3, p4, p5 = self.conv_encoder(p3), self.conv_encoder(p4), self.conv_encoder(p5)
...
def build_rtdetr_detector(
    *,
    in_channels: int,
    num_classes: int,
    variant: str = "rtdetr_tiny",
    width_mult: float = 1.0,
) -> nn.Module:
    name = str(variant).lower().strip()
    if name not in _VARIANTS:
        raise ValueError(f"Unknown RT-DETR variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
...
if __name__ == "__main__":
    ...
    m = build_rtdetr_detector(in_channels=3, num_classes=2, variant="rtdetr_tiny", width_mult=0.5)
    out = m(x)
    print("rtdetr_tiny", {k: tuple(v.shape) for k, v in out.items()})
