...
class DNDETRDetector(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        dn_zero = hs.mean() * 0.0
        return {"class_logits": class_logits, "boxes": boxes, "dn_zero": dn_zero}
...
def build_dn_detr_detector(
    *,
    in_channels: int,
    num_classes: int,
    variant: str = "dn_detr_tiny",
    width_mult: float = 1.0,
) -> nn.Module:
    name = str(variant).lower().strip()
    if name not in _VARIANTS:
        raise ValueError(f"Unknown DN-DETR variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
...
if __name__ == "__main__":
    ...
    m = build_dn_detr_detector(in_channels=3, num_classes=2, variant="dn_detr_tiny", width_mult=0.5)
    out = m(x)
    print("dn_detr_tiny", {k: tuple(v.shape) for k, v in out.items()})
