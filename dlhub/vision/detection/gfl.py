...
class GFLHead(nn.Module):
    ...
    def __init__(
        ...
        self.quality = nn.Conv2d(c, 1, kernel_size=3, padding=1)
        self.dist = nn.Conv2d(c, 4 * (rm + 1), kernel_size=3, padding=1)
        self.reg_max = rm
...
class GFLDetector(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> dict[str, list[torch.Tensor]]:
        ...
        cls_out, quality_out, dist_out = [], [], []
        for p in (p3, p4, p5):
            y = self.head.forward_single(p)
            cls_out.append(y["cls_logits"])
            quality_out.append(y["quality_logits"])
            dist_out.append(y["dist_logits"])
        return {"cls_logits": cls_out, "quality_logits": quality_out, "dist_logits": dist_out}
...
def build_gfl_detector(
    *,
    in_channels: int,
    num_classes: int,
    variant: str = "gfl_tiny",
    width_mult: float = 1.0,
) -> nn.Module:
    name = str(variant).lower().strip()
    if name not in _VARIANTS:
        raise ValueError(f"Unknown GFL variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
