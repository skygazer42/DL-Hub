...
class FreeAnchorHead(nn.Module):
    ...
    def __init__(self, *, channels: int, num_classes: int, num_convs: int = 4) -> None:
        ...
        self.obj = nn.Conv2d(c, 1, kernel_size=3, padding=1)
...
class FreeAnchorDetector(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> dict[str, list[torch.Tensor]]:
        ...
        obj_out, cls_out, box_out = [], [], []
        for p in (p3, p4, p5):
            y = self.head.forward_single(p)
            obj_out.append(y["obj_logits"])
            cls_out.append(y["cls_logits"])
            box_out.append(y["bbox_deltas"])
        return {"obj_logits": obj_out, "cls_logits": cls_out, "bbox_deltas": box_out}
...
def build_freeanchor_detector(
    *,
    in_channels: int,
    num_classes: int,
    variant: str = "freeanchor_tiny",
    width_mult: float = 1.0,
) -> nn.Module:
    name = str(variant).lower().strip()
    if name not in _VARIANTS:
        raise ValueError(f"Unknown FreeAnchor variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
