...
def build_conditional_detr_detector(
    *,
    ...
    if name not in _VARIANTS:
        raise ValueError(
            f"Unknown Conditional DETR variant: {variant!r}. Supported: {sorted(_VARIANTS)}"
        )
    spec = _VARIANTS[name]
...
if __name__ == "__main__":
    ...
    m = build_conditional_detr_detector(
        in_channels=3, num_classes=2, variant="conditional_detr_tiny", width_mult=0.5
    )
    out = m(x)
