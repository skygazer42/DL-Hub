...
class PAADetector(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.head = PAAHead(channels=out, num_classes=int(num_classes), num_convs=int(head_convs))
...
_VARIANTS: dict[str, dict] = {
    "paa_tiny": {"stem": 24, "c3": 48, "c4": 64, "c5": 80, "depth": 1, "fpn": 64, "head": 2},
    "paa_small": {"stem": 32, "c3": 64, "c4": 96, "c5": 128, "depth": 2, "fpn": 96, "head": 4},
    "paa_base": {"stem": 48, "c3": 96, "c4": 144, "c5": 192, "depth": 3, "fpn": 128, "head": 4},
}


def build_paa_detector(
    *,
    in_channels: int,
    num_classes: int,
    variant: str = "paa_tiny",
    width_mult: float = 1.0,
) -> nn.Module:
    name = str(variant).lower().strip()
    if name not in _VARIANTS:
        raise ValueError(f"Unknown PAA variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
...
if __name__ == "__main__":
    ...
    m = build_paa_detector(in_channels=3, num_classes=3, variant="paa_tiny", width_mult=0.5)
    out = m(x)
    print("paa_tiny", [tuple(t.shape) for t in out["cls_logits"]])
