...
class VFNetHead(nn.Module):
    ...
    def __init__(self, *, channels: int, num_classes: int, num_convs: int = 4) -> None:
        ...
        self.iou = nn.Conv2d(c, 1, kernel_size=3, padding=1)
...
class VFNetDetector(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.head = VFNetHead(channels=out, num_classes=int(num_classes), num_convs=int(head_convs))

    def forward(self, x: torch.Tensor) -> dict[str, list[torch.Tensor]]:
        x = check_nchw(x)
        c3, c4, c5 = self.backbone(x)
        p3, p4, p5 = self.fpn(c3, c4, c5)
        cls_out, box_out, iou_out = [], [], []
        for p in (p3, p4, p5):
            y = self.head.forward_single(p)
            cls_out.append(y["cls_logits"])
            box_out.append(y["bbox_deltas"])
            iou_out.append(y["iou_logits"])
        return {"cls_logits": cls_out, "bbox_deltas": box_out, "iou_logits": iou_out}


_VARIANTS: dict[str, dict] = {
    "vfnet_tiny": {"stem": 24, "c3": 48, "c4": 64, "c5": 80, "depth": 1, "fpn": 64, "head": 2},
    "vfnet_small": {"stem": 32, "c3": 64, "c4": 96, "c5": 128, "depth": 2, "fpn": 96, "head": 4},
    "vfnet_base": {"stem": 48, "c3": 96, "c4": 144, "c5": 192, "depth": 3, "fpn": 128, "head": 4},
}


def build_vfnet_detector(
    *,
    in_channels: int,
    num_classes: int,
    variant: str = "vfnet_tiny",
    width_mult: float = 1.0,
) -> nn.Module:
    name = str(variant).lower().strip()
    if name not in _VARIANTS:
        raise ValueError(f"Unknown VFNet variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
...
if __name__ == "__main__":
    ...
    m = build_vfnet_detector(in_channels=3, num_classes=3, variant="vfnet_tiny", width_mult=0.5)
    out = m(x)
    print("vfnet_tiny", [tuple(t.shape) for t in out["cls_logits"]])
