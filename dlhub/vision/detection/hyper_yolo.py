...
class HyperYoloDetector(nn.Module):
...
def build_hyper_yolo_detector(
    *,
    in_channels: int,
    num_classes: int,
    variant: str = "hyper_yolo_tiny",
...
if __name__ == "__main__":
    ...
    m = build_hyper_yolo_detector(in_channels=3, num_classes=3, variant="hyper_yolo_tiny", width_mult=0.5)
    out = m(x)
    print("hyper_yolo_tiny", [tuple(t.shape) for t in out["obj_logits"]])
    loss = (
