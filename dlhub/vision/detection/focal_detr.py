...
from dlhub.vision.detection._focal_detr_utils import (
    MLP,
    SimpleTransformer,
    flatten_hw,
...
class FocalDetrDetector(nn.Module):
...
def build_focal_focal_detr_detector(
    *,
    in_channels: int,
    num_classes: int,
...
if __name__ == "__main__":
    ...
    m = build_focal_focal_detr_detector(in_channels=3, num_classes=2, variant="focal_detr_tiny", width_mult=0.5)
    out = m(x)
    print("focal_detr_tiny", {k: tuple(v.shape) for k, v in out.items()})
