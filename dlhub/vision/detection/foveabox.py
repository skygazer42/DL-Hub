...
class FoveaBoxHead(nn.Module):
    ...
    def __init__(self, *, channels: int, num_classes: int, num_convs: int = 4) -> None:
        ...
        self.reg = nn.Conv2d(c, 4, kernel_size=3, padding=1)
        self.ctr = nn.Conv2d(c, 1, kernel_size=3, padding=1)
...
class FoveaBoxDetector(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> dict[str, list[torch.Tensor]]:
        ...
        for p in (p3, p4, p5):
            ...
            reg_out.append(y["reg"])
            ctr_out.append(y["centerness"])
        return {"cls_logits": cls_out, "reg": reg_out, "centerness": ctr_out}
...
def build_foveabox_detector(
    *,
    in_channels: int,
    num_classes: int,
    variant: str = "foveabox_tiny",
    width_mult: float = 1.0,
) -> nn.Module:
    name = str(variant).lower().strip()
    if name not in _VARIANTS:
        raise ValueError(f"Unknown FoveaBox variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
