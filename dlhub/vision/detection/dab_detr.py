...
class DABDETRDetector(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.ref_boxes = nn.Parameter(torch.rand(q, 4) * 0.5)  # (cx,cy,w,h) init
        self.class_head = nn.Linear(dm, nc)
        self.delta_head = MLP(dm, dm, 4, num_layers=3, act="relu")
    ...
    def forward(self, x: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        delta = self.delta_head(hs)
        boxes = torch.sigmoid(self.ref_boxes.unsqueeze(0) + delta)
...
def build_dab_detr_detector(
    *,
    in_channels: int,
    num_classes: int,
    variant: str = "dab_detr_tiny",
    width_mult: float = 1.0,
) -> nn.Module:
    name = str(variant).lower().strip()
    if name not in _VARIANTS:
        raise ValueError(f"Unknown DAB-DETR variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
