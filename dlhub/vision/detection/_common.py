...
class ConvTower(nn.Module):
    ...
    def __init__(self, channels: int, *, num_convs: int = 4, act: str = "relu") -> None:
        super().__init__()
        c = int(channels)
        n = int(num_convs)
        if c <= 0:
...
class BackboneC3C5(nn.Module):
    """Tiny conv backbone that returns (C3, C4, C5) feature maps.

    Strides are approximately (/8, /16, /32) relative to input.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.stem = nn.Sequential(
            ...
            ConvBNAct(stem, stem, kernel_size=3, stride=2, act=act),  # /4
        )

        def stage(in_ch: int, out_ch: int) -> nn.Sequential:
            layers: list[nn.Module] = []
            layers.append(ConvBNAct(in_ch, out_ch, kernel_size=3, stride=2, act=act))
            for _ in range(d - 1):
                layers.append(ConvBNAct(out_ch, out_ch, kernel_size=3, stride=1, act=act))
            return nn.Sequential(*layers)

        self.stage3 = stage(stem, int(c3_channels))  # /8
        self.stage4 = stage(int(c3_channels), int(c4_channels))  # /16
        self.stage5 = stage(int(c4_channels), int(c5_channels))  # /32
