...
class FSAFHead(nn.Module):
    ...
    def forward_single(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        return self.cls(self.cls_tower(x)), torch.relu(self.box(self.reg_tower(x)))
...
class FSAFDetector(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.head = FSAFHead(channels=out, num_classes=int(num_classes), num_convs=int(head_convs))
    ...
    def forward(self, x: torch.Tensor) -> dict[str, list[torch.Tensor]]:
        ...
        cls_out, box_out = [], []
        for p in (p3, p4, p5):
            c, b = self.head.forward_single(p)
            cls_out.append(c)
            box_out.append(b)
        return {"cls_logits": cls_out, "bbox_deltas": box_out}


_VARIANTS: dict[str, dict] = {
    "fsaf_tiny": {"stem": 24, "c3": 48, "c4": 64, "c5": 80, "depth": 1, "fpn": 64, "head": 2},
    "fsaf_small": {"stem": 32, "c3": 64, "c4": 96, "c5": 128, "depth": 2, "fpn": 96, "head": 4},
    "fsaf_base": {"stem": 48, "c3": 96, "c4": 144, "c5": 192, "depth": 3, "fpn": 128, "head": 4},
}


def build_fsaf_detector(
    *,
    in_channels: int,
    num_classes: int,
    variant: str = "fsaf_tiny",
    width_mult: float = 1.0,
) -> nn.Module:
    name = str(variant).lower().strip()
    if name not in _VARIANTS:
        raise ValueError(f"Unknown FSAF variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
...
if __name__ == "__main__":
    ...
    m = build_fsaf_detector(in_channels=3, num_classes=3, variant="fsaf_tiny", width_mult=0.5)
    out = m(x)
    print("fsaf_tiny", [tuple(t.shape) for t in out["cls_logits"]])
