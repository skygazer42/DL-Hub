...
from dlhub.vision.detection._align_detr_utils import (
    MLP,
    SimpleTransformer,
    flatten_hw,
...
class AlignDetrDetector(nn.Module):
...
def build_align_align_detr_detector(
    *,
    in_channels: int,
    num_classes: int,
...
if __name__ == "__main__":
    ...
    m = build_align_align_detr_detector(in_channels=3, num_classes=2, variant="align_detr_tiny", width_mult=0.5)
    out = m(x)
    print("align_detr_tiny", {k: tuple(v.shape) for k, v in out.items()})
