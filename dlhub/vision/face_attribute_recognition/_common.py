...
class TinyFaceAttributeBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.mode == "attr_cnn":
            ...
            local = local + (h - h.mean(dim=(-1, -2), keepdim=True))
...
def smoke_test_face_attribute_recognizer(builder, variant: str) -> None:
    ...
    print(variant, tuple(out["logits"].shape), tuple(out["attribute_map"].shape))
    print("ok")
