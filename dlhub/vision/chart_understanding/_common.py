...
class ToyModel(nn.Module):
    def __init__(
        self, *, family: str, in_channels: int, width: int, depth: int, num_answers: int = 12
    ):
        super().__init__()
        self.family = str(family)
        self.enc = TinyEncoder(in_channels, width, depth)
        self.head = nn.Linear(self.enc.out_channels, int(num_answers))

    def forward(self, image):
        feat = self.enc(image)
        pooled = F.adaptive_avg_pool2d(feat, (1, 1)).flatten(1)
        return {"answer_logits": self.head(pooled)}
...
def smoke_test_model(builder, variant: str):
    out = builder(in_channels=3, variant=variant, width_mult=0.5, num_answers=12)(
