...
def _split_arch_id(arch_id: str) -> tuple[str, str]:
    ...
    return split_arch_id(arch_id, default_prefix="sr")
...
def _extract_builder_name_from_source(src: str) -> str | None:
    ...
    for node in tree.body:
        if isinstance(node, ast.FunctionDef):
            name = str(node.name)
            if name.startswith("build_") and name.endswith("_super_resolver"):
                return name
    return None
...
def _make_lazy_builder(module_name: str, *, builder_name: str, variant: str) -> Builder:
    def _builder(cfg: BuildConfig):
        ...
        mod = importlib.import_module(f"dlhub.vision.super_resolution.{module_name}")
        fn = getattr(mod, builder_name, None)
        if fn is None:
            raise RuntimeError(f"Super-resolution module {module_name!r} missing {builder_name}()")
...
def _extend_registry(registry: dict[str, Builder]) -> None:
    ...
    module_dir = here / "super_resolution"
...
def list_local_arches() -> list[str]:
    ...
    return list_arch_ids(_REGISTRY, prefix="sr")
...
def build_local_model(
    ...
    *,
    ...
    if prefix in {"superres", "super_resolution", "super-resolution"}:
        prefix = "sr"
    if prefix not in {"sr", "local"}:
        raise ValueError(f"Unsupported super-resolution prefix: {prefix!r} (arch_id={arch_id!r})")

    builder = _REGISTRY.get(str(name).lower().strip())
    if builder is None:
        raise UnknownLocalArch(
            f"Unknown super-resolution arch: {arch_id!r}. Tip: run `python scripts/super_resolution_zoo.py --list`."
        )
    return builder(
        BuildConfig(
