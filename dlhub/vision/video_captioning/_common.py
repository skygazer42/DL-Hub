...
class ToyModel(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.frame = TinyEncoder(in_channels, width, depth)
        c = self.frame.out_channels
    ...
    def forward(self, video):
        ...
        feat = self.frame(video.view(b * t, c, h, w)).mean(dim=(2, 3)).view(b, t, -1)
        seq, _ = self.rnn(feat)
        logits = self.head(seq[:, : self.seq_len])
...
def smoke_test_model(builder, variant: str):
    out = builder(in_channels=3, variant=variant, width_mult=0.5, vocab_size=64, seq_len=12)(
