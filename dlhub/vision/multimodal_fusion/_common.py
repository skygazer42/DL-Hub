...
class ToyModel(nn.Module):
    def __init__(self, *, family:str, in_channels:int, width:int, depth:int):
        super().__init__(); self.family=str(family); self.enc=TinyEncoder(in_channels*2,width,depth); self.head=nn.Conv2d(self.enc.out_channels,int(in_channels),1)
    def forward(self, image1, image2): x=torch.cat([check_nchw(image1), check_nchw(image2)], dim=1); feat=self.enc(x); fused=torch.tanh(self.head(feat)); return {'fused': fused}
...
def smoke_test_model(builder, variant:str):
    x1=torch.randn(2,1,64,64); x2=torch.randn(2,1,64,64); out=builder(in_channels=1, variant=variant, width_mult=0.5)(x1,x2); print(variant, tuple(out['fused'].shape))
