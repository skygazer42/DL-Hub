...
class ToyAntiSpoofer(nn.Module):
    def __init__(self, *, family:str, in_channels:int, width:int, depth:int):
        super().__init__(); self.family=str(family); self.enc=TinyEncoder(in_channels,width,depth); c=self.enc.out_channels; self.map=nn.Conv2d(c,1,1); self.cls=nn.Linear(c,2)
    def forward(self,image): feat=self.enc(image); spoof_map=self.map(feat); pooled=F.adaptive_avg_pool2d(feat,(1,1)).flatten(1); logits=self.cls(pooled); return {'logits': logits, 'spoof_map': spoof_map}

def build_toy_spoofer(*, family:str, variants:dict[str,dict[str,int]], in_channels:int, variant:str, width_mult:float=1.0):
    spec=variants[str(variant)]; width=max(16,int(int(spec['width'])*float(width_mult))); return ToyAntiSpoofer(family=str(family), in_channels=int(in_channels), width=width, depth=int(spec['depth']))
