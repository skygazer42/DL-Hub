...
class GenLaneNetDetector(nn.Module):
    """Gen-LaneNet-style detector with camera-conditioned lane tokens."""
    ...
    def __init__(
        ...
        *,
        ...
        self.camera_head = GlobalContextHead(
            int(hidden_channels),
            self.model_dim,
            self.model_dim,
            dropout=float(dropout),
...
def build_genlanenet_lane_detector(
    *,
    in_channels: int,
    num_lanes: int,
    image_size: int = 64,
...
if __name__ == "__main__":
    ...
    m = build_genlanenet_lane_detector(
        in_channels=3, num_lanes=4, num_points=16, num_queries=6, variant="genlanenet_tiny"
    )
    out = m(x)
    print("genlanenet_tiny", {k: tuple(v.shape) for k, v in out.items()})
