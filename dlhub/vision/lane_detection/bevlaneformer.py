...
class BevlaneformerLaneDetector(nn.Module):
    """Toy lane detector for the bevlaneformer family."""
...
_VARIANTS: dict[str, dict[str, int | float]] = {
    "bevlaneformer_tiny": {"stem": 24, "hidden": 40, "depth": 1, "token_dim": 32, "dropout": 0.0},
    "bevlaneformer_small": {"stem": 32, "hidden": 56, "depth": 2, "token_dim": 48, "dropout": 0.0},
    "bevlaneformer_base": {"stem": 40, "hidden": 72, "depth": 3, "token_dim": 64, "dropout": 0.1},
}
...
def build_bevlaneformer_lane_detector(
    *,
    ...
    if name not in _VARIANTS:
        raise ValueError(f"Unknown bevlaneformer variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
