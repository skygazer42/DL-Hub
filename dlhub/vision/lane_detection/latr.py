...
class LATRLaneDetector(nn.Module):
    """LATR-style detector with anchor queries decoded against scene memory."""
    ...
    def __init__(
        ...
        *,
        ...
        self.anchor_queries = nn.Parameter(torch.randn(self.num_queries, self.model_dim) * 0.02)
    ...
    def forward(self, x: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        b = x.shape[0]
        anchor_queries = self.anchor_queries.unsqueeze(0).expand(b, -1, -1)
        decoded = self.decoder(tgt=anchor_queries, memory=memory)
...
_VARIANTS: dict[str, dict[str, int | float]] = {
    "latr_tiny": {"stem": 16, "hidden": 32, "depth": 1, "model_dim": 32, "dropout": 0.0},
    "latr_small": {"stem": 24, "hidden": 48, "depth": 2, "model_dim": 48, "dropout": 0.0},
    "latr_base": {"stem": 32, "hidden": 64, "depth": 3, "model_dim": 64, "dropout": 0.1},
}


def build_latr_lane_detector(
    *,
    in_channels: int,
    num_lanes: int,
    image_size: int = 64,
    ...
    if name not in _VARIANTS:
        raise ValueError(f"Unknown LATR variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
...
if __name__ == "__main__":
    ...
    m = build_latr_lane_detector(
        in_channels=3, num_lanes=4, num_points=16, num_queries=6, variant="latr_tiny"
    )
    out = m(x)
    print("latr_tiny", {k: tuple(v.shape) for k, v in out.items()})
