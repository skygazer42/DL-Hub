...
class TopoLaneDetector(nn.Module):
    """TopoLane-style detector with lane queries and pairwise topology reasoning."""
    ...
    def __init__(
        ...
        *,
        ...
        self.proj = nn.Conv2d(int(hidden_channels), self.model_dim, kernel_size=1)
    ...
    def forward(self, x: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        memory = self.proj(high).flatten(2).transpose(1, 2)
        b = x.shape[0]
        ...
        topology_logits = torch.matmul(decoded, decoded.transpose(1, 2)) / (self.model_dim**0.5)
        return {
...
_VARIANTS: dict[str, dict[str, int | float]] = {
    "topolane_tiny": {"stem": 16, "hidden": 32, "depth": 1, "model_dim": 32, "dropout": 0.0},
    "topolane_small": {"stem": 24, "hidden": 48, "depth": 2, "model_dim": 48, "dropout": 0.0},
    "topolane_base": {"stem": 32, "hidden": 64, "depth": 3, "model_dim": 64, "dropout": 0.1},
}


def build_topolane_lane_detector(
    *,
    in_channels: int,
    num_lanes: int,
    image_size: int = 64,
    ...
    if name not in _VARIANTS:
        raise ValueError(f"Unknown TopoLane variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
...
if __name__ == "__main__":
    ...
    m = build_topolane_lane_detector(
        in_channels=3, num_lanes=4, num_points=16, num_queries=6, variant="topolane_tiny"
    )
    out = m(x)
    print("topolane_tiny", {k: tuple(v.shape) for k, v in out.items()})
