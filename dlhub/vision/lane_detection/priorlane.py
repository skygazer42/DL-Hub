...
class PriorLaneDetector(nn.Module):
    """PriorLane-style detector with prior-conditioned lane queries."""
    ...
    def __init__(
        ...
        *,
        ...
        self.prior_context = GlobalContextHead(
            int(hidden_channels),
            self.model_dim,
            self.model_dim,
            dropout=float(dropout),
        )
        self.prior_embed = nn.Parameter(torch.randn(self.num_queries, self.model_dim) * 0.02)
    ...
    def forward(self, x: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        b = x.shape[0]
        prior_embeddings = self.prior_embed.unsqueeze(0).expand(b, -1, -1)
        prior_embeddings = prior_embeddings + self.prior_context(high).unsqueeze(1)
        decoded = self.decoder(tgt=prior_embeddings, memory=memory)
        ...
        return {
            ...
            "prior_embeddings": decoded,
        }


_VARIANTS: dict[str, dict[str, int | float]] = {
    "priorlane_tiny": {"stem": 16, "hidden": 32, "depth": 1, "model_dim": 32, "dropout": 0.0},
    "priorlane_small": {
        "stem": 24,
        "hidden": 48,
        "depth": 2,
    ...
    "priorlane_base": {"stem": 32, "hidden": 64, "depth": 3, "model_dim": 64, "dropout": 0.1},
}


def build_priorlane_lane_detector(
    *,
    in_channels: int,
    num_lanes: int,
    image_size: int = 64,
    ...
    if name not in _VARIANTS:
        raise ValueError(f"Unknown PriorLane variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
...
if __name__ == "__main__":
    ...
    m = build_priorlane_lane_detector(
        in_channels=3, num_lanes=4, num_points=16, num_queries=6, variant="priorlane_tiny"
    )
    out = m(x)
    print("priorlane_tiny", {k: tuple(v.shape) for k, v in out.items()})
