...
class Anchor3DLaneDetector(nn.Module):
    """Anchor3DLane-style detector with 3D anchor embeddings and heights."""
    ...
    def __init__(
        ...
        *,
        ...
        self.anchor_embed = nn.Parameter(torch.randn(self.num_queries, self.model_dim) * 0.02)
        ...
        self.height_head = nn.Linear(self.model_dim, self.num_points)
    ...
    def forward(self, x: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        b = x.shape[0]
        anchor_embeddings = self.anchor_embed.unsqueeze(0).expand(b, -1, -1)
        decoded = self.decoder(tgt=anchor_embeddings, memory=memory)
        ...
        lane_heights = torch.tanh(self.height_head(decoded))
        return {
            "anchor_embeddings": decoded,
...
def build_anchor3dlane_lane_detector(
    *,
    in_channels: int,
    num_lanes: int,
    image_size: int = 64,
    ...
    num_anchors: int = 24,
    num_queries: int = 6,
    variant: str = "anchor3dlane_small",
    ...
    if name not in _VARIANTS:
        raise ValueError(
            f"Unknown Anchor3DLane variant: {variant!r}. Supported: {sorted(_VARIANTS)}"
        )
    spec = _VARIANTS[name]
...
if __name__ == "__main__":
    ...
    m = build_anchor3dlane_lane_detector(
        in_channels=3, num_lanes=4, num_points=16, num_queries=6, variant="anchor3dlane_tiny"
    )
    out = m(x)
    print("anchor3dlane_tiny", {k: tuple(v.shape) for k, v in out.items()})
