...
class MambalaneLaneDetector(nn.Module):
    """Toy lane detector for the mambalane family."""
...
_VARIANTS: dict[str, dict[str, int | float]] = {
    "mambalane_tiny": {"stem": 24, "hidden": 40, "depth": 1, "token_dim": 32, "dropout": 0.0},
    "mambalane_small": {"stem": 32, "hidden": 56, "depth": 2, "token_dim": 48, "dropout": 0.0},
    "mambalane_base": {"stem": 40, "hidden": 72, "depth": 3, "token_dim": 64, "dropout": 0.1},
}


def build_mambalane_lane_detector(
    *,
    in_channels: int,
    num_lanes: int,
    image_size: int = 64,
    ...
    if name not in _VARIANTS:
        raise ValueError(f"Unknown mambalane variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
...
if __name__ == "__main__":
    ...
    m = build_mambalane_lane_detector(
        in_channels=3,
        num_lanes=4,
        num_points=16,
        variant="mambalane_tiny",
    )
    out = m(x)
    print("mambalane_tiny", {k: tuple(v.shape) for k, v in out.items()})
