...
class Laneformer2LaneDetector(nn.Module):
    """Toy lane detector for the laneformer2 family."""
...
_VARIANTS: dict[str, dict[str, int | float]] = {
    "laneformer2_tiny": {"stem": 16, "hidden": 32, "depth": 1, "token_dim": 32, "dropout": 0.0},
    "laneformer2_small": {"stem": 24, "hidden": 48, "depth": 2, "token_dim": 48, "dropout": 0.0},
    "laneformer2_base": {"stem": 32, "hidden": 64, "depth": 3, "token_dim": 64, "dropout": 0.1},
}


def build_laneformer2_lane_detector(
    *,
    in_channels: int,
    num_lanes: int,
    image_size: int = 64,
    ...
    if name not in _VARIANTS:
        raise ValueError(f"Unknown laneformer2 variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    ...
    return Laneformer2LaneDetector(
        in_channels=int(in_channels),
...
if __name__ == "__main__":
    ...
    m = build_laneformer2_lane_detector(
        in_channels=3,
        num_lanes=4,
        num_points=16,
        variant="laneformer2_tiny",
    )
    out = m(x)
    print("laneformer2_tiny", {k: tuple(v.shape) for k, v in out.items()})
