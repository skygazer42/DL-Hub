...
class LSTRLaneDetector(nn.Module):
    """A tiny query-based lane detector inspired by transformer lane models."""
    ...
    def __init__(
        ...
        *,
        ...
        self.input_proj = nn.Conv2d(int(hidden_channels), self.model_dim, kernel_size=1)
    ...
    def forward(self, x: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        memory = self.input_proj(high).flatten(2).transpose(1, 2)
        b = memory.shape[0]
...
_VARIANTS: dict[str, dict[str, int | float]] = {
    "lstr_tiny": {"stem": 16, "hidden": 32, "depth": 1, "model_dim": 32, "dropout": 0.0},
    "lstr_small": {"stem": 24, "hidden": 48, "depth": 2, "model_dim": 48, "dropout": 0.0},
    "lstr_base": {"stem": 32, "hidden": 64, "depth": 3, "model_dim": 64, "dropout": 0.1},
}


def build_lstr_lane_detector(
    *,
    in_channels: int,
    num_lanes: int,
    image_size: int = 64,
    ...
    if name not in _VARIANTS:
        raise ValueError(f"Unknown LSTR variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
...
if __name__ == "__main__":
    ...
    out = m(x)
    print("lstr_tiny", {k: tuple(v.shape) for k, v in out.items()})
