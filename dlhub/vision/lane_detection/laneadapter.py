...
class LaneadapterLaneDetector(nn.Module):
...
_VARIANTS: dict[str, dict[str, int | float]] = {
    "laneadapter_tiny": {"stem": 16, "hidden": 32, "depth": 1, "token_dim": 32, "dropout": 0.0},
    "laneadapter_small": {"stem": 24, "hidden": 48, "depth": 2, "token_dim": 48, "dropout": 0.0},
    "laneadapter_base": {"stem": 32, "hidden": 64, "depth": 3, "token_dim": 64, "dropout": 0.1},
}


def build_laneadapter_lane_detector(
    *,
    in_channels: int,
    num_lanes: int,
    image_size: int = 64,
    ...
    return LaneadapterLaneDetector(
        in_channels=int(in_channels),
...
if __name__ == "__main__":
    ...
    m = build_laneadapter_lane_detector(
        in_channels=3,
        num_lanes=4,
        num_points=16,
        variant="laneadapter_tiny",
    )
    out = m(x)
    print("laneadapter_tiny", {k: tuple(v.shape) for k, v in out.items()})
