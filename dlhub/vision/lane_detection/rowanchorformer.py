...
class RowanchorformerLaneDetector(nn.Module):
...
def build_rowanchorformer_lane_detector(
    *,
    in_channels: int,
    num_lanes: int,
    image_size: int = 64,
    ...
    num_anchors: int = 24,
    num_queries: int = 6,
    variant: str = "rowanchorformer_small",
    ...
    return RowanchorformerLaneDetector(
        in_channels=int(in_channels),
...
if __name__ == "__main__":
    ...
    m = build_rowanchorformer_lane_detector(
        in_channels=3,
        num_lanes=4,
        num_points=16,
        variant="rowanchorformer_tiny",
    )
    out = m(x)
    print("rowanchorformer_tiny", {k: tuple(v.shape) for k, v in out.items()})
