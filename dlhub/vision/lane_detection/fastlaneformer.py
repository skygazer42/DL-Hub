...
class FastlaneformerLaneDetector(nn.Module):
...
_VARIANTS: dict[str, dict[str, int | float]] = {
    "fastlaneformer_tiny": {"stem": 16, "hidden": 32, "depth": 1, "token_dim": 32, "dropout": 0.0},
    "fastlaneformer_small": {"stem": 24, "hidden": 48, "depth": 2, "token_dim": 48, "dropout": 0.0},
    "fastlaneformer_base": {"stem": 32, "hidden": 64, "depth": 3, "token_dim": 64, "dropout": 0.1},
}


def build_fastlaneformer_lane_detector(
    *,
    in_channels: int,
    num_lanes: int,
    image_size: int = 64,
    ...
    num_anchors: int = 24,
    num_queries: int = 6,
    variant: str = "fastlaneformer_small",
    ...
    return FastlaneformerLaneDetector(
        in_channels=int(in_channels),
...
if __name__ == "__main__":
    ...
    m = build_fastlaneformer_lane_detector(
        in_channels=3,
        num_lanes=4,
        num_points=16,
        variant="fastlaneformer_tiny",
    )
    out = m(x)
    print("fastlaneformer_tiny", {k: tuple(v.shape) for k, v in out.items()})
