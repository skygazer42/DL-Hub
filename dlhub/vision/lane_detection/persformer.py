...
class PersFormerLaneDetector(nn.Module):
    """PersFormer-style detector with query decoding and perspective features."""
    ...
    def forward(self, x: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        perspective_features = self.proj(high)
        memory = perspective_features.flatten(2).transpose(1, 2)
        b = x.shape[0]
...
def build_persformer_lane_detector(
    *,
    in_channels: int,
    num_lanes: int,
    image_size: int = 64,
    ...
    if name not in _VARIANTS:
        raise ValueError(f"Unknown PersFormer variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
...
if __name__ == "__main__":
    ...
    m = build_persformer_lane_detector(
        in_channels=3, num_lanes=4, num_points=16, num_queries=6, variant="persformer_tiny"
    )
    out = m(x)
    print("persformer_tiny", {k: tuple(v.shape) for k, v in out.items()})
