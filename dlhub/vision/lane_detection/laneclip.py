...
class LaneclipLaneDetector(nn.Module):
...
_VARIANTS: dict[str, dict[str, int | float]] = {
    "laneclip_tiny": {"stem": 16, "hidden": 32, "depth": 1, "token_dim": 32, "dropout": 0.0},
    "laneclip_small": {"stem": 24, "hidden": 48, "depth": 2, "token_dim": 48, "dropout": 0.0},
    "laneclip_base": {"stem": 32, "hidden": 64, "depth": 3, "token_dim": 64, "dropout": 0.1},
}


def build_laneclip_lane_detector(
    *,
    in_channels: int,
    num_lanes: int,
    image_size: int = 64,
...
if __name__ == "__main__":
    ...
    m = build_laneclip_lane_detector(
        in_channels=3,
        num_lanes=4,
        num_points=16,
        variant="laneclip_tiny",
    )
    out = m(x)
    print("laneclip_tiny", {k: tuple(v.shape) for k, v in out.items()})
