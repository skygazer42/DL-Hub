...
class SparseLaneformerLaneDetector(nn.Module):
    """Toy lane detector for the sparse_laneformer family."""
...
_VARIANTS: dict[str, dict[str, int | float]] = {
    "sparse_laneformer_tiny": {"stem": 16, "hidden": 32, "depth": 1, "token_dim": 32, "dropout": 0.0},
    "sparse_laneformer_small": {"stem": 24, "hidden": 48, "depth": 2, "token_dim": 48, "dropout": 0.0},
    "sparse_laneformer_base": {"stem": 32, "hidden": 64, "depth": 3, "token_dim": 64, "dropout": 0.1},
}


def build_sparse_laneformer_lane_detector(
    *,
    in_channels: int,
    num_lanes: int,
    image_size: int = 64,
    ...
    num_anchors: int = 24,
    num_queries: int = 4,
    variant: str = "sparse_laneformer_small",
    ...
    if name not in _VARIANTS:
        raise ValueError(f"Unknown sparse_laneformer variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    ...
    return SparseLaneformerLaneDetector(
        in_channels=int(in_channels),
...
if __name__ == "__main__":
    ...
    m = build_sparse_laneformer_lane_detector(
        in_channels=3,
        num_lanes=4,
        num_points=16,
        variant="sparse_laneformer_tiny",
    )
    out = m(x)
    print("sparse_laneformer_tiny", {k: tuple(v.shape) for k, v in out.items()})
