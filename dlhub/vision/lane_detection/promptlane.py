...
class PromptlaneLaneDetector(nn.Module):
    """Toy lane detector for the promptlane family."""
...
_VARIANTS: dict[str, dict[str, int | float]] = {
    "promptlane_tiny": {"stem": 16, "hidden": 32, "depth": 1, "token_dim": 32, "dropout": 0.0},
    "promptlane_small": {"stem": 24, "hidden": 48, "depth": 2, "token_dim": 48, "dropout": 0.0},
    "promptlane_base": {"stem": 32, "hidden": 64, "depth": 3, "token_dim": 64, "dropout": 0.1},
}


def build_promptlane_lane_detector(
    *,
    in_channels: int,
    num_lanes: int,
    image_size: int = 64,
    ...
    if name not in _VARIANTS:
        raise ValueError(f"Unknown promptlane variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
