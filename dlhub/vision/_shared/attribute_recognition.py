...
class CompactAttributeRecognizer(nn.Module):
    def __init__(
        ...
        self.cls = nn.Linear(self.enc.out_channels, int(num_attributes))
...
def smoke_test_attr(builder, variant: str):
    model = builder(in_channels=3, num_attributes=12, variant=variant, width_mult=0.5)
    out = model(torch.randn(2, 3, 128, 64))
    print(variant, tuple(out["attribute_logits"].shape))
