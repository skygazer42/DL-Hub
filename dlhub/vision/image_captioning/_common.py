...
class ToyModel(nn.Module):
    def __init__(self, *, family:str, in_channels:int, width:int, depth:int, vocab_size:int=64, seq_len:int=12):
        super().__init__(); self.family=str(family); self.seq_len=int(seq_len); self.enc=TinyEncoder(in_channels,width,depth); self.head=nn.Linear(self.enc.out_channels,int(vocab_size))
    def forward(self,image): feat=self.enc(image); logits=self.head(feat).unsqueeze(1).expand(-1,self.seq_len,-1); return {'logits': logits, 'tokens': logits.argmax(dim=-1)}
