...
def build_nts_net_fgvc_classifier(
    *, in_channels: int, num_classes: int, variant: str = "nts_net_small", image_size: int = 64, width_mult: float = 1.0, dropout: float = 0.1
) -> nn.Module:
    return build_fgvc_model(NTSNet, variants=_VARIANTS, in_channels=in_channels, num_classes=num_classes, variant=variant, image_size=image_size, width_mult=width_mult, dropout=dropout, family="nts_net")
