...
class TinyReflectionBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.mode in {"specular", "glass", "water", "edge", "contour"}:
...
class TinyReflectionDetector(nn.Module):
    ...
    def forward(self, image: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        return {
            ...
            "boundary": torch.sigmoid(self.boundary(feat)),
            "reflection_score": torch.sigmoid(self.score(pooled)),
        }
...
def smoke_test_reflection_detector(builder, variant: str) -> None:
    ...
    print(variant, tuple(out["mask"].shape), tuple(out["reflection_score"].shape))
    print("ok")
