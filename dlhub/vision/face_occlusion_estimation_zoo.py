    ...
    for family in _FAMILIES:
        for size in _SIZES:
            ...
            def _builder(cfg: BuildConfig, family: str = family, variant: str = variant):
                module = importlib.import_module(f"dlhub.vision.face_occlusion_estimation.{family}")
                fn = getattr(module, f"build_{family}_occlusion_estimator")
                return fn(
...
def list_local_arches() -> list[str]:
    return [f"faceocc:{name}" for name in sorted(_REGISTRY)]
