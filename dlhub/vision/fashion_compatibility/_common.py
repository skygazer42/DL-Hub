...
class ToyModel(nn.Module):
    def __init__(self, *, family:str, in_channels:int, width:int, depth:int):
        super().__init__(); self.family=str(family); self.enc=TinyEncoder(in_channels,width,depth); self.head=nn.Linear(self.enc.out_channels,1)
    def forward(self,item1,item2): f1=self.enc(item1).mean(dim=(2,3)); f2=self.enc(item2).mean(dim=(2,3)); score=torch.sigmoid(self.head(f1*f2)).squeeze(-1); return {'compatibility': score}
...
def smoke_test_model(builder, variant:str): out=builder(in_channels=3, variant=variant, width_mult=0.5)(torch.randn(2,3,64,64), torch.randn(2,3,64,64)); print(variant, tuple(out['compatibility'].shape))
