...
class CompactModel(nn.Module):
    def __init__(self, *, family: str, in_channels: int, width: int, depth: int):
        ...
        self.head = nn.Linear(self.enc.out_channels, 1)

    def forward(self, item1, item2):
        f1 = self.enc(item1).mean(dim=(2, 3))
        f2 = self.enc(item2).mean(dim=(2, 3))
        score = torch.sigmoid(self.head(f1 * f2)).squeeze(-1)
...
def smoke_test_model(builder, variant: str):
    ...
    print(variant, tuple(out["compatibility"].shape))
