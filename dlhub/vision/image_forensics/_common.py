...
def build_toy_anomaly_detector(*, family: str, variants: dict[str, dict[str, int]], in_channels: int, variant: str, width_mult: float = 1.0) -> nn.Module:
    spec = variants[str(variant)]
    width = max(16, int(int(spec['width']) * float(width_mult)))
    return TinyAnomalyDetector(family=str(family), in_channels=int(in_channels), width=width, depth=int(spec['depth']))

def smoke_test_anomaly(builder, variant: str) -> None:
    model = builder(in_channels=3, variant=variant, width_mult=0.5)
    out = model(torch.randn(2,3,64,64))
    print(variant, {k: tuple(v.shape) if torch.is_tensor(v) and v.ndim>0 else v.shape if hasattr(v,'shape') else type(v).__name__ for k,v in out.items()})
