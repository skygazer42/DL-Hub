...
def _split_arch_id(arch_id: str) -> tuple[str, str]:
    ...
    return split_arch_id(arch_id, default_prefix="outpaint")
...
def _registry() -> dict[str, Builder]:
    ...
    return make_lazy_family_registry(
        _FAMILIES,
        _SIZES,
        module_template="dlhub.vision.image_outpainting.{family}",
        builder_template="build_{family}_outpainter",
...
def list_local_arches() -> list[str]:
    ...
    return list_arch_ids(_REGISTRY, prefix="outpaint")
...
def build_local_model(arch_id: str, *, in_channels: int, width_mult: float = 1.0):
    ...
    if builder is None:
        raise UnknownLocalArch(
            f"Unknown image outpainting arch: {arch_id!r}. Tip: import `dlhub.vision.image_outpainting_zoo` and call `list_local_arches()`."
        )
