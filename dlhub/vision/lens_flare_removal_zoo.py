...
_FAMILIES = ["glow_flare", "streak_flare", "halo_flare", "residual_flare", "context_flare", "transformer_flare", "prompt_flare", "dual_flare", "frequency_flare", "mamba_flare"]
    ...
    for family in _FAMILIES:
        for size in _SIZES:
            ...
            def _builder(cfg: BuildConfig, family: str = family, variant: str = variant):
                module = importlib.import_module(f"dlhub.vision.lens_flare_removal.{family}")
                fn = getattr(module, f"build_{family}_flare_remover")
...
def list_local_arches() -> list[str]:
    return [f"flare:{name}" for name in sorted(_REGISTRY)]
...
def build_local_model(arch_id: str, *, in_channels: int, width_mult: float = 1.0):
    ...
    if builder is None:
        raise UnknownLocalArch(f"Unknown lens flare removal arch: {arch_id!r}. Tip: import `dlhub.vision.lens_flare_removal_zoo` and call `list_local_arches()`.")
