...
def _split_arch_id(arch_id: str) -> tuple[str, str]:
    ...
    return split_arch_id(arch_id, default_prefix="salbox")
...
def _registry() -> dict[str, Builder]:
    ...
    return make_lazy_family_registry(
        _FAMILIES,
        _SIZES,
        module_template="dlhub.vision.salient_object_detection_boxes.{family}",
        builder_template="build_{family}_box_detector",
...
def list_local_arches() -> list[str]:
    ...
    return list_arch_ids(_REGISTRY, prefix="salbox")
...
def build_local_model(arch_id: str, *, in_channels: int, width_mult: float = 1.0):
    ...
    if builder is None:
        raise UnknownLocalArch(
            f"Unknown salient object detection boxes arch: {arch_id!r}. Tip: import `dlhub.vision.salient_object_detection_boxes_zoo` and call `list_local_arches()`."
        )
