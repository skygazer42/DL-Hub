...
class ToyModel(nn.Module):
    def __init__(self, *, family:str, in_channels:int, width:int, depth:int, text_dim:int=32):
        super().__init__(); self.family=str(family); self.enc=TinyEncoder(in_channels,width,depth); c=self.enc.out_channels; self.txt=nn.Linear(text_dim,c); self.mask=nn.Conv2d(c,1,1)
    def forward(self,image,text_feat=None): x=check_nchw(image); feat=self.enc(x); if text_feat is None: text_feat=torch.randn(image.shape[0],32,device=image.device); bias=self.txt(text_feat.to(feat.dtype)).unsqueeze(-1).unsqueeze(-1); logits=self.mask(feat+bias); return {'logits': logits, 'mask': torch.sigmoid(logits)}
...
def smoke_test_model(builder, variant:str): out=builder(in_channels=3, variant=variant, width_mult=0.5)(torch.randn(2,3,64,64)); print(variant, tuple(out['mask'].shape))
