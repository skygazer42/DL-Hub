...
class ToyModel(nn.Module):
    def __init__(self, *, family:str, in_channels:int, width:int, depth:int):
        super().__init__(); self.family=str(family); self.enc=TinyEncoder(in_channels,width,depth); c=self.enc.out_channels; self.score=nn.Conv2d(c,1,1); self.offset=nn.Conv2d(c,4,1)
    def forward(self,image): feat=self.enc(image); return {'score_map': self.score(feat), 'segments': self.offset(feat)}
