...
_VARIANTS: dict[str, dict[str, float | int]] = {
    "lsanet_sr_tiny": {"embed": 24, "depth": 2, "heads": 4, "window": 4, "mlp": 2.0},
    "lsanet_sr_small": {"embed": 32, "depth": 3, "heads": 4, "window": 4, "mlp": 2.0},
    "lsanet_sr_base": {"embed": 48, "depth": 4, "heads": 6, "window": 4, "mlp": 2.0},
}


def build_lsanet_sr_super_resolver(
    *,
    in_channels: int,
    variant: str = "lsanet_sr_small",
...
if __name__ == "__main__":
    torch.manual_seed(0)
    m = build_lsanet_sr_super_resolver(in_channels=3, variant="lsanet_sr_tiny")
    x = torch.randn(2, 3, 16, 16)
    y = m(x)
    print("lsanet_sr_tiny", tuple(y["sr"].shape))
