...
_VARIANTS: dict[str, dict[str, int]] = _default_variants("edge_sr")


def build_edge_sr_super_resolver(
    *,
    in_channels: int,
    variant: str = "edge_sr_small",
    ...
    if name not in _VARIANTS:
        raise ValueError(f"Unknown EdgeSR variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
...
if __name__ == "__main__":
    torch.manual_seed(0)
    m = build_edge_sr_super_resolver(in_channels=3, variant="edge_sr_tiny")
    x = torch.randn(2, 3, 16, 16)
    y = m(x)
    print("edge_sr_tiny", tuple(y["sr"].shape))
