...
_VARIANTS: dict[str, dict[str, float | int]] = {
    "imdn_sr_tiny": {"features": 24, "num_blocks": 3, "res_scale": 0.1},
    "imdn_sr_small": {"features": 32, "num_blocks": 5, "res_scale": 0.1},
    "imdn_sr_base": {"features": 48, "num_blocks": 8, "res_scale": 0.1},
}


def build_imdn_sr_super_resolver(
    *,
    in_channels: int,
    variant: str = "imdn_sr_small",
...
if __name__ == "__main__":
    torch.manual_seed(0)
    m = build_imdn_sr_super_resolver(in_channels=3, variant="imdn_sr_tiny")
    x = torch.randn(2, 3, 16, 16)
    y = m(x)
    print("imdn_sr_tiny", tuple(y["sr"].shape))
