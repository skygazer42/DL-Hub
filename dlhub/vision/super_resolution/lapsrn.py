...
_VARIANTS: dict[str, dict[str, int]] = _default_variants("lapsrn")


def build_lapsrn_super_resolver(
    *,
    in_channels: int,
    variant: str = "lapsrn_small",
    ...
    if name not in _VARIANTS:
        raise ValueError(f"Unknown LapSRN variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
...
if __name__ == "__main__":
    torch.manual_seed(0)
    m = build_lapsrn_super_resolver(in_channels=3, variant="lapsrn_tiny")
    x = torch.randn(2, 3, 16, 16)
    y = m(x)
    print("lapsrn_tiny", tuple(y["sr"].shape))
