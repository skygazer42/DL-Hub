...
_VARIANTS: dict[str, dict[str, int]] = _default_variants("span_sr")


def build_span_sr_super_resolver(
    *,
    in_channels: int,
    variant: str = "span_sr_small",
    ...
    if name not in _VARIANTS:
        raise ValueError(f"Unknown SpanSR variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
...
if __name__ == "__main__":
    torch.manual_seed(0)
    m = build_span_sr_super_resolver(in_channels=3, variant="span_sr_tiny")
    x = torch.randn(2, 3, 16, 16)
    y = m(x)
    print("span_sr_tiny", tuple(y["sr"].shape))
