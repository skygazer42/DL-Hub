...
_VARIANTS: dict[str, dict[str, int]] = _default_variants("dlgsr")


def build_dlgsr_super_resolver(
    *,
    in_channels: int,
    variant: str = "dlgsr_small",
    ...
    if name not in _VARIANTS:
        raise ValueError(f"Unknown DLGSR variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
...
if __name__ == "__main__":
    torch.manual_seed(0)
    m = build_dlgsr_super_resolver(in_channels=3, variant="dlgsr_tiny")
    x = torch.randn(2, 3, 16, 16)
    y = m(x)
    print("dlgsr_tiny", tuple(y["sr"].shape))
