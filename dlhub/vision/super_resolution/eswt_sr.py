...
_VARIANTS: dict[str, dict[str, int]] = _default_variants("eswt_sr")


def build_eswt_sr_super_resolver(
    *,
    in_channels: int,
    variant: str = "eswt_sr_small",
    ...
    if name not in _VARIANTS:
        raise ValueError(f"Unknown EswtSR variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
...
if __name__ == "__main__":
    torch.manual_seed(0)
    m = build_eswt_sr_super_resolver(in_channels=3, variant="eswt_sr_tiny")
