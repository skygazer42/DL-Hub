...
_VARIANTS: dict[str, dict[str, int]] = _default_variants("drcn")


def build_drcn_super_resolver(
    *,
    in_channels: int,
    variant: str = "drcn_small",
    ...
    if name not in _VARIANTS:
        raise ValueError(f"Unknown DRCN variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
...
if __name__ == "__main__":
    torch.manual_seed(0)
    m = build_drcn_super_resolver(in_channels=3, variant="drcn_tiny")
    x = torch.randn(2, 3, 16, 16)
    y = m(x)
    print("drcn_tiny", tuple(y["sr"].shape))
