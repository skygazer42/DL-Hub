...
class GcnetClassifier(nn.Module):
    def __init__(
        self,
        *,
        in_channels: int,
        ...
        def make_stage(in_ch: int, out_ch: int, depth: int, *, stride: int) -> nn.Sequential:
            ...
            attn_mod = GlobalContextBlock(out_ch, reduction=16)
            blocks.append(_ResidualBlock(in_ch, out_ch, stride=int(stride), attn=attn_mod))
            for _ in range(int(depth) - 1):
                blocks.append(_ResidualBlock(out_ch, out_ch, stride=1, attn=GlobalContextBlock(out_ch, reduction=16)))
...
if __name__ == '__main__':
    ...
    m = build_gcnet_classifier(in_channels=3, num_classes=10, variant='tiny', width_mult=1.0)
    y = m(x)
    print('gcnet', tuple(y.shape))
