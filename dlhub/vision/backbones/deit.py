...
class DeitClassifier(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.pos = nn.Parameter(torch.zeros(1, int(num_patches) + 2, embed_dim))

        self.drop = nn.Dropout(p=float(dropout))
        self.blocks = nn.ModuleList(
            [
                TransformerEncoderBlock(embed_dim, heads, dropout=float(dropout))
    ...
    def forward(self, x: torch.Tensor):
        ...
        if hasattr(self, "dist_token"):
            ...
            tokens = torch.cat([cls, dist, tokens], dim=1)
        else:
            tokens = torch.cat([cls, dist, tokens], dim=1)
        ...
        cls, dist = tokens[:, 0], tokens[:, 1]
        return (self.head(cls), self.head(dist))
...
if __name__ == "__main__":
    ...
    m = build_deit_classifier(in_channels=3, num_classes=10, variant="tiny")
    y = m(x)
    if isinstance(y, tuple):
        print("deit", tuple(y[0].shape), tuple(y[1].shape))
    else:
        print("deit", tuple(y.shape))
