...
from dlhub.vision.backbones._blocks import ConvBNAct, GlobalAvgPoolHead, SKConv, scale_channels
...
class _ResidualBlock(nn.Module):
    def __init__(self, in_ch: int, out_ch: int, *, stride: int, attn: nn.Module) -> None:
        ...
        self.conv2 = nn.Sequential(
            SKConv(out_ch, kernel_sizes=(3, 5), reduction=16), nn.BatchNorm2d(out_ch)
        )
        self.attn = attn
...
class ResnestClassifier(nn.Module):
    def __init__(
        self,
        *,
        in_channels: int,
        ...
        def make_stage(in_ch: int, out_ch: int, depth: int, *, stride: int) -> nn.Sequential:
            ...
            attn_mod = nn.Identity()  # SK uses SKConv in conv path
...
if __name__ == "__main__":
    ...
    m = build_resnest_classifier(in_channels=3, num_classes=10, variant="tiny", width_mult=1.0)
    y = m(x)
    print("resnest", tuple(y.shape))
