...
class DaViTBlock(nn.Module):
    """DaViT-style dual attention (simplified): depthwise conv + SE + token MHSA."""

    def __init__(self, dim: int, *, num_heads: int, drop_path: float = 0.0) -> None:
        super().__init__()
        d = int(dim)
        self.local = nn.Sequential(
            nn.Conv2d(d, d, kernel_size=3, padding=1, groups=d, bias=False),
            nn.BatchNorm2d(d),
...
_VARIANTS: dict[str, dict] = {
    "davit_tiny": {"dims": (96, 192, 384, 768), "depths": (1, 1, 3, 1), "heads": (3, 6, 12, 24)},
    "davit_small": {"dims": (96, 192, 384, 768), "depths": (2, 2, 6, 2), "heads": (3, 6, 12, 24)},
}


def build_davit_classifier(
    *,
    in_channels: int,
    num_classes: int,
    variant: str = "davit_tiny",
    width_mult: float = 1.0,
    drop_path: float = 0.1,
    dropout: float = 0.1,
) -> nn.Module:
    ...
    if name not in _VARIANTS:
        raise ValueError(f"Unknown DaViT variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
    return DaViTClassifier(
