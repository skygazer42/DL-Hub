...
class CoordattnetClassifier(nn.Module):
    def __init__(
        self,
        *,
        in_channels: int,
        ...
        def make_stage(in_ch: int, out_ch: int, depth: int, *, stride: int) -> nn.Sequential:
            ...
            attn_mod = CoordAttention(out_ch, reduction=32)
            blocks.append(_ResidualBlock(in_ch, out_ch, stride=int(stride), attn=attn_mod))
            for _ in range(int(depth) - 1):
                blocks.append(_ResidualBlock(out_ch, out_ch, stride=1, attn=CoordAttention(out_ch, reduction=32)))
...
def build_coordattnet_classifier(
    *,
    in_channels: int,
    num_classes: int,
    variant: str = 'tiny',
...
if __name__ == '__main__':
    ...
    m = build_coordattnet_classifier(in_channels=3, num_classes=10, variant='tiny', width_mult=1.0)
    y = m(x)
    print('coordattnet', tuple(y.shape))
