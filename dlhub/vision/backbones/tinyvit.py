...
_VARIANTS: dict[str, dict] = {
    "tinyvit_5m": {"embed": 96, "depths": (2, 2, 6, 2), "heads": (3, 6, 12, 24), "window": 8},
    "tinyvit_11m": {"embed": 128, "depths": (2, 2, 12, 2), "heads": (4, 8, 16, 32), "window": 8},
}


def build_tinyvit_classifier(
    *,
    in_channels: int,
    num_classes: int,
    variant: str = "tinyvit_5m",
    image_size: int = 64,
    width_mult: float = 1.0,
    drop_path: float = 0.1,
    dropout: float = 0.1,
    ...
    if name not in _VARIANTS:
        raise ValueError(f"Unknown TinyViT variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
    return TinyViTClassifier(
