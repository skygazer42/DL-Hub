...
class PerceiverClassifier(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        inp = self.embed(x)  # (B, N, input_dim)
        lat = self.latents.expand(inp.shape[0], -1, -1)
        lat = lat + self.cross(lat, inp)
        lat = self.blocks(lat)
        lat = self.norm(lat)
        lat = self.drop(lat.mean(dim=1))
...
def build_perceiver_classifier(
    *,
    in_channels: int,
    num_classes: int,
    variant: str = "perceiver_tiny",
    image_size: int = 64,
    patch_size: int = 4,
    dropout: float = 0.1,
) -> nn.Module:
    ...
    if name not in _VARIANTS:
        raise ValueError(f"Unknown Perceiver variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
