...
class PNASNetClassifier(NASNetworkClassifier):
    def __init__(
        self,
        *,
        in_channels: int,
...
_VARIANTS: dict[str, dict] = {
    "pnasnet_tiny": {"init_channels": 12, "num_cells": 6, "stem_multiplier": 3},
    "pnasnet_small": {"init_channels": 16, "num_cells": 8, "stem_multiplier": 3},
    "pnasnet_base": {"init_channels": 24, "num_cells": 12, "stem_multiplier": 3},
}


def build_pnasnet_classifier(
    *,
    in_channels: int,
    num_classes: int,
    variant: str = "pnasnet_small",
    width_mult: float = 1.0,
    drop_path: float = 0.1,
    dropout: float = 0.1,
) -> nn.Module:
    ...
    if name not in _VARIANTS:
        raise ValueError(f"Unknown PNASNet variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
    return PNASNetClassifier(
        in_channels=int(in_channels),
        num_classes=int(num_classes),
