...
from dlhub.vision.backbones._blocks import ConvBNAct, GlobalAvgPoolHead, NonLocal2D, scale_channels
...
class NonlocalNetClassifier(nn.Module):
    def __init__(
        self,
        *,
        in_channels: int,
        ...
        def make_stage(in_ch: int, out_ch: int, depth: int, *, stride: int) -> nn.Sequential:
            ...
            attn_mod = NonLocal2D(out_ch, reduction=2)
            blocks.append(_ResidualBlock(in_ch, out_ch, stride=int(stride), attn=attn_mod))
            for _ in range(int(depth) - 1):
                blocks.append(
                    _ResidualBlock(out_ch, out_ch, stride=1, attn=NonLocal2D(out_ch, reduction=2))
                )
...
def build_nonlocal_net_classifier(
    *,
    in_channels: int,
    num_classes: int,
    variant: str = "tiny",
...
if __name__ == "__main__":
    ...
    m = build_nonlocal_net_classifier(in_channels=3, num_classes=10, variant="tiny", width_mult=1.0)
    y = m(x)
    print("nonlocal_net", tuple(y.shape))
