...
class AxialShift(nn.Module):
    """Axial shift token mixer (AS-MLP style, simplified)."""
...
class ASMLPClassifier(nn.Module):
    def __init__(
        self,
        *,
        in_channels: int,
...
_VARIANTS: dict[str, dict] = {
    "as_mlp_tiny": {"dim": 192, "depth": 10, "patch": 4},
    "as_mlp_small": {"dim": 256, "depth": 12, "patch": 4},
}


def build_as_mlp_classifier(
    *,
    in_channels: int,
    num_classes: int,
    variant: str = "as_mlp_tiny",
    width_mult: float = 1.0,
    dropout: float = 0.1,
) -> nn.Module:
    name = str(variant).lower().strip()
    if name not in _VARIANTS:
        raise ValueError(f"Unknown AS-MLP variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
    return ASMLPClassifier(
