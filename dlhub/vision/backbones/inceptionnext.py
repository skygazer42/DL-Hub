...
class InceptionNeXtBlock(nn.Module):
    """InceptionNeXt-inspired token mixer (multi-branch depthwise kernels)."""

    def __init__(self, dim: int, *, drop_path: float = 0.0) -> None:
        super().__init__()
        d = int(dim)
        self.pw1 = nn.Conv2d(d, d, kernel_size=1, bias=True)
        self.dw3 = nn.Conv2d(d, d, kernel_size=3, padding=1, groups=d, bias=False)
        self.dw5 = nn.Conv2d(d, d, kernel_size=5, padding=2, groups=d, bias=False)
        self.dw7 = nn.Conv2d(d, d, kernel_size=7, padding=3, groups=d, bias=False)
        self.bn = nn.BatchNorm2d(d)
        self.act = nn.GELU()
        self.pw2 = nn.Conv2d(d, d, kernel_size=1, bias=True)
...
class InceptionNeXtClassifier(nn.Module):
    def __init__(
        self,
        *,
        in_channels: int,
...
_VARIANTS: dict[str, dict] = {
    "inceptionnext_tiny": {"dims": (80, 160, 320, 640), "depths": (2, 2, 6, 2)},
    "inceptionnext_base": {"dims": (96, 192, 384, 768), "depths": (2, 2, 6, 2)},
    "inceptionnext_large": {"dims": (128, 256, 512, 1024), "depths": (2, 3, 9, 3)},
}


def build_inceptionnext_classifier(
    *,
    in_channels: int,
    num_classes: int,
    variant: str = "inceptionnext_base",
    width_mult: float = 1.0,
    drop_path: float = 0.1,
    dropout: float = 0.1,
) -> nn.Module:
