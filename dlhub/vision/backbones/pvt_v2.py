...
class SpatialReductionAttention(nn.Module):
    """PVT/SegFormer-like attention with spatial reduction for K/V (simplified)."""
...
def build_pvt_v2_classifier(
    *,
    in_channels: int,
    num_classes: int,
    variant: str = "pvt_v2_b1",
    width_mult: float = 1.0,
    drop_path: float = 0.1,
    dropout: float = 0.1,
) -> nn.Module:
    ...
    if name not in _VARIANTS:
        raise ValueError(f"Unknown PVTv2 variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
    return PVTV2Classifier(
