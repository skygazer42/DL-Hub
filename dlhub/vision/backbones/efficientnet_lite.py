...
class EfficientNetLiteClassifier(nn.Module):
    """EfficientNet-Lite style network (MBConv, ReLU6, simplified).

    EfficientNet-Lite typically removes swish and tweaks heads; this is a minimal,
    torch-only implementation that follows the stage layout idea.
    """
    ...
    def __init__(
        ...
        *,
        ...
        cfg = [
            # out, stride, exp, repeats, se
            (c(16), 1, 1.0, d(1), None),
            (c(24), 2, 6.0, d(2), None),
            (c(40), 2, 6.0, d(2), 0.25),
            (c(80), 2, 6.0, d(3), 0.25),
            (c(112), 1, 6.0, d(3), 0.25),
            (c(192), 2, 6.0, d(4), 0.25),
            (c(320), 1, 6.0, d(1), 0.25),
        ]
...
_VARIANTS: dict[str, dict] = {
    "efficientnet_lite0": {"w": 1.0, "d": 1.0},
    "efficientnet_lite1": {"w": 1.0, "d": 1.1},
    "efficientnet_lite2": {"w": 1.1, "d": 1.2},
    "efficientnet_lite3": {"w": 1.2, "d": 1.4},
}


def build_efficientnet_lite_classifier(
    *,
    in_channels: int,
    num_classes: int,
    variant: str = "efficientnet_lite0",
    dropout: float = 0.2,
) -> nn.Module:
