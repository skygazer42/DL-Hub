...
class TwinsSvtClassifier(nn.Module):
    def __init__(
        self,
        *,
        in_channels: int,
...
def build_twins_svt_classifier(
    *,
    in_channels: int,
    num_classes: int,
    image_size: int = 64,
...
if __name__ == "__main__":
    ...
    m = build_twins_svt_classifier(in_channels=3, num_classes=10, variant="tiny")
    y = m(x)
    if isinstance(y, tuple):
        print("twins_svt", tuple(y[0].shape), tuple(y[1].shape))
    else:
        print("twins_svt", tuple(y.shape))
