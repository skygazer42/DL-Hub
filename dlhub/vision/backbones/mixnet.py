...
class MixnetClassifier(nn.Module):
    def __init__(
        self,
        *,
        in_channels: int,
        ...
        def stage(in_ch: int, out_ch: int, depth: int, *, stride: int) -> nn.Sequential:
            ...
            blocks.append(InvertedResidual(in_ch, out_ch, stride=int(stride), expand_ratio=6.0, se_ratio=0.25, act='relu6'))
            for _ in range(int(depth) - 1):
                blocks.append(InvertedResidual(out_ch, out_ch, stride=1, expand_ratio=6.0, se_ratio=0.25, act='relu6'))
...
if __name__ == '__main__':
    ...
    m = build_mixnet_classifier(in_channels=3, num_classes=10, variant='tiny')
    y = m(x)
    print('mixnet', tuple(y.shape))
