...
class PerformerVitClassifier(nn.Module):
    def __init__(
        self,
        *,
        in_channels: int,
...
def build_performer_vit_classifier(
    *,
    in_channels: int,
    num_classes: int,
    image_size: int = 64,
...
if __name__ == "__main__":
    ...
    m = build_performer_vit_classifier(in_channels=3, num_classes=10, variant="tiny")
    y = m(x)
    if isinstance(y, tuple):
        print("performer_vit", tuple(y[0].shape), tuple(y[1].shape))
    else:
        print("performer_vit", tuple(y.shape))
