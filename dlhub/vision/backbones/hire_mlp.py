...
class HireMLPMixer(nn.Module):
    """Hire-MLP hierarchical mixing (simplified).

    Downsample -> mix -> upsample -> fuse.
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if y.shape[-2:] != x.shape[-2:]:
            y = nn.functional.interpolate(y, size=x.shape[-2:], mode="nearest")
        return self.fuse(y)
...
class HireMLPClassifier(nn.Module):
    def __init__(
        self,
        *,
        in_channels: int,
...
_VARIANTS: dict[str, dict] = {
    "hire_mlp_tiny": {"dim": 192, "depth": 10, "patch": 4},
    "hire_mlp_small": {"dim": 256, "depth": 12, "patch": 4},
}


def build_hire_mlp_classifier(
    *,
    in_channels: int,
    num_classes: int,
    variant: str = "hire_mlp_tiny",
    width_mult: float = 1.0,
    dropout: float = 0.1,
) -> nn.Module:
    name = str(variant).lower().strip()
    if name not in _VARIANTS:
        raise ValueError(f"Unknown Hire-MLP variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
    return HireMLPClassifier(
