...
class CompactModel(nn.Module):
    def __init__(
        self, *, family: str, in_channels: int, width: int, depth: int, num_fields: int = 8
    ):
        super().__init__()
        self.family = str(family)
        self.enc = TinyEncoder(in_channels, width, depth)
        c = self.enc.out_channels
        self.cls = nn.Linear(c, int(num_fields))

    def forward(self, image):
        feat = self.enc(image)
        pooled = F.adaptive_avg_pool2d(feat, (1, 1)).flatten(1)
        return {"field_logits": self.cls(pooled)}
...
def smoke_test_model(builder, variant: str):
    out = builder(in_channels=3, variant=variant, width_mult=0.5, num_fields=8)(
        torch.randn(2, 3, 128, 128)
    )
    print(variant, tuple(out["field_logits"].shape))
