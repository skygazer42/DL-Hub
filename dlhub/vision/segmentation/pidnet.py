...
class PIDNet(nn.Module):
    """PIDNet-style semantic segmentation (compact-first).
...
_VARIANTS: dict[str, dict] = {
    "pidnet_tiny": {"embed_dim": 64, "depth": 2, "num_heads": 4, "mlp_ratio": 4.0},
    "pidnet_small": {"embed_dim": 96, "depth": 3, "num_heads": 4, "mlp_ratio": 4.0},
    "pidnet_base": {"embed_dim": 128, "depth": 4, "num_heads": 8, "mlp_ratio": 4.0},
}


def build_pidnet_segmenter(
    *,
    in_channels: int,
    num_classes: int,
    variant: str = "pidnet_small",
    width_mult: float = 1.0,
) -> nn.Module:
    name = str(variant).lower().strip()
    if name not in _VARIANTS:
        raise ValueError(f"Unknown PIDNet variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
