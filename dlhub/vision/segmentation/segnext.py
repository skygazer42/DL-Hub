...
_VARIANTS: dict[str, dict] = {
    "segnext_tiny": {"embed_dim": 64, "depth": 2, "num_heads": 4, "mlp_ratio": 4.0},
    "segnext_small": {"embed_dim": 96, "depth": 3, "num_heads": 4, "mlp_ratio": 4.0},
    "segnext_base": {"embed_dim": 128, "depth": 4, "num_heads": 8, "mlp_ratio": 4.0},
}


def build_segnext_segmenter(
    *,
    in_channels: int,
    num_classes: int,
    variant: str = "segnext_small",
    width_mult: float = 1.0,
) -> nn.Module:
    name = str(variant).lower().strip()
    if name not in _VARIANTS:
        raise ValueError(f"Unknown SegNeXt variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
...
if __name__ == "__main__":
    ...
    m = build_segnext_segmenter(
        in_channels=3, num_classes=4, variant="segnext_tiny", width_mult=0.5
    )
    y = m(x)
    print("segnext_tiny", tuple(y.shape))
    loss = y.mean()
    loss.backward()
    print("ok")
