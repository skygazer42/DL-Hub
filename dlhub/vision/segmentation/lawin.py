...
class Lawin(nn.Module):
    """Lawin-style semantic segmentation (compact-first).
...
_VARIANTS: dict[str, dict] = {
    "lawin_tiny": {"embed_dim": 64, "depth": 2, "num_heads": 4, "mlp_ratio": 4.0},
    "lawin_small": {"embed_dim": 96, "depth": 3, "num_heads": 4, "mlp_ratio": 4.0},
    "lawin_base": {"embed_dim": 128, "depth": 4, "num_heads": 8, "mlp_ratio": 4.0},
}


def build_lawin_segmenter(
    *,
    in_channels: int,
    num_classes: int,
    variant: str = "lawin_small",
    width_mult: float = 1.0,
) -> nn.Module:
    name = str(variant).lower().strip()
    if name not in _VARIANTS:
        raise ValueError(f"Unknown Lawin variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
...
if __name__ == "__main__":
    ...
    m = build_lawin_segmenter(in_channels=3, num_classes=4, variant="lawin_tiny", width_mult=0.5)
    y = m(x)
    print("lawin_tiny", tuple(y.shape))
    loss = y.mean()
    loss.backward()
    print("ok")
