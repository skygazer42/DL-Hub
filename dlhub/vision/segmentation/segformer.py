...
class SegFormer(nn.Module):
    """SegFormer-style semantic segmentation (compact-first).
...
_VARIANTS: dict[str, dict] = {
    "segformer_tiny": {"embed_dim": 64, "depth": 2, "num_heads": 4, "mlp_ratio": 4.0},
    "segformer_small": {"embed_dim": 96, "depth": 3, "num_heads": 4, "mlp_ratio": 4.0},
    "segformer_base": {"embed_dim": 128, "depth": 4, "num_heads": 8, "mlp_ratio": 4.0},
}
...
def build_segformer_segmenter(
    *,
    in_channels: int,
    num_classes: int,
    variant: str = "segformer_small",
    width_mult: float = 1.0,
) -> nn.Module:
    name = str(variant).lower().strip()
    if name not in _VARIANTS:
        raise ValueError(f"Unknown SegFormer variant: {variant!r}. Supported: {sorted(_VARIANTS)}")
    spec = _VARIANTS[name]
...
if __name__ == "__main__":
    ...
    m = build_segformer_segmenter(
        in_channels=3, num_classes=4, variant="segformer_tiny", width_mult=0.5
    )
    y = m(x)
    print("segformer_tiny", tuple(y.shape))
    loss = y.mean()
    loss.backward()
    print("ok")
