...
class _ContextGuidedBlock(nn.Module):
    def __init__(self, channels: int, *, dilation: int = 2) -> None:
        ...
        mid = max(8, c // 2)
        self.reduce = ConvBNAct(c, mid, kernel_size=1, stride=1, padding=0, act="relu")
        ...
        self.context = nn.Sequential(
            nn.Conv2d(mid, mid, kernel_size=3, padding=dil, dilation=dil, bias=False),
            nn.BatchNorm2d(mid),
            nn.ReLU(inplace=True),
        )
        self.fuse = nn.Sequential(
            nn.Conv2d(mid * 2, c, kernel_size=1, bias=False),
            nn.BatchNorm2d(c),
        )
...
class CGNet(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        c = base * 4
        self.blocks = nn.Sequential(*[_ContextGuidedBlock(c, dilation=2) for _ in range(d)])
...
_VARIANTS: dict[str, dict] = {
    "cgnet_tiny": {"base_channels": 16, "depth": 2},
    "cgnet_small": {"base_channels": 24, "depth": 4},
    "cgnet_base": {"base_channels": 32, "depth": 6},
}


def build_cgnet_segmenter(
    *,
    in_channels: int,
    num_classes: int,
    variant: str = "cgnet_small",
...
if __name__ == "__main__":
    ...
    m = build_cgnet_segmenter(in_channels=3, num_classes=4, variant="cgnet_tiny", width_mult=0.5)
    y = m(x)
    print("cgnet_tiny", tuple(y.shape))
    loss = y.mean()
    loss.backward()
    print("ok")
