...
_VARIANTS: dict[str, dict] = {
    "seaformer_seg_tiny": {"embed_dim": 64, "depth": 2, "num_heads": 4, "mlp_ratio": 4.0},
    "seaformer_seg_small": {"embed_dim": 96, "depth": 3, "num_heads": 4, "mlp_ratio": 4.0},
    "seaformer_seg_base": {"embed_dim": 128, "depth": 4, "num_heads": 8, "mlp_ratio": 4.0},
}


def build_seaformer_seg_segmenter(
    *,
    in_channels: int,
    num_classes: int,
    variant: str = "seaformer_seg_small",
...
if __name__ == "__main__":
    ...
    m = build_seaformer_seg_segmenter(
        in_channels=3, num_classes=4, variant="seaformer_seg_tiny", width_mult=0.5
    )
    y = m(x)
    print("seaformer_seg_tiny", tuple(y.shape))
    loss = y.mean()
    loss.backward()
    print("ok")
