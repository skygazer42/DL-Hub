from __future__ import annotations

import torch
import torch.nn.functional as F
from torch import nn


def check_nchw(image: torch.Tensor) -> torch.Tensor:
    image = image.to(torch.float32)
    if image.ndim != 4:
        raise ValueError(f"Expected input shape (B, C, H, W), got {tuple(image.shape)}")
    return image


class TinyMoireBlock(nn.Module):
    def __init__(self, *, channels: int, mode: str) -> None:
        super().__init__()
        self.mode = str(mode)
        self.norm = nn.GroupNorm(1, int(channels))
        self.conv1 = nn.Conv2d(int(channels), int(channels), 3, padding=1)
        self.conv2 = nn.Conv2d(int(channels), int(channels), 3, padding=1)
        self.mix = nn.Conv2d(int(channels), int(channels), 1)
        self.depthwise = nn.Conv2d(int(channels), int(channels), 5, padding=2, groups=int(channels))
        self.prompt = nn.Parameter(torch.zeros(1, int(channels), 1, 1)) if self.mode == "prompt" else None

    def forward(self, x: torch.Tensor, guide: torch.Tensor) -> torch.Tensor:
        h = self.norm(x)
        if self.prompt is not None:
            h = h + self.prompt
        local = self.conv2(F.relu(self.conv1(h), inplace=True))
        if self.mode in {"cnn", "residual", "attention", "recursive"}:
            local = local + self.depthwise(h)
        elif self.mode == "frequency":
            low = F.avg_pool2d(h, 3, 1, 1)
            local = local + (h - low)
        elif self.mode == "wavelet":
            band = h - F.avg_pool2d(h, 5, 1, 2)
            local = local + band
        elif self.mode == "transformer":
            local = local * torch.sigmoid(self.mix(h)) + self.depthwise(h)
        elif self.mode == "dual":
            local = local + self.mix(guide)
        elif self.mode == "mamba":
            local = local + torch.tanh(self.depthwise(torch.roll(h, shifts=1, dims=-1)))
        return x + 0.2 * local


class TinyMoireRemover(nn.Module):
    def __init__(
        self,
        *,
        family: str,
        mode: str,
        in_channels: int,
        width: int,
        depth: int,
        passes: int,
    ) -> None:
        super().__init__()
        self.family = str(family)
        self.mode = str(mode)
        self.passes = max(1, int(passes))
        self.encoder = nn.Conv2d(int(in_channels), int(width), 3, padding=1)
        self.guide = nn.Conv2d(int(in_channels), int(width), 1)
        self.blocks = nn.ModuleList([TinyMoireBlock(channels=int(width), mode=str(mode)) for _ in range(max(1, int(depth)))])
        self.decoder = nn.Sequential(
            nn.Conv2d(int(width), int(width), 3, padding=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(int(width), int(in_channels), 3, padding=1),
        )
        self.map_head = nn.Conv2d(int(width), 1, 1)

    def forward(self, image: torch.Tensor) -> dict[str, torch.Tensor]:
        image = check_nchw(image)
        restored = image
        moire_map = None
        for _ in range(self.passes):
            feat = self.encoder(restored)
            guide = self.guide(image - restored)
            for block in self.blocks:
                feat = block(feat, guide)
            residual = self.decoder(feat)
            if self.mode in {"frequency", "wavelet"}:
                smooth = F.avg_pool2d(restored, 3, 1, 1)
                residual = 0.6 * residual + 0.4 * (restored - smooth)
            elif self.mode == "attention":
                residual = residual * torch.sigmoid(residual)
            elif self.mode == "recursive":
                residual = 0.7 * residual + 0.3 * torch.tanh(residual)
            restored = torch.clamp(restored - residual, -1.0, 1.0)
            moire_map = torch.sigmoid(self.map_head(feat))
        moire_residual = torch.clamp(image - restored, -1.0, 1.0)
        if moire_map is None:
            moire_map = moire_residual.mean(dim=1, keepdim=True).abs()
        return {"restored": restored, "moire_residual": moire_residual, "moire_map": moire_map}


def build_toy_moire_remover(
    *,
    family: str,
    mode: str,
    variants: dict[str, dict[str, int]],
    in_channels: int,
    variant: str,
    width_mult: float = 1.0,
) -> nn.Module:
    name = str(variant).lower().strip()
    if name not in variants:
        raise ValueError(f"Unknown variant for {family}: {variant!r}. Available: {sorted(variants)}")
    spec = dict(variants[name])
    width = max(12, int(int(spec["width"]) * float(width_mult)))
    depth = int(spec["depth"])
    passes = int(spec.get("passes", 1))
    return TinyMoireRemover(
        family=str(family),
        mode=str(mode),
        in_channels=int(in_channels),
        width=width,
        depth=depth,
        passes=passes,
    )


def smoke_test_moire_remover(builder, variant: str) -> None:
    model = builder(in_channels=3, variant=variant, width_mult=0.5)
    out = model(torch.randn(2, 3, 32, 32))
    print(variant, tuple(out["restored"].shape), tuple(out["moire_map"].shape))
    print("ok")
