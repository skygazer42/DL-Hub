...
_FAMILIES = ['siamese_face', 'triplet_face', 'arcface_verify', 'cosface_verify', 'relation_verify', 'transformer_verify', 'prompt_verify', 'occlusion_verify', 'contrastive_verify', 'mamba_verify']
    ...
    for family in _FAMILIES:
        for size in _SIZES:
            ...
            def _builder(cfg: BuildConfig, family: str = family, variant: str = variant):
                module = importlib.import_module(f"dlhub.vision.face_verification.{family}")
                fn = getattr(module, f"build_{family}_face_verifier")
                return fn(
...
def list_local_arches() -> list[str]:
    return [f"facever:{name}" for name in sorted(_REGISTRY)]
...
def build_local_model(arch_id: str, *, in_channels: int, width_mult: float = 1.0):
    ...
    if prefix not in {'facever', "local"}:
        raise ValueError(
            f"Unsupported face verification prefix: {prefix!r} (arch_id={arch_id!r})"
        )
