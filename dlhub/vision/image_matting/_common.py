...
class ToyModel(nn.Module):
    def __init__(self, *, family: str, in_channels: int, width: int, depth: int):
        ...
        self.alpha = nn.Conv2d(c, 1, 1)
        self.fg = nn.Conv2d(c, int(in_channels), 1)

    def forward(self, image):
        feat = self.enc(image)
        alpha = torch.sigmoid(self.alpha(feat))
        fg = torch.tanh(self.fg(feat))
        comp = alpha * fg + (1 - alpha) * check_nchw(image)
        return {"alpha": alpha, "foreground": fg, "composite": comp}
