...
class ToyModel(nn.Module):
    def __init__(self, *, family:str, in_channels:int, width:int, depth:int):
        super().__init__(); self.family=str(family); self.enc=TinyEncoder(in_channels,width,depth); c=self.enc.out_channels; self.alpha=nn.Conv2d(c,1,1); self.fg=nn.Conv2d(c,int(in_channels),1)
    def forward(self, image): feat=self.enc(image); alpha=torch.sigmoid(self.alpha(feat)); fg=torch.tanh(self.fg(feat)); comp=alpha*fg + (1-alpha)*check_nchw(image); return {'alpha': alpha, 'foreground': fg, 'composite': comp}
...
def smoke_test_model(builder, variant:str):
    out=builder(in_channels=3, variant=variant, width_mult=0.5)(torch.randn(2,3,64,64)); print(variant, {k:tuple(v.shape) for k,v in out.items()})
