...
class TinyHandClassifierBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.mode in {"cnn", "region", "attention", "contrastive", "skeleton"}:
...
class TinyHandClassifier(nn.Module):
    def __init__(self, *, family: str, mode: str, in_channels: int, width: int, depth: int, num_classes: int) -> None:
        super().__init__()
        self.family = str(family)
        self.mode = str(mode)
        self.stem = nn.Conv2d(int(in_channels), int(width), kernel_size=3, padding=1)
        self.blocks = nn.ModuleList([TinyHandClassifierBlock(channels=int(width), mode=str(mode)) for _ in range(max(1, int(depth)))])
        self.head = nn.Linear(int(width), int(num_classes))
        self.aux_head = nn.Conv2d(int(width), int(num_classes), kernel_size=1)
    ...
    def forward(self, image: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        return {"logits": logits, "probabilities": torch.softmax(logits, dim=-1), "aux_logits": self.aux_head(feat)}


def build_toy_hand_classifier(*, family: str, mode: str, variants: dict[str, dict[str, int]], in_channels: int, variant: str, width_mult: float = 1.0, num_classes: int = 4) -> nn.Module:
    ...
    classes = int(spec.get("num_classes", num_classes))
    return TinyHandClassifier(family=str(family), mode=str(mode), in_channels=int(in_channels), width=width, depth=depth, num_classes=classes)


def smoke_test_hand_classifier(builder, variant: str) -> None:
    model = builder(in_channels=3, variant=variant, width_mult=0.5)
    out = model(torch.randn(2, 3, 64, 64))
    print(variant, tuple(out["logits"].shape), tuple(out["aux_logits"].shape))
    print("ok")
