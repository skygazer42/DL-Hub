...
class CompactModel(nn.Module):
    def __init__(self, *, family: str, in_channels: int, width: int, depth: int):
        ...
        self.head = nn.Conv2d(self.enc.out_channels, 1, 1)

    def forward(self, left, right):
        x = torch.cat([check_nchw(left), check_nchw(right)], dim=1)
        disp = F.softplus(self.head(self.enc(x)))
...
def smoke_test_model(builder, variant: str):
    ...
    print(variant, tuple(out["disparity"].shape))
