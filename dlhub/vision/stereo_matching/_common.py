...
class ToyModel(nn.Module):
    def __init__(self, *, family:str, in_channels:int, width:int, depth:int):
        super().__init__(); self.family=str(family); self.enc=TinyEncoder(in_channels*2,width,depth); self.head=nn.Conv2d(self.enc.out_channels,1,1)
    def forward(self,left,right): x=torch.cat([check_nchw(left),check_nchw(right)],dim=1); disp=F.softplus(self.head(self.enc(x))); return {'disparity': disp}
...
def smoke_test_model(builder, variant:str): out=builder(in_channels=3,variant=variant,width_mult=0.5)(torch.randn(2,3,64,64),torch.randn(2,3,64,64)); print(variant, tuple(out['disparity'].shape))
