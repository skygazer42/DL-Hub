...
class TinyKeypointModel(nn.Module):
    def __init__(self, *, family: str, mode: str, in_channels: int, width: int, depth: int, num_points: int = 8) -> None:
        super().__init__()
        self.family = str(family)
        self.mode = str(mode)
        self.stem = nn.Conv2d(int(in_channels), int(width), 3, padding=1)
        self.blocks = nn.ModuleList([TinyKeypointBlock(channels=int(width), mode=str(mode)) for _ in range(max(1, int(depth)))])
    ...
    def forward(self, image: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        return {"points": points, "confidence": torch.sigmoid(self.conf(pooled)), "heatmaps": self.maps(feat)}


def build_toy_keypoint_model(*, family: str, mode: str, variants: dict[str, dict[str, int]], in_channels: int, variant: str, width_mult: float = 1.0) -> nn.Module:
    ...
    return TinyKeypointModel(family=str(family), mode=str(mode), in_channels=int(in_channels), width=width, depth=depth, num_points=num_points)
