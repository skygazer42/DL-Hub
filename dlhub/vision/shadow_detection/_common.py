...
class TinyShadowDetector(nn.Module):
    def __init__(self, *, family: str, mode: str, in_channels: int, width: int, depth: int) -> None:
        ...
        self.blocks = nn.ModuleList([TinyShadowBlock(channels=int(width), mode=str(mode)) for _ in range(max(1, int(depth)))])
    ...
    def forward(self, image: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        return {'logits': logits, 'mask': torch.sigmoid(logits), 'boundary': torch.sigmoid(self.boundary_head(feat))}

def build_toy_shadow_detector(*, family: str, mode: str, variants: dict[str, dict[str, int]], in_channels: int, variant: str, width_mult: float = 1.0) -> nn.Module:
    spec = dict(variants[str(variant).lower().strip()])
    width = max(12, int(int(spec['width']) * float(width_mult)))
    return TinyShadowDetector(family=str(family), mode=str(mode), in_channels=int(in_channels), width=width, depth=int(spec['depth']))
