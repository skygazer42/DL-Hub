...
class ToyModel(nn.Module):
    def __init__(
        self, *, family: str, in_channels: int, width: int, depth: int, num_cells: int = 16
    ):
        super().__init__()
        self.family = str(family)
        self.enc = TinyEncoder(in_channels, width, depth)
        c = self.enc.out_channels
        self.rows = nn.Linear(c, int(num_cells))
        self.cols = nn.Linear(c, int(num_cells))

    def forward(self, image):
        feat = self.enc(image)
        pooled = F.adaptive_avg_pool2d(feat, (1, 1)).flatten(1)
        return {"row_logits": self.rows(pooled), "col_logits": self.cols(pooled)}
...
def smoke_test_model(builder, variant: str):
    out = builder(in_channels=3, variant=variant, width_mult=0.5, num_cells=16)(
