...
class BiGRUTextClassifier(nn.Module):
    def __init__(self, cfg: BiGRUConfig) -> None:
        super().__init__()
        self.cfg = cfg
...
def build_bigru_classifier(
    *,
    ...
    if not name.startswith("bigru_"):
        raise ValueError("Expected variant like 'bigru_mean' or 'bigru_attn3l'")
...
def registry() -> dict[str, Builder]:
    ...
    for pool in pools:
        for n_layers in layers:
            name = f"bigru_{pool}" if n_layers == 1 else f"bigru_{pool}{n_layers}l"
            r[name] = make_builder(build_bigru_classifier, variant=name)
    return r
...
__all__ = ["BiGRUTextClassifier", "BiGRUConfig", "build_bigru_classifier", "registry"]
