...
class SRULayer(nn.Module):
    """A simple SRU-style recurrent layer (simplified, CPU-friendly)."""

    def __init__(self, in_dim: int, hidden_dim: int, *, dropout: float) -> None:
        super().__init__()
        self.in_dim = int(in_dim)
        self.hidden_dim = int(hidden_dim)
        self.proj = nn.Linear(self.in_dim, 3 * self.hidden_dim)
        self.skip = nn.Linear(self.in_dim, self.hidden_dim, bias=False)
        self.drop = nn.Dropout(p=float(dropout))

    def _scan(self, x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        # x: (B, T, D), mask: (B, T) float
        b, t, _ = x.shape
        u, f, r = self.proj(x).chunk(3, dim=-1)
        u = torch.tanh(u)
        f = torch.sigmoid(f)
        r = torch.sigmoid(r)
        skip = self.skip(x)

        c = torch.zeros((b, self.hidden_dim), device=x.device, dtype=torch.float32)
        out = torch.zeros((b, t, self.hidden_dim), device=x.device, dtype=torch.float32)
        for i in range(t):
            m = mask[:, i].view(b, 1).to(torch.float32)
            c_new = f[:, i] * c + (1.0 - f[:, i]) * u[:, i]
            c = m * c_new + (1.0 - m) * c
            h = r[:, i] * torch.tanh(c) + (1.0 - r[:, i]) * skip[:, i]
            out[:, i] = m * h
...
class SRUTextClassifier(nn.Module):
    def __init__(self, cfg: SRUConfig) -> None:
        super().__init__()
        self.cfg = cfg
        ...
        for _ in range(int(cfg.num_layers)):
            layers_fwd.append(SRULayer(in_dim, int(h), dropout=float(cfg.dropout)))
            if self.bidirectional:
                layers_bwd.append(SRULayer(in_dim, int(h), dropout=float(cfg.dropout)))
                in_dim = 2 * int(h)
            else:
                in_dim = int(h)
...
def build_sru_classifier(
    *,
    ...
    if not rest.startswith("sru_"):
        raise ValueError("Expected variant like 'sru_mean' or 'bisru_attn2l'")
...
def registry() -> dict[str, Builder]:
    ...
    # Family alias (historically `nl:sru`)
    r["sru"] = make_builder(build_sru_classifier, variant="sru_mean")

    for pool in pools:
        for n_layers in layers:
            name = f"sru_{pool}" if n_layers == 1 else f"sru_{pool}{n_layers}l"
            r[name] = make_builder(build_sru_classifier, variant=name)

            bi_name = f"bisru_{pool}" if n_layers == 1 else f"bisru_{pool}{n_layers}l"
            r[bi_name] = make_builder(build_sru_classifier, variant=bi_name)
...
__all__ = ["SRUTextClassifier", "SRUConfig", "build_sru_classifier", "registry"]
