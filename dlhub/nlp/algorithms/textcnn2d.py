...
class TextCNN2DClassifier(nn.Module):
    """Kim-style 2D CNN for sentence classification (conv over (time, embed))."""

    def __init__(self, cfg: TextCNN2DConfig) -> None:
        super().__init__()
        self.cfg = cfg
...
def build_textcnn2d_classifier(
    *,
    vocab_size: int,
    pad_id: int,
    max_length: int,
    ...
    if name in {"textcnn2d_tiny", "textcnn2d"}:
        embed_dim, num_filters, kernels = 96, 96, (3, 4, 5)
    elif name in {"textcnn2d_small"}:
...
def registry() -> dict[str, Builder]:
    r: dict[str, Builder] = {}
    r["textcnn2d"] = make_builder(build_textcnn2d_classifier, variant="textcnn2d_tiny")
    for name in ("textcnn2d_tiny", "textcnn2d_small", "textcnn2d_base"):
        r[name] = make_builder(build_textcnn2d_classifier, variant=name)
    return r
...
def _smoke() -> None:
    ...
    model = build_textcnn2d_classifier(
