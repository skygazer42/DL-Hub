...
def build_gru_classifier(
    *,
    ...
    if not name.startswith("gru_"):
        raise ValueError("Expected variant like 'gru_mean' or 'gru_attn3l'")
...
def registry() -> dict[str, Builder]:
    ...
    for pool in pools:
        for n_layers in layers:
            name = f"gru_{pool}" if n_layers == 1 else f"gru_{pool}{n_layers}l"
            r[name] = make_builder(build_gru_classifier, variant=name)
    return r
...
__all__ = ["GRUTextClassifier", "GRUConfig", "build_gru_classifier", "registry"]
