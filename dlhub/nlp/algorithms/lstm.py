...
def build_lstm_classifier(
    *,
    ...
    if not name.startswith("lstm_"):
        raise ValueError("Expected variant like 'lstm_mean' or 'lstm_attn3l'")
...
def registry() -> dict[str, Builder]:
    ...
    for pool in pools:
        for n_layers in layers:
            name = f"lstm_{pool}" if n_layers == 1 else f"lstm_{pool}{n_layers}l"
            r[name] = make_builder(build_lstm_classifier, variant=name)
    return r
...
__all__ = ["LSTMTextClassifier", "LSTMConfig", "build_lstm_classifier", "registry"]
