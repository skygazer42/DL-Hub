...
class T5Classifier(TransformerTextClassifier):
    pass


def build_t5_classifier(
    *,
    vocab_size: int,
    pad_id: int,
    max_length: int,
    ...
    if name in {"t5_tiny", "t5"}:
        embed_dim, heads, layers = 192, 4, 2
    elif name in {"t5_small"}:
        embed_dim, heads, layers = 256, 4, 3
    elif name in {"t5_base"}:
        embed_dim, heads, layers = 320, 5, 4
    else:
        raise ValueError("Unknown T5 variant. Supported: t5_tiny|t5_small|t5_base")
...
def registry() -> dict[str, Builder]:
    ...
    # Family alias (historically `nl:t5`)
    r["t5"] = make_builder(build_t5_classifier, variant="t5_tiny")

    for name in ("t5_tiny", "t5_small", "t5_base"):
        r[name] = make_builder(build_t5_classifier, variant=name)
...
__all__ = ["T5Classifier", "build_t5_classifier", "registry"]
