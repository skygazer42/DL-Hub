...
def build_bilstm_classifier(
    *,
    ...
    if not name.startswith("bilstm_"):
        raise ValueError("Expected variant like 'bilstm_mean' or 'bilstm_attn3l'")
...
def registry() -> dict[str, Builder]:
    ...
    # Family alias (historically `nl:bilstm`)
    r["bilstm"] = make_builder(build_bilstm_classifier, variant="bilstm_last")

    for pool in pools:
        for n_layers in layers:
            name = f"bilstm_{pool}" if n_layers == 1 else f"bilstm_{pool}{n_layers}l"
            r[name] = make_builder(build_bilstm_classifier, variant=name)
...
__all__ = ["BiLSTMTextClassifier", "BiLSTMConfig", "build_bilstm_classifier", "registry"]
