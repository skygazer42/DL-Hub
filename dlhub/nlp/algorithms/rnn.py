...
def build_rnn_classifier(
    *,
    ...
    if not name.startswith("rnn_"):
        raise ValueError("Expected variant like 'rnn_mean' or 'rnn_attn3l'")
...
def registry() -> dict[str, Builder]:
    ...
    for pool in pools:
        for n_layers in layers:
            name = f"rnn_{pool}" if n_layers == 1 else f"rnn_{pool}{n_layers}l"
            r[name] = make_builder(build_rnn_classifier, variant=name)
    return r
...
__all__ = ["RNNTextClassifier", "RNNConfig", "build_rnn_classifier", "registry"]
