...
class CompactRetriever(nn.Module):
    ...
    def forward(
        self, image: torch.Tensor, text: torch.Tensor | None = None
    ) -> dict[str, torch.Tensor]:
        x = check_nchw(image)
        image_embedding = self.pool(self.image_encoder(x)).flatten(1)
        text = (
        ...
        similarity = (image_embedding * text_embedding).sum(dim=1, keepdim=True)
        return {
...
def smoke_test_retriever(builder, variant: str) -> None:
    ...
    print(variant, tuple(out["image_embedding"].shape), tuple(out["similarity"].shape))
