...
def _split_arch_id(arch_id: str) -> tuple[str, str]:
    ...
    if ':' not in arch_id: return 'itr', arch_id
...
def list_local_arches() -> list[str]: return [f'itr:{name}' for name in sorted(_REGISTRY)]

def build_local_model(arch_id: str, *, in_channels: int, width_mult: float = 1.0):
    prefix_name, name = _split_arch_id(arch_id)
    if prefix_name not in {'itr', 'local'}: raise ValueError(f"Unsupported image-text retrieval prefix: {prefix_name!r} (arch_id={arch_id!r})")
    builder = _REGISTRY.get(str(name).lower().strip())
    if builder is None: raise UnknownLocalArch(f"Unknown image-text retrieval arch: {arch_id!r}. Tip: import `dlhub.multimodal.image_text_retrieval_zoo` and call `list_local_arches()`." )
