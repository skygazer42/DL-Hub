...
def _split_arch_id(arch_id: str) -> tuple[str, str]:
    ...
    if ':' not in arch_id: return 'docvlm', arch_id
    ...
    for family in _FAMILIES:
        for size in _SIZES:
            ...
            def _builder(cfg: BuildConfig, family: str = family, variant: str = variant):
                module = importlib.import_module(f"dlhub.multimodal.document_vlm.{family}")
                fn = getattr(module, f"build_{family}_document_vlm")
...
def list_local_arches() -> list[str]: return [f'docvlm:{name}' for name in sorted(_REGISTRY)]

def build_local_model(arch_id: str, *, in_channels: int, width_mult: float = 1.0):
    prefix_name, name = _split_arch_id(arch_id)
    if prefix_name not in {'docvlm', 'local'}: raise ValueError(f"Unsupported document VLM prefix: {prefix_name!r} (arch_id={arch_id!r})")
    builder = _REGISTRY.get(str(name).lower().strip())
    if builder is None: raise UnknownLocalArch(f"Unknown document VLM arch: {arch_id!r}. Tip: import `dlhub.multimodal.document_vlm_zoo` and call `list_local_arches()`." )
