...
class FedPromptStrategy(FederatedStrategy):
...
_VARIANTS: dict[str, dict[str, float | int]] = {
    "fedprompt_tiny": {"hidden": 8, "step_scale": 0.08},
    "fedprompt_small": {"hidden": 12, "step_scale": 0.12},
    "fedprompt_base": {"hidden": 16, "step_scale": 0.16},
}


def build_fedprompt_strategy(
    *,
    param_dim: int,
    num_clients: int,
    local_steps: int,
    variant: str = "fedprompt_tiny",
...
if __name__ == "__main__":
    smoke_test_strategy(build_fedprompt_strategy, "fedprompt_tiny")
