...
class FedCurriculumStrategy(FederatedStrategy):
...
_VARIANTS: dict[str, dict[str, float | int]] = {
    "fedcurriculum_tiny": {"hidden": 8, "step_scale": 0.08},
    "fedcurriculum_small": {"hidden": 12, "step_scale": 0.12},
    "fedcurriculum_base": {"hidden": 16, "step_scale": 0.16},
}


def build_fedcurriculum_strategy(
    *,
    param_dim: int,
    num_clients: int,
    local_steps: int,
    variant: str = "fedcurriculum_tiny",
    width_mult: float = 1.0,
):
    return build_federated_strategy(
        FedCurriculumStrategy,
        variants=_VARIANTS,
        param_dim=param_dim,
