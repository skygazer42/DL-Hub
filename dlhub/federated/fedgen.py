...
class FedGenStrategy(FederatedStrategy):
...
_VARIANTS: dict[str, dict[str, float | int]] = {
    "fedgen_tiny": {"hidden": 8, "step_scale": 0.08},
    "fedgen_small": {"hidden": 12, "step_scale": 0.12},
    "fedgen_base": {"hidden": 16, "step_scale": 0.16},
}


def build_fedgen_strategy(
    *,
    param_dim: int,
    num_clients: int,
    local_steps: int,
    variant: str = "fedgen_tiny",
...
if __name__ == "__main__":
    smoke_test_strategy(build_fedgen_strategy, "fedgen_tiny")
