...
class FedRoute2Strategy(FederatedStrategy):
...
_VARIANTS: dict[str, dict[str, float | int]] = {
    "fedroute2_tiny": {"hidden": 8, "step_scale": 0.08},
    "fedroute2_small": {"hidden": 12, "step_scale": 0.12},
    "fedroute2_base": {"hidden": 16, "step_scale": 0.16},
}


def build_fedroute2_strategy(
    *,
    param_dim: int,
    num_clients: int,
    local_steps: int,
    variant: str = "fedroute2_tiny",
...
if __name__ == "__main__":
    smoke_test_strategy(build_fedroute2_strategy, "fedroute2_tiny")
