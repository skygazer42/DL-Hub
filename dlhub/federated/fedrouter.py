...
class FedRouterStrategy(FederatedStrategy):
...
_VARIANTS: dict[str, dict[str, float | int]] = {
    "fedrouter_tiny": {"hidden": 8, "step_scale": 0.08},
    "fedrouter_small": {"hidden": 12, "step_scale": 0.12},
    "fedrouter_base": {"hidden": 16, "step_scale": 0.16},
}


def build_fedrouter_strategy(
    *,
    param_dim: int,
    num_clients: int,
    local_steps: int,
    variant: str = "fedrouter_tiny",
