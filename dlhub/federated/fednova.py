from __future__ import annotations

import torch

from ._common import (
    FederatedStrategy,
    build_federated_strategy,
    smoke_test_strategy,
    weighted_average,
)


class FedNovaStrategy(FederatedStrategy):
    def simulate_round(self, *, seed: int = 0) -> dict[str, torch.Tensor]:
        state = self._sample_round_state(seed=seed)
        normalized_updates = state.raw_updates / state.client_steps.unsqueeze(1)
        server_params = state.server_params + weighted_average(
            normalized_updates, state.client_weights
        )
        return {
            "server_params": server_params,
            "normalized_updates": normalized_updates,
            "client_steps": state.client_steps,
            "client_weights": state.client_weights,
        }


_VARIANTS: dict[str, dict[str, float | int]] = {
    "fednova_tiny": {"hidden": 8, "step_scale": 0.08},
    "fednova_small": {"hidden": 12, "step_scale": 0.12},
    "fednova_base": {"hidden": 16, "step_scale": 0.16},
}


def build_fednova_strategy(
    *,
    param_dim: int,
    num_clients: int,
    local_steps: int,
    variant: str = "fednova_tiny",
    width_mult: float = 1.0,
):
    return build_federated_strategy(
        FedNovaStrategy,
        variants=_VARIANTS,
        param_dim=param_dim,
        num_clients=num_clients,
        local_steps=local_steps,
        variant=variant,
        width_mult=width_mult,
        family="fednova",
    )


if __name__ == "__main__":
    smoke_test_strategy(build_fednova_strategy, "fednova_tiny")
