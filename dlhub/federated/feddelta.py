...
class FedDeltaStrategy(FederatedStrategy):
...
_VARIANTS: dict[str, dict[str, float | int]] = {
    "feddelta_tiny": {"hidden": 8, "step_scale": 0.08},
    "feddelta_small": {"hidden": 12, "step_scale": 0.12},
    "feddelta_base": {"hidden": 16, "step_scale": 0.16},
}


def build_feddelta_strategy(
    *,
    param_dim: int,
    num_clients: int,
    local_steps: int,
    variant: str = "feddelta_tiny",
...
if __name__ == "__main__":
    smoke_test_strategy(build_feddelta_strategy, "feddelta_tiny")
