...
class FedCoopStrategy(FederatedStrategy):
...
_VARIANTS: dict[str, dict[str, float | int]] = {
    "fedcoop_tiny": {"hidden": 8, "step_scale": 0.08},
    "fedcoop_small": {"hidden": 12, "step_scale": 0.12},
    "fedcoop_base": {"hidden": 16, "step_scale": 0.16},
}


def build_fedcoop_strategy(
    *,
    param_dim: int,
    num_clients: int,
    local_steps: int,
    variant: str = "fedcoop_tiny",
