...
class FedScopeStrategy(FederatedStrategy):
...
_VARIANTS: dict[str, dict[str, float | int]] = {
    "fedscope_tiny": {"hidden": 8, "step_scale": 0.08},
    "fedscope_small": {"hidden": 12, "step_scale": 0.12},
    "fedscope_base": {"hidden": 16, "step_scale": 0.16},
}


def build_fedscope_strategy(
    *,
    param_dim: int,
    num_clients: int,
    local_steps: int,
    variant: str = "fedscope_tiny",
...
if __name__ == "__main__":
    smoke_test_strategy(build_fedscope_strategy, "fedscope_tiny")
