...
class FedMoeStrategy(FederatedStrategy):
...
_VARIANTS: dict[str, dict[str, float | int]] = {
    "fedmoe_tiny": {"hidden": 8, "step_scale": 0.08},
    "fedmoe_small": {"hidden": 12, "step_scale": 0.12},
    "fedmoe_base": {"hidden": 16, "step_scale": 0.16},
}


def build_fedmoe_strategy(
    *,
    param_dim: int,
    num_clients: int,
    local_steps: int,
    variant: str = "fedmoe_tiny",
