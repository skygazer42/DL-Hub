...
class FedTeacherStrategy(FederatedStrategy):
...
_VARIANTS: dict[str, dict[str, float | int]] = {
    "fedteacher_tiny": {"hidden": 8, "step_scale": 0.08},
    "fedteacher_small": {"hidden": 12, "step_scale": 0.12},
    "fedteacher_base": {"hidden": 16, "step_scale": 0.16},
}


def build_fedteacher_strategy(
    *,
    param_dim: int,
    num_clients: int,
    local_steps: int,
    variant: str = "fedteacher_tiny",
...
if __name__ == "__main__":
    smoke_test_strategy(build_fedteacher_strategy, "fedteacher_tiny")
