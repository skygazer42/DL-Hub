...
class FedPrompt2Strategy(FederatedStrategy):
...
_VARIANTS: dict[str, dict[str, float | int]] = {
    "fedprompt2_tiny": {"hidden": 8, "step_scale": 0.08},
    "fedprompt2_small": {"hidden": 12, "step_scale": 0.12},
    "fedprompt2_base": {"hidden": 16, "step_scale": 0.16},
}


def build_fedprompt2_strategy(
    *,
    param_dim: int,
    num_clients: int,
    local_steps: int,
    variant: str = "fedprompt2_tiny",
...
if __name__ == "__main__":
    smoke_test_strategy(build_fedprompt2_strategy, "fedprompt2_tiny")
