...
class FedAdapterStrategy(FederatedStrategy):
...
_VARIANTS: dict[str, dict[str, float | int]] = {
    "fedadapter_tiny": {"hidden": 8, "step_scale": 0.08},
    "fedadapter_small": {"hidden": 12, "step_scale": 0.12},
    "fedadapter_base": {"hidden": 16, "step_scale": 0.16},
}


def build_fedadapter_strategy(
    *,
    param_dim: int,
    num_clients: int,
    local_steps: int,
    variant: str = "fedadapter_tiny",
...
if __name__ == "__main__":
    smoke_test_strategy(build_fedadapter_strategy, "fedadapter_tiny")
