...
class FedFomoStrategy(FederatedStrategy):
...
_VARIANTS: dict[str, dict[str, float | int]] = {
    "fedfomo_tiny": {"hidden": 8, "step_scale": 0.08},
    "fedfomo_small": {"hidden": 12, "step_scale": 0.12},
    "fedfomo_base": {"hidden": 16, "step_scale": 0.16},
}


def build_fedfomo_strategy(
    *,
    param_dim: int,
    num_clients: int,
    local_steps: int,
    variant: str = "fedfomo_tiny",
...
if __name__ == "__main__":
    smoke_test_strategy(build_fedfomo_strategy, "fedfomo_tiny")
