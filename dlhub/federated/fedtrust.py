...
class FedTrustStrategy(FederatedStrategy):
...
_VARIANTS: dict[str, dict[str, float | int]] = {
    "fedtrust_tiny": {"hidden": 8, "step_scale": 0.08},
    "fedtrust_small": {"hidden": 12, "step_scale": 0.12},
    "fedtrust_base": {"hidden": 16, "step_scale": 0.16},
}


def build_fedtrust_strategy(
    *,
    param_dim: int,
    num_clients: int,
    local_steps: int,
    variant: str = "fedtrust_tiny",
...
if __name__ == "__main__":
    smoke_test_strategy(build_fedtrust_strategy, "fedtrust_tiny")
