...
class FedAnchorStrategy(FederatedStrategy):
...
_VARIANTS: dict[str, dict[str, float | int]] = {
    "fedanchor_tiny": {"hidden": 8, "step_scale": 0.08},
    "fedanchor_small": {"hidden": 12, "step_scale": 0.12},
    "fedanchor_base": {"hidden": 16, "step_scale": 0.16},
}


def build_fedanchor_strategy(
    *,
    param_dim: int,
    num_clients: int,
    local_steps: int,
    variant: str = "fedanchor_tiny",
...
if __name__ == "__main__":
    smoke_test_strategy(build_fedanchor_strategy, "fedanchor_tiny")
