...
class FedMixStrategy(FederatedStrategy):
...
_VARIANTS: dict[str, dict[str, float | int]] = {
    "fedmix_tiny": {"hidden": 8, "step_scale": 0.08},
    "fedmix_small": {"hidden": 12, "step_scale": 0.12},
    "fedmix_base": {"hidden": 16, "step_scale": 0.16},
}


def build_fedmix_strategy(
    *,
    param_dim: int,
    num_clients: int,
    local_steps: int,
    variant: str = "fedmix_tiny",
...
if __name__ == "__main__":
    smoke_test_strategy(build_fedmix_strategy, "fedmix_tiny")
