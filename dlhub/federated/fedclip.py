...
class FedClipStrategy(FederatedStrategy):
...
_VARIANTS: dict[str, dict[str, float | int]] = {
    "fedclip_tiny": {"hidden": 8, "step_scale": 0.08},
    "fedclip_small": {"hidden": 12, "step_scale": 0.12},
    "fedclip_base": {"hidden": 16, "step_scale": 0.16},
}


def build_fedclip_strategy(
    *,
    param_dim: int,
    num_clients: int,
    local_steps: int,
    variant: str = "fedclip_tiny",
...
if __name__ == "__main__":
    smoke_test_strategy(build_fedclip_strategy, "fedclip_tiny")
