...
class FedGsaStrategy(FederatedStrategy):
...
_VARIANTS: dict[str, dict[str, float | int]] = {
    "fedgsa_tiny": {"hidden": 8, "step_scale": 0.08},
    "fedgsa_small": {"hidden": 12, "step_scale": 0.12},
    "fedgsa_base": {"hidden": 16, "step_scale": 0.16},
}


def build_fedgsa_strategy(
    *,
    param_dim: int,
    num_clients: int,
    local_steps: int,
    variant: str = "fedgsa_tiny",
...
if __name__ == "__main__":
    smoke_test_strategy(build_fedgsa_strategy, "fedgsa_tiny")
