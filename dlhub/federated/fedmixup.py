...
class FedMixupStrategy(FederatedStrategy):
...
_VARIANTS: dict[str, dict[str, float | int]] = {
    "fedmixup_tiny": {"hidden": 8, "step_scale": 0.08},
    "fedmixup_small": {"hidden": 12, "step_scale": 0.12},
    "fedmixup_base": {"hidden": 16, "step_scale": 0.16},
}


def build_fedmixup_strategy(
    *,
    param_dim: int,
    num_clients: int,
    local_steps: int,
    variant: str = "fedmixup_tiny",
