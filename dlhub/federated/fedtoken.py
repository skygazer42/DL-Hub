...
class FedTokenStrategy(FederatedStrategy):
...
_VARIANTS: dict[str, dict[str, float | int]] = {
    "fedtoken_tiny": {"hidden": 8, "step_scale": 0.08},
    "fedtoken_small": {"hidden": 12, "step_scale": 0.12},
    "fedtoken_base": {"hidden": 16, "step_scale": 0.16},
}


def build_fedtoken_strategy(
    *,
    param_dim: int,
    num_clients: int,
    local_steps: int,
    variant: str = "fedtoken_tiny",
...
if __name__ == "__main__":
    smoke_test_strategy(build_fedtoken_strategy, "fedtoken_tiny")
