...
class FedPerStrategy(FederatedStrategy):
    def simulate_round(self, *, seed: int = 0) -> dict[str, torch.Tensor]:
        ...
        shared_update = weighted_average(state.raw_updates[:, :split], state.client_weights)
        personalized_heads = state.server_params[split:].unsqueeze(0) + state.raw_updates[:, split:]
        server_params = torch.cat(
            [state.server_params[:split] + shared_update, state.server_params[split:]], dim=0
        )
        return {
...
_VARIANTS: dict[str, dict[str, float | int]] = {
    "fedper_tiny": {"hidden": 8, "step_scale": 0.08},
    "fedper_small": {"hidden": 12, "step_scale": 0.12},
    "fedper_base": {"hidden": 16, "step_scale": 0.16},
}


def build_fedper_strategy(
    *,
    param_dim: int,
    num_clients: int,
    local_steps: int,
    variant: str = "fedper_tiny",
...
if __name__ == "__main__":
    smoke_test_strategy(build_fedper_strategy, "fedper_tiny")
