...
class FedCalibStrategy(FederatedStrategy):
...
_VARIANTS: dict[str, dict[str, float | int]] = {
    "fedcalib_tiny": {"hidden": 8, "step_scale": 0.08},
    "fedcalib_small": {"hidden": 12, "step_scale": 0.12},
    "fedcalib_base": {"hidden": 16, "step_scale": 0.16},
}


def build_fedcalib_strategy(
    *,
    param_dim: int,
    num_clients: int,
    local_steps: int,
    variant: str = "fedcalib_tiny",
...
if __name__ == "__main__":
    smoke_test_strategy(build_fedcalib_strategy, "fedcalib_tiny")
