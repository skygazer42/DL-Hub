...
class FedLawStrategy(FederatedStrategy):
...
_VARIANTS: dict[str, dict[str, float | int]] = {
    "fedlaw_tiny": {"hidden": 8, "step_scale": 0.08},
    "fedlaw_small": {"hidden": 12, "step_scale": 0.12},
    "fedlaw_base": {"hidden": 16, "step_scale": 0.16},
}


def build_fedlaw_strategy(
    *,
    param_dim: int,
    num_clients: int,
    local_steps: int,
    variant: str = "fedlaw_tiny",
...
if __name__ == "__main__":
    smoke_test_strategy(build_fedlaw_strategy, "fedlaw_tiny")
