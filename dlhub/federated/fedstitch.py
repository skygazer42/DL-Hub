...
class FedStitchStrategy(FederatedStrategy):
...
_VARIANTS: dict[str, dict[str, float | int]] = {
    "fedstitch_tiny": {"hidden": 8, "step_scale": 0.08},
    "fedstitch_small": {"hidden": 12, "step_scale": 0.12},
    "fedstitch_base": {"hidden": 16, "step_scale": 0.16},
}


def build_fedstitch_strategy(
    *,
    param_dim: int,
    num_clients: int,
    local_steps: int,
    variant: str = "fedstitch_tiny",
...
if __name__ == "__main__":
    smoke_test_strategy(build_fedstitch_strategy, "fedstitch_tiny")
