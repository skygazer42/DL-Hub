...
def _split_arch_id(arch_id: str) -> tuple[str, str]:
    ...
    return split_arch_id(arch_id, default_prefix="v2v")
...
def _extract_builder_name_from_source(src: str) -> str | None:
    ...
    for node in tree.body:
        if isinstance(node, ast.FunctionDef):
            name = str(node.name)
            if name.startswith("build_") and name.endswith("_video_to_video"):
                return name
    return None
...
def _make_lazy_builder(module_name: str, *, builder_name: str, variant: str) -> Builder:
    def _builder(cfg: BuildConfig):
        ...
        mod = importlib.import_module(f"dlhub.generative.video_to_video.{module_name}")
...
def _extend_registry(registry: dict[str, Builder]) -> None:
    module_dir = Path(__file__).resolve().parent / "video_to_video"
...
def list_local_arches() -> list[str]:
    ...
    return list_arch_ids(_REGISTRY, prefix="v2v")
...
def build_local_model(arch_id: str, *, in_channels: int, width_mult: float = 1.0):
    ...
    if builder is None:
        raise UnknownLocalArch(
            f"Unknown video-to-video arch: {arch_id!r}. Tip: import `dlhub.generative.video_to_video_zoo` and call `list_local_arches()`."
        )
