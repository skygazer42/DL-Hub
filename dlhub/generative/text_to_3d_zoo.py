...
def _split_arch_id(arch_id: str) -> tuple[str, str]:
    ...
    return split_arch_id(arch_id, default_prefix="txt3d")
...
def _make_lazy_builder(module_name: str, *, builder_name: str, variant: str) -> Builder:
    def _builder(cfg: BuildConfig):
        ...
        mod = importlib.import_module(f"dlhub.generative.text_to_3d.{module_name}")
        fn = getattr(mod, builder_name, None)
        if fn is None:
            raise RuntimeError(f"Text-to-3D module {module_name!r} missing {builder_name}()")
...
def list_local_arches() -> list[str]:
    ...
    return list_arch_ids(_REGISTRY, prefix="txt3d")
...
def build_local_model(
    ...
    *,
    ...
    if prefix not in {"txt3d", "local"}:
        raise ValueError(f"Unsupported text-to-3D prefix: {prefix!r} (arch_id={arch_id!r})")
    builder = _REGISTRY.get(str(name).lower().strip())
    if builder is None:
        raise UnknownLocalArch(
            f"Unknown text-to-3D arch: {arch_id!r}. Tip: import `dlhub.generative.text_to_3d_zoo` and call `list_local_arches()`."
        )
    return builder(
        BuildConfig(
