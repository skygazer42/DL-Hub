...
def _split_arch_id(arch_id: str) -> tuple[str, str]:
    ...
    return split_arch_id(arch_id, default_prefix="t2v")
...
def _extract_builder_name_from_source(src: str) -> str | None:
    ...
    for node in tree.body:
        if isinstance(node, ast.FunctionDef):
            name = str(node.name)
            if name.startswith("build_") and name.endswith("_text_to_video"):
                return name
    return None
...
def _make_lazy_builder(module_name: str, *, builder_name: str, variant: str) -> Builder:
    def _builder(cfg: BuildConfig):
        ...
        mod = importlib.import_module(f"dlhub.generative.text_to_video.{module_name}")
        fn = getattr(mod, builder_name, None)
        if fn is None:
            raise RuntimeError(f"text-to-video module {module_name!r} missing {builder_name}()")
        kwargs = {
...
def _extend_registry(registry: dict[str, Builder]) -> None:
    module_dir = Path(__file__).resolve().parent / "text_to_video"
...
def list_local_arches() -> list[str]:
    ...
    return list_arch_ids(_REGISTRY, prefix="t2v")


def build_local_model(arch_id: str, *, in_channels: int, width_mult: float = 1.0):
    prefix_name, name = _split_arch_id(arch_id)
    if prefix_name not in {"t2v", "local"}:
        raise ValueError(f"Unsupported text-to-video prefix: {prefix_name!r} (arch_id={arch_id!r})")
    builder = _REGISTRY.get(str(name).lower().strip())
    if builder is None:
        raise UnknownLocalArch(
            f"Unknown text-to-video arch: {arch_id!r}. Tip: import `dlhub.generative.text_to_video_zoo` and call `list_local_arches()`."
        )
