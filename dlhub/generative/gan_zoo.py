...
def _extract_builder_name_from_source(src: str) -> str | None:
    ...
    for node in tree.body:
        if isinstance(node, ast.FunctionDef):
            name = str(node.name)
            if name.startswith("build_") and name.endswith("_gan"):
                return name
    return None
...
def _make_lazy_builder(module_name: str, *, builder_name: str, variant: str) -> Builder:
    def _builder(cfg: BuildConfig):
        ...
        mod = importlib.import_module(f"dlhub.generative.gan.{module_name}")
        fn = getattr(mod, builder_name, None)
        if fn is None:
            raise RuntimeError(f"GAN module {module_name!r} missing {builder_name}()")
...
def build_local_model(
    ...
    *,
    ...
    if prefix in {"g", "gmodel"}:
        prefix = "gan"
    if prefix not in {"gan", "local"}:
        raise ValueError(f"Unsupported GAN prefix: {prefix!r} (arch_id={arch_id!r})")

    builder = _REGISTRY.get(str(name).lower().strip())
    if builder is None:
        raise UnknownLocalArch(
            f"Unknown GAN arch: {arch_id!r}. Tip: run `python scripts/gan_zoo.py --list`."
        )
    return builder(
        BuildConfig(
