...
def build_diffusion_i2v_image_to_video(*, in_channels: int, variant: str = "diffusion_i2v_small", width_mult: float = 1.0) -> nn.Module:
    return build_toy_image_to_video(family="diffusion_i2v", mode="diffusion", variants=_VARIANTS, in_channels=int(in_channels), variant=str(variant), width_mult=float(width_mult))
