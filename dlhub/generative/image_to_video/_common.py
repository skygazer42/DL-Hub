...
class ToyImageToVideo(nn.Module):
    def __init__(
        ...
        self.frame_head = nn.Conv2d(int(width), int(in_channels), 3, padding=1)
        self.motion_head = nn.Conv2d(int(width), int(in_channels), 1)

    def forward(self, image: torch.Tensor) -> dict[str, torch.Tensor]:
        x = check_nchw(image)
        feat = self.encoder(x)
        base = self.frame_head(feat)
        motion = torch.tanh(self.motion_head(feat))
        frames = []
        for step in range(self.frames):
            alpha = float(step) / max(1, self.frames - 1)
            frames.append(torch.clamp(base + alpha * motion, -1.0, 1.0))
