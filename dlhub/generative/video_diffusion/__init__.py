"""Video diffusion family package with lazy builder imports.

Conventions:
- One video diffusion family per file.
- Each family exposes `build_<family>_video_diffusion(...)`.
...
def _import_attr(name: str) -> Any:
    if name.startswith("build_") and name.endswith("_video_diffusion"):
        stem = name[len("build_") : -len("_video_diffusion")]
