...
class BootstrapPointNet(nn.Module):
...
_VARIANTS: dict[str, dict] = {
    "bootstrap_point_pointnet_tiny": {"hidden": 32, "embed": 64, "proj": 64, "dropout": 0.0},
    "bootstrap_point_pointnet_small": {"hidden": 64, "embed": 128, "proj": 128, "dropout": 0.0},
    "bootstrap_point_pointnet_base": {"hidden": 96, "embed": 192, "proj": 192, "dropout": 0.0},
}


def build_bootstrap_point_pointnet(
    *,
    in_channels: int,
    variant: str = "bootstrap_point_pointnet_small",
...
if __name__ == "__main__":
    ...
    m = build_bootstrap_point_pointnet(in_channels=3, variant="bootstrap_point_pointnet_tiny")
    o1 = m.forward_online(v1)
    o2 = m.forward_online(v2)
    t1 = m.forward_target(v1)
    t2 = m.forward_target(v2)
    loss = bootstrap_point_loss(o1["p"], t2["z"], o2["p"], t1["z"])
    loss.backward()
