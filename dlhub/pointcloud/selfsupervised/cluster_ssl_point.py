...
class ClusterSSLPointPretrainer(nn.Module):
...
def build_cluster_ssl_point_pretrainer(
    *,
    in_channels: int,
    variant: str = "cluster_ssl_point_small",
    dropout: float = 0.0,
) -> nn.Module:
    ...
    spec = _VARIANTS[name]
    return ClusterSSLPointPretrainer(
        in_channels=int(in_channels),
