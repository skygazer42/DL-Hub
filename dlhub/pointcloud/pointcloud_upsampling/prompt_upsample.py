...
def build_prompt_upsample_upsampler(*, in_channels: int, variant: str = "prompt_upsample_small", width_mult: float = 1.0) -> nn.Module:
    return build_toy_upsampler(family="prompt_upsample", mode="prompt", variants=_VARIANTS, in_channels=int(in_channels), variant=str(variant), width_mult=float(width_mult))
