...
class TinyUpsampler(nn.Module):
    def __init__(
        self, *, family: str, mode: str, in_channels: int, width: int, depth: int, up_factor: int
    ) -> None:
        super().__init__()
        self.family = str(family)
        self.mode = str(mode)
        self.in_channels = int(in_channels)
        self.up_factor = max(2, int(up_factor))
        ...
        self.offset_head = nn.Linear(int(width), int(in_channels))
        self.mix_head = nn.Linear(int(width), int(in_channels))
    ...
    def forward(self, points: torch.Tensor) -> torch.Tensor:
        ...
        repeated = pts.repeat_interleave(self.up_factor, dim=1)
        offsets = self.offset_head(feat).repeat_interleave(self.up_factor, dim=1)
        mix = self.mix_head(feat).repeat_interleave(self.up_factor, dim=1)
        if self.mode in {"pugan", "diffusion"}:
            offsets = 0.5 * torch.tanh(offsets) + 0.1 * mix
        elif self.mode in {"punet", "mpu"}:
            offsets = 0.25 * offsets
...
def build_baseline_upsampler(
    *,
    ...
    return TinyUpsampler(
        family=str(family),
        mode=str(mode),
        in_channels=int(in_channels),
        width=width,
        depth=int(cfg["depth"]),
        up_factor=int(cfg.get("up_factor", 2)),
    )
