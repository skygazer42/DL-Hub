...
def check_bnc(x):
    ...
    if x.ndim != 3:
        raise ValueError(f"Expected input shape (B,N,C), got {tuple(x.shape)}")
    return x
...
class ToyRegistrar(nn.Module):
    ...
    def forward(self, src, tgt):
        s = self.proj(check_bnc(src))
        t = self.proj(check_bnc(tgt))
        _, hs = self.rnn(torch.cat([s, t], dim=1))
        pose = self.head(hs[-1])
        return {"pose6d": pose}
...
def smoke_test_model(builder, variant: str):
    out = builder(variant=variant, width_mult=0.5)(torch.randn(2, 128, 3), torch.randn(2, 128, 3))
    print(variant, tuple(out["pose6d"].shape))
