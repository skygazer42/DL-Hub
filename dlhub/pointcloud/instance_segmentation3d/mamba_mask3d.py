...
_VARIANTS: dict[str, dict[str, object]] = {
    "mamba_mask3d_tiny": {"d_model": 64, "depth": 2, "queries": 16},
    "mamba_mask3d_small": {"d_model": 96, "depth": 3, "queries": 24},
    "mamba_mask3d_base": {"d_model": 128, "depth": 4, "queries": 32},
}


class MambaMask3D(nn.Module):
    """MambaMask3D (compact): transformer point encoder + query mask head."""
...
def build_mamba_mamba_mask3d_instance_segmenter3d(
    in_channels: int,
    num_classes: int,
    *,
    variant: str = "mamba_mask3d_small",
...
if __name__ == "__main__":
    torch.manual_seed(0)
    m = build_mamba_mamba_mask3d_instance_segmenter3d(
        in_channels=3, num_classes=6, variant="mamba_mask3d_tiny"
    )
