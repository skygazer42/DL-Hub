...
_VARIANTS: dict[str, dict[str, object]] = {
    "openscene_inst_tiny": {"d_model": 64, "depth": 2, "queries": 16},
    "openscene_inst_small": {"d_model": 96, "depth": 3, "queries": 24},
    "openscene_inst_base": {"d_model": 128, "depth": 4, "queries": 32},
}


class OpenSceneInst(nn.Module):
    """OpenScene-Inst (toy): open-scene flavored query masks over point tokens."""
...
def build_openscene_inst_instance_segmenter3d(
    in_channels: int,
    num_classes: int,
    *,
    variant: str = "openscene_inst_small",
    width_mult: float = 1.0,
    dropout: float = 0.0,
) -> nn.Module:
    cfg = _VARIANTS[str(variant)]
    d_model = int(int(cfg["d_model"]) * float(width_mult))
    return OpenSceneInst(
...
if __name__ == "__main__":
    torch.manual_seed(0)
    m = build_openscene_inst_instance_segmenter3d(
        in_channels=3,
        num_classes=6,
        variant="openscene_inst_tiny",
    )
