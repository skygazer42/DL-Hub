...
_VARIANTS: dict[str, dict[str, object]] = {
    "voxelquery3d_tiny": {"d_model": 64, "depth": 2, "queries": 16},
    "voxelquery3d_small": {"d_model": 96, "depth": 3, "queries": 24},
    "voxelquery3d_base": {"d_model": 128, "depth": 4, "queries": 32},
}


class Voxelquery3D(nn.Module):
    """Voxelquery3D (compact): initial queries, then refine by pooling point features with masks."""
...
def build_voxelquery3d_instance_segmenter3d(
    in_channels: int,
    num_classes: int,
    *,
    variant: str = "voxelquery3d_small",
...
if __name__ == "__main__":
    torch.manual_seed(0)
    m = build_voxelquery3d_instance_segmenter3d(
        in_channels=3, num_classes=6, variant="voxelquery3d_tiny"
    )
