...
_VARIANTS: dict[str, dict[str, object]] = {
    "pointmask3d_tiny": {"d_model": 64, "depth": 2, "queries": 16},
    "pointmask3d_small": {"d_model": 96, "depth": 3, "queries": 24},
    "pointmask3d_base": {"d_model": 128, "depth": 4, "queries": 32},
}


class PointMask3D(nn.Module):
    """PointMask3D (compact): transformer point encoder + query mask head."""
...
def build_pointpointmask3d_instance_segmenter3d(
    in_channels: int,
    num_classes: int,
    *,
    variant: str = "pointmask3d_small",
...
if __name__ == "__main__":
    torch.manual_seed(0)
    m = build_pointpointmask3d_instance_segmenter3d(
        in_channels=3, num_classes=6, variant="pointmask3d_tiny"
    )
