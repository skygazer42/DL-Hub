...
_VARIANTS: dict[str, dict[str, object]] = {
    "panopticquery3d_tiny": {"d_model": 64, "depth": 2, "queries": 16},
    "panopticquery3d_small": {"d_model": 96, "depth": 3, "queries": 24},
    "panopticquery3d_base": {"d_model": 128, "depth": 4, "queries": 32},
}


class PanopticQuery3D(nn.Module):
    """PanopticQuery3D (compact): transformer point encoder + query mask head."""
...
def build_panopticquery3d_instance_segmenter3d(
    in_channels: int,
    num_classes: int,
    *,
    variant: str = "panopticquery3d_small",
    width_mult: float = 1.0,
    dropout: float = 0.0,
) -> nn.Module:
    cfg = _VARIANTS[str(variant)]
    d_model = int(int(cfg["d_model"]) * float(width_mult))
    return PanopticQuery3D(
...
if __name__ == "__main__":
    ...
    m = build_panopticquery3d_instance_segmenter3d(
        in_channels=3, num_classes=6, variant="panopticquery3d_tiny"
    )
