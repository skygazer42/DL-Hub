...
_VARIANTS: dict[str, dict[str, object]] = {
    "queryinst3d_tiny": {"d_model": 64, "depth": 2, "queries": 16},
    "queryinst3d_small": {"d_model": 96, "depth": 3, "queries": 24},
    "queryinst3d_base": {"d_model": 128, "depth": 4, "queries": 32},
}
...
class QueryInst3D(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        d = int(d_model)
        self.enc = TransformerPointEncoder(int(in_channels), d, depth=int(depth), dropout=float(dropout))
        self.head1 = QueryMaskHead(d, int(num_classes), num_queries=int(num_queries), dropout=float(dropout))
    ...
    def forward(self, points: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        d = feat.shape[-1]
        mask_logits = torch.einsum("bkd,bnd->bkn", l2_normalize(inst_feat2), l2_normalize(feat)) * math.sqrt(d)
        cls_logits = self.cls2(inst_feat2)
        return {"mask_logits": mask_logits, "cls_logits": cls_logits}


def build_queryinst3d_instance_segmenter3d(
    in_channels: int,
    num_classes: int,
    *,
    variant: str = "queryinst3d_small",
...
if __name__ == "__main__":
    torch.manual_seed(0)
    m = build_queryinst3d_instance_segmenter3d(in_channels=3, num_classes=6, variant="queryinst3d_tiny")
