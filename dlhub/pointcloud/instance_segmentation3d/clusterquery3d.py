...
_VARIANTS: dict[str, dict[str, object]] = {
    "clusterquery3d_tiny": {"d_model": 64, "depth": 2, "queries": 16},
    "clusterquery3d_small": {"d_model": 96, "depth": 3, "queries": 24},
    "clusterquery3d_base": {"d_model": 128, "depth": 4, "queries": 32},
}


class Clusterquery3D(nn.Module):
    """Clusterquery3D (compact): initial queries, then refine by pooling point features with masks."""
...
def build_clusterquery3d_instance_segmenter3d(
    in_channels: int,
    num_classes: int,
    *,
    variant: str = "clusterquery3d_small",
    width_mult: float = 1.0,
    dropout: float = 0.0,
) -> nn.Module:
    cfg = _VARIANTS[str(variant)]
    d_model = int(int(cfg["d_model"]) * float(width_mult))
    return Clusterquery3D(
...
if __name__ == "__main__":
    torch.manual_seed(0)
    m = build_clusterquery3d_instance_segmenter3d(
        in_channels=3, num_classes=6, variant="clusterquery3d_tiny"
    )
