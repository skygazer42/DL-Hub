...
_VARIANTS: dict[str, dict[str, object]] = {
    "instformer3d_tiny": {"d_model": 64, "depth": 2, "queries": 16},
    "instformer3d_small": {"d_model": 96, "depth": 3, "queries": 24},
    "instformer3d_base": {"d_model": 128, "depth": 4, "queries": 32},
}


class InstFormer3D(nn.Module):
    """InstFormer3D (compact): transformer point encoder + query mask head."""
...
def build_instformer3d_instance_segmenter3d(
    in_channels: int,
    num_classes: int,
    *,
    variant: str = "instformer3d_small",
...
if __name__ == "__main__":
    torch.manual_seed(0)
    m = build_instformer3d_instance_segmenter3d(
        in_channels=3, num_classes=6, variant="instformer3d_tiny"
    )
