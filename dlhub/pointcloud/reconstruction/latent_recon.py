...
class LatentReconAutoEncoder(nn.Module):
...
_VARIANTS: dict[str, dict] = {
    "latent_recon_tiny": {"hidden": 32, "latent": 64, "dropout": 0.0},
    "latent_recon_small": {"hidden": 64, "latent": 128, "dropout": 0.0},
    "latent_recon_base": {"hidden": 96, "latent": 192, "dropout": 0.0},
}


def build_latent_recon_autoencoder(
    *,
    in_channels: int,
    num_points: int,
    variant: str = "latent_recon_small",
...
if __name__ == "__main__":
    ...
    m = build_latent_recon_autoencoder(in_channels=3, num_points=128, variant="latent_recon_tiny")
    y = m(x)
    print("latent_recon_tiny", tuple(y.shape))
    loss = y.mean()
    loss.backward()
    print("ok")
