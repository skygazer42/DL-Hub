...
class PointrAutoEncoder(nn.Module):
...
_VARIANTS: dict[str, dict] = {
    "pointr_tiny": {"hidden": 32, "latent": 64, "dropout": 0.0},
    "pointr_small": {"hidden": 64, "latent": 128, "dropout": 0.0},
    "pointr_base": {"hidden": 96, "latent": 192, "dropout": 0.0},
}


def build_pointr_autoencoder(
    *,
    in_channels: int,
    num_points: int,
    variant: str = "pointr_small",
...
if __name__ == "__main__":
    ...
    m = build_pointr_autoencoder(in_channels=3, num_points=128, variant="pointr_tiny")
    y = m(x)
    print("pointr_tiny", tuple(y.shape))
    loss = y.mean()
    loss.backward()
    print("ok")
