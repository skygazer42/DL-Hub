...
class SeedformerReconAutoEncoder(nn.Module):
...
_VARIANTS: dict[str, dict] = {
    "seedformer_recon_tiny": {"hidden": 32, "latent": 64, "dropout": 0.0},
    "seedformer_recon_small": {"hidden": 64, "latent": 128, "dropout": 0.0},
    "seedformer_recon_base": {"hidden": 96, "latent": 192, "dropout": 0.0},
}


def build_seedformer_recon_autoencoder(
    *,
    in_channels: int,
    num_points: int,
    variant: str = "seedformer_recon_small",
    ...
    return SeedformerReconAutoEncoder(
        in_channels=int(in_channels),
        num_points=int(num_points),
