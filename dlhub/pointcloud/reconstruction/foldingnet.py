...
class FoldingNetAutoEncoder(nn.Module):
...
_VARIANTS: dict[str, dict] = {
    "foldingnet_tiny": {"hidden": 32, "latent": 64, "dropout": 0.0},
    "foldingnet_small": {"hidden": 64, "latent": 128, "dropout": 0.0},
    "foldingnet_base": {"hidden": 96, "latent": 192, "dropout": 0.0},
}


def build_foldingnet_autoencoder(
    *,
    in_channels: int,
    num_points: int,
    variant: str = "foldingnet_small",
...
if __name__ == "__main__":
    ...
    m = build_foldingnet_autoencoder(in_channels=3, num_points=128, variant="foldingnet_tiny")
    y = m(x)
    print("foldingnet_tiny", tuple(y.shape))
    loss = y.mean()
    loss.backward()
    print("ok")
