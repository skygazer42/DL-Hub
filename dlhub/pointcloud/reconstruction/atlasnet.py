...
class AtlasNetAutoEncoder(nn.Module):
...
_VARIANTS: dict[str, dict] = {
    "atlasnet_tiny": {"hidden": 32, "latent": 64, "dropout": 0.0},
    "atlasnet_small": {"hidden": 64, "latent": 128, "dropout": 0.0},
    "atlasnet_base": {"hidden": 96, "latent": 192, "dropout": 0.0},
}


def build_atlasnet_autoencoder(
    *,
    in_channels: int,
    num_points: int,
    variant: str = "atlasnet_small",
...
if __name__ == "__main__":
    ...
    m = build_atlasnet_autoencoder(in_channels=3, num_points=128, variant="atlasnet_tiny")
    y = m(x)
    print("atlasnet_tiny", tuple(y.shape))
    loss = y.mean()
    loss.backward()
    print("ok")
