...
class TopNetAutoEncoder(nn.Module):
...
_VARIANTS: dict[str, dict] = {
    "topnet_tiny": {"hidden": 32, "latent": 64, "dropout": 0.0},
    "topnet_small": {"hidden": 64, "latent": 128, "dropout": 0.0},
    "topnet_base": {"hidden": 96, "latent": 192, "dropout": 0.0},
}


def build_topnet_autoencoder(
    *,
    in_channels: int,
    num_points: int,
    variant: str = "topnet_small",
...
if __name__ == "__main__":
    ...
    m = build_topnet_autoencoder(in_channels=3, num_points=128, variant="topnet_tiny")
    y = m(x)
    print("topnet_tiny", tuple(y.shape))
    loss = y.mean()
    loss.backward()
    print("ok")
