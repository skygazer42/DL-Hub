...
class PointmambaReconAutoEncoder(nn.Module):
...
_VARIANTS: dict[str, dict] = {
    "pointmamba_recon_tiny": {"hidden": 32, "latent": 64, "dropout": 0.0},
    "pointmamba_recon_small": {"hidden": 64, "latent": 128, "dropout": 0.0},
    "pointmamba_recon_base": {"hidden": 96, "latent": 192, "dropout": 0.0},
}


def build_pointmamba_recon_autoencoder(
    *,
    in_channels: int,
    num_points: int,
    variant: str = "pointmamba_recon_small",
    ...
    return PointmambaReconAutoEncoder(
        in_channels=int(in_channels),
        num_points=int(num_points),
...
if __name__ == "__main__":
    ...
    m = build_pointmamba_recon_autoencoder(in_channels=3, num_points=128, variant="pointmamba_recon_tiny")
    y = m(x)
    print("pointmamba_recon_tiny", tuple(y.shape))
    loss = y.mean()
    loss.backward()
    print("ok")
