...
class GRNetAutoEncoder(nn.Module):
...
_VARIANTS: dict[str, dict] = {
    "grnet_tiny": {"hidden": 32, "latent": 64, "dropout": 0.0},
    "grnet_small": {"hidden": 64, "latent": 128, "dropout": 0.0},
    "grnet_base": {"hidden": 96, "latent": 192, "dropout": 0.0},
}


def build_grnet_autoencoder(
    *,
    in_channels: int,
    num_points: int,
    variant: str = "grnet_small",
...
if __name__ == "__main__":
    ...
    m = build_grnet_autoencoder(in_channels=3, num_points=128, variant="grnet_tiny")
    y = m(x)
    print("grnet_tiny", tuple(y.shape))
    loss = y.mean()
    loss.backward()
    print("ok")
