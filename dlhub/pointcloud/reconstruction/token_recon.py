...
class TokenReconAutoEncoder(nn.Module):
...
_VARIANTS: dict[str, dict] = {
    "token_recon_tiny": {"hidden": 32, "latent": 64, "dropout": 0.0},
    "token_recon_small": {"hidden": 64, "latent": 128, "dropout": 0.0},
    "token_recon_base": {"hidden": 96, "latent": 192, "dropout": 0.0},
}


def build_token_recon_autoencoder(
    *,
    in_channels: int,
    num_points: int,
    variant: str = "token_recon_small",
...
if __name__ == "__main__":
    ...
    m = build_token_recon_autoencoder(in_channels=3, num_points=128, variant="token_recon_tiny")
    y = m(x)
    print("token_recon_tiny", tuple(y.shape))
    loss = y.mean()
    loss.backward()
    print("ok")
