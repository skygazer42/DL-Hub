...
class SpareNetAutoEncoder(nn.Module):
...
_VARIANTS: dict[str, dict] = {
    "spare_net_tiny": {"hidden": 32, "latent": 64, "dropout": 0.0},
    "spare_net_small": {"hidden": 64, "latent": 128, "dropout": 0.0},
    "spare_net_base": {"hidden": 96, "latent": 192, "dropout": 0.0},
}


def build_spare_net_autoencoder(
    *,
    in_channels: int,
    num_points: int,
    variant: str = "spare_net_small",
...
if __name__ == "__main__":
    ...
    m = build_spare_net_autoencoder(in_channels=3, num_points=128, variant="spare_net_tiny")
    y = m(x)
    print("spare_net_tiny", tuple(y.shape))
    loss = y.mean()
    loss.backward()
    print("ok")
