...
class SnowflakeReconAutoEncoder(nn.Module):
...
_VARIANTS: dict[str, dict] = {
    "snowflake_recon_tiny": {"hidden": 32, "latent": 64, "dropout": 0.0},
    "snowflake_recon_small": {"hidden": 64, "latent": 128, "dropout": 0.0},
    "snowflake_recon_base": {"hidden": 96, "latent": 192, "dropout": 0.0},
}


def build_snowflake_recon_autoencoder(
    *,
    in_channels: int,
    num_points: int,
    variant: str = "snowflake_recon_small",
    ...
    return SnowflakeReconAutoEncoder(
        in_channels=int(in_channels),
        num_points=int(num_points),
