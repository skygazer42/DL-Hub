...
_VARIANTS: dict[str, dict] = {
    "pcn_tiny": {"hidden": 32, "latent": 64, "dropout": 0.0},
    "pcn_small": {"hidden": 64, "latent": 128, "dropout": 0.0},
    "pcn_base": {"hidden": 96, "latent": 192, "dropout": 0.0},
}


def build_pcn_autoencoder(
    *,
    in_channels: int,
    num_points: int,
    variant: str = "pcn_small",
...
if __name__ == "__main__":
    ...
    m = build_pcn_autoencoder(in_channels=3, num_points=128, variant="pcn_tiny")
    y = m(x)
    print("pcn_tiny", tuple(y.shape))
    loss = y.mean()
    loss.backward()
    print("ok")
