...
class SeedReconAutoEncoder(nn.Module):
...
_VARIANTS: dict[str, dict] = {
    "seed_recon_tiny": {"hidden": 32, "latent": 64, "dropout": 0.0},
    "seed_recon_small": {"hidden": 64, "latent": 128, "dropout": 0.0},
    "seed_recon_base": {"hidden": 96, "latent": 192, "dropout": 0.0},
}


def build_seed_recon_autoencoder(
    *,
    in_channels: int,
    num_points: int,
    variant: str = "seed_recon_small",
...
if __name__ == "__main__":
    ...
    m = build_seed_recon_autoencoder(in_channels=3, num_points=128, variant="seed_recon_tiny")
    y = m(x)
    print("seed_recon_tiny", tuple(y.shape))
    loss = y.mean()
    loss.backward()
    print("ok")
