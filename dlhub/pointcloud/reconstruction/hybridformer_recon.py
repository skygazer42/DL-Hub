...
class HybridformerReconAutoEncoder(nn.Module):
...
_VARIANTS: dict[str, dict] = {
    "hybridformer_recon_tiny": {"hidden": 32, "latent": 64, "dropout": 0.0},
    "hybridformer_recon_small": {"hidden": 64, "latent": 128, "dropout": 0.0},
    "hybridformer_recon_base": {"hidden": 96, "latent": 192, "dropout": 0.0},
}


def build_hybridformer_recon_autoencoder(
    *,
    in_channels: int,
    num_points: int,
    variant: str = "hybridformer_recon_small",
    ...
    return HybridformerReconAutoEncoder(
        in_channels=int(in_channels),
        num_points=int(num_points),
...
if __name__ == "__main__":
    ...
    m = build_hybridformer_recon_autoencoder(in_channels=3, num_points=128, variant="hybridformer_recon_tiny")
    y = m(x)
    print("hybridformer_recon_tiny", tuple(y.shape))
    loss = y.mean()
    loss.backward()
    print("ok")
