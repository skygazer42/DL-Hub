...
_VARIANTS: dict[str, dict[str, object]] = {
    ...
    "rangeformer_small": {"width": 96, "depth": 3, "h": 48, "w": 128},
    "rangeformer_base": {"width": 128, "depth": 4, "h": 64, "w": 160},
}
...
class RangeFormerSeg(nn.Module):
    ...
    def forward(self, points: torch.Tensor) -> torch.Tensor:
        ...
        rv = scatter_mean_2d(idx, p, h=int(self.grid.h), w=int(self.grid.w))  # (B,W,H,W)

        b, c, h, w = rv.shape
        tok = rv.permute(0, 2, 3, 1).reshape(b, h * w, c)
        tok = self.enc(tok)
...
def build_rangeformer_segmenter3d(
    in_channels: int,
    num_classes: int,
    *,
    variant: str = "rangeformer_small",
    width_mult: float = 1.0,
    dropout: float = 0.0,
) -> nn.Module:
    cfg = _VARIANTS[str(variant)]
    width = int(int(cfg["width"]) * float(width_mult))
    return RangeFormerSeg(
...
if __name__ == "__main__":
    torch.manual_seed(0)
    model = build_rangeformer_segmenter3d(in_channels=3, num_classes=6, variant="rangeformer_tiny")
