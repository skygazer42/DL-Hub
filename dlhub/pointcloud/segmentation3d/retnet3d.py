import torch
from torch import nn

from ._common import TransformerSegBase

_VARIANTS: dict[str, dict[str, object]] = {
    "retnet3d_tiny": {"d_model": 64, "depth": 2},
    "retnet3d_small": {"d_model": 96, "depth": 3},
    "retnet3d_base": {"d_model": 128, "depth": 4},
}


class RetNet3DSeg(nn.Module):
    """RetNet3D semantic segmentation (toy): transformer encoder on point tokens."""

    def __init__(
        self, *, in_channels: int, num_classes: int, d_model: int, depth: int, dropout: float = 0.0
    ) -> None:
        super().__init__()
        self.net = TransformerSegBase(
            in_channels=int(in_channels),
            num_classes=int(num_classes),
            d_model=int(d_model),
            depth=int(depth),
            dropout=float(dropout),
            pos_feats=16,
        )

    def forward(self, points: torch.Tensor) -> torch.Tensor:
        return self.net(points)


def build_retnet3d_segmenter3d(
    in_channels: int,
    num_classes: int,
    *,
    variant: str = "retnet3d_small",
    width_mult: float = 1.0,
    dropout: float = 0.0,
) -> nn.Module:
    cfg = _VARIANTS[str(variant)]
    d_model = int(int(cfg["d_model"]) * float(width_mult))
    return RetNet3DSeg(
        in_channels=int(in_channels),
        num_classes=int(num_classes),
        d_model=d_model,
        depth=int(cfg["depth"]),
        dropout=float(dropout),
    )


if __name__ == "__main__":
    torch.manual_seed(0)
    model = build_retnet3d_segmenter3d(
        in_channels=3, num_classes=6, variant="retnet3d_tiny"
    )
    x = torch.randn(2, 128, 3)
    y = model(x)
    (y.mean()).backward()
    print("logits:", tuple(y.shape))
