...
_VARIANTS: dict[str, dict[str, object]] = {
    "rangenetpp_tiny": {"width": 48, "h": 32, "w": 96},
    "rangenetpp_small": {"width": 64, "h": 48, "w": 128},
    "rangenetpp_base": {"width": 96, "h": 64, "w": 160},
}
...
def build_rangenetpp_segmenter3d(
    in_channels: int,
    num_classes: int,
    *,
    variant: str = "rangenetpp_small",
    width_mult: float = 1.0,
    dropout: float = 0.0,
) -> nn.Module:
    cfg = _VARIANTS[str(variant)]
    width = int(int(cfg["width"]) * float(width_mult))
    return RangeNetPPSeg(
...
if __name__ == "__main__":
    torch.manual_seed(0)
    model = build_rangenetpp_segmenter3d(in_channels=3, num_classes=6, variant="rangenetpp_tiny")
