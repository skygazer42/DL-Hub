...
_VARIANTS: dict[str, dict[str, object]] = {
    "stratified_mamba3d_tiny": {"width": 64, "depth": 2, "tokens": 48},
    "stratified_mamba3d_small": {"width": 96, "depth": 3, "tokens": 64},
    "stratified_mamba3d_base": {"width": 128, "depth": 4, "tokens": 96},
}


class StratifiedMamba3DSeg(nn.Module):
    """Stratified Mamba 3D semantic segmentation (toy): attend on sampled tokens then propagate."""
...
def build_stratified_mamba3d_segmenter3d(
    in_channels: int,
    num_classes: int,
    *,
    variant: str = "stratified_mamba3d_small",
    width_mult: float = 1.0,
    dropout: float = 0.0,
) -> nn.Module:
    cfg = _VARIANTS[str(variant)]
    width = int(int(cfg["width"]) * float(width_mult))
    return StratifiedMamba3DSeg(
...
if __name__ == "__main__":
    torch.manual_seed(0)
    model = build_stratified_mamba3d_segmenter3d(
        in_channels=3, num_classes=6, variant="stratified_mamba3d_tiny"
    )
