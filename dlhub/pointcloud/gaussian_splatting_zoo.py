...
def _split_arch_id(arch_id: str) -> tuple[str, str]:
    ...
    return split_arch_id(arch_id, default_prefix="pcgs")
...
def _extract_builder_name_from_source(src: str) -> str | None:
    ...
    for node in tree.body:
        if isinstance(node, ast.FunctionDef):
            name = str(node.name)
            if name.startswith("build_") and name.endswith("_splatter"):
                return name
    return None
...
def _extend_registry(r: dict[str, Builder]) -> None:
    ...
    module_dir = here / "gaussian_splatting"
...
def list_local_arches() -> list[str]:
    ...
    return list_arch_ids(_REGISTRY, prefix="pcgs")
...
def build_local_model(
    ...
    *,
    ...
    if prefix not in {"pcgs", "local"}:
        raise ValueError(f"Unsupported gaussian splatting prefix: {prefix!r} (arch_id={arch_id!r})")

    builder = _REGISTRY.get(str(name).lower().strip())
    if builder is None:
        raise UnknownLocalArch(
            f"Unknown gaussian splatting arch: {arch_id!r}. Tip: import `dlhub.pointcloud.gaussian_splatting_zoo` and call `list_local_arches()`."
        )
