...
_VARIANTS: dict[str, dict[str, object]] = {
    "fastbev_det_tiny": {"d_model": 64, "queries": 32},
    "fastbev_det_small": {"d_model": 96, "queries": 48},
    "fastbev_det_base": {"d_model": 128, "queries": 64},
}


class FastbevDet(nn.Module):
...
def build_fastbev_det_detector3d(
    in_channels: int,
    num_classes: int,
    *,
    variant: str = "fastbev_det_small",
...
if __name__ == "__main__":
    torch.manual_seed(0)
    m = build_fastbev_det_detector3d(in_channels=3, num_classes=5, variant="fastbev_det_tiny")
