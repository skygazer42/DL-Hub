...
_VARIANTS: dict[str, dict[str, object]] = {
    "bevnext_det_tiny": {"d_model": 64, "queries": 32},
    "bevnext_det_small": {"d_model": 96, "queries": 48},
    "bevnext_det_base": {"d_model": 128, "queries": 64},
}


class BevnextDet(nn.Module):
...
def build_bevnext_det_detector3d(
    in_channels: int,
    num_classes: int,
    *,
    variant: str = "bevnext_det_small",
...
if __name__ == "__main__":
    torch.manual_seed(0)
    m = build_bevnext_det_detector3d(in_channels=3, num_classes=5, variant="bevnext_det_tiny")
