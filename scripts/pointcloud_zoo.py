...
def main() -> int:
    ...
    from dlhub.pointcloud.local_zoo import build_local_model, list_local_arches

    args = parse_args()

    if not args.list and args.smoke is None:
        print("Nothing to do. Try one of:")
        print("- python scripts/pointcloud_zoo.py --list")
        print("- python scripts/pointcloud_zoo.py --smoke pc:pointnet")
        return 2
    ...
    if args.smoke is not None:
        ...
        if ":" not in arch_id:
            arch_id = f"pc:{arch_id}"

        import torch

        b = int(args.batch_size)
        n = int(args.num_points)
        c = int(args.in_channels)
        if c < 3:
            raise ValueError("--in-channels must be >= 3 for xyz")

        points = torch.randn(b, n, c, dtype=torch.float32)
        points[..., :3] = torch.rand(b, n, 3) * 2.0 - 1.0
