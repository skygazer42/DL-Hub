...
def _print_lines(lines: Iterable[str], *, limit: int = 120) -> None:
    lines = list(lines)
    if len(lines) <= limit:
        for line in lines:
            print(line)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="3D detection local model zoo utilities (no downloads).")
    ...
    parser.add_argument("--in-channels", type=int, default=3, help="Point channels for local detectors (>=3).")
    parser.add_argument("--num-classes", type=int, default=3, help="Detector classes.")
    parser.add_argument("--width-mult", type=float, default=1.0, help="Width multiplier for local detectors.")
    parser.add_argument("--dropout", type=float, default=0.0, help="Dropout rate for local detectors.")
...
def main() -> int:
    ...
    from dlhub.pointcloud.detection3d_zoo import build_local_model, list_local_arches

    args = parse_args()

    if not args.list and args.smoke is None:
        print("Nothing to do. Try one of:")
        print("- python scripts/detection3d_zoo.py --list")
        print("- python scripts/detection3d_zoo.py --smoke pcdet3d:pointpillars_tiny")
        return 2
    ...
    if args.smoke is not None:
        ...
        if ":" not in arch_id:
            arch_id = f"pcdet3d:{arch_id}"

        import torch

        x = torch.randn(int(args.batch_size), int(args.num_points), int(args.in_channels))
