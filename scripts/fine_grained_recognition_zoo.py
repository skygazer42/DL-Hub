...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fine-grained visual recognition local model zoo utilities (no downloads).")
    ...
    parser.add_argument("--timeline", action="store_true", help="Print a best-effort FGVC family timeline (by year).")
    ...
    if args.timeline:
        ...
        for fam in sorted(families):
            ...
            if entry is None:
                ...
                timeline.append((year, entry.group, entry.family, entry.method, f"dlfgvc:{entry.family}_tiny"))
