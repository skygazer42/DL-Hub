...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Co-segmentation local model zoo utilities (no downloads)."
    )
    ...
    parser.add_argument(
        "--smoke", type=str, default=None, metavar="ARCH_ID", help="Run a forward smoke."
    )
    parser.add_argument("--batch-size", type=int, default=2, help="Batch size for smoke inputs.")
    parser.add_argument("--set-size", type=int, default=3, help="Images per co-segmentation set.")
    parser.add_argument(
        "--image-size", type=int, default=64, help="Input image size for smoke inputs."
    )
...
def main() -> int:
    ...
    from dlhub.vision.co_segmentation_zoo import build_local_model, list_local_arches

    args = parse_args()
    if not args.list and args.smoke is None:
        print("Nothing to do. Try one of:")
        print("- python scripts/co_segmentation_zoo.py --list")
        print("- python scripts/co_segmentation_zoo.py --search transformer --list")
        print("- python scripts/co_segmentation_zoo.py --smoke coseg:siamese_coseg_tiny")
        return 2
    ...
    if args.list:
        print("Co-segmentation local zoo")
