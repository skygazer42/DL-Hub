...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Super-resolution local model zoo utilities (no downloads)."
    )
    ...
    parser.add_argument("--upscale-factor", type=int, default=2, help="Upscale factor.")
...
def main() -> int:
    ...
    if not args.list and args.smoke is None:
        ...
        print("- python scripts/super_resolution_zoo.py --list")
        print("- python scripts/super_resolution_zoo.py --search tiny --list")
        print("- python scripts/super_resolution_zoo.py --smoke sr:srcnn_tiny")
        return 2
    ...
    if args.list:
        print("Super-resolution local zoo")
    ...
    if args.smoke is not None:
        ...
        print(f"- low_res_shape={tuple(low_res.shape)}")
