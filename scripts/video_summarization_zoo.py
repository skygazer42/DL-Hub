...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Video summarization local model zoo utilities (no downloads)."
    )
    ...
    parser.add_argument("--seq-len", type=int, default=8, help="Video frame count for smoke inputs.")
    parser.add_argument("--image-size", type=int, default=32, help="Frame image size for smoke inputs.")
    parser.add_argument("--in-channels", type=int, default=3, help="Input channels for local summarizers.")
...
def main() -> int:
    ...
    from dlhub.vision.video_summarization_zoo import build_local_model, list_local_arches

    args = parse_args()
    if not args.list and args.smoke is None:
        print("Nothing to do. Try one of:")
        print("- python scripts/video_summarization_zoo.py --list")
        print("- python scripts/video_summarization_zoo.py --search tiny --list")
        print("- python scripts/video_summarization_zoo.py --smoke vsum:vasnet_tiny")
        return 2
    ...
    if args.list:
        print("Video summarization local zoo")
