...
class FaceIdentificationClassifier(nn.Module):
    def __init__(self, cfg: ModelConfig) -> None:
        ...
        self.head = nn.Sequential(
            ...
            nn.Linear(in_ch, int(cfg.num_identities)),
        )
...
def identification_loss(logits: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
    return torch.nn.functional.cross_entropy(logits, labels)


def identification_accuracy(logits: torch.Tensor, labels: torch.Tensor) -> float:
    with torch.no_grad():
