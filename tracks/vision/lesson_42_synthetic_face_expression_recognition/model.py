...
class FaceExpressionClassifier(nn.Module):
...
def expression_loss(logits: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
    return torch.nn.functional.cross_entropy(logits, labels)


def expression_accuracy(logits: torch.Tensor, labels: torch.Tensor) -> float:
    with torch.no_grad():
