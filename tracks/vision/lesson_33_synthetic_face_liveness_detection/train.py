...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 33 (Vision): synthetic face liveness detection."
    )
...
def _run_epoch(
    *,
    ...
    for step, (images, labels) in enumerate(loader):
        ...
        total_acc += liveness_accuracy(logits.detach(), labels.detach())
        total_batches += 1
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig, model_cfg: ModelConfig) -> int:
    ...
    paths = build_run_paths(
        track="vision",
        lesson="lesson_33_synthetic_face_liveness_detection",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("vision.face_liveness_detection", log_file=paths.logs_dir / "train.log")
    ...
    model = FaceLivenessClassifier(model_cfg).to(device_info.torch_device)
    ...
    save_checkpoint(
        ...
        extra={"track": "vision", "lesson": "lesson_33_synthetic_face_liveness_detection"},
    )
    return 0
