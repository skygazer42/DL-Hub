...
def _mask_to_box(mask: torch.Tensor) -> torch.Tensor:
    ...
    top = float(coords[:, 0].min().item())
    bottom = float(coords[:, 0].max().item() + 1)
    left = float(coords[:, 1].min().item())
    right = float(coords[:, 1].max().item() + 1)
    size = float(mask.shape[-1])
    center_x = (left + right) / (2.0 * size)
    center_y = (top + bottom) / (2.0 * size)
    width = (right - left) / size
    height = (bottom - top) / size
    return torch.tensor([center_x, center_y, width, height], dtype=torch.float32)


def make_salient_box_sample(cfg: DataConfig, idx: int) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    return image, _mask_to_box(salient_mask)


class SyntheticSalientObjectBoxesDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, torch.Tensor]:
        return make_salient_box_sample(self.cfg, int(idx))
