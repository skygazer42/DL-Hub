...
def edge_iou(logits: torch.Tensor, target: torch.Tensor, *, threshold: float = 0.5) -> float:
...
def edge_detection_loss(logits: torch.Tensor, target: torch.Tensor) -> tuple[torch.Tensor, dict[str, float]]:
...
__all__ = ["EdgeDetectionModel", "ModelConfig", "edge_detection_loss", "edge_iou"]
