...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig]:
    parser = argparse.ArgumentParser(description="Lesson 27 (Vision): synthetic edge detection.")
...
def _run_epoch(
    *,
    ...
    for step, (images, targets) in enumerate(loader):
        ...
        logits = model(images)
        loss, parts = edge_detection_loss(logits, targets)
        iou = edge_iou(logits.detach(), targets.detach())
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig, model_cfg: ModelConfig) -> int:
    ...
    paths = build_run_paths(
        track="vision",
        lesson="lesson_27_synthetic_edge_detection",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("vision.edge_detection", log_file=paths.logs_dir / "train.log")
    paths.run_dir.mkdir(parents=True, exist_ok=True)
    paths.checkpoints_dir.mkdir(parents=True, exist_ok=True)

    train_loader, val_loader = get_dataloaders(data_cfg)
    model = EdgeDetectionModel(model_cfg).to(device_info.torch_device)
    ...
    save_checkpoint(
        ...
        extra={"track": "vision", "lesson": "lesson_27_synthetic_edge_detection"},
    )
    return 0
