...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 44 (Vision): synthetic face verification."
    )
    ...
    parser.add_argument("--num-identities", type=int, default=24)
...
def _run_epoch(
    *,
    ...
    for step, (images_a, images_b, labels) in enumerate(loader):
        ...
        total_acc += verification_accuracy(logits.detach(), labels.detach())
        total_batches += 1
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig, model_cfg: ModelConfig) -> int:
    ...
    paths = build_run_paths(
        track="vision",
        lesson="lesson_44_synthetic_face_verification",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("vision.synthetic_face_verification", log_file=paths.logs_dir / "train.log")
    ...
    model = FaceVerificationModel(model_cfg).to(device_info.torch_device)
    ...
    save_checkpoint(
        ...
        extra={"track": "vision", "lesson": "lesson_44_synthetic_face_verification"},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.vision.lesson_44_synthetic_face_verification.train"
        )
