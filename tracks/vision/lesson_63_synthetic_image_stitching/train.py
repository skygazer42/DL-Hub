...
from .model import ModelConfig, StitchingModel, build_model, stitching_loss
...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig]:
    parser = argparse.ArgumentParser(description="Lesson 63 (Vision): synthetic image stitching.")
    ...
    parser.add_argument("--min-shapes", type=int, default=3)
...
def _run_epoch(
    *,
    ...
    for step, (inputs, target) in enumerate(loader):
        ...
        input_batch = {name: value.to(device) for name, value in inputs.items()}
        target = target.to(device)
        prediction = model(input_batch)
        loss, parts = stitching_loss(prediction, target)
        psnr = compute_psnr(prediction.detach(), target.detach())
        ...
        total_reconstruction += float(parts["reconstruction_loss"])
        total_seam += float(parts["seam_loss"])
        total_psnr += float(psnr.item())
        total_batches += 1

    if total_batches == 0:
        return Stats(loss=0.0, reconstruction_loss=0.0, seam_loss=0.0, psnr=0.0)
    return Stats(
        loss=total_loss / total_batches,
        reconstruction_loss=total_reconstruction / total_batches,
        seam_loss=total_seam / total_batches,
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig, model_cfg: ModelConfig) -> int:
    ...
    paths = build_run_paths(
        track="vision",
        lesson="lesson_63_synthetic_image_stitching",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("vision.image_stitching", log_file=paths.logs_dir / "train.log")
    ...
    save_checkpoint(
        ...
        extra={"track": "vision", "lesson": "lesson_63_synthetic_image_stitching"},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.vision.lesson_63_synthetic_image_stitching.train"
        )
