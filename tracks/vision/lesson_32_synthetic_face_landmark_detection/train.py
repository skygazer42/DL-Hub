...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 32 (Vision): synthetic face landmark detection."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig, model_cfg: ModelConfig) -> int:
    ...
    paths = build_run_paths(
        track="vision",
        lesson="lesson_32_synthetic_face_landmark_detection",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("vision.face_landmark_detection", log_file=paths.logs_dir / "train.log")
    ...
    model = FaceLandmarkRegressor(model_cfg).to(device_info.torch_device)
    ...
    save_checkpoint(
        ...
        extra={"track": "vision", "lesson": "lesson_32_synthetic_face_landmark_detection"},
    )
    return 0
...
def _evaluate_l2(
    *,
    ...
    for step, (images, targets) in enumerate(loader):
        ...
        total_error += mean_landmark_l2_pixels(
            preds.detach().cpu(),
            targets,
