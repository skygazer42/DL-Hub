...
from .model import HandPoseRegressor, ModelConfig, mean_pose_l2_pixels
...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig]:
    parser = argparse.ArgumentParser(description="Lesson 50 (Vision): synthetic hand pose estimation.")
    ...
    parser.add_argument("--line-sigma", type=float, default=1.2)
    parser.add_argument("--joint-sigma", type=float, default=1.7)
...
def _evaluate_l2(
    *,
    ...
    for step, (images, targets) in enumerate(loader):
        ...
        total_error += mean_pose_l2_pixels(
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig, model_cfg: ModelConfig) -> int:
    ...
    paths = build_run_paths(
        track="vision",
        lesson="lesson_50_synthetic_hand_pose_estimation",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("vision.hand_pose_estimation", log_file=paths.logs_dir / "train.log")
    ...
    model = HandPoseRegressor(model_cfg).to(device_info.torch_device)
    ...
    save_checkpoint(
        ...
        extra={"track": "vision", "lesson": "lesson_50_synthetic_hand_pose_estimation"},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.vision.lesson_50_synthetic_hand_pose_estimation.train"
        )
