...
def image_matching_loss(logits: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
    return torch.nn.functional.cross_entropy(logits, labels)


def image_matching_accuracy(logits: torch.Tensor, labels: torch.Tensor) -> float:
    with torch.no_grad():
        return float((logits.argmax(dim=1) == labels).to(torch.float32).mean().item())


__all__ = ["ImageMatchingModel", "ModelConfig", "image_matching_accuracy", "image_matching_loss"]
