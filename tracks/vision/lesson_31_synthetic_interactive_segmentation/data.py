...
def _validate_config(cfg: DataConfig) -> None:
    ...
    if int(cfg.click_radius) < 1:
        raise ValueError("click_radius must be >= 1")
    ...
    *,
    ...
    value = float(
        torch.empty((), dtype=torch.float32).uniform_(
            value_range[0], value_range[1], generator=generator
        ).item()
    )
...
def _make_click_map(mask: torch.Tensor, *, radius: int) -> torch.Tensor:
    coords = mask.nonzero(as_tuple=False)
    center = coords[len(coords) // 2]
    click = torch.zeros_like(mask)
    _draw_circle(click, int(center[0].item()), int(center[1].item()), radius, 1.0)
    return click


def make_interactive_sample(cfg: DataConfig, idx: int) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    target_mask = _draw_shape(
        image,
        generator=generator,
        size_range=(int(cfg.min_object_size) + 1, int(cfg.max_object_size) + 2),
        value_range=(0.8, 1.0),
    )
    click_map = _make_click_map(target_mask, radius=int(cfg.click_radius))
    image = image.clamp(0.0, 1.0).unsqueeze(0)
    return image, click_map.unsqueeze(0), target_mask.unsqueeze(0)


class SyntheticInteractiveSegmentationDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        return make_interactive_sample(self.cfg, int(idx))
