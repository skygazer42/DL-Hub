...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 31 (Vision): synthetic interactive segmentation from clicks."
    )
...
def _run_epoch(
    *,
    ...
    for step, (images, click_maps, targets) in enumerate(loader):
        if max_batches is not None and step >= int(max_batches):
            break
        images = images.to(device)
        click_maps = click_maps.to(device)
        targets = targets.to(device)
        logits = model(images, click_maps)
        loss, parts = interactive_segmentation_loss(logits, targets)
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig, model_cfg: ModelConfig) -> int:
    ...
    paths = build_run_paths(
        track="vision",
        lesson="lesson_31_synthetic_interactive_segmentation",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("vision.interactive_segmentation", log_file=paths.logs_dir / "train.log")
    paths.run_dir.mkdir(parents=True, exist_ok=True)
    paths.checkpoints_dir.mkdir(parents=True, exist_ok=True)

    train_loader, val_loader = get_dataloaders(data_cfg)
    model = InteractiveSegmentationModel(model_cfg).to(device_info.torch_device)
    ...
    save_checkpoint(
        ...
        extra={"track": "vision", "lesson": "lesson_31_synthetic_interactive_segmentation"},
    )
    return 0
