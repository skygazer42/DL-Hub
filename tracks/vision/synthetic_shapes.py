...
class SyntheticQuadrantSquares:
    def __init__(self, cfg: DataConfig) -> None:
        ...
        if int(cfg.num_classes) != 4:
            raise ValueError("This synthetic dataset currently uses 4 classes (quadrants).")
...
def get_dataloaders(cfg: DataConfig):
    """Return `(train_loader, val_loader)` for the synthetic quadrant squares task."""

    from torch.utils.data import DataLoader, Subset

    ds = SyntheticQuadrantSquares(cfg)
...
__all__ = ["DataConfig", "SyntheticQuadrantSquares", "get_dataloaders"]
