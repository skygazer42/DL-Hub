...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 36 (Vision): synthetic fixed-length text recognition."
    )
    ...
    parser.add_argument("--text-length", type=int, default=5)
...
def _run_epoch(
    *,
    ...
    for step, (images, labels) in enumerate(loader):
        ...
        total_acc += sequence_accuracy(logits.detach(), labels.detach())
        total_batches += 1
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig, model_cfg: ModelConfig) -> int:
    ...
    paths = build_run_paths(
        track="vision",
        lesson="lesson_36_synthetic_text_recognition",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("vision.text_recognition", log_file=paths.logs_dir / "train.log")
    ...
    resolved_model_cfg = ModelConfig(
        ...
        text_length=model_cfg.text_length,
        hidden_channels=model_cfg.hidden_channels,
        num_blocks=model_cfg.num_blocks,
        dropout=model_cfg.dropout,
    )
    model = TextRecognizer(resolved_model_cfg).to(device_info.torch_device)
    ...
    save_checkpoint(
        ...
        extra={"track": "vision", "lesson": "lesson_36_synthetic_text_recognition"},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.vision.lesson_36_synthetic_text_recognition.train"
        )
