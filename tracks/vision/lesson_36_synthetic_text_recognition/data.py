...
def _char_template(char_id: int) -> np.ndarray:
    rng = np.random.default_rng(int(char_id) * 10007)
    template = (rng.random((7, 5)) > 0.5).astype(np.float32)
...
class SyntheticWordDataset:
    """Generate fixed-length synthetic text crops with deterministic glyph templates."""

    def __init__(self, cfg: DataConfig, vocab: TextVocab | None = None) -> None:
        if int(cfg.image_height) < 16 or int(cfg.image_width) < 48:
            raise ValueError("image geometry is too small for synthetic text rendering")
        if int(cfg.text_length) < 3:
            raise ValueError("text_length must be >= 3")
        if not (0.0 < float(cfg.val_fraction) < 1.0):
            raise ValueError("val_fraction must be in (0, 1)")
        self.cfg = cfg
        self.vocab = TextVocab() if vocab is None else vocab
    ...
    def __getitem__(self, idx: int):
        ...
        rng = np.random.default_rng(int(self.cfg.seed) * 7_000_001 + int(idx))

        labels = rng.integers(1, self.vocab.size, size=text_length, dtype=np.int64)
        image = np.full((h, w), 0.97, dtype=np.float32)
        image[2 : h - 2, 2 : w - 2] = 0.9

        cell_w = (w - 4) // text_length
        baseline = h // 2 + 3
        for position, char_id in enumerate(labels.tolist()):
            ...
            start_y = baseline - glyph_h + int(rng.integers(-1, 2))
            start_y = int(np.clip(start_y, 2, h - 2 - glyph_h))
            ...
            image[start_y:end_y, start_x:end_x] -= 0.78 * crop
            if position < text_length - 1:
                gap_x = cell_x0 + cell_w - 1
                image[4 : h - 4, gap_x : gap_x + 1] += 0.02
...
def get_dataloaders(cfg: DataConfig):
    ...
    vocab = TextVocab()
    dataset = SyntheticWordDataset(cfg, vocab=vocab)
...
__all__ = ["DataConfig", "SyntheticWordDataset", "TextVocab", "get_dataloaders"]
