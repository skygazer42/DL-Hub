...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig]:
    parser = argparse.ArgumentParser(description="Lesson 61 (Vision): synthetic image retrieval.")
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig, model_cfg: ModelConfig) -> int:
    ...
    paths = build_run_paths(
        track="vision",
        lesson="lesson_61_synthetic_image_retrieval",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("vision.image_retrieval", log_file=paths.logs_dir / "train.log")
    ...
    model = ImageRetrievalEmbeddingNet(model_cfg).to(device_info.torch_device)
    ...
    save_checkpoint(
        ...
        extra={"track": "vision", "lesson": "lesson_61_synthetic_image_retrieval"},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.vision.lesson_61_synthetic_image_retrieval.train"
        )
