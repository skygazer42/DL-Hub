...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 09 (Vision): CNN backbones on a synthetic dataset."
    )
    ...
    parser.add_argument(
        "--include-timm",
        action="store_true",
        help="Include timm model names when listing architectures (may be a very large list).",
    )
    ...
    if args.list_arch:
        ...
        print("\n".join(list_supported_arches(include_timm=bool(args.include_timm))))
        raise SystemExit(0)
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="vision",
        lesson="lesson_09_cnn_backbones_compact_classification",
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "vision", "lesson": "lesson_09_cnn_backbones_compact_classification"},
    )
