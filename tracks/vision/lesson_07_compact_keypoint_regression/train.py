...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(description="Lesson 07 (Vision): Compact keypoint regression.")
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="vision", lesson="lesson_07_compact_keypoint_regression", run_name=train_cfg.run_name
    )
    logger = get_logger("vision.compact_keypoints", log_file=paths.logs_dir / "train.log")
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "vision", "lesson": "lesson_07_compact_keypoint_regression"},
    )
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.vision.lesson_07_compact_keypoint_regression.train"
        )
