...
from .model import HumanPoseRegressor, ModelConfig, mean_pose_l2_pixels
...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 49 (Vision): synthetic human pose estimation."
    )
    ...
    parser.add_argument("--line-sigma", type=float, default=1.3)
    parser.add_argument("--joint-sigma", type=float, default=1.8)
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig, model_cfg: ModelConfig) -> int:
    ...
    paths = build_run_paths(
        track="vision",
        lesson="lesson_49_synthetic_human_pose_estimation",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("vision.human_pose_estimation", log_file=paths.logs_dir / "train.log")
    ...
    model = HumanPoseRegressor(model_cfg).to(device_info.torch_device)
    ...
    save_checkpoint(
        ...
        extra={"track": "vision", "lesson": "lesson_49_synthetic_human_pose_estimation"},
    )
    return 0
