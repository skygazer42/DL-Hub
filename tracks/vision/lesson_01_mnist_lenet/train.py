...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(description="Lesson 01: Train LeNet on MNIST.")

    parser.add_argument("--dataset", type=str, default="mnist", help="mnist | fake")
    parser.add_argument(
        "--data-dir", type=str, default=".data", help="Where datasets are stored/downloaded."
    )
    ...
    parser.add_argument(
        "--device", type=str, default="auto", help="auto | cpu | cuda | mps | cuda:0"
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="vision", lesson="lesson_01_mnist_lenet", run_name=train_cfg.run_name
    )
    logger = get_logger("vision.mnist_lenet", log_file=paths.logs_dir / "train.log")
    ...
    model = LeNet().to(device_info.torch_device)
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "vision", "lesson": "lesson_01_mnist_lenet"},
    )
