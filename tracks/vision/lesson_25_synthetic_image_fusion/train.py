...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig]:
    parser = argparse.ArgumentParser(description="Lesson 25 (Vision): synthetic image fusion.")
    ...
    parser.add_argument("--blur-kernel-size", type=int, default=7)
...
def _run_epoch(
    *,
    ...
    for batch_idx, (inputs, target) in enumerate(loader):
        ...
        input_batch = {key: value.to(device) for key, value in inputs.items()}
        target = target.to(device)
        fused = model(input_batch)
        loss, parts = fusion_loss(fused, target)
        psnr = compute_psnr(fused.detach(), target.detach())
        ...
        total_consistency_loss += float(parts["consistency_loss"])
        total_psnr += float(psnr.item())
        total_batches += 1

    if total_batches == 0:
        return Stats(loss=0.0, reconstruction_loss=0.0, consistency_loss=0.0, psnr=0.0)
    return Stats(
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig, model_cfg: ModelConfig) -> int:
    ...
    paths = build_run_paths(
        track="vision",
        lesson="lesson_25_synthetic_image_fusion",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("vision.image_fusion", log_file=paths.logs_dir / "train.log")
    ...
    for epoch in range(1, int(train_cfg.epochs) + 1):
        ...
        append_jsonl(
            metrics_path,
            {
                ...
                "eval_consistency_loss": eval_stats.consistency_loss,
    ...
    save_checkpoint(
        ...
        extra={"track": "vision", "lesson": "lesson_25_synthetic_image_fusion"},
    )
    return 0
