...
from .model import ModelConfig, SalientObjectDetectionModel, mask_iou, salient_detection_loss
...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig]:
    parser = argparse.ArgumentParser(description="Lesson 28 (Vision): synthetic salient object detection.")
...
def _run_epoch(
    *,
    ...
    for step, (images, targets) in enumerate(loader):
        ...
        logits = model(images)
        loss, parts = salient_detection_loss(logits, targets)
        iou = mask_iou(logits.detach(), targets.detach())
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig, model_cfg: ModelConfig) -> int:
    ...
    paths = build_run_paths(
        track="vision",
        lesson="lesson_28_synthetic_salient_object_detection",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("vision.salient_object_detection", log_file=paths.logs_dir / "train.log")
    paths.run_dir.mkdir(parents=True, exist_ok=True)
    paths.checkpoints_dir.mkdir(parents=True, exist_ok=True)

    train_loader, val_loader = get_dataloaders(data_cfg)
    model = SalientObjectDetectionModel(model_cfg).to(device_info.torch_device)
    ...
    save_checkpoint(
        ...
        extra={"track": "vision", "lesson": "lesson_28_synthetic_salient_object_detection"},
    )
    return 0
