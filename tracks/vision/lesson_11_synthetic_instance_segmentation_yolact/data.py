...
class DataConfig:
    ...
    batch_size: int = 32
    image_size: int = 64
    stride: int = 8  # detection grid stride
...
class SyntheticRectInstanceSeg(Dataset):
    """Synthetic instance segmentation: one bright rectangle per image.

    Targets mimic an anchor-free per-cell assignment:
    - cls_target: (1, Gh, Gw) one-hot at the rectangle center cell
    - reg_target: (4, Gh, Gw) ltrb distances from cell center (pixels)
    - pos_mask: (1, Gh, Gw) one-hot mask for the positive cell
    - mask: (1, H, W) binary instance mask
    - box: (4,) xyxy
    """
    ...
    def _sample_box(self) -> tuple[int, int, int, int]:
        ...
        w = int(self.rng.integers(int(self.cfg.min_rect), int(self.cfg.max_rect) + 1))
        h = int(self.rng.integers(int(self.cfg.min_rect), int(self.cfg.max_rect) + 1))
    ...
    def __getitem__(self, idx: int):
        ...
        # Center cell for positive assignment.
        cx = 0.5 * (x1 + x2)
        cy = 0.5 * (y1 + y2)
        ...
        ltrb = np.array([cell_x - x1, cell_y - y1, x2 - cell_x, y2 - cell_y], dtype=np.float32)
