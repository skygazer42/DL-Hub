...
class SyntheticFingerCurvatureDataset:
    """Synthetic grayscale hand crops with a normalized finger-curvature target."""
    ...
    def __getitem__(self, idx: int):
        ...
        curvature = float(rng.uniform(0.02, 0.98))
        ...
        finger_sep = 0.085 * size
        finger_width = 0.028 * size
        finger_height = 0.12 * size
        ...
        for offset in (-1.5, -0.5, 0.5, 1.5):
            ...
            tip_shift = curvature * 0.085 * size * np.sign(offset if offset != 0 else 1.0)
            tip = np.exp(
                -((xx - (fx + tip_shift)) ** 2) / (2.0 * (0.85 * finger_width) ** 2)
                - ((yy - (fy - 0.08 * size)) ** 2) / (2.0 * (0.6 * finger_height) ** 2)
            ).astype(np.float32)
            image += 0.13 * base_finger + 0.11 * tip

        thumb_cx = cx - 0.18 * size
        ...
        target = np.array([curvature], dtype=np.float32)
...
__all__ = ["DataConfig", "SyntheticFingerCurvatureDataset", "get_dataloaders"]
