...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 19 (Vision): synthetic monocular depth estimation."
    )
    ...
    parser.add_argument("--near-depth", type=float, default=0.2)
    parser.add_argument("--far-depth", type=float, default=0.9)
    ...
    data_cfg = DataConfig(
        ...
        add_gradient_background=args.add_gradient_background,
...
def _run_epoch(
    *,
    ...
    for batch_idx, (images, targets) in enumerate(loader):
        ...
        depth = targets["depth"].to(device)
        pred = model(images)
        loss = torch.nn.functional.smooth_l1_loss(pred, depth)
        mae = torch.nn.functional.l1_loss(pred, depth)
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig, model_cfg: ModelConfig) -> int:
    ...
    paths = build_run_paths(
        track="vision",
        lesson="lesson_19_synthetic_monocular_depth_estimation",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("vision.monocular_depth", log_file=paths.logs_dir / "train.log")
    ...
    for epoch in range(1, int(train_cfg.epochs) + 1):
        ...
        logger.info(
            "Epoch %d/%d | train loss %.6f | val loss %.6f | val mae %.6f",
            epoch,
            train_cfg.epochs,
            train_stats.loss,
            eval_stats.loss,
    ...
    save_checkpoint(
        ...
        extra={"track": "vision", "lesson": "lesson_19_synthetic_monocular_depth_estimation"},
    )
    return 0
