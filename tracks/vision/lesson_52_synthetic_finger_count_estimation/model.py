...
class FingerCountClassifier(nn.Module):
...
def finger_count_loss(logits: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
    return torch.nn.functional.cross_entropy(logits, labels)


def finger_count_accuracy(logits: torch.Tensor, labels: torch.Tensor) -> float:
    with torch.no_grad():
