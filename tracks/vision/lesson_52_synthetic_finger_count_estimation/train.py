...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 52 (Vision): synthetic finger count estimation."
    )
...
def _run_epoch(
    *,
    ...
    for step, (images, labels) in enumerate(loader):
        ...
        total_acc += finger_count_accuracy(logits.detach(), labels.detach())
        total_batches += 1
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig, model_cfg: ModelConfig) -> int:
    ...
    paths = build_run_paths(
        track="vision",
        lesson="lesson_52_synthetic_finger_count_estimation",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("vision.finger_count_estimation", log_file=paths.logs_dir / "train.log")
    ...
    model = FingerCountClassifier(model_cfg).to(device_info.torch_device)
    ...
    save_checkpoint(
        ...
        extra={"track": "vision", "lesson": "lesson_52_synthetic_finger_count_estimation"},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.vision.lesson_52_synthetic_finger_count_estimation.train"
        )
