...
def gaze_loss(predictions: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
    return torch.nn.functional.mse_loss(predictions, targets)


def gaze_l1(predictions: torch.Tensor, targets: torch.Tensor) -> float:
    with torch.no_grad():
        return float(torch.mean(torch.abs(predictions - targets)).item())


__all__ = ["GazeRegressor", "ModelConfig", "gaze_l1", "gaze_loss"]
