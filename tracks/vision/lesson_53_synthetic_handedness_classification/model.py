...
class HandednessClassifier(nn.Module):
...
def handedness_loss(logits: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
    return torch.nn.functional.cross_entropy(logits, labels)


def handedness_accuracy(logits: torch.Tensor, labels: torch.Tensor) -> float:
    with torch.no_grad():
