...
from .model import FaceAlignmentRegressor, ModelConfig, mean_alignment_l2_pixels
...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig]:
    parser = argparse.ArgumentParser(description="Lesson 39 (Vision): synthetic face alignment.")
    ...
    parser.add_argument("--max-rotation-deg", type=float, default=18.0)
    parser.add_argument("--scale-jitter", type=float, default=0.12)
    parser.add_argument("--translation-jitter", type=float, default=0.08)
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig, model_cfg: ModelConfig) -> int:
    ...
    paths = build_run_paths(
        track="vision",
        lesson="lesson_39_synthetic_face_alignment",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("vision.face_alignment", log_file=paths.logs_dir / "train.log")
    ...
    model = FaceAlignmentRegressor(model_cfg).to(device_info.torch_device)
    ...
    save_checkpoint(
        ...
        extra={"track": "vision", "lesson": "lesson_39_synthetic_face_alignment"},
    )
    return 0
...
def _evaluate_l2(
    *,
    ...
    for step, (images, targets) in enumerate(loader):
        ...
        total_error += mean_alignment_l2_pixels(
            preds.detach().cpu(),
            targets,
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.vision.lesson_39_synthetic_face_alignment.train"
        )
