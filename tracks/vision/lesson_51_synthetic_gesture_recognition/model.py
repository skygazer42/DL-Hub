...
class GestureRecognitionClassifier(nn.Module):
...
def gesture_loss(logits: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
    return torch.nn.functional.cross_entropy(logits, labels)


def gesture_accuracy(logits: torch.Tensor, labels: torch.Tensor) -> float:
    with torch.no_grad():
