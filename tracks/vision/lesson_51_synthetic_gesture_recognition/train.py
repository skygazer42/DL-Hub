...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 51 (Vision): synthetic gesture recognition."
    )
    ...
    parser.add_argument("--line-sigma", type=float, default=1.4)
...
def _run_epoch(
    *,
    ...
    for step, (images, labels) in enumerate(loader):
        ...
        total_acc += gesture_accuracy(logits.detach(), labels.detach())
        total_batches += 1
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig, model_cfg: ModelConfig) -> int:
    ...
    paths = build_run_paths(
        track="vision",
        lesson="lesson_51_synthetic_gesture_recognition",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("vision.gesture_recognition", log_file=paths.logs_dir / "train.log")
    ...
    model = GestureRecognitionClassifier(model_cfg).to(device_info.torch_device)
    ...
    save_checkpoint(
        ...
        extra={"track": "vision", "lesson": "lesson_51_synthetic_gesture_recognition"},
    )
    return 0
