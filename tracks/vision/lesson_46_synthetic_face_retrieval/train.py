...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig]:
    parser = argparse.ArgumentParser(description="Lesson 46 (Vision): synthetic face retrieval.")
...
def _run_epoch(
    *,
    ...
    for step, (images, labels) in enumerate(loader):
        ...
        total_top1 += retrieval_top1_accuracy(embeddings.detach(), labels.detach())
        total_batches += 1

    return total_loss / max(1, total_batches), total_top1 / max(1, total_batches)
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig, model_cfg: ModelConfig) -> int:
    ...
    paths = build_run_paths(
        track="vision",
        lesson="lesson_46_synthetic_face_retrieval",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("vision.face_retrieval", log_file=paths.logs_dir / "train.log")
    ...
    model = FaceRetrievalEmbeddingNet(model_cfg).to(device_info.torch_device)
    ...
    for epoch in range(1, int(train_cfg.epochs) + 1):
        ...
        eval_loss, eval_top1 = _run_epoch(
            model=model,
            loader=val_loader,
            optimizer=None,
            device=device_info.torch_device,
    ...
    save_checkpoint(
        ...
        extra={"track": "vision", "lesson": "lesson_46_synthetic_face_retrieval"},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.vision.lesson_46_synthetic_face_retrieval.train"
        )
