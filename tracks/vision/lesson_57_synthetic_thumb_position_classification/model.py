...
class ThumbPositionClassifier(nn.Module):
...
def thumb_position_loss(logits: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
    return torch.nn.functional.cross_entropy(logits, labels)


def thumb_position_accuracy(logits: torch.Tensor, labels: torch.Tensor) -> float:
    with torch.no_grad():
