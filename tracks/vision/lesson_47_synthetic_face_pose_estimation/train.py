...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 47 (Vision): synthetic face pose estimation."
    )
...
def _run_epoch(
    *,
    ...
    for step, (images, poses) in enumerate(loader):
        ...
        total_mae += pose_mae(pred.detach(), poses.detach())
        total_batches += 1
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig, model_cfg: ModelConfig) -> int:
    ...
    paths = build_run_paths(
        track="vision",
        lesson="lesson_47_synthetic_face_pose_estimation",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("vision.face_pose_estimation", log_file=paths.logs_dir / "train.log")
    ...
    model = FacePoseRegressor(model_cfg).to(device_info.torch_device)
    ...
    save_checkpoint(
        ...
        extra={"track": "vision", "lesson": "lesson_47_synthetic_face_pose_estimation"},
    )
    return 0
