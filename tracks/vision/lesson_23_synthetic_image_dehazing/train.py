...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig]:
    parser = argparse.ArgumentParser(description="Lesson 23 (Vision): synthetic image dehazing.")
    ...
    parser.add_argument("--beta-min", type=float, default=0.8)
    parser.add_argument("--beta-max", type=float, default=1.8)
...
def _run_epoch(
    *,
    ...
    for batch_idx, (hazy, targets) in enumerate(loader):
        ...
        outputs = model(hazy)
        loss, parts = dehazing_loss(outputs, target_batch)
    ...
    if total_batches == 0:
        return Stats(loss=0.0, reconstruction_loss=0.0, transmission_loss=0.0, psnr=0.0)
    return Stats(
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig, model_cfg: ModelConfig) -> int:
    ...
    paths = build_run_paths(
        track="vision",
        lesson="lesson_23_synthetic_image_dehazing",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("vision.image_dehazing", log_file=paths.logs_dir / "train.log")
    ...
    save_checkpoint(
        ...
        extra={"track": "vision", "lesson": "lesson_23_synthetic_image_dehazing"},
    )
    return 0
