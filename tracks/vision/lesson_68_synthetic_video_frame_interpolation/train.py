...
from .model import ModelConfig, build_model, frame_interpolation_loss
...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 68 (Vision): synthetic video frame interpolation."
    )
    ...
    parser.add_argument("--motion-pixels", type=int, default=3)
    ...
    data_cfg = DataConfig(
        ...
        motion_pixels=args.motion_pixels,
...
def _run_epoch(
    *,
    ...
    for batch_idx, (endpoints, target) in enumerate(loader):
        ...
        outputs = model(endpoints)
        loss, parts = frame_interpolation_loss(outputs, target)
        psnr = compute_psnr(outputs["mid"].detach(), target.detach())
    ...
    if total_batches == 0:
        return Stats(loss=0.0, l1_loss=0.0, psnr=0.0)
    return Stats(
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig, model_cfg: ModelConfig) -> int:
    ...
    paths = build_run_paths(
        track="vision",
        lesson="lesson_68_synthetic_video_frame_interpolation",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("vision.video_frame_interpolation", log_file=paths.logs_dir / "train.log")
    ...
    for epoch in range(1, int(train_cfg.epochs) + 1):
        ...
        logger.info(
            "Epoch %d/%d | train loss %.6f | eval loss %.6f | eval psnr %.2f",
            epoch,
            train_cfg.epochs,
            train_stats.loss,
            eval_stats.loss,
    ...
    save_checkpoint(
        ...
        extra={"track": "vision", "lesson": "lesson_68_synthetic_video_frame_interpolation"},
    )
    return 0
