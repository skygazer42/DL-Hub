...
from .model import ModelConfig, build_model, restoration_loss
...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 69 (Vision): synthetic video restoration."
    )
...
def _run_epoch(
    *,
    ...
    for batch_idx, (degraded, clean) in enumerate(loader):
        ...
        loss, parts = restoration_loss(outputs["restored"], clean)
        ...
        total_temporal += float(parts["temporal_loss"])
        total_batches += 1

    if total_batches == 0:
        return Stats(loss=0.0, l1_loss=0.0, temporal_loss=0.0)
    return Stats(
        loss=total_loss / total_batches,
        l1_loss=total_l1 / total_batches,
        temporal_loss=total_temporal / total_batches,
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig, model_cfg: ModelConfig) -> int:
    ...
    paths = build_run_paths(
        track="vision",
        lesson="lesson_69_synthetic_video_restoration",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("vision.video_restoration", log_file=paths.logs_dir / "train.log")
    ...
    for epoch in range(1, int(train_cfg.epochs) + 1):
        ...
        append_jsonl(
            metrics_path,
            {
                ...
                "train_temporal_loss": train_stats.temporal_loss,
                "eval_loss": eval_stats.loss,
                "eval_l1_loss": eval_stats.l1_loss,
                "eval_temporal_loss": eval_stats.temporal_loss,
    ...
    save_checkpoint(
        ...
        extra={"track": "vision", "lesson": "lesson_69_synthetic_video_restoration"},
    )
    return 0
