...
class SignDigitClassifier(nn.Module):
...
def sign_digit_loss(logits: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
    return torch.nn.functional.cross_entropy(logits, labels)


def sign_digit_accuracy(logits: torch.Tensor, labels: torch.Tensor) -> float:
    with torch.no_grad():
