...
def parse_args() -> TrainConfig:
    parser = argparse.ArgumentParser(
        description="Lesson 09 (GNN): metapath2vec-style embeddings on a compact heterogeneous graph."
    )
...
def run_training(cfg: TrainConfig) -> int:
    ...
    paths = build_run_paths(
        track="gnn", lesson="lesson_09_metapath2vec_compact_hetero_embedding", run_name=cfg.run_name
    )
    logger = get_logger("gnn.metapath2vec_compact", log_file=paths.logs_dir / "train.log")
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "gnn", "lesson": "lesson_09_metapath2vec_compact_hetero_embedding"},
    )
