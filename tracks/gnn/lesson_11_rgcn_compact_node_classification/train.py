...
from .data import DataConfig, load_synthetic_rel_graph
...
def parse_args() -> TrainConfig:
    parser = argparse.ArgumentParser(
        description="Lesson 11 (GNN): R-GCN on a synthetic relational graph."
    )
...
def run_training(cfg: TrainConfig) -> int:
    ...
    paths = build_run_paths(
        track="gnn", lesson="lesson_11_rgcn_compact_node_classification", run_name=cfg.run_name
    )
    logger = get_logger("gnn.rgcn_compact", log_file=paths.logs_dir / "train.log")
    ...
    data = load_synthetic_rel_graph(data_cfg)
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "gnn", "lesson": "lesson_11_rgcn_compact_node_classification"},
    )
