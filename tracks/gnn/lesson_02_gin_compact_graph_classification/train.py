...
from .model import GINGraphClassifier, ModelConfig
...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 02 (GNN): synthetic graph classification with GIN."
    )
    ...
    train_cfg = TrainConfig(
        ...
        neighbor_pooling=args.neighbor_pooling,
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="gnn", lesson="lesson_02_gin_compact_graph_classification", run_name=train_cfg.run_name
    )
    logger = get_logger("gnn.compact_gin", log_file=paths.logs_dir / "train.log")
    ...
    model = GINGraphClassifier(
        ModelConfig(
            ...
            num_mlp_layers=train_cfg.num_mlp_layers,
            num_classes=2,
            neighbor_pooling=train_cfg.neighbor_pooling,
            graph_pooling=train_cfg.graph_pooling,
            learn_eps=train_cfg.learn_eps,
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "gnn", "lesson": "lesson_02_gin_compact_graph_classification"},
    )
