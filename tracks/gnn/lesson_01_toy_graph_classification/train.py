...
from .model import GCNGraphClassifier, ModelConfig
...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(description="Lesson 01 (GNN): toy graph classification with GCN.")
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="gnn", lesson="lesson_01_toy_graph_classification", run_name=train_cfg.run_name
    )
    logger = get_logger("gnn.toy_gcn", log_file=paths.logs_dir / "train.log")
    ...
    model = GCNGraphClassifier(
        ModelConfig(in_features=2, hidden_features=train_cfg.hidden_features, num_classes=2)
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "gnn", "lesson": "lesson_01_toy_graph_classification"},
    )
