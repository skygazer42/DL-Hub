...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 01 (GNN): synthetic graph classification with GCN."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="gnn", lesson="lesson_01_compact_graph_classification", run_name=train_cfg.run_name
    )
    logger = get_logger("gnn.compact_gcn", log_file=paths.logs_dir / "train.log")
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "gnn", "lesson": "lesson_01_compact_graph_classification"},
    )
