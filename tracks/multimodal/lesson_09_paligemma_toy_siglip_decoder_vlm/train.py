...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 09 (Multimodal): PaliGemma-lite SigLIP decoder toy VLM."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="multimodal",
        lesson="lesson_09_paligemma_toy_siglip_decoder_vlm",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("multimodal.paligemma_toy", log_file=paths.logs_dir / "train.log")
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.multimodal.lesson_09_paligemma_toy_siglip_decoder_vlm.train"
        )
