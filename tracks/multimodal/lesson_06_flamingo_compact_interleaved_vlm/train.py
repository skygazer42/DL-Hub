...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 06 (Multimodal): Flamingo-lite interleaved compact VLM."
    )

    parser.add_argument("--num-samples", type=int, default=512)
    parser.add_argument("--batch-size", type=int, default=32)
    parser.add_argument("--image-size", type=int, default=16)
    parser.add_argument("--max-text-length", type=int, default=28)
    ...
    *,
    ...
    for idx in range(min(4, int(generated.shape[0]))):
        ...
        rows.append(
            {
                "epoch": int(epoch),
                "task_token": batch["task_token"][idx],
                "attribute_name": batch["attribute_name"][idx],
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="multimodal",
        lesson="lesson_06_flamingo_compact_interleaved_vlm",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("multimodal.flamingo_compact", log_file=paths.logs_dir / "train.log")
    ...
    for epoch in range(1, int(train_cfg.epochs) + 1):
        ...
        logger.info(
            "Epoch %d/%d | train loss %.4f tok %.3f em %.3f | eval loss %.4f tok %.3f em %.3f",
            epoch,
            train_cfg.epochs,
            train_stats.loss,
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.multimodal.lesson_06_flamingo_compact_interleaved_vlm.train"
        )
