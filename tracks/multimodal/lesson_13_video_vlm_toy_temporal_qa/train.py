...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 13 (Multimodal): Video-VLM-lite toy temporal QA."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="multimodal",
        lesson="lesson_13_video_vlm_toy_temporal_qa",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("multimodal.video_vlm_toy", log_file=paths.logs_dir / "train.log")
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.multimodal.lesson_13_video_vlm_toy_temporal_qa.train"
        )
