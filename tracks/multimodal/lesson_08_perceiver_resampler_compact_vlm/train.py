...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 08 (Multimodal): Perceiver-resampler-lite compact VLM."
    )

    parser.add_argument("--num-samples", type=int, default=512)
    parser.add_argument("--batch-size", type=int, default=32)
    parser.add_argument("--image-size", type=int, default=16)
    parser.add_argument("--scene-size", type=int, default=32)
    ...
    parser.add_argument("--num-latents", type=int, default=6)
...
def _yes_no_accuracy(logits: torch.Tensor, labels: torch.Tensor, question_types: list[str]) -> float:
    ...
    selected = [idx for idx, qtype in enumerate(question_types) if qtype == "yes_no"]
    if not selected:
        return 0.0
...
def _write_samples(
    *,
    ...
    generated = model.greedy_generate(
        images=moved["images"],
        question_ids=moved["question_ids"],
        max_new_tokens=3,
    ).cpu()

    rows = []
    for idx in range(min(4, int(generated.shape[0]))):
        prompt_len = int((batch["question_ids"][idx] != vocab.pad_id).to(torch.long).sum().item())
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="multimodal",
        lesson="lesson_08_perceiver_resampler_compact_vlm",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("multimodal.perceiver_compact", log_file=paths.logs_dir / "train.log")
    ...
    model = CompactPerceiverResamplerModel(
        PerceiverModelConfig(
            ...
            num_latents=int(train_cfg.num_latents),
        )
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.multimodal.lesson_08_perceiver_resampler_compact_vlm.train"
        )
