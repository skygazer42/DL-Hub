...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 10 (Multimodal): OWL-ViT-lite open-vocabulary compact detection."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="multimodal",
        lesson="lesson_10_owlvit_compact_open_vocab_detection",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("multimodal.owlvit_compact", log_file=paths.logs_dir / "train.log")
