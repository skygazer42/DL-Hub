...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(description="Lesson 54 (Multimodal): toy handedness VLM reasoning.")
...
def _run_epoch(
    *,
    model: ToyHandednessReasoningModel,
    loader,
    device: torch.device,
    ...
    for step, batch in enumerate(loader):
        ...
        outputs = model(moved)
        loss = handedness_loss(outputs["logits"], moved["target_handedness"])
        if is_train:
            loss.backward()
            optimizer.step()

        batch_size = int(moved["target_handedness"].shape[0])
        total_examples += batch_size
        total_loss += float(loss.item()) * batch_size
        total_accuracy += compute_accuracy(outputs["logits"], moved["target_handedness"]) * batch_size
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="multimodal",
        lesson="lesson_54_handedness_vlm_reasoning",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("multimodal.handedness_vlm_reasoning", log_file=paths.logs_dir / "train.log")
    ...
    save_checkpoint(
        ...
        extra={"track": "multimodal", "lesson": "lesson_54_handedness_vlm_reasoning"},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.multimodal.lesson_54_handedness_vlm_reasoning.train"
        )
