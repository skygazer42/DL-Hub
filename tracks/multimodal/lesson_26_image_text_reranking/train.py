...
def _run_epoch(
    *,
    ...
    for step, batch in enumerate(loader):
        ...
        outputs = model(moved)
        loss = reranking_loss(outputs["scores"], moved["label_index"])
        if is_train:
            loss.backward()
            optimizer.step()

        batch_size = int(moved["label_index"].shape[0])
        total_examples += batch_size
        total_loss += float(loss.item()) * batch_size
        total_acc += reranking_accuracy(outputs["scores"], moved["label_index"]) * batch_size
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    model = ToyImageTextReranker(
        ImageTextRerankerConfig(
            ...
            pad_id=vocab.pad_id,
            num_candidates=int(data_cfg.num_candidates),
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "multimodal", "lesson": "lesson_26_image_text_reranking"},
    )
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.multimodal.lesson_26_image_text_reranking.train"
        )
