...
_ACTION_TYPES: tuple[str, ...] = ("jump", "wave", "sit")
...
def _build_record(
    *,
    ...
    del idx
    action_type = _ACTION_TYPES[int(torch.randint(0, len(_ACTION_TYPES), (1,), generator=generator).item())]
    ...
    query_tokens = ["locate", "action", action_type, "start", "end"]
    query_ids, attention_mask = vocab.encode_query(query_tokens, max_length=int(cfg.max_text_length))

    action_mask = torch.zeros(num_frames, dtype=torch.float32)
    action_mask[seg_start : seg_end + 1] = 1.0
    ...
    return {
        ...
        "segment": torch.tensor([seg_start, seg_end], dtype=torch.long),
        "query_text": " ".join(query_tokens),
        "action_type": action_type,
    }


class SyntheticVideoTextActionDataset(Dataset[dict[str, object]]):
...
def get_dataloaders(cfg: DataConfig) -> tuple[DataLoader[dict[str, object]], DataLoader[dict[str, object]], Vocab]:
    dataset = SyntheticVideoTextActionDataset(cfg)
...
__all__ = [
    "DataConfig",
    "SyntheticVideoTextActionDataset",
    "Vocab",
    "get_dataloaders",
]
