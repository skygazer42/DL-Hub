...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 32 (Multimodal): compact video-text temporal action localization."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="multimodal",
        lesson="lesson_32_video_text_action_localization",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("multimodal.action_localization_compact", log_file=paths.logs_dir / "train.log")
    ...
    for epoch in range(1, int(train_cfg.epochs) + 1):
        ...
        logger.info(
            "Epoch %d/%d | train loss %.4f tiou %.3f r1@0.5 %.3f | eval loss %.4f tiou %.3f r1@0.5 %.3f",
            epoch,
            train_cfg.epochs,
            train_stats.loss,
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.multimodal.lesson_32_video_text_action_localization.train"
        )
