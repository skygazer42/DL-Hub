...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 38 (Multimodal): compact face verification VLM reasoning."
    )
    ...
    parser.add_argument("--identity-pool", type=int, default=24)
...
def _run_epoch(
    *,
    ...
    for step, batch in enumerate(loader):
        ...
        outputs = model(moved)
        loss = verification_loss(outputs["logits"], moved["labels"])
        if is_train:
            loss.backward()
            optimizer.step()
        ...
        total_acc += verification_accuracy(outputs["logits"], moved["labels"]) * batch_size
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="multimodal",
        lesson="lesson_38_face_verification_vlm_reasoning",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("multimodal.face_verification_vlm_reasoning", log_file=paths.logs_dir / "train.log")
    ...
    save_checkpoint(
        ...
        extra={"track": "multimodal", "lesson": "lesson_38_face_verification_vlm_reasoning"},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.multimodal.lesson_38_face_verification_vlm_reasoning.train"
        )
