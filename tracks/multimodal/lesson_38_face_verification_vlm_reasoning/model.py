...
class CompactFaceVerificationModel(nn.Module):
    def __init__(self, cfg: FaceVerificationConfig) -> None:
        ...
        pair_dim = int(cfg.vision_width) * 4 + int(cfg.text_dim)
        self.classifier = nn.Sequential(
            nn.Linear(pair_dim, int(cfg.hidden_dim)),
            nn.ReLU(),
            nn.Linear(int(cfg.hidden_dim), int(cfg.num_classes)),
        )

    def forward(self, batch: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        feat_a = self.face_encoder(batch["image_a"])
        feat_b = self.face_encoder(batch["image_b"])
        feat_diff = torch.abs(feat_a - feat_b)
        feat_mul = feat_a * feat_b
        query_feat = self.query_encoder(batch["query_ids"], batch["attention_mask"])
        fused = torch.cat([feat_a, feat_b, feat_diff, feat_mul, query_feat], dim=-1)
