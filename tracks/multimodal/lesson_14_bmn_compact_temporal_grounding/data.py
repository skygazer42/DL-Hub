...
def _build_segment_targets(num_frames: int, gt_start: int, gt_end: int) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    start_labels = torch.zeros(int(num_frames), dtype=torch.float32)
    end_labels = torch.zeros(int(num_frames), dtype=torch.float32)
    proposal_labels = torch.zeros((int(num_frames), int(num_frames)), dtype=torch.float32)
    proposal_mask = torch.zeros((int(num_frames), int(num_frames)), dtype=torch.float32)

    start_labels[int(gt_start)] = 1.0
    end_labels[int(gt_end)] = 1.0

    for start_idx in range(int(num_frames)):
        for end_idx in range(start_idx, int(num_frames)):
            proposal_mask[start_idx, end_idx] = 1.0
            proposal_labels[start_idx, end_idx] = _temporal_iou(start_idx, end_idx, gt_start, gt_end)

    return start_labels, end_labels, proposal_labels, proposal_mask
...
def _build_record(
    *,
    ...
    start_labels, end_labels, proposal_labels, proposal_mask = _build_segment_targets(
        int(cfg.num_frames), gt_start, gt_end
    )
...
class SyntheticBmnTemporalGroundingDataset(Dataset):
    def __init__(self, cfg: DataConfig, *, vocab: Vocab) -> None:
        self.cfg = cfg
        self.vocab = vocab
        if int(cfg.num_samples) <= 0:
            raise ValueError("num_samples must be positive")
        if int(cfg.num_frames) < 6:
            raise ValueError("num_frames must be >= 6")
...
def get_dataloaders(cfg: DataConfig) -> tuple[DataLoader, DataLoader, Vocab]:
    vocab = _build_vocab()
    dataset = SyntheticBmnTemporalGroundingDataset(cfg, vocab=vocab)
    ...
    def _collate(batch: list[dict[str, object]]) -> dict[str, object]:
        return {
            ...
            "start_labels": torch.stack([sample["start_labels"] for sample in batch], dim=0),
            "end_labels": torch.stack([sample["end_labels"] for sample in batch], dim=0),
            "proposal_labels": torch.stack([sample["proposal_labels"] for sample in batch], dim=0),
            "proposal_mask": torch.stack([sample["proposal_mask"] for sample in batch], dim=0),
...
__all__ = ["DataConfig", "SyntheticBmnTemporalGroundingDataset", "Vocab", "get_dataloaders"]
