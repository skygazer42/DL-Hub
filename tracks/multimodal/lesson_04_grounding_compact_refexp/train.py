...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="multimodal",
        lesson="lesson_04_grounding_compact_refexp",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("multimodal.grounding_compact", log_file=paths.logs_dir / "train.log")
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.multimodal.lesson_04_grounding_compact_refexp.train"
        )
