...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    ...
    parser.add_argument("--trajectory-len", type=int, default=6)
    parser.add_argument("--image-size", type=int, default=20)
    parser.add_argument("--max-question-length", type=int, default=12)
    ...
    parser.add_argument("--traj-width", type=int, default=24)
    ...
    *,
    ...
    for step, batch in enumerate(loader):
        ...
        with torch.set_grad_enabled(is_train):
            outputs = model(moved)
            loss = eqa_loss(outputs["logits"], moved["answer_id"])
            if is_train:
                loss.backward()
                optimizer.step()

        batch_size = int(moved["trajectory"].shape[0])
        acc = eqa_accuracy(outputs["logits"], moved["answer_id"])
...
def _write_samples(
    *,
    ...
    row = {
        ...
        "question_text": list(batch["question_text"][:4]),
        "answer_text": list(batch["answer_text"][:4]),
        "target_step": [int(v) for v in batch["target_step"][:4].tolist()],
        "pred_answer_id": pred[:4],
    }
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="multimodal",
        lesson="lesson_23_embodied_question_answering",
    ...
    model = CompactEmbodiedQaModel(
        EmbodiedQaConfig(
            ...
            trajectory_len=int(data_cfg.trajectory_len),
            num_answers=4,
            hidden_dim=int(train_cfg.hidden_dim),
            vision_width=int(train_cfg.vision_width),
            traj_width=int(train_cfg.traj_width),
