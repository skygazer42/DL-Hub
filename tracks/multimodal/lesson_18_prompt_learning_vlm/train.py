...
def _run_epoch(
    *,
    ...
    for step, batch in enumerate(loader):
        ...
        i2t, t2i = retrieval_accuracy(outputs["logits_per_image"], outputs["logits_per_text"])
        total_examples += batch_size
        total_loss += float(loss.item()) * batch_size
        total_i2t += i2t * batch_size
        total_t2i += t2i * batch_size
...
def _write_samples(
    *,
    ...
    moved = _move_batch(batch, device)
    outputs = model(moved)
    ranked = outputs["logits_per_image"].argmax(dim=-1).cpu().tolist()
    rows = []
    for idx in range(min(4, len(ranked))):
        rows.append(
            {
                "epoch": int(epoch),
                "vocab_size": int(vocab.size),
                "concept_name": batch["concept_name"][idx],
                "predicted_match_index": int(ranked[idx]),
            }
        )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    model = CompactPromptLearningVLM(
        PromptLearningConfig(
            ...
            prompt_length=int(train_cfg.prompt_length),
    ...
    logger.info("Trainable parameters: %d", sum(parameter.numel() for parameter in trainable_params))
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.multimodal.lesson_18_prompt_learning_vlm.train"
        )
