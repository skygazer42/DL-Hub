...
class FaceCaptionGroundingConfig:
    vocab_size: int
    pad_id: int
    num_classes: int = 2
    hidden_dim: int = 64
...
class ToyFaceCaptionGroundingModel(nn.Module):
    def __init__(self, cfg: FaceCaptionGroundingConfig) -> None:
        ...
        self.caption_encoder = MaskedTextEncoder(
            vocab_size=int(cfg.vocab_size),
            pad_id=int(cfg.pad_id),
            text_dim=int(cfg.text_dim),
        )
        self.caption_to_vision = nn.Linear(int(cfg.text_dim), int(cfg.vision_width))
    ...
    def forward(self, batch: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        caption_feat = self.caption_encoder(batch["caption_ids"], batch["caption_mask"])
        caption_vision = self.caption_to_vision(caption_feat)
        feat_diff = torch.abs(image_feat - caption_vision)
        feat_mul = image_feat * caption_vision
        fused = torch.cat([image_feat, caption_feat, feat_diff, feat_mul], dim=-1)
...
def grounding_loss(logits: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
    return F.cross_entropy(logits, labels.to(torch.long))


@torch.no_grad()
def grounding_accuracy(logits: torch.Tensor, labels: torch.Tensor) -> float:
