...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 46 (Multimodal): toy face detection VLM reasoning."
    )
...
def _run_epoch(
    *,
    model: ToyFaceDetectionReasoningModel,
    loader,
    device: torch.device,
    ...
    for step, batch in enumerate(loader):
        ...
        outputs = model(moved)
        loss = face_detection_loss(outputs["pred_box"], moved["target_box"])
        if is_train:
            loss.backward()
            optimizer.step()

        batch_size = int(moved["target_box"].shape[0])
        total_examples += batch_size
        total_loss += float(loss.item()) * batch_size
        total_iou += float(box_iou_xyxy(outputs["pred_box"], moved["target_box"]).mean().item()) * batch_size

    if total_examples == 0:
        return {"loss": 0.0, "mean_iou": 0.0}
    return {"loss": total_loss / total_examples, "mean_iou": total_iou / total_examples}
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="multimodal",
        lesson="lesson_46_face_detection_vlm_reasoning",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("multimodal.face_detection_vlm_reasoning", log_file=paths.logs_dir / "train.log")
    ...
    for epoch in range(1, int(train_cfg.epochs) + 1):
        ...
        logger.info(
            "Epoch %d/%d | train loss %.4f iou %.4f | eval loss %.4f iou %.4f",
            epoch,
            train_cfg.epochs,
            train_stats["loss"],
    ...
    save_checkpoint(
        ...
        extra={"track": "multimodal", "lesson": "lesson_46_face_detection_vlm_reasoning"},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.multimodal.lesson_46_face_detection_vlm_reasoning.train"
        )
