...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 47 (Multimodal): compact face retrieval VLM reasoning."
    )
    ...
    parser.add_argument("--num-identities", type=int, default=5)
...
def _run_epoch(
    *,
    ...
    for step, batch in enumerate(loader):
        ...
        outputs = model(moved)
        loss = face_retrieval_loss(outputs["logits"], moved["target_identity"])
        if is_train:
            loss.backward()
            optimizer.step()

        batch_size = int(moved["target_identity"].shape[0])
        total_examples += batch_size
        total_loss += float(loss.item()) * batch_size
        total_top1 += retrieval_top1_accuracy(outputs["logits"], moved["target_identity"]) * batch_size

    if total_examples == 0:
        return {"loss": 0.0, "top1": 0.0}
    return {"loss": total_loss / total_examples, "top1": total_top1 / total_examples}
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="multimodal",
        lesson="lesson_47_face_retrieval_vlm_reasoning",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("multimodal.face_retrieval_vlm_reasoning", log_file=paths.logs_dir / "train.log")
    ...
    model = CompactFaceRetrievalReasoningModel(
        FaceRetrievalReasoningConfig(
            ...
            num_identities=int(train_cfg.num_identities),
        )
    ...
    for epoch in range(1, int(train_cfg.epochs) + 1):
        ...
        logger.info(
            "Epoch %d/%d | train loss %.4f top1 %.3f | eval loss %.4f top1 %.3f",
            epoch,
            train_cfg.epochs,
            train_stats["loss"],
            train_stats["top1"],
    ...
    save_checkpoint(
        ...
        extra={"track": "multimodal", "lesson": "lesson_47_face_retrieval_vlm_reasoning"},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.multimodal.lesson_47_face_retrieval_vlm_reasoning.train"
        )
