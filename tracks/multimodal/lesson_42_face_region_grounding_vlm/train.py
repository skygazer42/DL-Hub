...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 42 (Multimodal): compact face-region grounding with VLM fusion."
    )
...
def _run_epoch(
    *,
    ...
    for step, batch in enumerate(loader):
        ...
        outputs = model(moved)
        loss = face_region_grounding_loss(outputs["pred_boxes"], moved["target_boxes"])
        if is_train:
            loss.backward()
            optimizer.step()

        batch_size = int(moved["target_boxes"].shape[0])
        total_examples += batch_size
        total_loss += float(loss.item()) * batch_size
        total_iou += float(box_iou_xyxy(outputs["pred_boxes"], moved["target_boxes"]).mean().item()) * batch_size
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="multimodal",
        lesson="lesson_42_face_region_grounding_vlm",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("multimodal.face_region_grounding_vlm", log_file=paths.logs_dir / "train.log")
    ...
    save_checkpoint(
        ...
        extra={"track": "multimodal", "lesson": "lesson_42_face_region_grounding_vlm"},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.multimodal.lesson_42_face_region_grounding_vlm.train"
        )
