...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    ...
    parser.add_argument("--max-query-length", type=int, default=10)
...
def _run_epoch(
    *,
    ...
    for step, batch in enumerate(loader):
        ...
        outputs = model(moved)
        loss = hoi_loss(outputs["logits"], moved["labels"])
        if is_train:
            loss.backward()
            optimizer.step()
        ...
        total_acc += hoi_accuracy(outputs["logits"], moved["labels"]) * batch_size
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "multimodal", "lesson": "lesson_29_human_object_interaction_reasoning"},
    )
