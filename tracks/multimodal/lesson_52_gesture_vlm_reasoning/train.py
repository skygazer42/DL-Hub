...
from .model import GestureReasoningConfig, CompactGestureReasoningModel, compute_accuracy, gesture_loss
...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(description="Lesson 52 (Multimodal): compact gesture VLM reasoning.")
...
def _run_epoch(
    *,
    ...
    for step, batch in enumerate(loader):
        ...
        outputs = model(moved)
        loss = gesture_loss(outputs["logits"], moved["target_gesture"])
        if is_train:
            loss.backward()
            optimizer.step()

        batch_size = int(moved["target_gesture"].shape[0])
        total_examples += batch_size
        total_loss += float(loss.item()) * batch_size
        total_accuracy += compute_accuracy(outputs["logits"], moved["target_gesture"]) * batch_size
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="multimodal",
        lesson="lesson_52_gesture_vlm_reasoning",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("multimodal.gesture_vlm_reasoning", log_file=paths.logs_dir / "train.log")
    ...
    save_checkpoint(
        ...
        extra={"track": "multimodal", "lesson": "lesson_52_gesture_vlm_reasoning"},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.multimodal.lesson_52_gesture_vlm_reasoning.train"
        )
