...
from .model import FingerCountReasoningConfig, CompactFingerCountReasoningModel, compute_accuracy, finger_count_loss
...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(description="Lesson 53 (Multimodal): compact finger-count VLM reasoning.")
...
def _run_epoch(
    *,
    ...
    for step, batch in enumerate(loader):
        ...
        outputs = model(moved)
        loss = finger_count_loss(outputs["logits"], moved["target_finger_count"])
        if is_train:
            loss.backward()
            optimizer.step()

        batch_size = int(moved["target_finger_count"].shape[0])
        total_examples += batch_size
        total_loss += float(loss.item()) * batch_size
        total_accuracy += compute_accuracy(outputs["logits"], moved["target_finger_count"]) * batch_size
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="multimodal",
        lesson="lesson_53_finger_count_vlm_reasoning",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("multimodal.finger_count_vlm_reasoning", log_file=paths.logs_dir / "train.log")
    ...
    save_checkpoint(
        ...
        extra={"track": "multimodal", "lesson": "lesson_53_finger_count_vlm_reasoning"},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.multimodal.lesson_53_finger_count_vlm_reasoning.train"
        )
