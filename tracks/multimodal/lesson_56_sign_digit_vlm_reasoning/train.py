...
from .model import SignDigitReasoningConfig, CompactSignDigitReasoningModel, compute_accuracy, sign_digit_loss
...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(description="Lesson 56 (Multimodal): compact sign-digit VLM reasoning.")
...
def _run_epoch(
    *,
    ...
    for step, batch in enumerate(loader):
        ...
        outputs = model(moved)
        loss = sign_digit_loss(outputs["logits"], moved["target_sign_digit"])
        if is_train:
            loss.backward()
            optimizer.step()

        batch_size = int(moved["target_sign_digit"].shape[0])
        total_examples += batch_size
        total_loss += float(loss.item()) * batch_size
        total_accuracy += compute_accuracy(outputs["logits"], moved["target_sign_digit"]) * batch_size
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="multimodal",
        lesson="lesson_56_sign_digit_vlm_reasoning",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("multimodal.sign_digit_vlm_reasoning", log_file=paths.logs_dir / "train.log")
    ...
    save_checkpoint(
        ...
        extra={"track": "multimodal", "lesson": "lesson_56_sign_digit_vlm_reasoning"},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.multimodal.lesson_56_sign_digit_vlm_reasoning.train"
        )
