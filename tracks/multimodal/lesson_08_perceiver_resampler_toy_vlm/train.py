...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 08 (Multimodal): Perceiver-resampler-lite toy VLM."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="multimodal",
        lesson="lesson_08_perceiver_resampler_toy_vlm",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("multimodal.perceiver_toy", log_file=paths.logs_dir / "train.log")
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.multimodal.lesson_08_perceiver_resampler_toy_vlm.train"
        )
