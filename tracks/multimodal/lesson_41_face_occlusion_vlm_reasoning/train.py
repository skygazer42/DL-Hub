...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    ...
    parser.add_argument("--heavy-threshold", type=float, default=0.22)
...
def _run_epoch(
    *,
    ...
    for step, batch in enumerate(loader):
        ...
        outputs = model(moved)
        loss = occlusion_loss(outputs["logits"], moved["labels"])
        if is_train:
            loss.backward()
            optimizer.step()
        ...
        total_acc += occlusion_accuracy(outputs["logits"], moved["labels"]) * batch_size
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="multimodal",
        lesson="lesson_41_face_occlusion_vlm_reasoning",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("multimodal.face_occlusion_vlm_reasoning", log_file=paths.logs_dir / "train.log")
    ...
    save_checkpoint(
        ...
        extra={"track": "multimodal", "lesson": "lesson_41_face_occlusion_vlm_reasoning"},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.multimodal.lesson_41_face_occlusion_vlm_reasoning.train"
        )
