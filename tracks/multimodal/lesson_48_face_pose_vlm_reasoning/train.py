...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 48 (Multimodal): compact face pose VLM reasoning."
    )
    parser.add_argument("--num-samples", type=int, default=512)
    parser.add_argument("--batch-size", type=int, default=16)
    parser.add_argument("--image-size", type=int, default=48)
    parser.add_argument("--max-text-length", type=int, default=14)
...
def _run_epoch(
    *,
    ...
    for step, batch in enumerate(loader):
        ...
        outputs = model(moved)
        loss = face_pose_loss(outputs["pred_pose"], moved["target_pose"])
        if is_train:
            loss.backward()
            optimizer.step()

        batch_size = int(moved["target_pose"].shape[0])
        total_examples += batch_size
        total_loss += float(loss.item()) * batch_size
        total_mae += float(pose_mae(outputs["pred_pose"], moved["target_pose"]).mean().item()) * batch_size

    if total_examples == 0:
        return {"loss": 0.0, "mean_mae": 0.0}
    return {"loss": total_loss / total_examples, "mean_mae": total_mae / total_examples}
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="multimodal",
        lesson="lesson_48_face_pose_vlm_reasoning",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("multimodal.face_pose_vlm_reasoning", log_file=paths.logs_dir / "train.log")
    ...
    for epoch in range(1, int(train_cfg.epochs) + 1):
        ...
        append_jsonl(
            metrics_path,
            {
                ...
                "train_mean_mae": train_stats["mean_mae"],
                "eval_loss": eval_stats["loss"],
                "eval_mean_mae": eval_stats["mean_mae"],
    ...
    save_checkpoint(
        ...
        extra={"track": "multimodal", "lesson": "lesson_48_face_pose_vlm_reasoning"},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.multimodal.lesson_48_face_pose_vlm_reasoning.train"
        )
