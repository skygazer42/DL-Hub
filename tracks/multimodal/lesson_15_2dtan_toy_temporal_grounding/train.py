...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 15 (Multimodal): 2D-TAN-lite toy temporal grounding."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="multimodal",
        lesson="lesson_15_2dtan_toy_temporal_grounding",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("multimodal.2dtan_toy", log_file=paths.logs_dir / "train.log")
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.multimodal.lesson_15_2dtan_toy_temporal_grounding.train"
        )
