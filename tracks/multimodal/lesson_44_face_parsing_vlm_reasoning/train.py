...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 44 (Multimodal): toy query-conditioned face parsing with VLM fusion."
    )
...
def _run_epoch(
    *,
    model: ToyFaceParsingReasoningModel,
    loader,
    device: torch.device,
    ...
    for step, batch in enumerate(loader):
        ...
        outputs = model(moved)
        loss = face_parsing_loss(outputs["mask_logits"], moved["target_mask"])
        if is_train:
            loss.backward()
            optimizer.step()

        batch_size = int(moved["target_mask"].shape[0])
        total_examples += batch_size
        total_loss += float(loss.item()) * batch_size
        total_iou += float(mask_iou(outputs["pred_mask"], moved["target_mask"]).mean().item()) * batch_size
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="multimodal",
        lesson="lesson_44_face_parsing_vlm_reasoning",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("multimodal.face_parsing_vlm_reasoning", log_file=paths.logs_dir / "train.log")
    ...
    save_checkpoint(
        ...
        extra={"track": "multimodal", "lesson": "lesson_44_face_parsing_vlm_reasoning"},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.multimodal.lesson_44_face_parsing_vlm_reasoning.train"
        )
