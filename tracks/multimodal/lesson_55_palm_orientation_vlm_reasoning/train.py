...
def _run_epoch(
    *,
    ...
    for step, batch in enumerate(loader):
        ...
        outputs = model(moved)
        loss = palm_orientation_loss(outputs["prediction"], moved["target_palm_orientation"])
        if is_train:
            loss.backward()
            optimizer.step()

        batch_size = int(moved["target_palm_orientation"].shape[0])
        total_examples += batch_size
        total_loss += float(loss.item()) * batch_size
        total_mae += compute_mae(outputs["prediction"], moved["target_palm_orientation"]) * batch_size
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="multimodal",
        lesson="lesson_55_palm_orientation_vlm_reasoning",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("multimodal.palm_orientation_vlm_reasoning", log_file=paths.logs_dir / "train.log")
    ...
    save_checkpoint(
        ...
        extra={"track": "multimodal", "lesson": "lesson_55_palm_orientation_vlm_reasoning"},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.multimodal.lesson_55_palm_orientation_vlm_reasoning.train"
        )
