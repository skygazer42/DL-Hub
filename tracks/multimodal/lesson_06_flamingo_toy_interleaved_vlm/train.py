...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 06 (Multimodal): Flamingo-lite interleaved toy VLM."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="multimodal",
        lesson="lesson_06_flamingo_toy_interleaved_vlm",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("multimodal.flamingo_toy", log_file=paths.logs_dir / "train.log")
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.multimodal.lesson_06_flamingo_toy_interleaved_vlm.train"
        )
