...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 16 (Multimodal): multi-scale 2D-TAN-lite compact temporal grounding."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="multimodal",
        lesson="lesson_16_multiscale_2dtan_compact_temporal_grounding",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("multimodal.multiscale_2dtan_compact", log_file=paths.logs_dir / "train.log")
