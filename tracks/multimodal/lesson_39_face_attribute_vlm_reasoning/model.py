...
class ToyFaceAttributeReasoner(nn.Module):
    ...
    def forward(self, batch: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        fused = torch.cat([image_feat, query_feat], dim=-1)
        logits = self.classifier(fused)
        probabilities = torch.softmax(logits, dim=-1)
        return {"logits": logits, "probabilities": probabilities}


def attribute_loss(logits: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
    return F.cross_entropy(logits, labels.to(torch.long))


@torch.no_grad()
def attribute_accuracy(logits: torch.Tensor, labels: torch.Tensor) -> float:
