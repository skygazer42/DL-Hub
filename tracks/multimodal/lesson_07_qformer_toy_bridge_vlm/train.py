...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 07 (Multimodal): Q-Former-lite bridge toy VLM."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="multimodal",
        lesson="lesson_07_qformer_toy_bridge_vlm",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("multimodal.qformer_toy", log_file=paths.logs_dir / "train.log")
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.multimodal.lesson_07_qformer_toy_bridge_vlm.train"
        )
