...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 12 (Multimodal): key-value OCR-lite compact document VLM."
    )
...
def _run_epoch(
    *,
    ...
    for step, batch in enumerate(loader):
        ...
        total_present += present_accuracy(
            outputs["logits"], batch["labels"], none_id=int(none_id)
        ) * batch_size
    ...
    return Stats(
        ...
        present_acc=total_present / total_examples,
    )
...
def _write_samples(
    *,
    ...
    generated = model.greedy_generate(
        image=moved["image"],
        prompt_ids=moved["prompt_ids"],
        max_new_tokens=3,
    ).cpu()

    rows = []
    for idx in range(min(4, int(generated.shape[0]))):
        prompt_len = int((batch["prompt_ids"][idx] != vocab.pad_id).to(torch.long).sum().item())
        answer_tokens = vocab.decode_ids(generated[idx, prompt_len:].tolist())
        pred_text = " ".join(answer_tokens)
        rows.append(
            {
                "epoch": int(epoch),
                ...
                "target_present": int(round(float(batch["present"][idx].item()))),
                "pred_present": int(pred_text != "none"),
            }
        )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="multimodal",
        lesson="lesson_12_key_value_ocr_compact_doc_vlm",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("multimodal.key_value_ocr_compact", log_file=paths.logs_dir / "train.log")
    ...
    for epoch in range(1, int(train_cfg.epochs) + 1):
        ...
        logger.info(
            "Epoch %d/%d | train loss %.4f tok %.3f em %.3f present %.3f | eval loss %.4f tok %.3f em %.3f present %.3f",
            epoch,
            train_cfg.epochs,
            train_stats.loss,
            train_stats.answer_token_acc,
            train_stats.answer_exact_match,
            train_stats.present_acc,
            eval_stats.loss,
            eval_stats.answer_token_acc,
            eval_stats.answer_exact_match,
            eval_stats.present_acc,
        )
        append_jsonl(
            metrics_path,
            {
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.multimodal.lesson_12_key_value_ocr_compact_doc_vlm.train"
        )
