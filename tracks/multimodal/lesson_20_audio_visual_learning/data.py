...
_EVENTS: tuple[EventSpec, ...] = (
    EventSpec("red_square_siren", "red", "square", "left", "rising", 0.18, 0.20),
    EventSpec("green_circle_hum", "green", "circle", "right", "falling", 0.20, 0.75),
    EventSpec("blue_cross_click", "blue", "cross", "up", "staccato", 0.18, 0.45),
    EventSpec("yellow_square_spin", "yellow", "square", "around", "ripple", 0.22, 0.58),
    EventSpec("blue_circle_beacon", "blue", "circle", "left", "rising", 0.16, 0.32),
    EventSpec("red_cross_alarm", "red", "cross", "right", "falling", 0.21, 0.68),
)
...
def _build_samples(cfg: DataConfig) -> list[tuple[int, int]]:
    _validate_config(cfg)
    base = [(event_id, variant_id) for variant_id in range(4) for event_id in range(num_events())]
...
def _render_video(spec: EventSpec, cfg: DataConfig, variant_id: int) -> torch.Tensor:
    frames = []
    offset = int(variant_id) % int(cfg.num_frames)
    size_scale = float(spec.size_scale) + 0.01 * float(variant_id - 1)
...
def _render_audio(spec: EventSpec, cfg: DataConfig, variant_id: int) -> torch.Tensor:
    ...
    if spec.audio_pattern == "rising":
        ...
        gate = ((steps + int(variant_id)) % 3 != 0).to(torch.float32)
    ...
    width = 1.4 + 0.1 * float(variant_id)
    energy = torch.exp(-0.5 * ((mel - center) / width).pow(2))
    spectrogram = 0.02 + 0.9 * energy * gate
...
class SyntheticAudioVisualLearningDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> dict[str, object]:
        event_id, variant_id = self.samples[int(idx)]
        spec = _EVENTS[int(event_id)]
        motion_id = _MOTIONS.index(spec.motion_name)
        return {
            "video": _render_video(spec, self.cfg, variant_id),
            "audio_spectrogram": _render_audio(spec, self.cfg, variant_id),
            ...
            "audio_pattern": spec.audio_pattern,
        }
...
def get_dataloaders(cfg: DataConfig) -> tuple[DataLoader, DataLoader]:
    ...
    def _collate(batch: list[dict[str, object]]) -> dict[str, object]:
        return {
            ...
            "event_id": torch.stack([sample["event_id"] for sample in batch], dim=0),
            ...
            "audio_pattern": [str(sample["audio_pattern"]) for sample in batch],
        }
