...
class FaceLandmarkReasoningConfig:
    vocab_size: int
    pad_id: int
    hidden_dim: int = 64
    text_dim: int = 32
...
class CompactFaceLandmarkReasoningModel(nn.Module):
    ...
    def forward(self, batch: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        pred_point = torch.sigmoid(self.point_head(torch.cat([image_feat, query_feat], dim=-1)))
        return {"pred_point": pred_point}


def face_landmark_distance(pred_point: torch.Tensor, target_point: torch.Tensor) -> torch.Tensor:
    return torch.linalg.norm(pred_point - target_point.to(torch.float32), dim=-1)


def face_landmark_loss(pred_point: torch.Tensor, target_point: torch.Tensor) -> torch.Tensor:
    reg = F.smooth_l1_loss(pred_point, target_point.to(torch.float32))
    point_l2 = face_landmark_distance(pred_point, target_point).mean()
