...
def _make_question_answer(
    ...
    *,
    ...
    if qtype == "color":
        return qtype, ["what", "color", "is", "the", "object"], [color_name]
    if qtype == "shape":
...
class SyntheticQFormerDataset(Dataset):
    ...
    def _build_record(
        ...
        color_name, shape_name, size_name, location = concept
        question_type, question_tokens, answer_tokens = _make_question_answer(concept, idx=idx)
        question_ids = self.vocab.encode_question(question_tokens, max_length=int(self.cfg.max_text_length))
...
def get_dataloaders(cfg: DataConfig) -> tuple[DataLoader, DataLoader, Vocab]:
    vocab = _build_vocab()
    dataset = SyntheticQFormerDataset(cfg, vocab=vocab)
...
__all__ = ["DataConfig", "SyntheticQFormerDataset", "Vocab", "get_dataloaders"]
