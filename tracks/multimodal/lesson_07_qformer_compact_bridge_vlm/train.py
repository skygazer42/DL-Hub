...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 07 (Multimodal): Q-Former-lite bridge compact VLM."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="multimodal",
        lesson="lesson_07_qformer_compact_bridge_vlm",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("multimodal.qformer_compact", log_file=paths.logs_dir / "train.log")
    ...
    model = CompactQFormerModel(
        QFormerModelConfig(
            ...
            num_query_tokens=int(train_cfg.num_query_tokens),
        )
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.multimodal.lesson_07_qformer_compact_bridge_vlm.train"
        )
