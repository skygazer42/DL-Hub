...
def _run_epoch(
    *,
    ...
    for step, batch in enumerate(loader):
        ...
        outputs = model(moved)
        loss = finger_spread_loss(outputs["prediction"], moved["target_finger_spread"])
        if is_train:
            loss.backward()
            optimizer.step()

        batch_size = int(moved["target_finger_spread"].shape[0])
        total_examples += batch_size
        total_loss += float(loss.item()) * batch_size
        total_mae += compute_mae(outputs["prediction"], moved["target_finger_spread"]) * batch_size
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="multimodal",
        lesson="lesson_57_finger_spread_vlm_reasoning",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("multimodal.finger_spread_vlm_reasoning", log_file=paths.logs_dir / "train.log")
    ...
    save_checkpoint(
        ...
        extra={"track": "multimodal", "lesson": "lesson_57_finger_spread_vlm_reasoning"},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.multimodal.lesson_57_finger_spread_vlm_reasoning.train"
        )
