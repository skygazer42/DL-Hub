...
def _run_epoch(
    *,
    ...
    for step, batch in enumerate(loader):
        ...
        outputs = model(moved)
        loss = face_alignment_loss(outputs["pred_points"], moved["target_points"])
        if is_train:
            loss.backward()
            optimizer.step()

        batch_size = int(moved["target_points"].shape[0])
        total_examples += batch_size
        total_loss += float(loss.item()) * batch_size
        total_l2 += float(mean_alignment_l2(outputs["pred_points"], moved["target_points"]).mean().item()) * batch_size
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="multimodal",
        lesson="lesson_45_face_alignment_vlm_reasoning",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("multimodal.face_alignment_vlm_reasoning", log_file=paths.logs_dir / "train.log")
    ...
    model = CompactFaceAlignmentReasoningModel(
        FaceAlignmentReasoningConfig(
            ...
            num_points=int(train_cfg.num_points),
        )
    ...
    save_checkpoint(
        ...
        extra={"track": "multimodal", "lesson": "lesson_45_face_alignment_vlm_reasoning"},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.multimodal.lesson_45_face_alignment_vlm_reasoning.train"
        )
