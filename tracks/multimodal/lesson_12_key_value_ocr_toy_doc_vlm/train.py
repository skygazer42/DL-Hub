...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 12 (Multimodal): key-value OCR-lite toy document VLM."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="multimodal",
        lesson="lesson_12_key_value_ocr_toy_doc_vlm",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("multimodal.key_value_ocr_toy", log_file=paths.logs_dir / "train.log")
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.multimodal.lesson_12_key_value_ocr_toy_doc_vlm.train"
        )
