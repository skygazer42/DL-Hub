...
- render a toy document image with several `key:value` rows
...
```bash
python -m tracks.multimodal.lesson_12_key_value_ocr_toy_doc_vlm.train --epochs 1 --max-train-batches 2 --max-eval-batches 1 --device cpu
```
