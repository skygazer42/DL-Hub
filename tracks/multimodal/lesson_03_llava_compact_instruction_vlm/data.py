...
class Vocab:
    ...
    def encode_instruction(self, tokens: list[str], *, max_length: int) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    def encode_example(
        self, instruction_tokens: list[str], answer_tokens: list[str], *, max_length: int
    ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        prompt_ids = [self.bos_id, *[int(self.token_to_id[token]) for token in instruction_tokens], self.sep_id]
        ...
        instruction_ids, instruction_mask = self.encode_instruction(
            instruction_tokens, max_length=int(max_length)
        )
        return (
            instruction_ids,
            instruction_mask,
...
def _make_question_answer(
    ...
    *,
    ...
    if qtype == "color":
        return qtype, ["what", "color", "is", "the", "shape"], [color_name]
    if qtype == "shape":
        return qtype, ["what", "shape", "is", "the", "object"], [shape_name]
    if qtype == "size":
        return qtype, ["what", "size", "is", "the", "object"], [size_name]
    if qtype == "location":
        return qtype, ["where", "is", "the", "object"], list(location)

    yes_no_mode = (int(idx) // len(_QUESTION_TYPES)) % 4
    truth = ((int(idx) // (len(_QUESTION_TYPES) * 4)) % 2) == 0
    if yes_no_mode == 0:
        target = color_name if truth else next(name for name in _COLORS if name != color_name)
        question = ["is", "the", "object", target]
    elif yes_no_mode == 1:
        target = shape_name if truth else next(name for name in _SHAPES if name != shape_name)
        question = ["is", "the", "object", target]
    elif yes_no_mode == 2:
        target = size_name if truth else next(name for name in _SIZES if name != size_name)
        question = ["is", "the", "object", target]
    else:
        target = location if truth else next(name for name in _LOCATIONS if name != location)
        question = ["is", "the", "object", "at", target[0], target[1]]
    return "yes_no", question, ["yes" if truth else "no"]


class SyntheticLLaVADataset(Dataset):
    ...
    def _build_record(
        ...
        color_name, shape_name, size_name, location = concept
        question_type, instruction_tokens, answer_tokens = _make_question_answer(concept, idx=idx)
        instruction_ids, instruction_mask, input_ids, labels, attention_mask = self.vocab.encode_example(
            instruction_tokens,
            answer_tokens,
...
def get_dataloaders(cfg: DataConfig) -> tuple[DataLoader, DataLoader, Vocab]:
    vocab = _build_vocab()
    dataset = SyntheticLLaVADataset(cfg, vocab=vocab)
...
__all__ = ["DataConfig", "SyntheticLLaVADataset", "Vocab", "get_dataloaders"]
