...
def _run_epoch(
    *,
    ...
    for step, batch in enumerate(loader):
        ...
        outputs = model(moved)
        loss = recognition_loss(outputs["logits"], moved["label_ids"])
        if is_train:
            loss.backward()
            optimizer.step()

        batch_size = int(moved["label_ids"].shape[0])
        total_examples += batch_size
        total_loss += float(loss.item()) * batch_size
        total_acc += recognition_accuracy(outputs["logits"], moved["label_ids"]) * batch_size
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "multimodal", "lesson": "lesson_27_scene_text_vlm_recognition"},
    )
