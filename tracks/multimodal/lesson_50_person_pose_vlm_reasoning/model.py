...
class PersonPoseReasoningConfig:
    vocab_size: int
    pad_id: int
    hidden_dim: int = 72
    text_dim: int = 32
...
class CompactPersonPoseReasoningModel(nn.Module):
    def __init__(self, cfg: PersonPoseReasoningConfig) -> None:
        ...
        self.pose_head = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(int(cfg.hidden_dim), 6),
        )

    def forward(self, batch: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        image_feat = self.person_encoder(batch["image"])
...
def person_pose_loss(pred_pose: torch.Tensor, target_pose: torch.Tensor) -> torch.Tensor:
