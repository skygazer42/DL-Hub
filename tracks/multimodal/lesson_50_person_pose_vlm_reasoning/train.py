...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 50 (Multimodal): compact person pose VLM reasoning."
    )
...
def _run_epoch(
    *,
    ...
    for step, batch in enumerate(loader):
        ...
        outputs = model(moved)
        loss = person_pose_loss(outputs["pred_pose"], moved["target_pose"])
        if is_train:
            loss.backward()
            optimizer.step()
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="multimodal",
        lesson="lesson_50_person_pose_vlm_reasoning",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("multimodal.person_pose_vlm_reasoning", log_file=paths.logs_dir / "train.log")
    ...
    save_checkpoint(
        ...
        extra={"track": "multimodal", "lesson": "lesson_50_person_pose_vlm_reasoning"},
    )
    return 0
