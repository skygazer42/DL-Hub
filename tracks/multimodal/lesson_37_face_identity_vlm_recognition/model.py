...
class CompactFaceIdentityVLM(nn.Module):
    def __init__(self, cfg: FaceIdentityConfig) -> None:
        ...
        self.identity_encoder = MaskedTextEncoder(
    ...
    def forward(self, batch: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        identity_features = self.identity_encoder(batch["identity_ids"], batch["identity_mask"])
        fused = torch.cat([image_features, identity_features], dim=-1)
...
def face_identity_loss(logits: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
    return F.cross_entropy(logits, labels.to(torch.long))


@torch.no_grad()
def face_identity_accuracy(logits: torch.Tensor, labels: torch.Tensor) -> float:
