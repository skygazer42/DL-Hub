...
def _build_map_targets(num_frames: int, gt_start: int, gt_end: int) -> tuple[torch.Tensor, torch.Tensor]:
    map_labels = torch.zeros((int(num_frames), int(num_frames)), dtype=torch.float32)
    map_mask = torch.zeros((int(num_frames), int(num_frames)), dtype=torch.float32)
    for start_idx in range(int(num_frames)):
        for end_idx in range(start_idx, int(num_frames)):
            map_mask[start_idx, end_idx] = 1.0
            map_labels[start_idx, end_idx] = _temporal_iou(start_idx, end_idx, gt_start, gt_end)
...
def _build_record(
    *,
    ...
    map_labels, map_mask = _build_map_targets(int(cfg.num_frames), gt_start, gt_end)
...
class SyntheticTwoDtanTemporalGroundingDataset(Dataset):
...
def get_dataloaders(cfg: DataConfig) -> tuple[DataLoader, DataLoader, Vocab]:
    vocab = _build_vocab()
    dataset = SyntheticTwoDtanTemporalGroundingDataset(cfg, vocab=vocab)
    ...
    def _collate(batch: list[dict[str, object]]) -> dict[str, object]:
        return {
            ...
            "map_labels": torch.stack([sample["map_labels"] for sample in batch], dim=0),
            "map_mask": torch.stack([sample["map_mask"] for sample in batch], dim=0),
...
__all__ = ["DataConfig", "SyntheticTwoDtanTemporalGroundingDataset", "Vocab", "get_dataloaders"]
