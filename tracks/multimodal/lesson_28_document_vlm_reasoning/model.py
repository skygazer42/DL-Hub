...
class DocumentVlmReasoningConfig:
    vocab_size: int
    pad_id: int
    num_classes: int = 2
    hidden_dim: int = 64
...
class ToyDocumentVlmReasoningModel(nn.Module):
    def __init__(self, cfg: DocumentVlmReasoningConfig) -> None:
        ...
        self.doc_encoder = MaskedTextEncoder(
    ...
    def forward(self, batch: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        doc_features = self.doc_encoder(batch["doc_input_ids"], batch["doc_attention_mask"])
        query_features = self.query_encoder(batch["query_input_ids"], batch["query_attention_mask"])
        fused = torch.cat([image_features, doc_features, query_features], dim=-1)
...
@torch.no_grad()
def reasoning_accuracy(logits: torch.Tensor, labels: torch.Tensor) -> float:
