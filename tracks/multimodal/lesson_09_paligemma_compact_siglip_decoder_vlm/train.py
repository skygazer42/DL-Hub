...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 09 (Multimodal): PaliGemma-lite SigLIP decoder compact VLM."
    )
...
def _yes_no_accuracy(logits: torch.Tensor, labels: torch.Tensor, task_types: list[str]) -> float:
    pred = logits.argmax(dim=-1)
    mask = labels != -100
    exact = ((pred == labels) | (~mask)).all(dim=1).to(torch.float32)
    selected = [idx for idx, task_type in enumerate(task_types) if task_type == "yes_no"]
    if not selected:
        return 0.0
    ...
    *,
    ...
    for step, batch in enumerate(loader):
        ...
        total_yes_no += _yes_no_accuracy(
            outputs["logits"], batch["labels"], list(batch["task_type"])
        ) * batch_size
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="multimodal",
        lesson="lesson_09_paligemma_compact_siglip_decoder_vlm",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("multimodal.paligemma_compact", log_file=paths.logs_dir / "train.log")
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.multimodal.lesson_09_paligemma_compact_siglip_decoder_vlm.train"
        )
