...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig, DiffusionSchedule]:
    parser = argparse.ArgumentParser(
        description="Lesson 32 (Generative): compact diffusion layout-preserving editing from layout and edit map."
    )
...
def _evaluate(
    model: CompactLayoutPreservingDiffusionModel,
    ...
    *,
    ...
    total_seen = 0
    with torch.no_grad():
        for batch_idx, (layout, edit, target) in enumerate(loader):
...
def run_training(
    ...
    paths = build_run_paths(
        track="generative",
        lesson="lesson_32_compact_diffusion_layout_preserving_editing",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("generative.compact_diffusion_layout_preserving_editing", log_file=paths.logs_dir / "train.log")
    ...
    model = CompactLayoutPreservingDiffusionModel(model_cfg).to(device_info.torch_device)
    ...
    for epoch in range(1, int(train_cfg.epochs) + 1):
        ...
        for batch_idx, (layout, edit, target) in enumerate(train_loader):
    ...
    sample_layout, sample_edit, sample_target = next(iter(val_loader))
    ...
    torch.save(
        ...
        paths.run_dir / "layout_editing_triplets.pt",
    )
    ...
    save_checkpoint(
        ...
        extra={"track": "generative", "lesson": "lesson_32_compact_diffusion_layout_preserving_editing"},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.generative.lesson_32_compact_diffusion_layout_preserving_editing.train"
        )
