...
from .model import ModelConfig, CompactLayoutToImageModel
...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 12 (Generative): compact layout-to-image with a simple encoder-decoder model."
    )
...
def _evaluate(
    model: CompactLayoutToImageModel,
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig, model_cfg: ModelConfig) -> int:
    ...
    paths = build_run_paths(
        track="generative",
        lesson="lesson_12_compact_layout_to_image",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("generative.compact_layout_to_image", log_file=paths.logs_dir / "train.log")
    ...
    model = CompactLayoutToImageModel(model_cfg).to(device_info.torch_device)
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "generative", "lesson": "lesson_12_compact_layout_to_image"},
    )
