...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig, DiffusionSchedule]:
    parser = argparse.ArgumentParser(
        description="Lesson 28 (Generative): toy diffusion subject-driven generation from subject and guidance."
    )
...
def _evaluate(
    model: ToySubjectDrivenDiffusionModel,
...
def run_training(
    ...
    paths = build_run_paths(
        track="generative",
        lesson="lesson_28_toy_diffusion_subject_driven_generation",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("generative.toy_diffusion_subject_driven_generation", log_file=paths.logs_dir / "train.log")
    ...
    model = ToySubjectDrivenDiffusionModel(model_cfg).to(device_info.torch_device)
    ...
    save_checkpoint(
        ...
        extra={"track": "generative", "lesson": "lesson_28_toy_diffusion_subject_driven_generation"},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.generative.lesson_28_toy_diffusion_subject_driven_generation.train"
        )
