...
from .model import DiffusionSchedule, ModelConfig, ToySuperResolutionDiffusionModel, q_sample
...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig, DiffusionSchedule]:
    parser = argparse.ArgumentParser(
        description="Lesson 15 (Generative): toy diffusion-style super-resolution with low-res conditioning."
    )
...
def _evaluate(
    model: ToySuperResolutionDiffusionModel,
...
def run_training(
    ...
    paths = build_run_paths(
        track="generative",
        lesson="lesson_15_toy_diffusion_super_resolution",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("generative.toy_diffusion_super_resolution", log_file=paths.logs_dir / "train.log")
    ...
    model = ToySuperResolutionDiffusionModel(model_cfg).to(device_info.torch_device)
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "generative", "lesson": "lesson_15_toy_diffusion_super_resolution"},
    )
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.generative.lesson_15_toy_diffusion_super_resolution.train"
        )
