...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig, DiffusionSchedule]:
    parser = argparse.ArgumentParser(
        description="Lesson 25 (Generative): compact diffusion compositional generation from structure and style."
    )
...
def _evaluate(
    model: CompactCompositionalDiffusionModel,
    ...
    *,
    ...
    total_seen = 0
    with torch.no_grad():
        for batch_idx, (structure, style, target) in enumerate(loader):
...
def run_training(
    ...
    paths = build_run_paths(
        track="generative",
        lesson="lesson_25_compact_diffusion_compositional_generation",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("generative.compact_diffusion_compositional_generation", log_file=paths.logs_dir / "train.log")
    ...
    model = CompactCompositionalDiffusionModel(model_cfg).to(device_info.torch_device)
    ...
    for epoch in range(1, int(train_cfg.epochs) + 1):
        ...
        for batch_idx, (structure, style, target) in enumerate(train_loader):
            if train_cfg.max_train_batches is not None and batch_idx >= train_cfg.max_train_batches:
                break
            structure = structure.to(device_info.torch_device)
    ...
    sample_structure, sample_style, sample_target = next(iter(val_loader))
    sample_structure = sample_structure[:16]
    ...
    save_checkpoint(
        ...
        extra={"track": "generative", "lesson": "lesson_25_compact_diffusion_compositional_generation"},
    )
    return 0
