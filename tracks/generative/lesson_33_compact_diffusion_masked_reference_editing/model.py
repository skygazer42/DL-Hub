...
class CompactMaskedReferenceEditingDiffusionModel(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        features = torch.cat([xt, source, reference, mask, t_map], dim=1)
    ...
    def sample(
        ...
        *,
        ...
        xt = source * (1.0 - mask) + torch.randn_like(source) * mask
        ...
        for step in range(step_count - 1, -1, -1):
            ...
            pred_noise = self(xt=xt, source=source, reference=reference, mask=mask, timesteps=t)
            ...
            if step > 0:
                xt = source * (1.0 - mask) + (mean + torch.sqrt(beta_t) * torch.randn_like(xt)) * mask
            else:
                xt = source * (1.0 - mask) + mean * mask
...
__all__ = ["DiffusionSchedule", "ModelConfig", "CompactMaskedReferenceEditingDiffusionModel", "q_sample"]
