...
class CompactLayoutAttributeFusionDiffusionModel(nn.Module):
    def __init__(self, cfg: ModelConfig) -> None:
        ...
        net_in_channels = in_channels * 2 + attr_dim + 1
    ...
    def forward(
        ...
        *,
        ...
        attr_dim = int(self.cfg.attr_dim)
        attr_map = (
            attribute.to(dtype=xt.dtype)
            .view(batch_size, attr_dim, 1, 1)
            .expand(batch_size, attr_dim, height, width)
        )
        features = torch.cat([xt, layout, attr_map, t_map], dim=1)
    ...
    def sample(
        ...
        *,
        ...
        attribute = attribute.to(device=device, dtype=torch.float32)
        ...
        for step in range(step_count - 1, -1, -1):
            ...
            pred_noise = self(xt=xt, layout=layout, attribute=attribute, timesteps=t)
...
__all__ = ["DiffusionSchedule", "ModelConfig", "CompactLayoutAttributeFusionDiffusionModel", "q_sample"]
