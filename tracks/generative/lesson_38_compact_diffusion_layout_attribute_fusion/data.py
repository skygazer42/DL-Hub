...
def _make_synthetic_layout_attribute_fusion_data(cfg: DataConfig) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    if attr_dim != 4:
        raise ValueError("attr_dim must be 4 for this synthetic lesson")
...
class SyntheticLayoutAttributeFusionDataset(Dataset):
    def __init__(self, cfg: DataConfig) -> None:
        self.cfg = cfg
        self._layout, self._attribute, self._target = _make_synthetic_layout_attribute_fusion_data(cfg)
...
def get_dataloaders(cfg: DataConfig) -> tuple[DataLoader, DataLoader]:
    dataset: Dataset = SyntheticLayoutAttributeFusionDataset(cfg)
...
__all__ = ["DataConfig", "SyntheticLayoutAttributeFusionDataset", "get_dataloaders"]
