...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 49 (Generative): compact text-to-video generation."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig, model_cfg: ModelConfig) -> int:
    ...
    paths = build_run_paths(
        track="generative",
        lesson="lesson_49_compact_text_to_video",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("generative.compact_text_to_video", log_file=paths.logs_dir / "train.log")
    ...
    model = CompactTextToVideoModel(model_cfg).to(device_info.torch_device)
    ...
    save_checkpoint(
        ...
        extra={"track": "generative", "lesson": "lesson_49_compact_text_to_video"},
    )
    return 0
