...
from .model import ModelConfig, CompactWorldModelsModel, reward_mae, world_models_loss
...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 51 (Generative): compact world-model training."
    )
...
def run_training(
    ...
    paths = build_run_paths(
        track="generative",
        lesson="lesson_51_compact_world_models",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("generative.compact_world_models", log_file=paths.logs_dir / "train.log")
    ...
    model = CompactWorldModelsModel(model_cfg).to(device_info.torch_device)
    ...
    save_checkpoint(
        ...
        extra={"track": "generative", "lesson": "lesson_51_compact_world_models"},
    )
    return 0
