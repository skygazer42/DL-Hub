...
def _stroke_mask(region_mask: torch.Tensor, generator: torch.Generator) -> torch.Tensor:
    coords = torch.nonzero(region_mask[0] > 0.5, as_tuple=False)
    if coords.size(0) < 2:
        return region_mask.clone()

    stroke = torch.zeros_like(region_mask)
    ...
    num_segments = int(torch.randint(2, 5, (1,), generator=generator).item())
    anchor_ids = torch.randperm(coords.size(0), generator=generator)[: num_segments + 1]
    anchors = coords[anchor_ids].to(torch.float32)

    for segment_idx in range(num_segments):
        y0, x0 = anchors[segment_idx]
        y1, x1 = anchors[segment_idx + 1]
        dy = y1 - y0
        dx = x1 - x0
        ...
        radius = _uniform(generator, 1.2, 2.6)
        patch = (((xx - proj_x) ** 2 + (yy - proj_y) ** 2) <= radius**2).to(torch.float32)
        stroke[0] = torch.maximum(stroke[0], patch)

    return (stroke * region_mask).clamp(0.0, 1.0)
...
def _make_target(
    *,
    ...
    target = source * (1.0 - stroke_mask) + edited * stroke_mask
...
def _make_synthetic_stroke_mask_editing_data(cfg: DataConfig) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    generator = torch.Generator().manual_seed(int(cfg.seed))
    sources: list[torch.Tensor] = []
    stroke_masks: list[torch.Tensor] = []
    targets: list[torch.Tensor] = []

    for _ in range(int(cfg.num_samples)):
        region_mask = _region_mask(int(cfg.image_size), generator)
        stroke_mask = _stroke_mask(region_mask, generator)
        source = _make_source(region_mask, generator)
        target = _make_target(source=source, stroke_mask=stroke_mask, generator=generator)
    ...
    return (
        ...
        torch.stack(stroke_masks, dim=0),
        torch.stack(targets, dim=0),
    )


class SyntheticStrokeMaskEditingDataset(Dataset):
    def __init__(self, cfg: DataConfig) -> None:
        self.cfg = cfg
        self._source, self._stroke_mask, self._target = _make_synthetic_stroke_mask_editing_data(cfg)
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        i = int(idx)
        return self._source[i], self._stroke_mask[i], self._target[i]


def get_dataloaders(cfg: DataConfig) -> tuple[DataLoader, DataLoader]:
    dataset: Dataset = SyntheticStrokeMaskEditingDataset(cfg)
...
__all__ = ["DataConfig", "SyntheticStrokeMaskEditingDataset", "get_dataloaders"]
