...
class CompactStrokeMaskEditingDiffusionModel(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        features = torch.cat([xt, source, stroke_mask, t_map], dim=1)
    ...
    def sample(
        ...
        *,
        ...
        stroke_mask = stroke_mask.to(device=device, dtype=torch.float32)
        ...
        xt = source * (1.0 - stroke_mask) + torch.randn_like(source) * stroke_mask
        ...
        for step in range(step_count - 1, -1, -1):
            ...
            pred_noise = self(xt=xt, source=source, stroke_mask=stroke_mask, timesteps=t)
            ...
            if step > 0:
                xt = source * (1.0 - stroke_mask) + (mean + torch.sqrt(beta_t) * torch.randn_like(xt)) * stroke_mask
            else:
                xt = source * (1.0 - stroke_mask) + mean * stroke_mask
...
__all__ = ["DiffusionSchedule", "ModelConfig", "CompactStrokeMaskEditingDiffusionModel", "q_sample"]
