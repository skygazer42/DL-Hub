...
class CompactScribbleMaskEditingDiffusionModel(nn.Module):
    def __init__(self, cfg: ModelConfig) -> None:
        super().__init__()
        self.cfg = cfg
        in_channels = int(cfg.in_channels)
        hidden = int(cfg.hidden_channels)
        net_in_channels = in_channels * 2 + 2
    ...
    def forward(
        ...
        *,
        ...
        features = torch.cat([xt, source, scribble_mask, t_map], dim=1)
    ...
    def sample(
        ...
        *,
        ...
        scribble_mask = scribble_mask.to(device=device, dtype=torch.float32)
        ...
        xt = source * (1.0 - scribble_mask) + torch.randn_like(source) * scribble_mask
        ...
        for step in range(step_count - 1, -1, -1):
            ...
            pred_noise = self(xt=xt, source=source, scribble_mask=scribble_mask, timesteps=t)
            ...
            if step > 0:
                xt = source * (1.0 - scribble_mask) + (mean + torch.sqrt(beta_t) * torch.randn_like(xt)) * scribble_mask
            else:
                xt = source * (1.0 - scribble_mask) + mean * scribble_mask
...
__all__ = ["DiffusionSchedule", "ModelConfig", "CompactScribbleMaskEditingDiffusionModel", "q_sample"]
