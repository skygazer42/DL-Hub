# Lesson 20: Toy Diffusion Reflection Removal
...
```bash
python -m tracks.generative.lesson_20_toy_diffusion_reflection_removal.train --epochs 1 --num-samples 64 --batch-size 8 --max-train-batches 2 --max-eval-batches 1 --device cpu --run-name smoke
```

Or via the shared launcher:

```bash
python scripts/run_lesson.py generative lesson_20_toy_diffusion_reflection_removal -- --epochs 1 --device cpu
```

## Outputs

`outputs/generative/lesson_20_toy_diffusion_reflection_removal/<run_name>/`
