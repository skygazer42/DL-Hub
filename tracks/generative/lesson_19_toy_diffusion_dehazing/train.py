...
from .model import DiffusionSchedule, ModelConfig, ToyDehazingDiffusionModel, q_sample
...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig, DiffusionSchedule]:
    parser = argparse.ArgumentParser(
        description="Lesson 19 (Generative): toy diffusion-style image dehazing (去雾) from paired hazy/clean images."
    )
...
def _evaluate(
    model: ToyDehazingDiffusionModel,
...
def run_training(
    ...
    paths = build_run_paths(
        track="generative",
        lesson="lesson_19_toy_diffusion_dehazing",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("generative.toy_diffusion_dehazing", log_file=paths.logs_dir / "train.log")
    ...
    model = ToyDehazingDiffusionModel(model_cfg).to(device_info.torch_device)
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "generative", "lesson": "lesson_19_toy_diffusion_dehazing"},
    )
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.generative.lesson_19_toy_diffusion_dehazing.train"
        )
