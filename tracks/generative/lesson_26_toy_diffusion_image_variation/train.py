...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig, DiffusionSchedule]:
    parser = argparse.ArgumentParser(
        description="Lesson 26 (Generative): toy conditional diffusion for image variation from source image."
    )
...
def _evaluate(
    model: ToyImageVariationDiffusionModel,
...
def run_training(
    ...
    paths = build_run_paths(
        track="generative",
        lesson="lesson_26_toy_diffusion_image_variation",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("generative.toy_diffusion_image_variation", log_file=paths.logs_dir / "train.log")
    ...
    model = ToyImageVariationDiffusionModel(model_cfg).to(device_info.torch_device)
    ...
    save_checkpoint(
        ...
        extra={"track": "generative", "lesson": "lesson_26_toy_diffusion_image_variation"},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.generative.lesson_26_toy_diffusion_image_variation.train"
        )
