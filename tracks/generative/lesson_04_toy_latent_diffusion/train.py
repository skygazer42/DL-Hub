...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 04 (Generative): toy latent diffusion with a compact autoencoder."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig, model_cfg: ModelConfig) -> int:
    ...
    paths = build_run_paths(
        track="generative", lesson="lesson_04_toy_latent_diffusion", run_name=train_cfg.run_name
    )
    logger = get_logger("generative.toy_latent_diffusion", log_file=paths.logs_dir / "train.log")
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "generative", "lesson": "lesson_04_toy_latent_diffusion"},
    )
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.generative.lesson_04_toy_latent_diffusion.train"
        )
