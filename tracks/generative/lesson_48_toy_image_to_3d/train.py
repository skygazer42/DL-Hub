...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 48 (Generative): toy image-to-3D lesson with lightweight density/mesh targets."
    )
    ...
    parser.add_argument("--family", type=str, default="zero123_toy")
    parser.add_argument("--variant", type=str, default="zero123_toy_tiny")
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig, model_cfg: ModelConfig) -> int:
    ...
    paths = build_run_paths(
        track="generative",
        lesson="lesson_48_toy_image_to_3d",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("generative.toy_image_to_3d", log_file=paths.logs_dir / "train.log")
    ...
    model = ToyImageTo3DModel(model_cfg).to(device_info.torch_device)
    ...
    save_checkpoint(
        ...
        extra={"track": "generative", "lesson": "lesson_48_toy_image_to_3d"},
    )
    return 0
