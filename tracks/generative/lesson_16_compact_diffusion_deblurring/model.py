...
class CompactDeblurringDiffusionModel(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        features = torch.cat([xt, blurry, t_map], dim=1)
    ...
    def sample(
        ...
        *,
        ...
        blurry = blurry.to(device=device, dtype=torch.float32)
        n = int(blurry.shape[0])
        ...
        for step in range(step_count - 1, -1, -1):
            ...
            pred_noise = self(xt=xt, blurry=blurry, timesteps=t)
        ...
        deblurred = xt.clamp(0.0, 1.0).cpu()
        if frames is None:
            return deblurred
        return torch.stack(frames[1:] + [deblurred], dim=0)
