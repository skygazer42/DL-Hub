...
from .model import DiffusionSchedule, ModelConfig, CompactDeblurringDiffusionModel, q_sample
...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig, DiffusionSchedule]:
    parser = argparse.ArgumentParser(
        description="Lesson 16 (Generative): compact diffusion-style image deblurring from blurry/sharp pairs."
    )
...
def _evaluate(
    model: CompactDeblurringDiffusionModel,
...
def run_training(
    ...
    paths = build_run_paths(
        track="generative",
        lesson="lesson_16_compact_diffusion_deblurring",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("generative.compact_diffusion_deblurring", log_file=paths.logs_dir / "train.log")
    ...
    model = CompactDeblurringDiffusionModel(model_cfg).to(device_info.torch_device)
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "generative", "lesson": "lesson_16_compact_diffusion_deblurring"},
    )
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.generative.lesson_16_compact_diffusion_deblurring.train"
        )
