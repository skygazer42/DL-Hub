...
class CompactSubjectDrivenDiffusionModel(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        features = torch.cat([xt, subject, guidance, t_map], dim=1)
    ...
    def sample(
        ...
        *,
        ...
        subject = subject.to(device=device, dtype=torch.float32)
        guidance = guidance.to(device=device, dtype=torch.float32)
        ...
        for step in range(step_count - 1, -1, -1):
            ...
            pred_noise = self(xt=xt, subject=subject, guidance=guidance, timesteps=t)
...
__all__ = ["DiffusionSchedule", "ModelConfig", "CompactSubjectDrivenDiffusionModel", "q_sample"]
