...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig, DiffusionSchedule]:
    parser = argparse.ArgumentParser(
        description="Lesson 22 (Generative): compact diffusion-style content/style transfer."
    )
...
def _evaluate(
    model: CompactStyleTransferDiffusionModel,
    ...
    *,
    ...
    total_seen = 0
    with torch.no_grad():
        for batch_idx, (content, style, target) in enumerate(loader):
...
def run_training(
    ...
    paths = build_run_paths(
        track="generative",
        lesson="lesson_22_compact_diffusion_style_transfer",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("generative.compact_diffusion_style_transfer", log_file=paths.logs_dir / "train.log")
    ...
    model = CompactStyleTransferDiffusionModel(model_cfg).to(device_info.torch_device)
    ...
    for epoch in range(1, int(train_cfg.epochs) + 1):
        ...
        for batch_idx, (content, style, target) in enumerate(train_loader):
    ...
    sample_content, sample_style, sample_target = next(iter(val_loader))
    sample_content = sample_content[:16]
    sample_style = sample_style[:16]
    ...
    torch.save({"frames": frames}, paths.run_dir / "style_trajectory.pt")
    ...
    save_checkpoint(
        ...
        extra={"track": "generative", "lesson": "lesson_22_compact_diffusion_style_transfer"},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.generative.lesson_22_compact_diffusion_style_transfer.train"
        )
