...
class CompactPathMaskEditingDiffusionModel(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        features = torch.cat([xt, source, path_mask, t_map], dim=1)
    ...
    def sample(
        ...
        *,
        ...
        path_mask = path_mask.to(device=device, dtype=torch.float32)
        ...
        xt = source * (1.0 - path_mask) + torch.randn_like(source) * path_mask
        ...
        for step in range(step_count - 1, -1, -1):
            ...
            pred_noise = self(xt=xt, source=source, path_mask=path_mask, timesteps=t)
            ...
            if step > 0:
                xt = source * (1.0 - path_mask) + (mean + torch.sqrt(beta_t) * torch.randn_like(xt)) * path_mask
            else:
                xt = source * (1.0 - path_mask) + mean * path_mask
...
__all__ = ["DiffusionSchedule", "ModelConfig", "CompactPathMaskEditingDiffusionModel", "q_sample"]
