...
class CompactInpaintingDiffusionModel(nn.Module):
    def __init__(self, cfg: ModelConfig) -> None:
        super().__init__()
        self.cfg = cfg
        in_channels = int(cfg.in_channels)
        net_in_channels = in_channels * 3 + 2
    ...
    def forward(
        ...
        *,
        ...
        features = torch.cat([xt, context, mask, xt * mask, t_map], dim=1)
    ...
    def sample(
        ...
        *,
        ...
        for step in range(step_count - 1, -1, -1):
            ...
            pred_noise = self(xt=xt, context=context, mask=mask, timesteps=t)
        ...
        inpainted = xt.clamp(0.0, 1.0)
        inpainted = context * (1.0 - mask) + inpainted * mask
        inpainted = inpainted.cpu()
        if frames is None:
            return inpainted
        return torch.stack(frames[1:] + [inpainted], dim=0)
