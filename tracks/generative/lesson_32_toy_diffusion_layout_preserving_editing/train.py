...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig, DiffusionSchedule]:
    parser = argparse.ArgumentParser(
        description="Lesson 32 (Generative): toy diffusion layout-preserving editing from layout and edit map."
    )
...
def _evaluate(
    model: ToyLayoutPreservingDiffusionModel,
...
def run_training(
    ...
    paths = build_run_paths(
        track="generative",
        lesson="lesson_32_toy_diffusion_layout_preserving_editing",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("generative.toy_diffusion_layout_preserving_editing", log_file=paths.logs_dir / "train.log")
    ...
    model = ToyLayoutPreservingDiffusionModel(model_cfg).to(device_info.torch_device)
    ...
    save_checkpoint(
        ...
        extra={"track": "generative", "lesson": "lesson_32_toy_diffusion_layout_preserving_editing"},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.generative.lesson_32_toy_diffusion_layout_preserving_editing.train"
        )
