# Lesson 23: Toy Diffusion Multi-Focus Fusion
...
```bash
python -m tracks.generative.lesson_23_toy_diffusion_multi_focus_fusion.train --epochs 1 --num-samples 64 --batch-size 8 --max-train-batches 2 --max-eval-batches 1 --device cpu --run-name smoke
```

Or via the shared launcher:

```bash
python scripts/run_lesson.py generative lesson_23_toy_diffusion_multi_focus_fusion -- --epochs 1 --device cpu
```

## Outputs

`outputs/generative/lesson_23_toy_diffusion_multi_focus_fusion/<run_name>/`
