...
from .model import DiffusionSchedule, ModelConfig, ToyDenoisingDiffusionModel, q_sample
...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig, DiffusionSchedule]:
    parser = argparse.ArgumentParser(
        description="Lesson 17 (Generative): toy diffusion-style denoising from paired noisy/clean synthetic images."
    )
...
def _evaluate(
    model: ToyDenoisingDiffusionModel,
...
def run_training(
    ...
    paths = build_run_paths(
        track="generative",
        lesson="lesson_17_toy_diffusion_denoising",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("generative.toy_diffusion_denoising", log_file=paths.logs_dir / "train.log")
    ...
    model = ToyDenoisingDiffusionModel(model_cfg).to(device_info.torch_device)
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "generative", "lesson": "lesson_17_toy_diffusion_denoising"},
    )
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.generative.lesson_17_toy_diffusion_denoising.train"
        )
