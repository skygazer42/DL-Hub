# Lesson 17: Toy Diffusion Denoising (去噪)
...
```bash
python -m tracks.generative.lesson_17_toy_diffusion_denoising.train --epochs 1 --num-samples 64 --batch-size 8 --max-train-batches 2 --max-eval-batches 1 --device cpu --run-name smoke
```

Or via the shared launcher:

```bash
python scripts/run_lesson.py generative lesson_17_toy_diffusion_denoising -- --epochs 1 --device cpu
```

## Outputs

`outputs/generative/lesson_17_toy_diffusion_denoising/<run_name>/`
