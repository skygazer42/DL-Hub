...
def _make_target(
    *,
    ...
    shifted_source = torch.roll(source, shifts=1, dims=2)
    reference_mix = 0.62 * reference + 0.38 * shifted_source
    target = source * (1.0 - 0.55 * mask) + reference_mix * (0.55 * mask)
...
def _make_synthetic_reference_editing_data(cfg: DataConfig) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    return torch.stack(sources, dim=0), torch.stack(references, dim=0), torch.stack(targets, dim=0)


class SyntheticReferenceEditingDataset(Dataset):
    def __init__(self, cfg: DataConfig) -> None:
        self.cfg = cfg
        self._source, self._reference, self._target = _make_synthetic_reference_editing_data(cfg)
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        i = int(idx)
        return self._source[i], self._reference[i], self._target[i]


def get_dataloaders(cfg: DataConfig) -> tuple[DataLoader, DataLoader]:
    dataset: Dataset = SyntheticReferenceEditingDataset(cfg)
...
__all__ = ["DataConfig", "SyntheticReferenceEditingDataset", "get_dataloaders"]
