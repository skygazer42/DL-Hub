...
class CompactControlNetDenoiser(nn.Module):
    def __init__(self, cfg: ModelConfig) -> None:
        ...
        self.main_stem = nn.Conv2d(in_channels + 1, hidden, kernel_size=3, padding=1)
    ...
    def forward(self, xt: torch.Tensor, structure_hint: torch.Tensor, timesteps: torch.Tensor) -> torch.Tensor:
        b, _, h, w = xt.shape
        ...
        base_features = self.main_stem(torch.cat([xt, t_map], dim=1))
        control_features = self.control_branch(structure_hint.to(dtype=xt.dtype))
        fused = base_features + control_features
        return self.main_body(fused)
    ...
    def sample(
        ...
        schedule: DiffusionSchedule,
        *,
        structure_hint: torch.Tensor,
        device: torch.device | str,
        num_steps: int | None = None,
        return_all: bool = False,
    ) -> torch.Tensor:
        device = torch.device(device)
        structure_hint = structure_hint.to(device=device, dtype=torch.float32)
        n = int(structure_hint.shape[0])
        shape = (
            n,
            int(self.cfg.in_channels),
            int(self.cfg.image_size),
            int(self.cfg.image_size),
        )
        ...
        for step in range(step_count - 1, -1, -1):
            ...
            pred_noise = self(xt, structure_hint, t)
