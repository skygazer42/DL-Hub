...
from .model import DiffusionSchedule, ModelConfig, CompactControlNetDenoiser, q_sample
...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig, DiffusionSchedule]:
    parser = argparse.ArgumentParser(
        description="Lesson 11 (Generative): compact ControlNet-style diffusion with structural guidance."
    )
...
def _evaluate(
    model: CompactControlNetDenoiser,
    ...
    *,
    ...
    total_seen = 0
    with torch.no_grad():
        for batch_idx, (target, structure_hint) in enumerate(loader):
            if max_batches is not None and batch_idx >= max_batches:
                break
            target = target.to(device)
            structure_hint = structure_hint.to(device)
...
def run_training(
    ...
    paths = build_run_paths(
        track="generative",
        lesson="lesson_11_compact_controlnet",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("generative.compact_controlnet", log_file=paths.logs_dir / "train.log")
    ...
    model = CompactControlNetDenoiser(model_cfg).to(device_info.torch_device)
    ...
    for epoch in range(1, train_cfg.epochs + 1):
        ...
        for batch_idx, (target, structure_hint) in enumerate(train_loader):
            ...
            structure_hint = structure_hint.to(device_info.torch_device)
            noise = torch.randn_like(target)
    ...
    _, sample_hint = next(iter(val_loader))
    sample_hint = sample_hint[:16]
    samples = model.sample(
        schedule,
        structure_hint=sample_hint,
        device=device_info.torch_device,
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "generative", "lesson": "lesson_11_compact_controlnet"},
    )
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.generative.lesson_11_compact_controlnet.train"
        )
