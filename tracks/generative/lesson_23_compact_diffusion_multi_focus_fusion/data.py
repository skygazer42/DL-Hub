...
class SyntheticMultiFocusFusionDataset(Dataset):
    def __init__(self, cfg: DataConfig) -> None:
        self.cfg = cfg
        self._focus_a, self._focus_b, self._target = _make_synthetic_multi_focus_data(cfg)
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        i = int(idx)
        return self._focus_a[i], self._focus_b[i], self._target[i]
...
def _make_focus_views(target: torch.Tensor, generator: torch.Generator) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    slope = float(torch.empty((1,)).uniform_(-0.8, 0.8, generator=generator).item())
    bias = float(torch.empty((1,)).uniform_(-0.2, 0.2, generator=generator).item())
    sharp_band = (yy - slope * xx - bias) > 0
    mask_a = sharp_band.to(dtype=target.dtype) * 0.85 + 0.05
    mask_b = (~sharp_band).to(dtype=target.dtype) * 0.85 + 0.05

    blurred = _gaussian_blur(target)
    blurred = _gaussian_blur(blurred)
    noise_a = 0.02 * torch.randn(target.shape, generator=generator, dtype=target.dtype)
    noise_b = 0.02 * torch.randn(target.shape, generator=generator, dtype=target.dtype)

    focus_a = target * mask_a.unsqueeze(0) + blurred * (1.0 - mask_a.unsqueeze(0)) + noise_a
    shifted = torch.roll(blurred, shifts=1, dims=2)
    focus_b = target * mask_b.unsqueeze(0) + shifted * (1.0 - mask_b.unsqueeze(0)) + noise_b
    return focus_a.clamp(0.0, 1.0), focus_b.clamp(0.0, 1.0)


def _make_synthetic_multi_focus_data(cfg: DataConfig) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    g = torch.Generator().manual_seed(int(cfg.seed))
    focus_a: list[torch.Tensor] = []
    focus_b: list[torch.Tensor] = []
    target: list[torch.Tensor] = []

    for _ in range(int(cfg.num_samples)):
        clean = _paint_shape(int(cfg.image_size), g)
        view_a, view_b = _make_focus_views(clean, g)
        focus_a.append(view_a)
        focus_b.append(view_b)
        target.append(clean)

    return torch.stack(focus_a, dim=0), torch.stack(focus_b, dim=0), torch.stack(target, dim=0)


def get_dataloaders(cfg: DataConfig) -> tuple[DataLoader, DataLoader]:
    dataset: Dataset = SyntheticMultiFocusFusionDataset(cfg)
