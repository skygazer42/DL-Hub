...
class CompactMultiFocusFusionDiffusionModel(nn.Module):
    ...
    def forward(
        ...
        *,
        xt: torch.Tensor,
        focus_a: torch.Tensor,
        focus_b: torch.Tensor,
        timesteps: torch.Tensor,
    ) -> torch.Tensor:
        b, _, h, w = xt.shape
        ...
        features = torch.cat([xt, focus_a, focus_b, t_map], dim=1)
    ...
    def sample(
        ...
        *,
        ...
        focus_a = focus_a.to(device=device, dtype=torch.float32)
        focus_b = focus_b.to(device=device, dtype=torch.float32)
        n = int(focus_a.shape[0])
        ...
        for step in range(step_count - 1, -1, -1):
            ...
            pred_noise = self(xt=xt, focus_a=focus_a, focus_b=focus_b, timesteps=t)
