...
class CompactLayoutLightingFusionDiffusionModel(nn.Module):
    def __init__(self, cfg: ModelConfig) -> None:
        super().__init__()
        self.cfg = cfg
        in_channels = int(cfg.in_channels)
        layout_channels = int(cfg.layout_channels)
        hidden = int(cfg.hidden_channels)
        lighting_dim = int(cfg.lighting_dim)
        net_in_channels = in_channels + layout_channels + lighting_dim + 1
    ...
    def forward(
        ...
        *,
        ...
        lighting_dim = int(self.cfg.lighting_dim)
        lighting_map = (
            lighting_code.to(dtype=xt.dtype)
            .view(batch_size, lighting_dim, 1, 1)
            .expand(batch_size, lighting_dim, height, width)
        )
        features = torch.cat([xt, layout.to(dtype=xt.dtype), lighting_map, t_map], dim=1)
    ...
    def sample(
        ...
        *,
        ...
        layout = layout.to(device=device, dtype=torch.float32)
        lighting_code = lighting_code.to(device=device, dtype=torch.float32)
        n, _, height, width = layout.shape
        ...
        xt = torch.randn((n, int(self.cfg.in_channels), height, width), device=device, dtype=torch.float32)
        ...
        for step in range(step_count - 1, -1, -1):
            ...
            pred_noise = self(xt=xt, layout=layout, lighting_code=lighting_code, timesteps=t)
...
__all__ = ["DiffusionSchedule", "ModelConfig", "CompactLayoutLightingFusionDiffusionModel", "q_sample"]
