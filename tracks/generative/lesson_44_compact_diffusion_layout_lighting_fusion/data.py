...
def _sample_lighting_code(lighting_dim: int, generator: torch.Generator) -> torch.Tensor:
    if lighting_dim != 4:
        raise ValueError("lighting_dim must be 4 for this synthetic lesson")
...
def _make_synthetic_layout_lighting_fusion_data(cfg: DataConfig) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
...
class SyntheticLayoutLightingFusionDataset(Dataset):
    def __init__(self, cfg: DataConfig) -> None:
        self.cfg = cfg
        self._layout, self._lighting_code, self._target = _make_synthetic_layout_lighting_fusion_data(cfg)
...
def get_dataloaders(cfg: DataConfig) -> tuple[DataLoader, DataLoader]:
    dataset: Dataset = SyntheticLayoutLightingFusionDataset(cfg)
...
__all__ = ["DataConfig", "SyntheticLayoutLightingFusionDataset", "get_dataloaders"]
