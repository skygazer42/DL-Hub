...
from .model import DiffusionSchedule, ModelConfig, CompactTextConditionedDenoiser, q_sample
...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig, DiffusionSchedule]:
    parser = argparse.ArgumentParser(
        description="Lesson 13 (Generative): compact text-to-image diffusion with prompt conditioning."
    )
...
def _evaluate(
    model: CompactTextConditionedDenoiser,
...
def run_training(
    ...
    paths = build_run_paths(
        track="generative",
        lesson="lesson_13_compact_text_to_image_diffusion",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("generative.compact_text_to_image_diffusion", log_file=paths.logs_dir / "train.log")
    ...
    model = CompactTextConditionedDenoiser(model_cfg).to(device_info.torch_device)
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "generative", "lesson": "lesson_13_compact_text_to_image_diffusion"},
    )
