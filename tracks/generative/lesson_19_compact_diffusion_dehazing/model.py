...
class CompactDehazingDiffusionModel(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        features = torch.cat([xt, hazy, t_map], dim=1)
    ...
    def sample(
        ...
        *,
        ...
        hazy = hazy.to(device=device, dtype=torch.float32)
        n = int(hazy.shape[0])
        ...
        for step in range(step_count - 1, -1, -1):
            ...
            pred_noise = self(xt=xt, hazy=hazy, timesteps=t)
        ...
        dehazed = xt.clamp(0.0, 1.0).cpu()
        if frames is None:
            return dehazed
        return torch.stack(frames[1:] + [dehazed], dim=0)
