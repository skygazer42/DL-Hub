...
from .model import DiffusionSchedule, ModelConfig, CompactDehazingDiffusionModel, q_sample
...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig, DiffusionSchedule]:
    parser = argparse.ArgumentParser(
        description="Lesson 19 (Generative): compact diffusion-style image dehazing (去雾) from paired hazy/clean images."
    )
...
def _evaluate(
    model: CompactDehazingDiffusionModel,
...
def run_training(
    ...
    paths = build_run_paths(
        track="generative",
        lesson="lesson_19_compact_diffusion_dehazing",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("generative.compact_diffusion_dehazing", log_file=paths.logs_dir / "train.log")
    ...
    model = CompactDehazingDiffusionModel(model_cfg).to(device_info.torch_device)
    ...
    for epoch in range(1, train_cfg.epochs + 1):
        ...
        for batch_idx, (hazy, clean) in enumerate(train_loader):
            if train_cfg.max_train_batches is not None and batch_idx >= train_cfg.max_train_batches:
                break
            hazy = hazy.to(device_info.torch_device)
    ...
    sample_hazy, sample_clean = next(iter(val_loader))
    sample_hazy = sample_hazy[:16]
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "generative", "lesson": "lesson_19_compact_diffusion_dehazing"},
    )
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.generative.lesson_19_compact_diffusion_dehazing.train"
        )
