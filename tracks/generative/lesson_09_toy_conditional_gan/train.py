...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 09 (Generative): toy class-conditional GAN on synthetic images."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig, model_cfg: ModelConfig) -> int:
    ...
    paths = build_run_paths(
        track="generative",
        lesson="lesson_09_toy_conditional_gan",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("generative.toy_conditional_gan", log_file=paths.logs_dir / "train.log")
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "generative", "lesson": "lesson_09_toy_conditional_gan"},
    )
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.generative.lesson_09_toy_conditional_gan.train"
        )
