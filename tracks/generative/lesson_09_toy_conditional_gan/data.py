...
class _ConditionedToyDataset(Dataset):
...
def _make_toy_images_and_labels(cfg: DataConfig) -> tuple[torch.Tensor, torch.Tensor]:
    size = int(cfg.image_size)
    if size != 28:
        raise ValueError(f"Toy conditional GAN expects image_size=28, got {size}")
...
def get_dataloaders(cfg: DataConfig) -> tuple[DataLoader, DataLoader]:
    images, labels = _make_toy_images_and_labels(cfg)
    base_ds: Dataset = _ConditionedToyDataset(images, labels)
