...
from .model import DiffusionSchedule, ModelConfig, ToyControlNetDenoiser, q_sample
...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig, DiffusionSchedule]:
    parser = argparse.ArgumentParser(
        description="Lesson 11 (Generative): toy ControlNet-style diffusion with structural guidance."
    )
...
def run_training(
    ...
    paths = build_run_paths(
        track="generative",
        lesson="lesson_11_toy_controlnet",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("generative.toy_controlnet", log_file=paths.logs_dir / "train.log")
    ...
    model = ToyControlNetDenoiser(model_cfg).to(device_info.torch_device)
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "generative", "lesson": "lesson_11_toy_controlnet"},
    )
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.generative.lesson_11_toy_controlnet.train"
        )
