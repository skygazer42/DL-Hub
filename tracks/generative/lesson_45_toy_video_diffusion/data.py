...
def _sample_motion_code(motion_dim: int, generator: torch.Generator) -> torch.Tensor:
    if motion_dim != 3:
        raise ValueError("motion_dim must be 3 for this toy lesson")
...
class ToyVideoDiffusionDataset(Dataset):
...
def get_dataloaders(cfg: DataConfig) -> tuple[DataLoader, DataLoader]:
    dataset: Dataset = ToyVideoDiffusionDataset(cfg)
...
__all__ = ["DataConfig", "ToyVideoDiffusionDataset", "get_dataloaders"]
