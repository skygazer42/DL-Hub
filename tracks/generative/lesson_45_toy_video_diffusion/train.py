...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig, DiffusionSchedule]:
    parser = argparse.ArgumentParser(
        description="Lesson 45 (Generative): toy video diffusion from a keyframe and motion code."
    )
...
def run_training(
    ...
    paths = build_run_paths(
        track="generative",
        lesson="lesson_45_toy_video_diffusion",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("generative.toy_video_diffusion", log_file=paths.logs_dir / "train.log")
    ...
    model = ToyVideoDiffusionModel(model_cfg).to(device_info.torch_device)
    ...
    save_checkpoint(
        ...
        extra={"track": "generative", "lesson": "lesson_45_toy_video_diffusion"},
    )
    return 0
