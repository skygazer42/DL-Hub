...
class CompactSuperResolutionDiffusionModel(nn.Module):
    def __init__(self, cfg: ModelConfig) -> None:
        ...
        self.net = nn.Sequential(
            nn.Conv2d(in_channels * 2 + 1, hidden, kernel_size=3, padding=1),
            nn.SiLU(),
    ...
    def forward(self, *, xt: torch.Tensor, low_res: torch.Tensor, timesteps: torch.Tensor) -> torch.Tensor:
        b, _, h, w = xt.shape
        lr_up = F.interpolate(low_res, size=(h, w), mode="bilinear", align_corners=False)
        ...
        features = torch.cat([xt, lr_up, t_map], dim=1)
    ...
    def sample(
        ...
        *,
        ...
        low_res = low_res.to(device=device, dtype=torch.float32)
        n = int(low_res.shape[0])
        h = int(low_res.shape[-2]) * 2
        w = int(low_res.shape[-1]) * 2
        ...
        xt = torch.randn((n, int(self.cfg.in_channels), h, w), device=device, dtype=torch.float32)
        ...
        for step in range(step_count - 1, -1, -1):
            ...
            pred_noise = self(xt=xt, low_res=low_res, timesteps=t)
            alpha_t = alphas[step]
