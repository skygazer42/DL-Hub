...
from .model import DiffusionSchedule, ModelConfig, CompactSuperResolutionDiffusionModel, q_sample
...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig, DiffusionSchedule]:
    parser = argparse.ArgumentParser(
        description="Lesson 15 (Generative): compact diffusion-style super-resolution with low-res conditioning."
    )
...
def _evaluate(
    model: CompactSuperResolutionDiffusionModel,
...
def run_training(
    ...
    paths = build_run_paths(
        track="generative",
        lesson="lesson_15_compact_diffusion_super_resolution",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("generative.compact_diffusion_super_resolution", log_file=paths.logs_dir / "train.log")
    ...
    model = CompactSuperResolutionDiffusionModel(model_cfg).to(device_info.torch_device)
    ...
    for epoch in range(1, train_cfg.epochs + 1):
        ...
        for batch_idx, (low_res, high_res) in enumerate(train_loader):
            if train_cfg.max_train_batches is not None and batch_idx >= train_cfg.max_train_batches:
                break
            low_res = low_res.to(device_info.torch_device)
            high_res = high_res.to(device_info.torch_device)

            noise = torch.randn_like(high_res)
            timesteps = torch.randint(0, schedule.num_steps, (high_res.size(0),), device=device_info.torch_device)
            xt = q_sample(schedule, high_res, timesteps, noise)
    ...
    sample_low_res, sample_high_res = next(iter(val_loader))
    sample_low_res = sample_low_res[:16]
    sample_high_res = sample_high_res[:16]
    samples = model.sample(
        schedule=schedule,
        low_res=sample_low_res,
        device=device_info.torch_device,
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "generative", "lesson": "lesson_15_compact_diffusion_super_resolution"},
    )
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.generative.lesson_15_compact_diffusion_super_resolution.train"
        )
