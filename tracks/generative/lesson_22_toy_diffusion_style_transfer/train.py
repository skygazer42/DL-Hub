...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig, DiffusionSchedule]:
    parser = argparse.ArgumentParser(
        description="Lesson 22 (Generative): toy diffusion-style content/style transfer."
    )
...
def _evaluate(
    model: ToyStyleTransferDiffusionModel,
...
def run_training(
    ...
    paths = build_run_paths(
        track="generative",
        lesson="lesson_22_toy_diffusion_style_transfer",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("generative.toy_diffusion_style_transfer", log_file=paths.logs_dir / "train.log")
    ...
    model = ToyStyleTransferDiffusionModel(model_cfg).to(device_info.torch_device)
    ...
    save_checkpoint(
        ...
        extra={"track": "generative", "lesson": "lesson_22_toy_diffusion_style_transfer"},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.generative.lesson_22_toy_diffusion_style_transfer.train"
        )
