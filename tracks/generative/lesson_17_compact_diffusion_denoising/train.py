...
from .model import DiffusionSchedule, ModelConfig, CompactDenoisingDiffusionModel, q_sample
...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig, DiffusionSchedule]:
    parser = argparse.ArgumentParser(
        description="Lesson 17 (Generative): compact diffusion-style denoising from paired noisy/clean synthetic images."
    )
    ...
    parser.add_argument("--impulse-prob", type=float, default=0.015)
...
def _evaluate(
    model: CompactDenoisingDiffusionModel,
    ...
    *,
    ...
    with torch.no_grad():
        for batch_idx, (noisy, clean) in enumerate(loader):
            ...
            pred_noise = model(xt=xt, noisy=noisy, timesteps=timesteps)
...
def run_training(
    ...
    paths = build_run_paths(
        track="generative",
        lesson="lesson_17_compact_diffusion_denoising",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("generative.compact_diffusion_denoising", log_file=paths.logs_dir / "train.log")
    ...
    model = CompactDenoisingDiffusionModel(model_cfg).to(device_info.torch_device)
    ...
    for epoch in range(1, train_cfg.epochs + 1):
        ...
        for batch_idx, (noisy, clean) in enumerate(train_loader):
            if train_cfg.max_train_batches is not None and batch_idx >= train_cfg.max_train_batches:
                break
            noisy = noisy.to(device_info.torch_device)
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "generative", "lesson": "lesson_17_compact_diffusion_denoising"},
    )
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.generative.lesson_17_compact_diffusion_denoising.train"
        )
