...
class CompactDenoisingDiffusionModel(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        features = torch.cat([xt, noisy, t_map], dim=1)
    ...
    def sample(
        ...
        *,
        ...
        for step in range(step_count - 1, -1, -1):
            ...
            pred_noise = self(xt=xt, noisy=noisy, timesteps=t)
        ...
        denoised = xt.clamp(0.0, 1.0).cpu()
        if frames is None:
            return denoised
        return torch.stack(frames[1:] + [denoised], dim=0)
