...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig, DiffusionSchedule]:
    parser = argparse.ArgumentParser(
        description="Lesson 34 (Generative): toy diffusion layout-reference fusion from layout and reference images."
    )
...
def _evaluate(
    model: ToyLayoutReferenceFusionDiffusionModel,
...
def run_training(
    ...
    paths = build_run_paths(
        track="generative",
        lesson="lesson_34_toy_diffusion_layout_reference_fusion",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("generative.toy_diffusion_layout_reference_fusion", log_file=paths.logs_dir / "train.log")
    ...
    model = ToyLayoutReferenceFusionDiffusionModel(model_cfg).to(device_info.torch_device)
    ...
    save_checkpoint(
        ...
        extra={"track": "generative", "lesson": "lesson_34_toy_diffusion_layout_reference_fusion"},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.generative.lesson_34_toy_diffusion_layout_reference_fusion.train"
        )
