...
class CompactLayoutPaletteFusionDiffusionModel(nn.Module):
    def __init__(self, cfg: ModelConfig) -> None:
        ...
        palette_dim = int(cfg.palette_dim)
        net_in_channels = in_channels + layout_channels + palette_dim + 1
    ...
    def forward(
        ...
        *,
        ...
        palette_dim = int(self.cfg.palette_dim)
        palette_map = (
            palette_code.to(dtype=xt.dtype)
            .view(batch_size, palette_dim, 1, 1)
            .expand(batch_size, palette_dim, height, width)
        )
        features = torch.cat([xt, layout.to(dtype=xt.dtype), palette_map, t_map], dim=1)
    ...
    def sample(
        ...
        *,
        ...
        palette_code = palette_code.to(device=device, dtype=torch.float32)
        ...
        for step in range(step_count - 1, -1, -1):
            ...
            pred_noise = self(xt=xt, layout=layout, palette_code=palette_code, timesteps=t)
...
__all__ = ["DiffusionSchedule", "ModelConfig", "CompactLayoutPaletteFusionDiffusionModel", "q_sample"]
