...
def _sample_palette_code(palette_dim: int, generator: torch.Generator) -> torch.Tensor:
    if palette_dim != 6:
        raise ValueError("palette_dim must be 6 for this synthetic lesson")
...
def _make_synthetic_layout_palette_fusion_data(cfg: DataConfig) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
...
class SyntheticLayoutPaletteFusionDataset(Dataset):
    def __init__(self, cfg: DataConfig) -> None:
        self.cfg = cfg
        self._layout, self._palette_code, self._target = _make_synthetic_layout_palette_fusion_data(cfg)
...
def get_dataloaders(cfg: DataConfig) -> tuple[DataLoader, DataLoader]:
    dataset: Dataset = SyntheticLayoutPaletteFusionDataset(cfg)
...
__all__ = ["DataConfig", "SyntheticLayoutPaletteFusionDataset", "get_dataloaders"]
