...
class CompactBoxMaskEditingDiffusionModel(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        features = torch.cat([xt, source, box_mask, t_map], dim=1)
    ...
    def sample(
        ...
        *,
        ...
        box_mask = box_mask.to(device=device, dtype=torch.float32)
        ...
        xt = source * (1.0 - box_mask) + torch.randn_like(source) * box_mask
        ...
        for step in range(step_count - 1, -1, -1):
            ...
            pred_noise = self(xt=xt, source=source, box_mask=box_mask, timesteps=t)
            ...
            if step > 0:
                xt = source * (1.0 - box_mask) + (mean + torch.sqrt(beta_t) * torch.randn_like(xt)) * box_mask
            else:
                xt = source * (1.0 - box_mask) + mean * box_mask
...
__all__ = ["DiffusionSchedule", "ModelConfig", "CompactBoxMaskEditingDiffusionModel", "q_sample"]
