...
from .model import DiffusionSchedule, ModelConfig, CompactDerainingDiffusionModel, q_sample
...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig, DiffusionSchedule]:
    parser = argparse.ArgumentParser(
        description="Lesson 18 (Generative): compact diffusion-style image deraining (去雨) from paired rainy/clean images."
    )
...
def _evaluate(
    model: CompactDerainingDiffusionModel,
    ...
    *,
    ...
    total_seen = 0
    with torch.no_grad():
        for batch_idx, (rainy, clean) in enumerate(loader):
...
def run_training(
    ...
    paths = build_run_paths(
        track="generative",
        lesson="lesson_18_compact_diffusion_deraining",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("generative.compact_diffusion_deraining", log_file=paths.logs_dir / "train.log")
    ...
    model = CompactDerainingDiffusionModel(model_cfg).to(device_info.torch_device)
    ...
    for epoch in range(1, train_cfg.epochs + 1):
        ...
        for batch_idx, (rainy, clean) in enumerate(train_loader):
            if train_cfg.max_train_batches is not None and batch_idx >= train_cfg.max_train_batches:
                break
            rainy = rainy.to(device_info.torch_device)
    ...
    sample_rainy, sample_clean = next(iter(val_loader))
    sample_rainy = sample_rainy[:16]
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "generative", "lesson": "lesson_18_compact_diffusion_deraining"},
    )
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.generative.lesson_18_compact_diffusion_deraining.train"
        )
