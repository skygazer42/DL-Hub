...
class CompactDerainingDiffusionModel(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        features = torch.cat([xt, rainy, t_map], dim=1)
    ...
    def sample(
        ...
        *,
        ...
        rainy = rainy.to(device=device, dtype=torch.float32)
        n = int(rainy.shape[0])
        ...
        for step in range(step_count - 1, -1, -1):
            ...
            pred_noise = self(xt=xt, rainy=rainy, timesteps=t)
        ...
        derained = xt.clamp(0.0, 1.0).cpu()
        if frames is None:
            return derained
        return torch.stack(frames[1:] + [derained], dim=0)
