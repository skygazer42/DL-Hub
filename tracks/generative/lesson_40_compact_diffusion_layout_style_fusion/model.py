...
class CompactLayoutStyleFusionDiffusionModel(nn.Module):
    def __init__(self, cfg: ModelConfig) -> None:
        ...
        net_in_channels = in_channels * 2 + style_dim + 1
    ...
    def forward(
        ...
        *,
        ...
        style_dim = int(self.cfg.style_dim)
        style_map = (
            style_code.to(dtype=xt.dtype)
            .view(batch_size, style_dim, 1, 1)
            .expand(batch_size, style_dim, height, width)
        )
        features = torch.cat([xt, layout, style_map, t_map], dim=1)
    ...
    def sample(
        ...
        *,
        ...
        style_code = style_code.to(device=device, dtype=torch.float32)
        n = int(layout.shape[0])
        ...
        for step in range(step_count - 1, -1, -1):
            ...
            pred_noise = self(xt=xt, layout=layout, style_code=style_code, timesteps=t)
...
__all__ = ["DiffusionSchedule", "ModelConfig", "CompactLayoutStyleFusionDiffusionModel", "q_sample"]
