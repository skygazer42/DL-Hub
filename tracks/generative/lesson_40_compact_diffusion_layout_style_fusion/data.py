...
def _make_style_map(style_code: torch.Tensor, image_size: int, generator: torch.Generator) -> torch.Tensor:
    if int(style_code.numel()) != 3:
        raise ValueError("style_dim must be 3 for this synthetic lesson")
...
def _make_synthetic_layout_style_fusion_data(cfg: DataConfig) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
...
class SyntheticLayoutStyleFusionDataset(Dataset):
    def __init__(self, cfg: DataConfig) -> None:
        self.cfg = cfg
        self._layout, self._style_code, self._target = _make_synthetic_layout_style_fusion_data(cfg)
...
def get_dataloaders(cfg: DataConfig) -> tuple[DataLoader, DataLoader]:
    dataset: Dataset = SyntheticLayoutStyleFusionDataset(cfg)
...
__all__ = ["DataConfig", "SyntheticLayoutStyleFusionDataset", "get_dataloaders"]
