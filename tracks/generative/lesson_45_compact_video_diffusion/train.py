...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig, DiffusionSchedule]:
    parser = argparse.ArgumentParser(
        description="Lesson 45 (Generative): compact video diffusion from a keyframe and motion code."
    )
    ...
    parser.add_argument("--motion-dim", type=int, default=3)

    parser.add_argument("--in-channels", type=int, default=1)
    parser.add_argument("--hidden-channels", type=int, default=16)
    parser.add_argument("--time-embed-dim", type=int, default=16)

    parser.add_argument("--num-diffusion-steps", type=int, default=12)
...
def _evaluate(
    model: CompactVideoDiffusionModel,
    ...
    *,
    ...
    total_seen = 0
    with torch.no_grad():
        for batch_idx, (keyframe, motion_code, target_video) in enumerate(loader):
...
def run_training(
    ...
    paths = build_run_paths(
        track="generative",
        lesson="lesson_45_compact_video_diffusion",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("generative.compact_video_diffusion", log_file=paths.logs_dir / "train.log")
    ...
    model = CompactVideoDiffusionModel(model_cfg).to(device_info.torch_device)
    ...
    for epoch in range(1, int(train_cfg.epochs) + 1):
        ...
        for batch_idx, (keyframe, motion_code, target_video) in enumerate(train_loader):
            if train_cfg.max_train_batches is not None and batch_idx >= train_cfg.max_train_batches:
                break
            keyframe = keyframe.to(device_info.torch_device)
            motion_code = motion_code.to(device_info.torch_device)
    ...
    model.eval()
    sample_keyframe, sample_motion_code, sample_target = next(iter(val_loader))
    sample_keyframe = sample_keyframe[:8]
    sample_motion_code = sample_motion_code[:8]
    sample_target = sample_target[:8]
    samples = model.sample(
        schedule=schedule,
        keyframe=sample_keyframe,
        motion_code=sample_motion_code,
        device=device_info.torch_device,
    ...
    torch.save(
        ...
        paths.run_dir / "video_diffusion_triplets.pt",
    )
    ...
    save_checkpoint(
        ...
        extra={"track": "generative", "lesson": "lesson_45_compact_video_diffusion"},
    )
    return 0
