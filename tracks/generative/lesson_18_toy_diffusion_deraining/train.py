...
from .model import DiffusionSchedule, ModelConfig, ToyDerainingDiffusionModel, q_sample
...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig, DiffusionSchedule]:
    parser = argparse.ArgumentParser(
        description="Lesson 18 (Generative): toy diffusion-style image deraining (去雨) from paired rainy/clean images."
    )
...
def _evaluate(
    model: ToyDerainingDiffusionModel,
...
def run_training(
    ...
    paths = build_run_paths(
        track="generative",
        lesson="lesson_18_toy_diffusion_deraining",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("generative.toy_diffusion_deraining", log_file=paths.logs_dir / "train.log")
    ...
    model = ToyDerainingDiffusionModel(model_cfg).to(device_info.torch_device)
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "generative", "lesson": "lesson_18_toy_diffusion_deraining"},
    )
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.generative.lesson_18_toy_diffusion_deraining.train"
        )
