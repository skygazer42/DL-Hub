...
def _identity_texture(image_size: int, generator: torch.Generator) -> torch.Tensor:
    yy, xx = torch.meshgrid(
        torch.linspace(-1.0, 1.0, image_size, dtype=torch.float32),
        torch.linspace(-1.0, 1.0, image_size, dtype=torch.float32),
        indexing="ij",
...
def _make_subject(identity_mask: torch.Tensor, texture: torch.Tensor, generator: torch.Generator) -> torch.Tensor:
    background = 0.06 + 0.08 * torch.rand(identity_mask.shape, generator=generator, dtype=torch.float32)
    subject = background * (1.0 - identity_mask) + (0.16 + 0.78 * texture) * identity_mask
    subject += 0.02 * torch.rand(identity_mask.shape, generator=generator, dtype=torch.float32)
...
def _make_target(subject: torch.Tensor, edit: torch.Tensor, generator: torch.Generator) -> torch.Tensor:
    # Blend an edit effect with the original subject so texture cues remain recognizable.
    local_shift = torch.roll(subject, shifts=1, dims=2)
    edited = (1.0 - 0.65 * edit) * subject + (0.65 * edit) * (0.7 * local_shift + 0.3 * (1.0 - subject))
    target = 0.86 * edited + 0.14 * subject
    target += 0.02 * torch.randn(subject.shape, generator=generator, dtype=torch.float32)
    target += 0.01 * torch.rand(subject.shape, generator=generator, dtype=torch.float32)
    return target.clamp(0.0, 1.0)


def _make_synthetic_identity_preserving_data(cfg: DataConfig) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    for _ in range(int(cfg.num_samples)):
        identity_mask = _shape_mask(int(cfg.image_size), generator)
        texture = _identity_texture(int(cfg.image_size), generator)
        subject = _make_subject(identity_mask, texture, generator)
        edit = _make_edit_map(int(cfg.image_size), generator)
        target = _make_target(subject, edit, generator)
        subjects.append(subject)
        edits.append(edit)
        targets.append(target)

    return torch.stack(subjects, dim=0), torch.stack(edits, dim=0), torch.stack(targets, dim=0)


class SyntheticIdentityPreservingEditingDataset(Dataset):
    def __init__(self, cfg: DataConfig) -> None:
        self.cfg = cfg
        self._subject, self._edit, self._target = _make_synthetic_identity_preserving_data(cfg)
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        i = int(idx)
        return self._subject[i], self._edit[i], self._target[i]


def get_dataloaders(cfg: DataConfig) -> tuple[DataLoader, DataLoader]:
    dataset: Dataset = SyntheticIdentityPreservingEditingDataset(cfg)
...
__all__ = ["DataConfig", "SyntheticIdentityPreservingEditingDataset", "get_dataloaders"]
