...
class CompactPolygonMaskEditingDiffusionModel(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        features = torch.cat([xt, source, polygon_mask, t_map], dim=1)
    ...
    def sample(
        ...
        *,
        ...
        polygon_mask = polygon_mask.to(device=device, dtype=torch.float32)
        ...
        xt = source * (1.0 - polygon_mask) + torch.randn_like(source) * polygon_mask
        ...
        for step in range(step_count - 1, -1, -1):
            ...
            pred_noise = self(xt=xt, source=source, polygon_mask=polygon_mask, timesteps=t)
            ...
            if step > 0:
                xt = source * (1.0 - polygon_mask) + (mean + torch.sqrt(beta_t) * torch.randn_like(xt)) * polygon_mask
            else:
                xt = source * (1.0 - polygon_mask) + mean * polygon_mask
...
__all__ = ["DiffusionSchedule", "ModelConfig", "CompactPolygonMaskEditingDiffusionModel", "q_sample"]
