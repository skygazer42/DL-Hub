...
def _make_condition(mask: torch.Tensor, generator: torch.Generator) -> torch.Tensor:
    noise = 0.06 * torch.rand(mask.shape, generator=generator, dtype=torch.float32)
    return (0.12 + 0.62 * mask + noise).clamp(0.0, 1.0)


def _make_target(mask: torch.Tensor, texture: torch.Tensor, generator: torch.Generator) -> torch.Tensor:
    background = 0.08 + 0.05 * torch.rand(mask.shape, generator=generator, dtype=torch.float32)
    foreground = 0.18 + 0.75 * texture
    target = background * (1.0 - mask) + foreground * mask
    target += 0.03 * torch.rand(mask.shape, generator=generator, dtype=torch.float32)
    return target.clamp(0.0, 1.0)


def _make_synthetic_synthesis_data(cfg: DataConfig) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    for _ in range(int(cfg.num_samples)):
        ...
        tex = _texture(int(cfg.image_size), generator)
        conditions.append(_make_condition(mask, generator))
        targets.append(_make_target(mask, tex, generator))

    return torch.stack(conditions, dim=0), torch.stack(targets, dim=0)


class SyntheticImageSynthesisDataset(Dataset):
    def __init__(self, cfg: DataConfig) -> None:
        self.cfg = cfg
        self._condition, self._target = _make_synthetic_synthesis_data(cfg)
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, torch.Tensor]:
        i = int(idx)
        return self._condition[i], self._target[i]


def get_dataloaders(cfg: DataConfig) -> tuple[DataLoader, DataLoader]:
    dataset: Dataset = SyntheticImageSynthesisDataset(cfg)
...
__all__ = ["DataConfig", "SyntheticImageSynthesisDataset", "get_dataloaders"]
