...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig, DiffusionSchedule]:
    parser = argparse.ArgumentParser(
        description=(
            "Lesson 24 (Generative): compact conditional diffusion image synthesis "
            "with synthetic condition-target pairs."
        )
    )
...
def _evaluate(
    model: CompactImageSynthesisDiffusionModel,
    ...
    *,
    ...
    with torch.no_grad():
        for batch_idx, (condition, target) in enumerate(loader):
            ...
            pred_noise = model(xt=xt, condition=condition, timesteps=timesteps)
...
def run_training(
    ...
    paths = build_run_paths(
        track="generative",
        lesson="lesson_24_compact_diffusion_image_synthesis",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("generative.compact_diffusion_image_synthesis", log_file=paths.logs_dir / "train.log")
    ...
    model = CompactImageSynthesisDiffusionModel(model_cfg).to(device_info.torch_device)
    ...
    sample_condition, sample_target = next(iter(val_loader))
    ...
    torch.save({"frames": trajectory}, paths.run_dir / "trajectory.pt")
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "generative", "lesson": "lesson_24_compact_diffusion_image_synthesis"},
    )
