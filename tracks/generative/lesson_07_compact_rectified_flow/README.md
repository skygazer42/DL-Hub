# Lesson 07: Compact Rectified Flow

This lesson continues from compact flow matching with a rectified-flow framing:
...
```bash
python -m tracks.generative.lesson_07_compact_rectified_flow.train --epochs 1 --num-samples 48 --batch-size 8 --max-train-batches 2 --max-eval-batches 1 --device cpu --run-name smoke
```

## Outputs

`outputs/generative/lesson_07_compact_rectified_flow/<run_name>/`
