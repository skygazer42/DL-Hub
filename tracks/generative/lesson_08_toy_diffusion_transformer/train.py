...
from .model import DiTTiny, DiffusionSchedule, ModelConfig, q_sample
...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig, DiffusionSchedule]:
    parser = argparse.ArgumentParser(
        description="Lesson 08 (Generative): toy diffusion with a tiny DiT-style denoiser."
    )
...
def run_training(
    ...
    paths = build_run_paths(
        track="generative",
        lesson="lesson_08_toy_diffusion_transformer",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("generative.toy_diffusion_transformer", log_file=paths.logs_dir / "train.log")
    ...
    model = DiTTiny(model_cfg).to(device_info.torch_device)
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "generative", "lesson": "lesson_08_toy_diffusion_transformer"},
    )
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.generative.lesson_08_toy_diffusion_transformer.train"
        )
