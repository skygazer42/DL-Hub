...
class CompactMultiReferenceDiffusionModel(nn.Module):
    def __init__(self, cfg: ModelConfig) -> None:
        super().__init__()
        self.cfg = cfg
        in_channels = int(cfg.in_channels)
        hidden = int(cfg.hidden_channels)
        net_in_channels = in_channels * 4 + 1
    ...
    def forward(
        ...
        *,
        ...
        features = torch.cat([xt, reference_a, reference_b, condition, t_map], dim=1)
    ...
    def sample(
        ...
        *,
        ...
        reference_a = reference_a.to(device=device, dtype=torch.float32)
        reference_b = reference_b.to(device=device, dtype=torch.float32)
        condition = condition.to(device=device, dtype=torch.float32)
        n = int(reference_a.shape[0])
        ...
        xt = torch.randn_like(reference_a)
...
__all__ = ["DiffusionSchedule", "ModelConfig", "CompactMultiReferenceDiffusionModel", "q_sample"]
