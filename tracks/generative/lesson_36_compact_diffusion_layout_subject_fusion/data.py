...
def _subject_texture(image_size: int, generator: torch.Generator) -> torch.Tensor:
    ...
    if mode == 0:
        ...
        texture = 0.5 + 0.5 * torch.sin(xx * 7.5 + yy * 2.5 + phase)
    elif mode == 1:
        texture = 0.5 + 0.5 * torch.cos(yy * 10.0)
    elif mode == 2:
        texture = (((xx * 7.0).floor() + (yy * 7.0).floor()) % 2.0 == 0.0).to(torch.float32)
    elif mode == 3:
        radial = (xx.pow(2) + yy.pow(2)).sqrt()
        texture = 0.5 + 0.5 * torch.cos(radial * 12.0)
    else:
        texture = 0.5 + 0.5 * torch.sin((xx + yy) * 5.5) * torch.cos((xx - yy) * 4.5)
    texture = texture.unsqueeze(0)
    texture += 0.05 * torch.rand((1, image_size, image_size), generator=generator, dtype=torch.float32)
    return texture.clamp(0.0, 1.0)


def _make_subject_style(image_size: int, generator: torch.Generator) -> torch.Tensor:
    texture = _subject_texture(image_size, generator)
    envelope = 0.45 + 0.55 * _shape_mask(image_size, generator)
    subject_style = 0.2 + 0.8 * texture * envelope
    subject_style += 0.03 * torch.rand(subject_style.shape, generator=generator, dtype=torch.float32)
    return subject_style.clamp(0.0, 1.0)


def _make_target(layout: torch.Tensor, subject_style: torch.Tensor, generator: torch.Generator) -> torch.Tensor:
    structure = (layout > 0.3).to(layout.dtype)
    shifted_style = torch.roll(subject_style, shifts=1, dims=2)
    mirrored_style = torch.flip(subject_style, dims=(1,))
    fused_style = 0.6 * shifted_style + 0.4 * mirrored_style
    foreground = (0.15 + 0.75 * fused_style) * structure
    contour = 0.22 * torch.roll(layout, shifts=1, dims=1) + 0.78 * layout
    background = (0.05 + 0.14 * (1.0 - layout)) * (1.0 - structure)
    target = foreground + background
    target = 0.68 * target + 0.32 * contour
...
def _make_synthetic_layout_subject_fusion_data(cfg: DataConfig) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    generator = torch.Generator().manual_seed(int(cfg.seed))
    layouts: list[torch.Tensor] = []
    subject_styles: list[torch.Tensor] = []
    targets: list[torch.Tensor] = []

    for _ in range(int(cfg.num_samples)):
        layout = _make_layout(int(cfg.image_size), generator)
        subject_style = _make_subject_style(int(cfg.image_size), generator)
        target = _make_target(layout, subject_style, generator)
        layouts.append(layout)
        subject_styles.append(subject_style)
        targets.append(target)

    return torch.stack(layouts, dim=0), torch.stack(subject_styles, dim=0), torch.stack(targets, dim=0)


class SyntheticLayoutSubjectFusionDataset(Dataset):
    def __init__(self, cfg: DataConfig) -> None:
        self.cfg = cfg
        self._layout, self._subject_style, self._target = _make_synthetic_layout_subject_fusion_data(cfg)
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        i = int(idx)
        return self._layout[i], self._subject_style[i], self._target[i]


def get_dataloaders(cfg: DataConfig) -> tuple[DataLoader, DataLoader]:
    dataset: Dataset = SyntheticLayoutSubjectFusionDataset(cfg)
...
__all__ = ["DataConfig", "SyntheticLayoutSubjectFusionDataset", "get_dataloaders"]
