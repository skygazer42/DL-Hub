...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig, DiffusionSchedule]:
    parser = argparse.ArgumentParser(
        description="Lesson 25 (Generative): toy diffusion compositional generation from structure and style."
    )
...
def _evaluate(
    model: ToyCompositionalDiffusionModel,
...
def run_training(
    ...
    paths = build_run_paths(
        track="generative",
        lesson="lesson_25_toy_diffusion_compositional_generation",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("generative.toy_diffusion_compositional_generation", log_file=paths.logs_dir / "train.log")
    ...
    model = ToyCompositionalDiffusionModel(model_cfg).to(device_info.torch_device)
    ...
    save_checkpoint(
        ...
        extra={"track": "generative", "lesson": "lesson_25_toy_diffusion_compositional_generation"},
    )
    return 0
