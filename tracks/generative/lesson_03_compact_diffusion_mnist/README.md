# Lesson 03: Compact Diffusion (MNIST-like, minimal DDPM)

This lesson keeps diffusion compact-first:
...
```bash
python -m tracks.generative.lesson_03_compact_diffusion_mnist.train --dataset fake --epochs 1 --max-train-batches 2 --max-eval-batches 1 --device cpu --run-name smoke
```

If `torchvision` is installed, you can switch to real MNIST:

```bash
python -m tracks.generative.lesson_03_compact_diffusion_mnist.train --dataset mnist --epochs 5
```

## Outputs

`outputs/generative/lesson_03_compact_diffusion_mnist/<run_name>/`
