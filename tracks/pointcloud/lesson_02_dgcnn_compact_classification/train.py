...
from .model import DGCNNClassifier, ModelConfig
...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 02 (PointCloud): DGCNN compact classification (Cube vs Sphere)."
    )
    ...
    parser.add_argument("--dynamic-graph", action=argparse.BooleanOptionalAction, default=True)
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="pointcloud", lesson="lesson_02_dgcnn_compact_classification", run_name=train_cfg.run_name
    )
    logger = get_logger("pointcloud.compact_dgcnn", log_file=paths.logs_dir / "train.log")
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "pointcloud", "lesson": "lesson_02_dgcnn_compact_classification"},
    )
