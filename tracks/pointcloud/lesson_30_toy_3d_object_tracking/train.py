...
from .model import ModelConfig, ToyObjectTracker, tracking_loss
...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 30 (PointCloud): toy 3D object tracking with synthetic pointcloud pairs."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig, model_cfg: ModelConfig) -> int:
    ...
    paths = build_run_paths(
        track="pointcloud",
        lesson="lesson_30_toy_3d_object_tracking",
    ...
    model = ToyObjectTracker(model_cfg).to(device_info.torch_device)
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "pointcloud", "lesson": "lesson_30_toy_3d_object_tracking"},
    )
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.pointcloud.lesson_30_toy_3d_object_tracking.train"
        )
