...
class TrainConfig:
    ...
    arch: str = "swav_pointnet:swav_pointnet_small"
    dropout: float = 0.0
...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(description="Lesson 16 (PointCloud): self-supervised SwAV (toy-first).")
    ...
    parser.add_argument(
        "--arch",
        type=str,
        default="swav_pointnet:swav_pointnet_small",
        help="Supported: swav_pointnet:<variant> (try --list-arch)",
    )
    ...
    parser.add_argument("--sinkhorn-iters", type=int, default=3)
    ...
    *,
    ...
    for step, (v1, v2, _y) in enumerate(loader):
        ...
        loss = swav_loss(
            ...
            sinkhorn_epsilon=float(train_cfg.sinkhorn_epsilon),
            sinkhorn_iters=int(train_cfg.sinkhorn_iters),
        )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="pointcloud",
        lesson="lesson_16_pointcloud_selfsupervised_swav",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("pointcloud.ssl_swav", log_file=paths.logs_dir / "train.log")
    ...
    model = build_model(
        ModelConfig(
            ...
            num_prototypes=train_cfg.num_prototypes,
        )
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "pointcloud", "lesson": "lesson_16_pointcloud_selfsupervised_swav"},
    )
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.pointcloud.lesson_16_pointcloud_selfsupervised_swav.train"
        )
