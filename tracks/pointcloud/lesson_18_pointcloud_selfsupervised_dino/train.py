...
class TrainConfig:
    ...
    arch: str = "dino_pointnet:dino_pointnet_small"
    dropout: float = 0.0
...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(description="Lesson 18 (PointCloud): self-supervised DINO (toy-first).")
    ...
    parser.add_argument(
        "--arch",
        type=str,
        default="dino_pointnet:dino_pointnet_small",
        help="Supported: dino_pointnet:<variant> (try --list-arch)",
    )
    parser.add_argument("--list-arch", action="store_true", help="Print supported architectures and exit.")
    parser.add_argument("--dropout", type=float, default=0.0)
    parser.add_argument("--out-dim", type=int, default=None, help="Override #prototypes/out_dim.")
    ...
    *,
    ...
    for step, (v1, v2, _y) in enumerate(loader):
        ...
        if is_train:
            ...
            model.update_center([t1, t2], center_momentum=float(train_cfg.center_momentum))
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="pointcloud",
        lesson="lesson_18_pointcloud_selfsupervised_dino",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("pointcloud.ssl_dino", log_file=paths.logs_dir / "train.log")
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "pointcloud", "lesson": "lesson_18_pointcloud_selfsupervised_dino"},
    )
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.pointcloud.lesson_18_pointcloud_selfsupervised_dino.train"
        )
