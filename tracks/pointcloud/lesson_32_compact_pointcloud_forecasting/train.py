...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 32 (PointCloud): compact pointcloud forecasting."
    )
    ...
    parser.add_argument("--motion-scale", type=float, default=0.18)
    ...
    parser.add_argument(
        "--arch",
        type=str,
        default="trajpoint_forecast:trajpoint_forecast_small",
        help="Supported: trajpoint_forecast:<variant> (try --list-arch).",
    )
...
def _run_epoch(
    *,
    ...
    for batch_idx, (history, targets) in enumerate(loader):
        ...
        loss, parts = forecasting_loss(outputs, target_batch)
        ...
        total_mae += float(parts["step_mae"])
        total_batches += 1

    if total_batches == 0:
        return Stats(loss=0.0, mae=0.0)
    return Stats(loss=total_loss / total_batches, mae=total_mae / total_batches)
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig, model_cfg: ModelConfig) -> int:
    ...
    paths = build_run_paths(
        track="pointcloud",
        lesson="lesson_32_compact_pointcloud_forecasting",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("pointcloud.forecasting", log_file=paths.logs_dir / "train.log")
    ...
    for epoch in range(1, int(train_cfg.epochs) + 1):
        ...
        logger.info(
            "Epoch %d/%d | train loss %.6f | train mae %.6f | eval loss %.6f | eval mae %.6f",
            epoch,
            train_cfg.epochs,
            train_stats.loss,
            train_stats.mae,
    ...
    save_checkpoint(
        ...
        extra={"track": "pointcloud", "lesson": "lesson_32_compact_pointcloud_forecasting"},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.pointcloud.lesson_32_compact_pointcloud_forecasting.train"
        )
