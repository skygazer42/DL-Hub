...
from .model import ModelConfig, ToyInstanceSegmentation3DNet, instance_segmentation_loss
...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 29 (PointCloud): toy 3D instance segmentation."
    )
...
def _run_epoch(
    *,
    model: ToyInstanceSegmentation3DNet,
    loader,
    device: torch.device,
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig, model_cfg: ModelConfig) -> int:
    ...
    paths = build_run_paths(
        track="pointcloud",
        lesson="lesson_29_toy_3d_instance_segmentation",
    ...
    model = ToyInstanceSegmentation3DNet(model_cfg).to(device_info.torch_device)
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "pointcloud", "lesson": "lesson_29_toy_3d_instance_segmentation"},
    )
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.pointcloud.lesson_29_toy_3d_instance_segmentation.train"
        )
