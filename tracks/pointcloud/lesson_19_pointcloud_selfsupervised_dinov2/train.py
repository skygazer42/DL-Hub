...
class TrainConfig:
    ...
    arch: str = "dinov2_pointmae:dinov2_pointmae_small"
    dropout: float = 0.0
...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(description="Lesson 19 (PointCloud): self-supervised DINOv2 (toy-first).")
    ...
    parser.add_argument(
        "--arch",
        type=str,
        default="dinov2_pointmae:dinov2_pointmae_small",
        help="Supported: dinov2_pointmae:<variant> (try --list-arch)",
    )
    ...
    parser.add_argument("--lambda-ibot", type=float, default=1.0)
...
def _run_epoch(
    *,
    ...
    for step, (v1, v2, _y) in enumerate(loader):
        ...
        loss = loss_cls + float(train_cfg.lambda_ibot) * loss_patch
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="pointcloud",
        lesson="lesson_19_pointcloud_selfsupervised_dinov2",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("pointcloud.ssl_dinov2", log_file=paths.logs_dir / "train.log")
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "pointcloud", "lesson": "lesson_19_pointcloud_selfsupervised_dinov2"},
    )
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.pointcloud.lesson_19_pointcloud_selfsupervised_dinov2.train"
        )
