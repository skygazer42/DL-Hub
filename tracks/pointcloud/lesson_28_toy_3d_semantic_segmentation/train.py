...
from .model import ModelConfig, ToyPointNetSemanticSeg3D, segmentation3d_loss
...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 28 (PointCloud): toy 3D semantic segmentation."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig, model_cfg: ModelConfig) -> int:
    ...
    paths = build_run_paths(
        track="pointcloud",
        lesson="lesson_28_toy_3d_semantic_segmentation",
    ...
    model = ToyPointNetSemanticSeg3D(model_cfg).to(device_info.torch_device)
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "pointcloud", "lesson": "lesson_28_toy_3d_semantic_segmentation"},
    )
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.pointcloud.lesson_28_toy_3d_semantic_segmentation.train"
        )
