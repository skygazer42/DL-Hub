...
class TrainConfig:
    ...
    arch: str = "simsiam_pointnet:simsiam_pointnet_small"
    dropout: float = 0.0


def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 15 (PointCloud): self-supervised SimSiam (compact-first)."
    )
...
def _run_epoch(
    *,
    ...
    for step, (v1, v2, _y) in enumerate(loader):
        ...
        out1 = model(v1)
        out2 = model(v2)
        loss = simsiam_loss(out1["p"], out2["z"], out2["p"], out1["z"])
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="pointcloud",
        lesson="lesson_15_pointcloud_selfsupervised_simsiam",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("pointcloud.ssl_simsiam", log_file=paths.logs_dir / "train.log")
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "pointcloud", "lesson": "lesson_15_pointcloud_selfsupervised_simsiam"},
    )
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.pointcloud.lesson_15_pointcloud_selfsupervised_simsiam.train"
        )
