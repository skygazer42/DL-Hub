...
class TrainConfig:
    ...
    run_name: str = "dev"
    arch: str = "pointnet_ae:pointnet_ae_tiny"
    dropout: float = 0.0


def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 24 (PointCloud): compact pointcloud completion with a PointNet autoencoder."
    )
    ...
    parser.add_argument("--visible-fraction", type=float, default=0.55)
    parser.add_argument("--p-sphere", type=float, default=0.5)
    parser.add_argument("--partial-noise-std", type=float, default=0.02)
    ...
    parser.add_argument(
        "--arch",
        type=str,
        default="pointnet_ae:pointnet_ae_tiny",
    ...
    data_cfg = DataConfig(
        ...
        partial_noise_std=args.partial_noise_std,
...
def _run_epoch(
    *,
    ...
    total = 0
    total_loss = 0.0
    for step, (partial, complete) in enumerate(loader):
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="pointcloud",
        lesson="lesson_24_compact_pointcloud_completion",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("pointcloud.completion", log_file=paths.logs_dir / "train.log")
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "pointcloud", "lesson": "lesson_24_compact_pointcloud_completion"},
    )
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.pointcloud.lesson_24_compact_pointcloud_completion.train"
        )
