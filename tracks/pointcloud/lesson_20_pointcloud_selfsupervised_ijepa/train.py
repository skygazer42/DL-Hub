...
class TrainConfig:
    ...
    arch: str = "ijepa_pointmae:ijepa_pointmae_small"
    dropout: float = 0.0
...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 20 (PointCloud): self-supervised I-JEPA (compact-first)."
    )
...
def _run_epoch(
    *,
    ...
    for step, (points, _y) in enumerate(loader):
        ...
        out = model.forward_student(points, mask_ratio=float(train_cfg.mask_ratio))
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="pointcloud",
        lesson="lesson_20_pointcloud_selfsupervised_ijepa",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("pointcloud.ssl_ijepa", log_file=paths.logs_dir / "train.log")
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "pointcloud", "lesson": "lesson_20_pointcloud_selfsupervised_ijepa"},
    )
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.pointcloud.lesson_20_pointcloud_selfsupervised_ijepa.train"
        )
