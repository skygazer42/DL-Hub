...
class TrainConfig:
    ...
    arch: str = "byol_pointnet:byol_pointnet_small"
    dropout: float = 0.0
    ema_decay: float = 0.99


def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(description="Lesson 11 (PointCloud): self-supervised BYOL (toy-first).")
    ...
    parser.add_argument(
        "--arch",
        type=str,
        default="byol_pointnet:byol_pointnet_small",
        help="Supported: byol_pointnet:<variant> (try --list-arch)",
    )
    ...
    *,
    ...
    for step, (v1, v2, _y) in enumerate(loader):
        ...
        if is_train:
            loss.backward()
            optimizer.step()
            model.update_target(ema_decay=float(ema_decay))
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="pointcloud",
        lesson="lesson_11_pointcloud_selfsupervised_byol",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("pointcloud.ssl_byol", log_file=paths.logs_dir / "train.log")
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "pointcloud", "lesson": "lesson_11_pointcloud_selfsupervised_byol"},
    )
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.pointcloud.lesson_11_pointcloud_selfsupervised_byol.train"
        )
