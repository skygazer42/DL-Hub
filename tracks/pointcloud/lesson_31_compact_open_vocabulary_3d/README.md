...
The compact setup uses synonym variants in the text prompts to mimic open-vocabulary
behavior while staying lightweight for CPU smoke runs.

## Run

```bash
python -m tracks.pointcloud.lesson_31_compact_open_vocabulary_3d.train --run-name dev
python scripts/run_lesson.py pointcloud lesson_31_compact_open_vocabulary_3d --dry-run
```

## Outputs

Runs are written to `outputs/pointcloud/lesson_31_compact_open_vocabulary_3d/<run-name>/`
