...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 31 (PointCloud): compact open-vocabulary 3D recognition and grounding."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="pointcloud",
        lesson="lesson_31_compact_open_vocabulary_3d",
    ...
    vocab_ds = SyntheticOpenVocabulary3DDataset(data_cfg)
    ...
    model = CompactOpenVocabulary3DModel(model_cfg).to(device_info.torch_device)
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "pointcloud", "lesson": "lesson_31_compact_open_vocabulary_3d"},
    )
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.pointcloud.lesson_31_compact_open_vocabulary_3d.train"
        )
