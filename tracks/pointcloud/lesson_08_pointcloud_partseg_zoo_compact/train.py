...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 08 (PointCloud): part-seg zoo on compact partseg dataset."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="pointcloud",
        lesson="lesson_08_pointcloud_partseg_zoo_compact",
    ...
    model = build_model(
        ModelConfig(
            ...
            hidden_features=int(train_cfg.hidden_features),
            dropout=float(train_cfg.dropout),
            k=int(train_cfg.k),
            dynamic_graph=bool(train_cfg.dynamic_graph),
        )
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "pointcloud", "lesson": "lesson_08_pointcloud_partseg_zoo_compact"},
    )
