...
def _augment(
    ...
    *,
    ...
    js = float(jitter_std)
    if js > 0:
        noise = torch.randn(x.shape, generator=g, dtype=torch.float32) * js
        x = x + noise

    # Point dropout.
    dp = float(drop_p)
    if dp > 0:
        mask = torch.rand((x.shape[0],), generator=g, dtype=torch.float32) < dp
...
class DataConfig:
    ...
    # Strong (student) vs weak (teacher) augmentation strengths.
    strong_jitter_std: float = 0.02
    strong_drop_p: float = 0.2
    weak_jitter_std: float = 0.005
...
class ToyReSSLViewsDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        strong = _augment(
            ...
            g=g1,
            cfg=cfg,
            jitter_std=float(cfg.strong_jitter_std),
            drop_p=float(cfg.strong_drop_p),
        )
        weak = _augment(
            base,
            g=g2,
            cfg=cfg,
            jitter_std=float(cfg.weak_jitter_std),
            drop_p=float(cfg.weak_drop_p),
        )
...
def get_dataloaders(cfg: DataConfig) -> tuple[DataLoader, DataLoader]:
    ...
    def _collate(batch):
        v_strong = torch.stack([b[0] for b in batch], dim=0)
        v_weak = torch.stack([b[1] for b in batch], dim=0)
        y = torch.stack([b[2] for b in batch], dim=0)
        return v_strong, v_weak, y
