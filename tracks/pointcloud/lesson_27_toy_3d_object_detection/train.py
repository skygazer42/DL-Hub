...
from .model import ModelConfig, ToyDetector3D, detection3d_loss
...
def parse_args() -> tuple[TrainConfig, DataConfig, ModelConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 27 (PointCloud): toy 3D object detection on synthetic point clouds."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig, model_cfg: ModelConfig) -> int:
    ...
    paths = build_run_paths(
        track="pointcloud",
        lesson="lesson_27_toy_3d_object_detection",
    ...
    model = ToyDetector3D(model_cfg).to(device_info.torch_device)
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "pointcloud", "lesson": "lesson_27_toy_3d_object_detection"},
    )
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.pointcloud.lesson_27_toy_3d_object_detection.train"
        )
