...
def _augment(points: torch.Tensor, *, g: torch.Generator, cfg) -> torch.Tensor:
    """Point cloud augmentations for contrastive learning."""
    ...
    # Shuffle order (set is unordered anyway; helpful for robustness).
...
class ToySSLViewsDataset(Dataset):
    """Return two augmented views + label (label used only for optional evaluation)."""
...
def get_dataloaders(cfg: DataConfig) -> tuple[DataLoader, DataLoader]:
    ...
    def _collate(batch):
        v1 = torch.stack([b[0] for b in batch], dim=0)  # (B, N, 3)
