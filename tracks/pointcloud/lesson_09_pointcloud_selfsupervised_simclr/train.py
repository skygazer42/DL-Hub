...
class TrainConfig:
    ...
    arch: str = "simclr_pointnet:simclr_pointnet_small"
    dropout: float = 0.0
    temperature: float = 0.2


def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(description="Lesson 09 (PointCloud): self-supervised SimCLR (toy-first).")
    ...
    parser.add_argument(
        ...
        help="Supported: simclr_pointnet:<variant> (try --list-arch)",
    )
...
def _run_epoch(
    *,
    ...
    for step, (v1, v2, _y) in enumerate(loader):
        ...
        z1 = out1["z"]
        z2 = out2["z"]
        loss = nt_xent_loss(z1, z2, temperature=float(temperature))
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="pointcloud",
        lesson="lesson_09_pointcloud_selfsupervised_simclr",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("pointcloud.ssl_simclr", log_file=paths.logs_dir / "train.log")
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "pointcloud", "lesson": "lesson_09_pointcloud_selfsupervised_simclr"},
    )
