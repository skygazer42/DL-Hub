...
class TrainConfig:
    ...
    arch: str = "barlowtwins_pointnet:barlowtwins_pointnet_small"
    dropout: float = 0.0
...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(description="Lesson 17 (PointCloud): self-supervised Barlow Twins (toy-first).")
    ...
    parser.add_argument(
        "--arch",
        type=str,
        default="barlowtwins_pointnet:barlowtwins_pointnet_small",
        help="Supported: barlowtwins_pointnet:<variant> (try --list-arch)",
    )
    parser.add_argument("--list-arch", action="store_true", help="Print supported architectures and exit.")
    parser.add_argument("--dropout", type=float, default=0.0)

    parser.add_argument("--lambda-offdiag", type=float, default=0.005)
...
def _run_epoch(
    *,
    ...
    for step, (v1, v2, _y) in enumerate(loader):
        ...
        out1 = model(v1)
        out2 = model(v2)
        loss = barlow_twins_loss(out1["z"], out2["z"], lambda_offdiag=float(lambda_offdiag))
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="pointcloud",
        lesson="lesson_17_pointcloud_selfsupervised_barlowtwins",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("pointcloud.ssl_barlowtwins", log_file=paths.logs_dir / "train.log")
    ...
    for epoch in range(1, int(train_cfg.epochs) + 1):
        train_loss = _run_epoch(
            ...
            lambda_offdiag=float(train_cfg.lambda_offdiag),
        )
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "pointcloud", "lesson": "lesson_17_pointcloud_selfsupervised_barlowtwins"},
    )
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.pointcloud.lesson_17_pointcloud_selfsupervised_barlowtwins.train"
        )
