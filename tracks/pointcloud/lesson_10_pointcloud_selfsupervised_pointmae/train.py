...
class TrainConfig:
    ...
    arch: str = "pointmae:pointmae_small"
    dropout: float = 0.0
    mask_ratio: float = 0.6


def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(description="Lesson 10 (PointCloud): self-supervised PointMAE (toy-first).")
    ...
    parser.add_argument(
        "--arch",
        type=str,
        default="pointmae:pointmae_small",
        help="Supported: pointmae:<variant> (try --list-arch)",
    )
...
def _run_epoch(
    *,
    ...
    for step, (points, _y) in enumerate(loader):
        ...
        out = model(points, mask_ratio=float(mask_ratio))
        pred = out["pred"]  # (B, M, K, 3)
        target = out["target"]
        pred_f = pred.reshape(-1, pred.shape[-2], 3)
        target_f = target.reshape(-1, target.shape[-2], 3)
        loss = chamfer_distance(pred_f, target_f)

        if is_train:
            loss.backward()
            optimizer.step()

        bs = int(points.size(0))
        total += bs
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="pointcloud",
        lesson="lesson_10_pointcloud_selfsupervised_pointmae",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("pointcloud.ssl_pointmae", log_file=paths.logs_dir / "train.log")
    ...
    for epoch in range(1, int(train_cfg.epochs) + 1):
        ...
        logger.info("Epoch %d/%d | train loss %.6f | eval loss %.6f", epoch, train_cfg.epochs, train_loss, eval_loss)
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "pointcloud", "lesson": "lesson_10_pointcloud_selfsupervised_pointmae"},
    )
