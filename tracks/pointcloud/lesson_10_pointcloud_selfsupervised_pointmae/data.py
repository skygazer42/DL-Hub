...
def _sample_sphere(*, num_points: int, g: torch.Generator, noise_std: float = 0.0) -> torch.Tensor:
...
def _sample_cube_surface(
    *, num_points: int, g: torch.Generator, noise_std: float = 0.0
    ...
    ns = float(noise_std)
    if ns > 0.0:
        pts = pts + torch.randn((n, 3), generator=g, dtype=torch.float32) * ns
    return pts
...
class SyntheticMAEDataset(Dataset):
    """Single-view SSL dataset: returns (points, label). Label is for optional probing only."""
...
__all__ = ["DataConfig", "SyntheticMAEDataset", "get_dataloaders"]
