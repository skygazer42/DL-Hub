...
from .model import ModelConfig, PointNetClassifier
...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 01 (PointCloud): PointNet compact classification (Cube vs Sphere)."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="pointcloud",
        lesson="lesson_01_pointnet_compact_classification",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("pointcloud.compact_pointnet", log_file=paths.logs_dir / "train.log")
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "pointcloud", "lesson": "lesson_01_pointnet_compact_classification"},
    )
