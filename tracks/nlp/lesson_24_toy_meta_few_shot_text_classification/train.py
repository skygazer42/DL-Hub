...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 24 (NLP): toy meta few-shot text classification."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="nlp",
        lesson="lesson_24_toy_meta_few_shot_text_classification",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("nlp.toy_meta_few_shot_text", log_file=paths.logs_dir / "train.log")
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.nlp.lesson_24_toy_meta_few_shot_text_classification.train"
        )
