...
_TASKS = (
    {"name": "weather", "keyword": "rain", "context": "forecast", "action": "umbrella", "slot": "outside"},
    {"name": "navigation", "keyword": "route", "context": "traffic", "action": "map", "slot": "downtown"},
    {"name": "music", "keyword": "playlist", "context": "melody", "action": "speaker", "slot": "studio"},
    {"name": "booking", "keyword": "ticket", "context": "travel", "action": "hotel", "slot": "airport"},
    {"name": "reminder", "keyword": "alarm", "context": "calendar", "action": "schedule", "slot": "morning"},
    {"name": "lighting", "keyword": "lamp", "context": "brightness", "action": "switch", "slot": "livingroom"},
    {
        "name": "translation",
        "keyword": "phrase",
        "context": "language",
    ...
    {"name": "finance", "keyword": "budget", "context": "expense", "action": "report", "slot": "monthly"},
)

_SUPPORT_TEMPLATES = (
    "meta adapt task {name} uses {keyword} with {context} and {action}",
    "adapt support for {name} keeps {keyword} near {slot} during {context}",
    "episode support task {name} asks for {action} when {keyword} appears",
)

_QUERY_TEMPLATES = (
    "meta adapt query for {name} needs {action} after {keyword} in {context}",
    "which task adapts to {keyword} with {context} around {slot}",
    "episode query about {name} wants {action} plus {keyword}",
)
...
def _make_episode_specs(config: DataConfig) -> list[EpisodeSpec]:
    ...
    for _ in range(int(config.num_episodes)):
        chosen = rng.choice(len(_TASKS), size=int(config.num_ways), replace=False)
        ...
        for class_id, concept_idx in enumerate(chosen):
            concept = dict(_TASKS[int(concept_idx)])
            task_names.append(concept["name"])
...
class EpisodeDataset:
    ...
    def __getitem__(self, idx: int):
        ...
        return {
            ...
            "task_names": list(episode.task_names),
        }
...
__all__ = ["DataConfig", "EpisodeDataset", "EpisodeSpec", "get_dataloaders"]
