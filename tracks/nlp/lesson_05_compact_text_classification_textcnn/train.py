...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 05 (NLP): compact text classification with TextCNN."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="nlp", lesson="lesson_05_compact_text_classification_textcnn", run_name=train_cfg.run_name
    )
    logger = get_logger("nlp.compact_textcnn", log_file=paths.logs_dir / "train.log")
    ...
    model = TextCNNClassifier(
        ModelConfig(
            ...
            num_filters=train_cfg.num_filters,
        )
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={
            "track": "nlp",
            "lesson": "lesson_05_compact_text_classification_textcnn",
            "vocab_size": vocab.size,
        },
    )
