...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 10 (NLP): toy prompt tuning classification with a frozen encoder."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="nlp",
        lesson="lesson_10_toy_prompt_tuning_classifier",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("nlp.prompt_tuning", log_file=paths.logs_dir / "train.log")
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.nlp.lesson_10_toy_prompt_tuning_classifier.train"
        )
