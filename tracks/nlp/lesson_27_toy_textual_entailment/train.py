...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(description="Lesson 27 (NLP): toy textual entailment.")
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="nlp",
        lesson="lesson_27_toy_textual_entailment",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("nlp.toy_textual_entailment", log_file=paths.logs_dir / "train.log")
    ...
    save_checkpoint(
        ...
        extra={"track": "nlp", "lesson": "lesson_27_toy_textual_entailment", "vocab_size": vocab.size},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.nlp.lesson_27_toy_textual_entailment.train"
        )
