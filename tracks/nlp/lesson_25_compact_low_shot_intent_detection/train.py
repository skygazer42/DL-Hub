...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 25 (NLP): compact low-shot intent detection."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="nlp",
        lesson="lesson_25_compact_low_shot_intent_detection",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("nlp.compact_low_shot_intent", log_file=paths.logs_dir / "train.log")
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.nlp.lesson_25_compact_low_shot_intent_detection.train"
        )
