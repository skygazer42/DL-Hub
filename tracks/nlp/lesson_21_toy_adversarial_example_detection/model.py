...
class AdversarialExampleDetector(nn.Module):
...
def detection_accuracy(logits: torch.Tensor, labels: torch.Tensor) -> float:
    pred = (torch.sigmoid(logits) >= 0.5).to(torch.float32)
    return float((pred == labels.to(torch.float32)).to(torch.float32).mean().item())


__all__ = ["AdversarialExampleDetector", "ModelConfig", "detection_accuracy", "detection_loss"]
