...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 21 (NLP): toy adversarial example detection."
    )
    parser.add_argument("--num-samples", type=int, default=1024)
    parser.add_argument("--batch-size", type=int, default=64)
    parser.add_argument("--max-length", type=int, default=16)
    parser.add_argument("--adversarial-fraction", type=float, default=0.5)
...
def _run_epoch(
    *,
    ...
    for step, batch in enumerate(loader):
        ...
        total_acc += detection_accuracy(logits.detach(), batch["labels"].detach())
        total_batches += 1
    if total_batches == 0:
        return EpochStats(loss=0.0, acc=0.0)
    return EpochStats(loss=total_loss / total_batches, acc=total_acc / total_batches)
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="nlp",
        lesson="lesson_21_toy_adversarial_example_detection",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("nlp.toy_adversarial_example_detection", log_file=paths.logs_dir / "train.log")
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.nlp.lesson_21_toy_adversarial_example_detection.train"
        )
