...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 22 (NLP): toy weak-supervision text classification."
    )
    parser.add_argument("--num-samples", type=int, default=1024)
    parser.add_argument("--batch-size", type=int, default=64)
    parser.add_argument("--max-length", type=int, default=16)
    parser.add_argument("--num-labeling-functions", type=int, default=3)
    ...
    data_cfg = DataConfig(
        ...
        num_labeling_functions=args.num_labeling_functions,
...
def _run_epoch(
    *,
    ...
    for step, batch in enumerate(loader):
        ...
        total_acc += weak_supervision_accuracy(logits.detach(), batch["gold_labels"].detach())
        total_batches += 1
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="nlp",
        lesson="lesson_22_toy_weak_supervision_text_classification",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("nlp.toy_weak_supervision", log_file=paths.logs_dir / "train.log")
    ...
    model = WeakSupervisionTextClassifier(
        ModelConfig(
            ...
            num_labeling_functions=int(data_cfg.num_labeling_functions),
    ...
    save_checkpoint(
        ...
        extra={"track": "nlp", "lesson": "lesson_22_toy_weak_supervision_text_classification"},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.nlp.lesson_22_toy_weak_supervision_text_classification.train"
        )
