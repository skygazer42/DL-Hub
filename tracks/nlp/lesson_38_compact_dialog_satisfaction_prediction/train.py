...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(description="Lesson 38 (NLP): compact dialog satisfaction prediction.")
...
def _run_epoch(
    *,
    model: DialogSatisfactionClassifier,
    loader,
    device: torch.device,
    ...
    for step, batch in enumerate(loader):
        ...
        logits = model(batch)
        loss = dialog_satisfaction_loss(logits, batch["labels"])
        if is_train:
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="nlp",
        lesson="lesson_38_compact_dialog_satisfaction_prediction",
        run_name=train_cfg.run_name,
    )
    logger = get_logger(
        "nlp.compact_dialog_satisfaction_prediction",
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.nlp.lesson_38_compact_dialog_satisfaction_prediction.train"
        )
