...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 04 (NLP): compact seq2seq generation with attention."
    )
...
def _run_epoch(
    *,
    ...
    for step, (inputs, targets) in enumerate(loader):
        ...
        loss = criterion(logits.view(-1, logits.shape[-1]), tgt_out_ids.view(-1))
        ...
        with torch.no_grad():
            b, t = tgt_out_ids.shape
            pred = logits.argmax(dim=2)

            mask = (tgt_mask > 0).to(torch.bool)
            correct_tokens += int(((pred == tgt_out_ids) & mask).sum().item())
            total_tokens += int(mask.sum().item())

            # exact match over non-pad positions
            is_exact = ((pred == tgt_out_ids) | (~mask)).all(dim=1)
            exact += int(is_exact.sum().item())
            total_seqs += int(b)

            total_loss += float(loss.item()) * int(b)

    denom_b = max(1, total_seqs)
    denom_tok = max(1, total_tokens)
    return Stats(
        loss=total_loss / denom_b,
        token_acc=float(correct_tokens) / denom_tok,
        exact_match=float(exact) / denom_b,
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="nlp",
        lesson="lesson_04_compact_seq2seq_attention_generation",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("nlp.compact_seq2seq_att", log_file=paths.logs_dir / "train.log")
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.nlp.lesson_04_compact_seq2seq_attention_generation.train"
        )
