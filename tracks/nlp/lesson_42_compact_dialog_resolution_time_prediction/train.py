...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(description="Lesson 42 (NLP): compact dialog resolution time prediction.")
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="nlp",
        lesson="lesson_42_compact_dialog_resolution_time_prediction",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("nlp.compact_dialog_resolution_time_prediction", log_file=paths.logs_dir / "train.log")
    ...
    model = DialogResolutionTimeClassifier(model_cfg).to(device_info.torch_device)
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.nlp.lesson_42_compact_dialog_resolution_time_prediction.train"
        )
