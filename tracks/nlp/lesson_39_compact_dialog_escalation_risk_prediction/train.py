...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(description="Lesson 39 (NLP): compact dialog escalation risk prediction.")
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="nlp",
        lesson="lesson_39_compact_dialog_escalation_risk_prediction",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("nlp.compact_dialog_escalation_risk_prediction", log_file=paths.logs_dir / "train.log")
    ...
    model = DialogEscalationRiskClassifier(model_cfg).to(device_info.torch_device)
    ...
    save_checkpoint(
        ...
        extra={"track": "nlp", "lesson": "lesson_39_compact_dialog_escalation_risk_prediction", "vocab_size": vocab.size},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.nlp.lesson_39_compact_dialog_escalation_risk_prediction.train"
        )
