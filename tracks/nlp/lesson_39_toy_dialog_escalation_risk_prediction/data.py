...
RISK_TO_ID = {name: idx for idx, name in enumerate(RISK_LEVELS)}

ISSUES = ("billing", "refund", "delivery", "reservation")
CHANNELS = ("dialog", "chat", "call")
MOODS = ("calm", "frustrated", "upset")
AGENT_STATES = ("available", "delayed", "offline")
...
def _build_example(risk_level: str, rng: np.random.Generator) -> tuple[str, int]:
    ...
    if risk_level == "low":
        text = (
            f"customer {channel} issue {issue} mood calm agent {agent_state} response clear "
            f"followup scheduled escalation risk low"
        )
    elif risk_level == "medium":
        text = (
            f"customer {channel} issue {issue} mood frustrated agent delayed response partial "
            f"update requested escalation risk medium"
        )
    else:
        text = (
            f"customer {channel} issue {issue} mood upset urgent agent offline response failed "
            f"manager needed escalation risk high"
        )
    return text, int(RISK_TO_ID[risk_level])
...
def _make_examples(config: DataConfig) -> list[tuple[str, int]]:
    ...
    for _ in range(int(config.num_samples)):
        risk_level = str(rng.choice(RISK_LEVELS))
        examples.append(_build_example(risk_level, rng))
    rng.shuffle(examples)
    return examples


class DialogEscalationRiskDataset:
...
def get_dataloaders(config: DataConfig):
    ...
    dataset = DialogEscalationRiskDataset(examples=examples, vocab=vocab, max_length=int(config.max_length))
...
__all__ = ["DataConfig", "DialogEscalationRiskDataset", "get_dataloaders"]
