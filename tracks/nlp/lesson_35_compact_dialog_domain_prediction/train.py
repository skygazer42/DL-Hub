...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(description="Lesson 35 (NLP): compact dialog domain prediction.")
...
def _run_epoch(
    *,
    ...
    for step, batch in enumerate(loader):
        ...
        logits = model(batch)
        loss = dialog_domain_loss(logits, batch["labels"])
        if is_train:
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="nlp",
        lesson="lesson_35_compact_dialog_domain_prediction",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("nlp.compact_dialog_domain_prediction", log_file=paths.logs_dir / "train.log")
    ...
    save_checkpoint(
        ...
        extra={"track": "nlp", "lesson": "lesson_35_compact_dialog_domain_prediction", "vocab_size": vocab.size},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.nlp.lesson_35_compact_dialog_domain_prediction.train"
        )
