...
TRANSFER_LEVELS = ("low", "medium", "high")
TRANSFER_TO_ID = {name: idx for idx, name in enumerate(TRANSFER_LEVELS)}
...
def _build_example(transfer_level: str, rng: np.random.Generator) -> tuple[str, int]:
    ...
    if transfer_level == "low":
        text = (
            f"customer {channel} issue {issue} mood calm agent {agent_state} "
            f"transfer optional specialist standby transfer low"
        )
    elif transfer_level == "medium":
        text = (
            f"customer {channel} issue {issue} mood frustrated agent delayed "
            f"transfer suggested specialist review transfer medium"
        )
    else:
        text = (
            f"customer {channel} issue {issue} mood upset urgent agent blocked "
            f"transfer required specialist needed transfer high"
        )
    return text, int(TRANSFER_TO_ID[transfer_level])
...
def _make_examples(config: DataConfig) -> list[tuple[str, int]]:
    ...
    for _ in range(int(config.num_samples)):
        transfer_level = str(rng.choice(TRANSFER_LEVELS))
        examples.append(_build_example(transfer_level, rng))
    rng.shuffle(examples)
    return examples


class DialogTransferDataset:
...
def get_dataloaders(config: DataConfig):
    ...
    dataset = DialogTransferDataset(examples=examples, vocab=vocab, max_length=int(config.max_length))
...
__all__ = ["DataConfig", "DialogTransferDataset", "get_dataloaders"]
