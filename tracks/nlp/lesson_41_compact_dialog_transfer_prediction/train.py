...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(description="Lesson 41 (NLP): compact dialog transfer prediction.")
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="nlp",
        lesson="lesson_41_compact_dialog_transfer_prediction",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("nlp.compact_dialog_transfer_prediction", log_file=paths.logs_dir / "train.log")
    ...
    model = DialogTransferClassifier(model_cfg).to(device_info.torch_device)
    ...
    save_checkpoint(
        ...
        extra={"track": "nlp", "lesson": "lesson_41_compact_dialog_transfer_prediction", "vocab_size": vocab.size},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.nlp.lesson_41_compact_dialog_transfer_prediction.train"
        )
