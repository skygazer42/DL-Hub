...
class ModelConfig:
    vocab_size: int
    pad_id: int
    num_satisfaction_levels: int = 3
    embed_dim: int = 64
    dropout: float = 0.1


class DialogSatisfactionClassifier(nn.Module):
    def __init__(self, config: ModelConfig) -> None:
        ...
        self.head = nn.Linear(int(config.embed_dim), int(config.num_satisfaction_levels))
...
def dialog_satisfaction_loss(logits: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
