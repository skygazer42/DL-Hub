...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 06 (NLP): compact text classification with BiLSTM."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="nlp", lesson="lesson_06_compact_text_classification_bilstm", run_name=train_cfg.run_name
    )
    logger = get_logger("nlp.compact_bilstm_cls", log_file=paths.logs_dir / "train.log")
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={
            "track": "nlp",
            "lesson": "lesson_06_compact_text_classification_bilstm",
            "vocab_size": vocab.size,
        },
    )
