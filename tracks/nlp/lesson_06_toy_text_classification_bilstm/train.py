...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(description="Lesson 06 (NLP): toy text classification with BiLSTM.")
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="nlp", lesson="lesson_06_toy_text_classification_bilstm", run_name=train_cfg.run_name
    )
    logger = get_logger("nlp.toy_bilstm_cls", log_file=paths.logs_dir / "train.log")
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "nlp", "lesson": "lesson_06_toy_text_classification_bilstm", "vocab_size": vocab.size},
    )
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.nlp.lesson_06_toy_text_classification_bilstm.train"
        )
