...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 09 (NLP): compact encoder-decoder transformer summarization."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="nlp",
        lesson="lesson_09_compact_transformer_summarization",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("nlp.compact_transformer_summarization", log_file=paths.logs_dir / "train.log")
