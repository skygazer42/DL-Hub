...
class ModelConfig:
    vocab_size: int
    pad_id: int
    num_sla_levels: int = 2
    embed_dim: int = 64
    dropout: float = 0.1


class DialogSlaBreachClassifier(nn.Module):
    def __init__(self, config: ModelConfig) -> None:
        ...
        self.head = nn.Linear(int(config.embed_dim), int(config.num_sla_levels))
...
def dialog_sla_breach_loss(logits: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
...
__all__ = ["DialogSlaBreachClassifier", "ModelConfig", "compute_accuracy", "dialog_sla_breach_loss"]
