...
SLA_LEVELS = ("ok", "breach")
SLA_TO_ID = {name: idx for idx, name in enumerate(SLA_LEVELS)}

ISSUES = ("billing", "refund", "delivery", "reservation")
CHANNELS = ("dialog", "chat", "call")
MOODS = ("calm", "frustrated", "urgent")
WAIT_BUCKETS = ("short", "medium", "long")
...
def _build_example(sla_level: str, rng: np.random.Generator) -> tuple[str, int]:
    issue = str(rng.choice(ISSUES))
    channel = str(rng.choice(CHANNELS))
    mood = str(rng.choice(MOODS))
    wait_bucket = str(rng.choice(WAIT_BUCKETS))
    agent_state = str(rng.choice(AGENT_STATES))
    minutes = int(rng.integers(2, 90))

    if sla_level == "ok":
        text = (
            f"customer {channel} issue {issue} mood calm agent {agent_state} "
            f"sla ok minutes {minutes} wait {wait_bucket}"
        )
    else:
        text = (
            f"customer {channel} issue {issue} mood {mood} agent delayed "
            f"sla breach minutes {minutes} wait long breach"
        )
    return text, int(SLA_TO_ID[sla_level])
...
def _make_examples(config: DataConfig) -> list[tuple[str, int]]:
    ...
    for _ in range(int(config.num_samples)):
        sla_level = str(rng.choice(SLA_LEVELS))
        examples.append(_build_example(sla_level, rng))
    rng.shuffle(examples)
    return examples


class DialogSlaBreachDataset:
...
def get_dataloaders(config: DataConfig):
    ...
    dataset = DialogSlaBreachDataset(examples=examples, vocab=vocab, max_length=int(config.max_length))
...
__all__ = ["DataConfig", "DialogSlaBreachDataset", "get_dataloaders"]
