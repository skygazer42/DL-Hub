...
class SlotCarryoverPredictor(nn.Module):
    def __init__(self, config: ModelConfig) -> None:
        ...
        self.cuisine_head = nn.Linear(hidden_dim, 2)
        self.area_head = nn.Linear(hidden_dim, 2)
        self.party_head = nn.Linear(hidden_dim, 2)
...
def compute_slot_carryover_metrics(
    cuisine_logits: torch.Tensor,
    area_logits: torch.Tensor,
    party_logits: torch.Tensor,
    cuisine_labels: torch.Tensor,
    ...
    with torch.no_grad():
        ...
        joint_carry_acc = (cuisine_acc * area_acc * party_acc).mean()
    return {"slot_acc": float(slot_acc.item()), "joint_carry_acc": float(joint_carry_acc.item())}
