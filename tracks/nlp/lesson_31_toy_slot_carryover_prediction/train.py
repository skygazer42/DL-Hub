...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(description="Lesson 31 (NLP): toy slot carryover prediction.")
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="nlp",
        lesson="lesson_31_toy_slot_carryover_prediction",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("nlp.toy_slot_carryover_prediction", log_file=paths.logs_dir / "train.log")
    ...
    save_checkpoint(
        ...
        extra={"track": "nlp", "lesson": "lesson_31_toy_slot_carryover_prediction", "vocab_size": vocab.size},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.nlp.lesson_31_toy_slot_carryover_prediction.train"
        )
