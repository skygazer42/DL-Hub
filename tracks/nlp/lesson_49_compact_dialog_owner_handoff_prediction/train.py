...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(description="Lesson 49 (NLP): compact dialog owner handoff prediction.")
...
def _run_epoch(
    *,
    model: DialogOwnerHandoffClassifier,
    loader,
    device: torch.device,
    ...
    for step, batch in enumerate(loader):
        ...
        logits = model(batch)
        loss = dialog_owner_handoff_loss(logits, batch["labels"])
        if is_train:
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="nlp",
        lesson="lesson_49_compact_dialog_owner_handoff_prediction",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("nlp.compact_dialog_owner_handoff_prediction", log_file=paths.logs_dir / "train.log")
    ...
    model = DialogOwnerHandoffClassifier(model_cfg).to(device_info.torch_device)
    ...
    save_checkpoint(
        ...
        extra={"track": "nlp", "lesson": "lesson_49_compact_dialog_owner_handoff_prediction", "vocab_size": vocab.size},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.nlp.lesson_49_compact_dialog_owner_handoff_prediction.train"
        )
