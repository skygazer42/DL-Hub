...
DOMAINS = ("restaurant", "taxi", "hotel", "weather")
...
ROOM_TYPES = ("standard", "suite", "deluxe")
DAYS = ("today", "tomorrow", "friday")
...
def _build_example(domain: str, rng: np.random.Generator) -> tuple[str, int]:
    ...
    while dst == src:
        dst = str(rng.choice(PLACES))
    nights = str(rng.choice(NIGHTS))
    room = str(rng.choice(ROOM_TYPES))
    day = str(rng.choice(DAYS))
    state = str(rng.choice(WEATHER_STATES))

    if domain == "restaurant":
        text = (
            f"user domain restaurant need table for {party_size} in {area} at {time} "
            f"assistant confirm reservation details"
        )
    elif domain == "taxi":
        text = (
            f"user domain taxi pickup {src} destination {dst} at {time} "
            f"assistant confirm ride request"
        )
    elif domain == "hotel":
        text = (
            f"user domain hotel stay {nights} nights near {area} room {room} "
            f"assistant confirm booking details"
        )
    else:
        text = (
            f"user domain weather forecast for {area} on {day} expect {state} "
            f"assistant provide outlook"
        )
    return text, int(DOMAIN_TO_ID[domain])
...
def _make_examples(config: DataConfig) -> list[tuple[str, int]]:
    ...
    for _ in range(int(config.num_samples)):
        ...
        examples.append(_build_example(domain, rng))
    rng.shuffle(examples)
    return examples


class DialogDomainDataset:
...
def get_dataloaders(config: DataConfig):
    ...
    dataset = DialogDomainDataset(examples=examples, vocab=vocab, max_length=int(config.max_length))
...
__all__ = ["DataConfig", "DialogDomainDataset", "get_dataloaders"]
