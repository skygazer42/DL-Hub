...
class ModelConfig:
    vocab_size: int
    pad_id: int
    num_transfer_levels: int = 3
    embed_dim: int = 64
    dropout: float = 0.1


class DialogTransferClassifier(nn.Module):
    def __init__(self, config: ModelConfig) -> None:
        ...
        self.head = nn.Linear(int(config.embed_dim), int(config.num_transfer_levels))
...
def dialog_transfer_loss(logits: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
...
__all__ = ["DialogTransferClassifier", "ModelConfig", "compute_accuracy", "dialog_transfer_loss"]
