...
def _make_synthetic_examples(num_samples: int, seed: int) -> list[tuple[str, int]]:
...
def get_dataloaders(config: DataConfig):
    """Return `(train_loader, val_loader, vocab)` for the synthetic text task."""

    import torch
    from torch.utils.data import DataLoader, Subset

    examples = _make_synthetic_examples(num_samples=config.num_samples, seed=config.seed)
    vocab = _build_vocab([t for t, _ in examples])
    ds = SyntheticTextDataset(examples=examples, vocab=vocab, max_length=config.max_length)
...
__all__ = ["DataConfig", "Vocab", "SyntheticTextDataset", "get_dataloaders", "simple_tokenize"]
