...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(description="Lesson 45 (NLP): compact dialog followup channel prediction.")
...
def _run_epoch(
    *,
    model: DialogFollowupChannelClassifier,
    loader,
    device: torch.device,
    ...
    for step, batch in enumerate(loader):
        ...
        logits = model(batch)
        loss = dialog_followup_channel_loss(logits, batch["labels"])
        if is_train:
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="nlp",
        lesson="lesson_45_compact_dialog_followup_channel_prediction",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("nlp.compact_dialog_followup_channel_prediction", log_file=paths.logs_dir / "train.log")
    ...
    model = DialogFollowupChannelClassifier(model_cfg).to(device_info.torch_device)
    ...
    save_checkpoint(
        ...
        extra={"track": "nlp", "lesson": "lesson_45_compact_dialog_followup_channel_prediction", "vocab_size": vocab.size},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.nlp.lesson_45_compact_dialog_followup_channel_prediction.train"
        )
