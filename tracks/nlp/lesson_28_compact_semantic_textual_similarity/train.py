...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(description="Lesson 28 (NLP): compact semantic textual similarity.")
...
def _run_epoch(
    *,
    ...
    for step, batch in enumerate(loader):
        ...
        total_mae += mean_absolute_error(predictions.detach(), batch["scores"].detach())
        total_batches += 1
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="nlp",
        lesson="lesson_28_compact_semantic_textual_similarity",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("nlp.compact_semantic_textual_similarity", log_file=paths.logs_dir / "train.log")
    ...
    save_checkpoint(
        ...
        extra={"track": "nlp", "lesson": "lesson_28_compact_semantic_textual_similarity", "vocab_size": vocab.size},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.nlp.lesson_28_compact_semantic_textual_similarity.train"
        )
