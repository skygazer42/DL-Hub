...
from .model import ModelConfig, MetaFewShotTextClassifier, episode_accuracy, meta_episode_loss
...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 24 (NLP): compact meta few-shot text classification."
    )
...
def _run_epoch(
    *,
    ...
    for step, batch in enumerate(loader):
        ...
        assert isinstance(batch["query_labels"], torch.Tensor)
        ...
        batch_examples = int(batch["query_labels"].numel())
        total_examples += batch_examples
        total_loss += float(loss.item()) * batch_examples
        total_acc += episode_accuracy(outputs["logits"].detach(), batch["query_labels"].detach()) * batch_examples

    return Stats(
        loss=total_loss / max(1, total_examples),
        episode_acc=total_acc / max(1, total_examples),
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="nlp",
        lesson="lesson_24_compact_meta_few_shot_text_classification",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("nlp.compact_meta_few_shot_text", log_file=paths.logs_dir / "train.log")
    ...
    for epoch in range(1, int(train_cfg.epochs) + 1):
        ...
        logger.info(
            ...
            train_cfg.epochs,
            train_stats.loss,
            train_stats.episode_acc,
            eval_stats.loss,
            eval_stats.episode_acc,
        )
        append_jsonl(
            metrics_path,
            {
                ...
                "train_episode_acc": train_stats.episode_acc,
                "eval_loss": eval_stats.loss,
                "eval_episode_acc": eval_stats.episode_acc,
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.nlp.lesson_24_compact_meta_few_shot_text_classification.train"
        )
