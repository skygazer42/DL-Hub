...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 14 (NLP): compact contrastive sentence embeddings with two augmented views."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="nlp",
        lesson="lesson_14_compact_contrastive_sentence_embedding",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("nlp.compact_contrastive_sentence_embedding", log_file=paths.logs_dir / "train.log")
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.nlp.lesson_14_compact_contrastive_sentence_embedding.train"
        )
