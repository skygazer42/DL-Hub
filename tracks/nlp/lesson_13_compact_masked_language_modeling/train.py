...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 13 (NLP): compact masked language modeling with a tiny transformer encoder."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="nlp",
        lesson="lesson_13_compact_masked_language_modeling",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("nlp.compact_masked_lm", log_file=paths.logs_dir / "train.log")
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.nlp.lesson_13_compact_masked_language_modeling.train"
        )
