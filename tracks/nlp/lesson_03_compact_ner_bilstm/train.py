...
from .model import BiLstmNerTagger, ModelConfig
...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(description="Lesson 03 (NLP): compact NER with a BiLSTM tagger.")
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="nlp", lesson="lesson_03_compact_ner_bilstm", run_name=train_cfg.run_name
    )
    logger = get_logger("nlp.compact_ner_bilstm", log_file=paths.logs_dir / "train.log")
    ...
    train_loader, val_loader, vocab, tag_vocab = get_dataloaders(data_cfg)
    ...
    write_json(paths.run_dir / "tags.json", tag_vocab.to_dict())

    criterion = torch.nn.CrossEntropyLoss(ignore_index=int(data_cfg.ignore_index))
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={
            "track": "nlp",
            "lesson": "lesson_03_compact_ner_bilstm",
            "vocab_size": vocab.size,
            "num_tags": tag_vocab.size,
        },
    )
