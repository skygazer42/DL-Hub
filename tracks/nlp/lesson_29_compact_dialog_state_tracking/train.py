...
from .data import AREA_TO_ID, CUISINE_TO_ID, DataConfig, PARTY_TO_ID, get_dataloaders
...
class EpochStats:
    loss: float
    slot_acc: float
    joint_goal_acc: float


def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(description="Lesson 29 (NLP): compact dialog state tracking.")
...
def _run_epoch(
    *,
    ...
    for step, batch in enumerate(loader):
        ...
        total_joint_goal_acc += metrics["joint_goal_acc"]
        total_batches += 1
    ...
    return EpochStats(
        ...
        joint_goal_acc=total_joint_goal_acc / denom,
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="nlp",
        lesson="lesson_29_compact_dialog_state_tracking",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("nlp.compact_dialog_state_tracking", log_file=paths.logs_dir / "train.log")
    ...
    model = DialogStateTracker(
        ModelConfig(
            ...
            pad_id=vocab.pad_id,
            num_cuisine_states=len(CUISINE_TO_ID),
            num_area_states=len(AREA_TO_ID),
            num_party_states=len(PARTY_TO_ID),
    ...
    for epoch in range(1, int(train_cfg.epochs) + 1):
        ...
        logger.info(
            ...
            train_stats.joint_goal_acc,
            eval_stats.loss,
            eval_stats.slot_acc,
            eval_stats.joint_goal_acc,
        )
        append_jsonl(
            metrics_path,
            {
                ...
                "train_joint_goal_acc": train_stats.joint_goal_acc,
                "eval_loss": eval_stats.loss,
                "eval_slot_acc": eval_stats.slot_acc,
                "eval_joint_goal_acc": eval_stats.joint_goal_acc,
    ...
    save_checkpoint(
        ...
        extra={"track": "nlp", "lesson": "lesson_29_compact_dialog_state_tracking", "vocab_size": vocab.size},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.nlp.lesson_29_compact_dialog_state_tracking.train"
        )
