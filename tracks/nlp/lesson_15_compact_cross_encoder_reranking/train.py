...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 15 (NLP): compact cross-encoder reranking for query/document pairs."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="nlp",
        lesson="lesson_15_compact_cross_encoder_reranking",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("nlp.compact_cross_encoder_reranking", log_file=paths.logs_dir / "train.log")
