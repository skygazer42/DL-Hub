...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(description="Lesson 17 (NLP): toy text anomaly detection.")
...
def _run_epoch(
    *,
    ...
    for step, batch in enumerate(loader):
        ...
        total_acc += anomaly_accuracy(logits.detach(), batch["labels"].detach())
        total_batches += 1
    if total_batches == 0:
        return EpochStats(loss=0.0, anomaly_acc=0.0)
    return EpochStats(loss=total_loss / total_batches, anomaly_acc=total_acc / total_batches)
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="nlp",
        lesson="lesson_17_toy_text_anomaly_detection",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("nlp.toy_text_anomaly_detection", log_file=paths.logs_dir / "train.log")
    ...
    for epoch in range(1, int(train_cfg.epochs) + 1):
        ...
        logger.info(
            "Epoch %d/%d | train loss %.4f anomaly_acc %.3f | eval loss %.4f anomaly_acc %.3f",
            epoch,
            train_cfg.epochs,
            train_stats.loss,
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.nlp.lesson_17_toy_text_anomaly_detection.train"
        )
