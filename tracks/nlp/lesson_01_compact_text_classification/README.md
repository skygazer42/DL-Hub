# Lesson 01：Compact 文本分类（Embedding + Mean Pooling）
...
> 为什么是 compact？因为这样你可以先把“数据管线与模型形状”弄明白，再把同样结构迁移到 IMDb/AGNews 等真实数据集。
...
```
outputs/nlp/lesson_01_compact_text_classification/<run_name>/
  config.json
  metrics.jsonl
  vocab.json
