...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 10 (LLM): toy GRPO group-relative policy alignment."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="llm",
        lesson="lesson_10_toy_grpo_alignment",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("llm.toy_grpo_alignment", log_file=paths.logs_dir / "train.log")
    ...
    policy = ToyGrpoPolicyLM(model_cfg).to(device_info.torch_device)
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "llm", "lesson": "lesson_10_toy_grpo_alignment", "vocab_size": vocab.size},
    )
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.llm.lesson_10_toy_grpo_alignment.train"
        )
