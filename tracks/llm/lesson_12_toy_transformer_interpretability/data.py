...
class ToyInterpretabilityDataset:
...
def get_dataloaders(cfg: DataConfig):
    ...
    if int(cfg.seq_length) < 10:
        raise ValueError("seq_length must be >= 10 for toy transformer interpretability sequences")

    vocab = Vocab(base_vocab_size=int(cfg.base_vocab_size))
    input_ids, labels = _make_dataset_arrays(cfg, vocab)
    ds = ToyInterpretabilityDataset(input_ids=input_ids, labels=labels, vocab=vocab)
...
__all__ = ["DataConfig", "ToyInterpretabilityDataset", "Vocab", "get_dataloaders"]
