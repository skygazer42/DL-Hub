...
class Vocab:
    ...
    header_token_id: int = 2
    column_token_id: int = 3
    pipe_token_id: int = 4
    divider_token_id: int = 5
    newline_token_id: int = 6
    markdown_table_token_id: int = 7
    ...
    def to_dict(self) -> dict[str, int]:
        return {
            ...
            "header_token_id": int(self.header_token_id),
            "column_token_id": int(self.column_token_id),
            "pipe_token_id": int(self.pipe_token_id),
            "divider_token_id": int(self.divider_token_id),
            "newline_token_id": int(self.newline_token_id),
            "markdown_table_token_id": int(self.markdown_table_token_id),
...
def _build_example(*, prompt_id: int, vocab: Vocab, seq_length: int) -> tuple[np.ndarray, np.ndarray]:
    col_a = _offset_content_token(prompt_id, offset=1, vocab=vocab)
    col_b = _offset_content_token(prompt_id, offset=2, vocab=vocab)
    val_a = _offset_content_token(prompt_id, offset=3, vocab=vocab)
    val_b = _offset_content_token(prompt_id, offset=4, vocab=vocab)

    tokens = [
        int(vocab.prompt_token_id),
        int(prompt_id),
        int(vocab.separator_token_id),
        int(vocab.header_token_id),
        int(vocab.separator_token_id),
        int(vocab.column_token_id),
        int(col_a),
        int(vocab.pipe_token_id),
        int(col_b),
        int(vocab.newline_token_id),
        int(vocab.markdown_table_token_id),
        int(vocab.pipe_token_id),
        int(col_a),
        int(vocab.pipe_token_id),
        int(col_b),
    ...
    table_pos = tokens.index(int(vocab.markdown_table_token_id))
    targets = [
        int(vocab.markdown_table_token_id),
        int(vocab.pipe_token_id),
        int(col_a),
...
class SyntheticMarkdownTableConstrainedPromptingDataset:
...
def get_dataloaders(cfg: DataConfig):
    ...
    if int(cfg.seq_length) < 30:
        raise ValueError("seq_length must be >= 30 for synthetic markdown-table constrained prompting template")

    vocab = Vocab(base_vocab_size=int(cfg.base_vocab_size))
    input_ids, labels = _make_dataset_arrays(cfg, vocab)
    dataset = SyntheticMarkdownTableConstrainedPromptingDataset(input_ids=input_ids, labels=labels, vocab=vocab)
...
__all__ = ["DataConfig", "SyntheticMarkdownTableConstrainedPromptingDataset", "Vocab", "get_dataloaders"]
