...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(description="Lesson 41 (LLM): compact markdown-table constrained prompting.")
...
def generate_response(
    *,
    model: MarkdownTableConstrainedPromptingTransformerLM,
    device: torch.device,
    prompt_ids: list[int],
    max_new_tokens: int,
    pad_id: int,
...
def _sample_prompt(vocab: Vocab) -> list[int]:
    ...
    col_a = int(vocab.content_start_id + 1)
    col_b = int(vocab.content_start_id + 2)
    return [
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="llm",
        lesson="lesson_41_compact_markdown_table_constrained_prompting",
        run_name=train_cfg.run_name,
    )
    logger = get_logger(
        "llm.compact_markdown_table_constrained_prompting",
    ...
    save_checkpoint(
        ...
        extra={"track": "llm", "lesson": "lesson_41_compact_markdown_table_constrained_prompting"},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.llm.lesson_41_compact_markdown_table_constrained_prompting.train"
        )
