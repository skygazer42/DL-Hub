...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 29 (LLM): compact constraint-repair prompting with candidate repair spans."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="llm",
        lesson="lesson_29_compact_constraint_repair_prompting",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("llm.compact_constraint_repair_prompting", log_file=paths.logs_dir / "train.log")
