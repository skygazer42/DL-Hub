...
class Vocab:
    ...
    constraint_token_id: int = 2
    candidate_token_id: int = 3
    violates_token_id: int = 4
    repair_token_id: int = 5
    ...
    def to_dict(self) -> dict[str, int]:
        return {
            ...
            "constraint_token_id": int(self.constraint_token_id),
            "candidate_token_id": int(self.candidate_token_id),
            "violates_token_id": int(self.violates_token_id),
            "repair_token_id": int(self.repair_token_id),
...
def _build_example(*, prompt_id: int, vocab: Vocab, seq_length: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    repair_pos = tokens.index(int(vocab.repair_token_id))
    targets = repaired_answer + [int(vocab.eos_id)]
    for offset, token_id in enumerate(targets):
        pos = repair_pos + offset
...
class SyntheticConstraintRepairPromptingDataset:
...
def get_dataloaders(cfg: DataConfig):
    ...
    if int(cfg.seq_length) < 18:
        raise ValueError("seq_length must be >= 18 for synthetic constraint-repair prompting template")

    vocab = Vocab(base_vocab_size=int(cfg.base_vocab_size))
    input_ids, labels = _make_dataset_arrays(cfg, vocab)
    dataset = SyntheticConstraintRepairPromptingDataset(input_ids=input_ids, labels=labels, vocab=vocab)
...
__all__ = ["DataConfig", "SyntheticConstraintRepairPromptingDataset", "Vocab", "get_dataloaders"]
