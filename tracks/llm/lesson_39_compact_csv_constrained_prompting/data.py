...
class Vocab:
    ...
    def to_dict(self) -> dict[str, int]:
        return {
            ...
            "comma_token_id": int(self.comma_token_id),
            "newline_token_id": int(self.newline_token_id),
            "csv_token_id": int(self.csv_token_id),
...
def _build_example(*, prompt_id: int, vocab: Vocab, seq_length: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    tokens = [
        ...
        int(col_a),
        int(vocab.comma_token_id),
        int(col_b),
        int(vocab.newline_token_id),
        int(vocab.csv_token_id),
        int(col_a),
        int(vocab.comma_token_id),
        int(col_b),
        int(vocab.newline_token_id),
    ...
    csv_pos = tokens.index(int(vocab.csv_token_id))
    targets = [
        int(vocab.csv_token_id),
        int(col_a),
        int(vocab.comma_token_id),
...
class SyntheticCsvConstrainedPromptingDataset:
...
def get_dataloaders(cfg: DataConfig):
    ...
    if int(cfg.seq_length) < 22:
        raise ValueError("seq_length must be >= 22 for synthetic CSV constrained prompting template")

    vocab = Vocab(base_vocab_size=int(cfg.base_vocab_size))
    input_ids, labels = _make_dataset_arrays(cfg, vocab)
    dataset = SyntheticCsvConstrainedPromptingDataset(input_ids=input_ids, labels=labels, vocab=vocab)
...
__all__ = ["DataConfig", "SyntheticCsvConstrainedPromptingDataset", "Vocab", "get_dataloaders"]
