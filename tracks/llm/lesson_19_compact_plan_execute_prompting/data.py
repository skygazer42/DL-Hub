...
class Vocab:
    ...
    def to_dict(self) -> dict[str, int]:
        return {
            ...
            "plan_token_id": int(self.plan_token_id),
            "execute_token_id": int(self.execute_token_id),
...
def _build_example(*, prompt_id: int, vocab: Vocab, seq_length: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    tokens = [
        ...
        int(vocab.plan_token_id),
        *plan_ids,
        int(vocab.separator_token_id),
        int(vocab.execute_token_id),
        *execute_ids,
        int(vocab.eos_id),
    ]
    ...
    execute_pos = tokens.index(int(vocab.execute_token_id))
    targets = execute_ids + [int(vocab.eos_id)]
    for offset, target in enumerate(targets):
        pos = execute_pos + offset
...
class SyntheticPlanExecutePromptingDataset:
...
def get_dataloaders(cfg: DataConfig):
    ...
    if int(cfg.seq_length) < 12:
        raise ValueError("seq_length must be >= 12 for synthetic plan-execute prompting template")

    vocab = Vocab(base_vocab_size=int(cfg.base_vocab_size))
    input_ids, labels = _make_dataset_arrays(cfg, vocab)
    ds = SyntheticPlanExecutePromptingDataset(input_ids=input_ids, labels=labels, vocab=vocab)
...
__all__ = ["DataConfig", "SyntheticPlanExecutePromptingDataset", "Vocab", "get_dataloaders"]
