...
def _build_example(topic_id: int, vocab: Vocab, seq_length: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    assistant_pos = tokens.index(int(vocab.assistant_token_id))
...
def get_dataloaders(cfg: DataConfig):
    ...
    if int(cfg.seq_length) < 10:
        raise ValueError("seq_length must be >= 10 for the synthetic chat template")

    vocab = Vocab(base_vocab_size=int(cfg.base_vocab_size))
    input_ids, labels = _make_dataset_arrays(cfg, vocab)
    ds = SyntheticChatSFTDataset(input_ids=input_ids, labels=labels, vocab=vocab)
...
__all__ = ["DataConfig", "Vocab", "SyntheticChatSFTDataset", "get_dataloaders"]
