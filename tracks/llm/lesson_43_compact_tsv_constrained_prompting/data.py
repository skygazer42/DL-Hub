...
class Vocab:
    ...
    def to_dict(self) -> dict[str, int]:
        return {
            ...
            "tab_token_id": int(self.tab_token_id),
            "newline_token_id": int(self.newline_token_id),
            "tsv_token_id": int(self.tsv_token_id),
...
def _build_example(*, prompt_id: int, vocab: Vocab, seq_length: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    tokens = [
        ...
        int(col_a),
        int(vocab.tab_token_id),
        int(col_b),
        int(vocab.newline_token_id),
        int(vocab.tsv_token_id),
        int(col_a),
        int(vocab.tab_token_id),
        int(col_b),
        int(vocab.newline_token_id),
    ...
    table_pos = tokens.index(int(vocab.tsv_token_id))
    targets = [
        int(vocab.tsv_token_id),
        int(col_a),
        int(vocab.tab_token_id),
...
class SyntheticTsvConstrainedPromptingDataset:
...
def get_dataloaders(cfg: DataConfig):
    ...
    if int(cfg.seq_length) < 20:
        raise ValueError("seq_length must be >= 20 for synthetic TSV constrained prompting")

    vocab = Vocab(base_vocab_size=int(cfg.base_vocab_size))
    input_ids, labels = _make_dataset_arrays(cfg, vocab)
    dataset = SyntheticTsvConstrainedPromptingDataset(input_ids=input_ids, labels=labels, vocab=vocab)
...
__all__ = ["DataConfig", "SyntheticTsvConstrainedPromptingDataset", "Vocab", "get_dataloaders"]
