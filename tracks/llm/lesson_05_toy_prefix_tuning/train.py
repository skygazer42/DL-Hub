...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 02 (LLM): toy chat-format supervised fine-tuning."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(track="llm", lesson="lesson_02_toy_chat_sft", run_name=train_cfg.run_name)
    logger = get_logger("llm.toy_chat_sft", log_file=paths.logs_dir / "train.log")
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "llm", "lesson": "lesson_02_toy_chat_sft", "vocab_size": vocab.size},
    )
