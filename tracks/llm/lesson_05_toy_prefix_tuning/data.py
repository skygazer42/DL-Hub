...
def get_dataloaders(cfg: DataConfig):
    ...
    if int(cfg.seq_length) < 10:
        raise ValueError("seq_length must be >= 10 for the toy chat template")

    vocab = Vocab(base_vocab_size=int(cfg.base_vocab_size))
    input_ids, labels = _make_dataset_arrays(cfg, vocab)
    ds = ToyChatSFTDataset(input_ids=input_ids, labels=labels, vocab=vocab)
...
__all__ = ["DataConfig", "Vocab", "ToyChatSFTDataset", "get_dataloaders"]
