...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 20 (LLM): compact ReAct-style think/act/observe/final prompting."
    )
    parser.add_argument("--num-samples", type=int, default=4096)
    parser.add_argument("--batch-size", type=int, default=64)
    parser.add_argument("--seq-length", type=int, default=28)
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="llm",
        lesson="lesson_20_compact_react_tool_prompting",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("llm.compact_react_tool_prompting", log_file=paths.logs_dir / "train.log")
