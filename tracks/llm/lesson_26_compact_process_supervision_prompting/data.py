...
class Vocab:
    ...
    check_token_id: int = 3
    pass_token_id: int = 4
    fail_token_id: int = 5
    process_token_id: int = 6
    ...
    def to_dict(self) -> dict[str, int]:
        return {
            ...
            "check_token_id": int(self.check_token_id),
            "pass_token_id": int(self.pass_token_id),
            "fail_token_id": int(self.fail_token_id),
            "process_token_id": int(self.process_token_id),
...
def _build_example(*, prompt_id: int, vocab: Vocab, seq_length: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    process_pos = tokens.index(int(vocab.process_token_id))
    targets = refined_answer + [int(vocab.eos_id)]
    for offset, token_id in enumerate(targets):
        pos = process_pos + offset
...
class SyntheticProcessSupervisionPromptingDataset:
...
def get_dataloaders(cfg: DataConfig):
    ...
    if int(cfg.seq_length) < 20:
        raise ValueError("seq_length must be >= 20 for synthetic process supervision prompting")

    vocab = Vocab(base_vocab_size=int(cfg.base_vocab_size))
    input_ids, labels = _make_dataset_arrays(cfg, vocab)
    dataset = SyntheticProcessSupervisionPromptingDataset(input_ids=input_ids, labels=labels, vocab=vocab)
...
__all__ = ["DataConfig", "SyntheticProcessSupervisionPromptingDataset", "Vocab", "get_dataloaders"]
