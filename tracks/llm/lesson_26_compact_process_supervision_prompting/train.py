...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 26 (LLM): compact process supervision prompting with explicit checks."
    )
...
def generate_response(
    *,
    model: ProcessSupervisionTransformerLM,
    device: torch.device,
    prompt_ids: list[int],
    max_new_tokens: int,
    pad_id: int,
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="llm",
        lesson="lesson_26_compact_process_supervision_prompting",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("llm.compact_process_supervision_prompting", log_file=paths.logs_dir / "train.log")
    ...
    save_checkpoint(
        ...
        extra={"track": "llm", "lesson": "lesson_26_compact_process_supervision_prompting"},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.llm.lesson_26_compact_process_supervision_prompting.train"
        )
