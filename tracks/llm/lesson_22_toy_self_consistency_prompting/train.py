...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 22 (LLM): toy self-consistency prompting with sampled answers and voting."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="llm",
        lesson="lesson_22_toy_self_consistency_prompting",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("llm.toy_self_consistency_prompting", log_file=paths.logs_dir / "train.log")
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.llm.lesson_22_toy_self_consistency_prompting.train"
        )
