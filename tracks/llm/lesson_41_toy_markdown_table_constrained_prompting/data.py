...
class ToyMarkdownTableConstrainedPromptingDataset:
...
def get_dataloaders(cfg: DataConfig):
    ...
    if int(cfg.seq_length) < 30:
        raise ValueError("seq_length must be >= 30 for toy markdown-table constrained prompting template")

    vocab = Vocab(base_vocab_size=int(cfg.base_vocab_size))
    input_ids, labels = _make_dataset_arrays(cfg, vocab)
    dataset = ToyMarkdownTableConstrainedPromptingDataset(input_ids=input_ids, labels=labels, vocab=vocab)
...
__all__ = ["DataConfig", "ToyMarkdownTableConstrainedPromptingDataset", "Vocab", "get_dataloaders"]
