...
class ToyPlanExecutePromptingDataset:
...
def get_dataloaders(cfg: DataConfig):
    ...
    if int(cfg.seq_length) < 12:
        raise ValueError("seq_length must be >= 12 for toy plan-execute prompting template")

    vocab = Vocab(base_vocab_size=int(cfg.base_vocab_size))
    input_ids, labels = _make_dataset_arrays(cfg, vocab)
    ds = ToyPlanExecutePromptingDataset(input_ids=input_ids, labels=labels, vocab=vocab)
...
__all__ = ["DataConfig", "ToyPlanExecutePromptingDataset", "Vocab", "get_dataloaders"]
