...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 19 (LLM): toy two-stage plan-then-execute prompting."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="llm",
        lesson="lesson_19_toy_plan_execute_prompting",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("llm.toy_plan_execute_prompting", log_file=paths.logs_dir / "train.log")
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.llm.lesson_19_toy_plan_execute_prompting.train"
        )
