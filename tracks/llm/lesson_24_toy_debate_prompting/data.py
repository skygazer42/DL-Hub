...
class ToyDebatePromptingDataset:
...
def get_dataloaders(cfg: DataConfig):
    ...
    if int(cfg.seq_length) < 16:
        raise ValueError("seq_length must be >= 16 for toy debate prompting template")

    vocab = Vocab(base_vocab_size=int(cfg.base_vocab_size))
    input_ids, labels = _make_dataset_arrays(cfg, vocab)
    dataset = ToyDebatePromptingDataset(input_ids=input_ids, labels=labels, vocab=vocab)
...
__all__ = ["DataConfig", "ToyDebatePromptingDataset", "Vocab", "get_dataloaders"]
