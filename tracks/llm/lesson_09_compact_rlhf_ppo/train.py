...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(description="Lesson 09 (LLM): compact RLHF PPO policy fine-tuning.")
...
def ppo_policy_loss(
    *,
    ...
    # Compact simplification: the "old policy" in the ratio is the frozen
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="llm",
        lesson="lesson_09_compact_rlhf_ppo",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("llm.compact_rlhf_ppo", log_file=paths.logs_dir / "train.log")
    ...
    policy = CompactPolicyLM(model_cfg).to(device_info.torch_device)
    reference = CompactPolicyLM(model_cfg).to(device_info.torch_device)
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "llm", "lesson": "lesson_09_compact_rlhf_ppo", "vocab_size": vocab.size},
    )
