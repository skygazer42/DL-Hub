...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 01 (LLM): compact causal LM with Transformer decoder."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="llm", lesson="lesson_01_compact_causal_lm_transformer", run_name=train_cfg.run_name
    )
    logger = get_logger("llm.compact_causal_lm", log_file=paths.logs_dir / "train.log")
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={
            "track": "llm",
            "lesson": "lesson_01_compact_causal_lm_transformer",
            "vocab_size": vocab.size,
        },
    )
