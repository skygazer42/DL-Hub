...
class Vocab:
    ...
    score_token_id: int = 3
    rerank_token_id: int = 4
    ...
    def to_dict(self) -> dict[str, int]:
        return {
            ...
            "score_token_id": int(self.score_token_id),
            "rerank_token_id": int(self.rerank_token_id),
...
def _build_example(*, prompt_id: int, vocab: Vocab, seq_length: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    tokens = [
        ...
        *cand_a,
        int(vocab.score_token_id),
        int(vocab.separator_token_id),
        int(vocab.candidate_token_id),
        *cand_b,
        int(vocab.score_token_id),
        int(vocab.separator_token_id),
        int(vocab.rerank_token_id),
        *reranked,
        int(vocab.eos_id),
    ]
    ...
    rerank_pos = tokens.index(int(vocab.rerank_token_id))
    targets = reranked + [int(vocab.eos_id)]
    for offset, token_id in enumerate(targets):
        pos = rerank_pos + offset
...
class SyntheticCriticRerankPromptingDataset:
...
def get_dataloaders(cfg: DataConfig):
    ...
    if int(cfg.seq_length) < 18:
        raise ValueError("seq_length must be >= 18 for synthetic critic-rerank prompting template")

    vocab = Vocab(base_vocab_size=int(cfg.base_vocab_size))
    input_ids, labels = _make_dataset_arrays(cfg, vocab)
    dataset = SyntheticCriticRerankPromptingDataset(input_ids=input_ids, labels=labels, vocab=vocab)
...
__all__ = ["DataConfig", "SyntheticCriticRerankPromptingDataset", "Vocab", "get_dataloaders"]
