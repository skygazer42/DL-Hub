...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 23 (LLM): compact critic-rerank prompting with candidate scoring."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="llm",
        lesson="lesson_23_compact_critic_rerank_prompting",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("llm.compact_critic_rerank_prompting", log_file=paths.logs_dir / "train.log")
