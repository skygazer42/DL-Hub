...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 07 (LLM): toy reward modeling over chosen/rejected completions."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="llm",
        lesson="lesson_07_toy_reward_modeling",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("llm.toy_reward_modeling", log_file=paths.logs_dir / "train.log")
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "llm", "lesson": "lesson_07_toy_reward_modeling", "vocab_size": vocab.size},
    )
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.llm.lesson_07_toy_reward_modeling.train"
        )
