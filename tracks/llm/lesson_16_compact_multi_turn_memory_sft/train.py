...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 16 (LLM): compact multi-turn memory chat supervised fine-tuning."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="llm",
        lesson="lesson_16_compact_multi_turn_memory_sft",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("llm.compact_multi_turn_memory_sft", log_file=paths.logs_dir / "train.log")
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.llm.lesson_16_compact_multi_turn_memory_sft.train"
        )
