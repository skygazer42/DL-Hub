...
class ToyCitationGroundedPromptingDataset:
...
def get_dataloaders(cfg: DataConfig):
    ...
    if int(cfg.seq_length) < 16:
        raise ValueError("seq_length must be >= 16 for toy citation-grounded prompting")

    vocab = Vocab(base_vocab_size=int(cfg.base_vocab_size))
    input_ids, labels = _make_dataset_arrays(cfg, vocab)
    dataset = ToyCitationGroundedPromptingDataset(input_ids=input_ids, labels=labels, vocab=vocab)
...
__all__ = ["DataConfig", "ToyCitationGroundedPromptingDataset", "Vocab", "get_dataloaders"]
