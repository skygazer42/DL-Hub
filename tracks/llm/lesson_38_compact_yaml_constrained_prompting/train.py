...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(description="Lesson 38 (LLM): compact YAML constrained prompting.")
...
def _sample_prompt(vocab: Vocab) -> list[int]:
    ...
    first_key = int(vocab.content_start_id + 1)
    second_key = int(vocab.content_start_id + 2)
    return [
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="llm",
        lesson="lesson_38_compact_yaml_constrained_prompting",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("llm.compact_yaml_constrained_prompting", log_file=paths.logs_dir / "train.log")
    ...
    save_checkpoint(
        ...
        extra={"track": "llm", "lesson": "lesson_38_compact_yaml_constrained_prompting"},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.llm.lesson_38_compact_yaml_constrained_prompting.train"
        )
