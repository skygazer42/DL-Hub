...
class Vocab:
    ...
    key_token_id: int = 2
    value_token_id: int = 3
    colon_token_id: int = 4
    ...
    def to_dict(self) -> dict[str, int]:
        return {
            ...
            "colon_token_id": int(self.colon_token_id),
            "newline_token_id": int(self.newline_token_id),
            "yaml_token_id": int(self.yaml_token_id),
...
def _build_example(*, prompt_id: int, vocab: Vocab, seq_length: int) -> tuple[np.ndarray, np.ndarray]:
    first_key = _offset_content_token(prompt_id, offset=1, vocab=vocab)
    second_key = _offset_content_token(prompt_id, offset=2, vocab=vocab)
    first_value = _offset_content_token(prompt_id, offset=3, vocab=vocab)
    second_value = _offset_content_token(prompt_id, offset=4, vocab=vocab)
    ...
    tokens = [
        ...
        int(second_key),
        int(vocab.newline_token_id),
        int(vocab.yaml_token_id),
        int(first_key),
        int(vocab.colon_token_id),
        int(first_value),
        int(vocab.newline_token_id),
        int(second_key),
        int(vocab.colon_token_id),
    ...
    yaml_pos = tokens.index(int(vocab.yaml_token_id))
    targets = [
        int(vocab.yaml_token_id),
        int(first_key),
        int(vocab.colon_token_id),
...
class SyntheticYamlConstrainedPromptingDataset:
...
def get_dataloaders(cfg: DataConfig):
    ...
    if int(cfg.seq_length) < 20:
        raise ValueError("seq_length must be >= 20 for synthetic YAML constrained prompting template")

    vocab = Vocab(base_vocab_size=int(cfg.base_vocab_size))
    input_ids, labels = _make_dataset_arrays(cfg, vocab)
    dataset = SyntheticYamlConstrainedPromptingDataset(input_ids=input_ids, labels=labels, vocab=vocab)
...
__all__ = ["DataConfig", "SyntheticYamlConstrainedPromptingDataset", "Vocab", "get_dataloaders"]
