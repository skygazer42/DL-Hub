...
class ToyProcessSupervisionPromptingDataset:
...
def get_dataloaders(cfg: DataConfig):
    ...
    if int(cfg.seq_length) < 20:
        raise ValueError("seq_length must be >= 20 for toy process supervision prompting")

    vocab = Vocab(base_vocab_size=int(cfg.base_vocab_size))
    input_ids, labels = _make_dataset_arrays(cfg, vocab)
    dataset = ToyProcessSupervisionPromptingDataset(input_ids=input_ids, labels=labels, vocab=vocab)
...
__all__ = ["DataConfig", "ToyProcessSupervisionPromptingDataset", "Vocab", "get_dataloaders"]
