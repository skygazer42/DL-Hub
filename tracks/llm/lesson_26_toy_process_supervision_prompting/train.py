...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 26 (LLM): toy process supervision prompting with explicit checks."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="llm",
        lesson="lesson_26_toy_process_supervision_prompting",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("llm.toy_process_supervision_prompting", log_file=paths.logs_dir / "train.log")
    ...
    save_checkpoint(
        ...
        extra={"track": "llm", "lesson": "lesson_26_toy_process_supervision_prompting"},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.llm.lesson_26_toy_process_supervision_prompting.train"
        )
