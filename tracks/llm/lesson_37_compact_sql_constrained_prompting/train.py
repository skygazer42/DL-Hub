...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(description="Lesson 37 (LLM): compact SQL constrained prompting.")
...
def _sample_prompt(vocab: Vocab) -> list[int]:
    ...
    table_id = int(vocab.content_start_id + 1)
    column_id = int(vocab.content_start_id + 2)
    return [
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="llm",
        lesson="lesson_37_compact_sql_constrained_prompting",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("llm.compact_sql_constrained_prompting", log_file=paths.logs_dir / "train.log")
