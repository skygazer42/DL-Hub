...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 28 (LLM): toy reference-grounded prompting with explicit evidence tokens."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="llm",
        lesson="lesson_28_toy_reference_grounded_prompting",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("llm.toy_reference_grounded_prompting", log_file=paths.logs_dir / "train.log")
    ...
    save_checkpoint(
        ...
        extra={"track": "llm", "lesson": "lesson_28_toy_reference_grounded_prompting", "vocab_size": vocab.size},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.llm.lesson_28_toy_reference_grounded_prompting.train"
        )
