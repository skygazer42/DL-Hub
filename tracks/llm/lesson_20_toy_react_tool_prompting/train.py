...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 20 (LLM): toy ReAct-style think/act/observe/final prompting."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="llm",
        lesson="lesson_20_toy_react_tool_prompting",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("llm.toy_react_tool_prompting", log_file=paths.logs_dir / "train.log")
