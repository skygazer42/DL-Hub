...
class Vocab:
    ...
    def to_dict(self) -> dict[str, int]:
        return {
            ...
            "cite_token_id": int(self.cite_token_id),
...
def _build_example(*, prompt_id: int, vocab: Vocab, seq_length: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    citation_span = [reference_span[0], reference_span[2]]
    ...
    tokens = [
        ...
        *reference_span,
        int(vocab.separator_token_id),
        int(vocab.answer_token_id),
        *answer_stub,
        int(vocab.separator_token_id),
        int(vocab.cite_token_id),
        *citation_span,
        int(vocab.eos_id),
    ]
    ...
    cite_pos = tokens.index(int(vocab.cite_token_id))
    targets = citation_span + [int(vocab.eos_id)]
...
class SyntheticCitationGroundedPromptingDataset:
...
def get_dataloaders(cfg: DataConfig):
    ...
    if int(cfg.seq_length) < 16:
        raise ValueError("seq_length must be >= 16 for synthetic citation-grounded prompting")

    vocab = Vocab(base_vocab_size=int(cfg.base_vocab_size))
    input_ids, labels = _make_dataset_arrays(cfg, vocab)
    dataset = SyntheticCitationGroundedPromptingDataset(input_ids=input_ids, labels=labels, vocab=vocab)
...
__all__ = ["DataConfig", "SyntheticCitationGroundedPromptingDataset", "Vocab", "get_dataloaders"]
