...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 06 (LLM): compact pairwise preference optimization."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="llm",
        lesson="lesson_06_compact_preference_optimization",
        run_name=train_cfg.run_name,
    )
    logger = get_logger(
        "llm.compact_preference_optimization",
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.llm.lesson_06_compact_preference_optimization.train"
        )
