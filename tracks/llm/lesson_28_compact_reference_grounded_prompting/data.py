...
class Vocab:
    ...
    reference_token_id: int = 2
    query_token_id: int = 3
    ground_token_id: int = 4
    ...
    def to_dict(self) -> dict[str, int]:
        return {
            ...
            "reference_token_id": int(self.reference_token_id),
            "query_token_id": int(self.query_token_id),
            "ground_token_id": int(self.ground_token_id),
...
def _build_example(*, prompt_id: int, vocab: Vocab, seq_length: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    tokens = [
        ...
        int(vocab.reference_token_id),
        *reference_span,
        int(vocab.separator_token_id),
        int(vocab.query_token_id),
        *query_span,
        int(vocab.separator_token_id),
        int(vocab.ground_token_id),
        *grounded_answer,
        int(vocab.eos_id),
    ]
    ...
    ground_pos = tokens.index(int(vocab.ground_token_id))
    targets = grounded_answer + [int(vocab.eos_id)]
    for offset, token_id in enumerate(targets):
        pos = ground_pos + offset
...
class SyntheticReferenceGroundedPromptingDataset:
...
def get_dataloaders(cfg: DataConfig):
    ...
    if int(cfg.seq_length) < 16:
        raise ValueError("seq_length must be >= 16 for synthetic reference-grounded prompting")

    vocab = Vocab(base_vocab_size=int(cfg.base_vocab_size))
    input_ids, labels = _make_dataset_arrays(cfg, vocab)
    dataset = SyntheticReferenceGroundedPromptingDataset(input_ids=input_ids, labels=labels, vocab=vocab)
...
__all__ = ["DataConfig", "SyntheticReferenceGroundedPromptingDataset", "Vocab", "get_dataloaders"]
