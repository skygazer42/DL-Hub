...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 33 (LLM): compact function-signature prompting with signature-compatible call targets."
    )
...
def _sample_prompt(vocab: Vocab) -> list[int]:
    function_id = int(vocab.content_start_id)
    return [
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="llm",
        lesson="lesson_33_compact_function_signature_prompting",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("llm.compact_function_signature_prompting", log_file=paths.logs_dir / "train.log")
