...
class Vocab:
    ...
    function_token_id: int = 2
    arg_token_id: int = 3
    type_token_id: int = 4
    return_token_id: int = 5
    call_token_id: int = 6
    ...
    def to_dict(self) -> dict[str, int]:
        return {
            ...
            "function_token_id": int(self.function_token_id),
            "arg_token_id": int(self.arg_token_id),
            "type_token_id": int(self.type_token_id),
            "return_token_id": int(self.return_token_id),
...
def _build_example(*, prompt_id: int, vocab: Vocab, seq_length: int) -> tuple[np.ndarray, np.ndarray]:
    arg_a = _offset_content_token(prompt_id, offset=1, vocab=vocab)
    arg_b = _offset_content_token(prompt_id, offset=2, vocab=vocab)
    type_a = _offset_content_token(prompt_id, offset=3, vocab=vocab)
    type_b = _offset_content_token(prompt_id, offset=4, vocab=vocab)
    return_type = _offset_content_token(prompt_id, offset=5, vocab=vocab)
    value_a = _offset_content_token(prompt_id, offset=6, vocab=vocab)
    value_b = _offset_content_token(prompt_id, offset=7, vocab=vocab)

    tokens = [
        int(vocab.prompt_token_id),
        int(prompt_id),
        int(vocab.separator_token_id),
        int(vocab.function_token_id),
        int(prompt_id),
        int(vocab.separator_token_id),
        int(vocab.arg_token_id),
        int(arg_a),
        int(vocab.type_token_id),
        int(type_a),
        int(vocab.separator_token_id),
        int(vocab.arg_token_id),
        int(arg_b),
    ...
    call_pos = tokens.index(int(vocab.call_token_id))
    targets = [
        int(prompt_id),
        int(value_a),
        int(vocab.separator_token_id),
...
class SyntheticFunctionSignaturePromptingDataset:
...
def get_dataloaders(cfg: DataConfig):
    ...
    if int(cfg.seq_length) < 26:
        raise ValueError("seq_length must be >= 26 for synthetic function-signature prompting template")

    vocab = Vocab(base_vocab_size=int(cfg.base_vocab_size))
    input_ids, labels = _make_dataset_arrays(cfg, vocab)
    dataset = SyntheticFunctionSignaturePromptingDataset(input_ids=input_ids, labels=labels, vocab=vocab)
...
__all__ = ["DataConfig", "SyntheticFunctionSignaturePromptingDataset", "Vocab", "get_dataloaders"]
