...
class Vocab:
    ...
    pattern_token_id: int = 2
    literal_token_id: int = 3
    charset_token_id: int = 4
    quantifier_token_id: int = 5
    regex_token_id: int = 6
    ...
    def to_dict(self) -> dict[str, int]:
        return {
            ...
            "pattern_token_id": int(self.pattern_token_id),
            "literal_token_id": int(self.literal_token_id),
            "charset_token_id": int(self.charset_token_id),
            "quantifier_token_id": int(self.quantifier_token_id),
            "regex_token_id": int(self.regex_token_id),
...
def _build_example(*, prompt_id: int, vocab: Vocab, seq_length: int) -> tuple[np.ndarray, np.ndarray]:
    literal_a = _offset_content_token(prompt_id, offset=1, vocab=vocab)
    literal_b = _offset_content_token(prompt_id, offset=2, vocab=vocab)
    charset = _offset_content_token(prompt_id, offset=3, vocab=vocab)
    quantifier = _offset_content_token(prompt_id, offset=4, vocab=vocab)

    tokens = [
        int(vocab.prompt_token_id),
        int(prompt_id),
        int(vocab.separator_token_id),
        int(vocab.pattern_token_id),
        int(prompt_id),
        int(vocab.separator_token_id),
        int(vocab.literal_token_id),
        int(literal_a),
        int(vocab.separator_token_id),
        int(vocab.charset_token_id),
        int(charset),
        int(vocab.separator_token_id),
        int(vocab.quantifier_token_id),
        int(quantifier),
        int(vocab.separator_token_id),
        int(vocab.literal_token_id),
        int(literal_b),
        int(vocab.separator_token_id),
        int(vocab.regex_token_id),
        int(literal_a),
        int(charset),
        int(quantifier),
        int(literal_b),
    ...
    regex_pos = tokens.index(int(vocab.regex_token_id))
    targets = [
        int(vocab.regex_token_id),
        int(literal_a),
        int(charset),
...
class SyntheticRegexConstrainedPromptingDataset:
...
def get_dataloaders(cfg: DataConfig):
    ...
    if int(cfg.seq_length) < 25:
        raise ValueError("seq_length must be >= 25 for synthetic regex-constrained prompting template")

    vocab = Vocab(base_vocab_size=int(cfg.base_vocab_size))
    input_ids, labels = _make_dataset_arrays(cfg, vocab)
    dataset = SyntheticRegexConstrainedPromptingDataset(input_ids=input_ids, labels=labels, vocab=vocab)
...
__all__ = ["DataConfig", "SyntheticRegexConstrainedPromptingDataset", "Vocab", "get_dataloaders"]
