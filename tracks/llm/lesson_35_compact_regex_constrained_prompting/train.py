...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 35 (LLM): compact regex-constrained prompting with regex continuation targets."
    )
...
def _sample_prompt(vocab: Vocab) -> list[int]:
    pattern_id = int(vocab.content_start_id)
    return [
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="llm",
        lesson="lesson_35_compact_regex_constrained_prompting",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("llm.compact_regex_constrained_prompting", log_file=paths.logs_dir / "train.log")
