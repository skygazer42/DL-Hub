...
class SyntheticJsonConstrainedPromptingDataset:
...
def get_dataloaders(cfg: DataConfig):
    ...
    if int(cfg.seq_length) < 22:
        raise ValueError("seq_length must be >= 22 for synthetic JSON-constrained prompting template")

    vocab = Vocab(base_vocab_size=int(cfg.base_vocab_size))
    input_ids, labels = _make_dataset_arrays(cfg, vocab)
    dataset = SyntheticJsonConstrainedPromptingDataset(input_ids=input_ids, labels=labels, vocab=vocab)
...
__all__ = ["DataConfig", "SyntheticJsonConstrainedPromptingDataset", "Vocab", "get_dataloaders"]
