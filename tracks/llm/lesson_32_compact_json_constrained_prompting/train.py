...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 32 (LLM): compact JSON-constrained prompting with structured output targets."
    )
...
def _sample_prompt(vocab: Vocab) -> list[int]:
    ...
    field_a = int(vocab.content_start_id + 1)
    field_b = int(vocab.content_start_id + 2)
    return [
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="llm",
        lesson="lesson_32_compact_json_constrained_prompting",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("llm.compact_json_constrained_prompting", log_file=paths.logs_dir / "train.log")
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.llm.lesson_32_compact_json_constrained_prompting.train"
        )
