...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 11 (LLM): compact RAG language model with synthetic retrieval."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="llm",
        lesson="lesson_11_compact_rag_language_model",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("llm.compact_rag_language_model", log_file=paths.logs_dir / "train.log")
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.llm.lesson_11_compact_rag_language_model.train"
        )
