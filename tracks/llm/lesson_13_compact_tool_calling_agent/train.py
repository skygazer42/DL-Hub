...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 13 (LLM): compact tool-calling agent with synthetic calculator/lookup tasks."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="llm",
        lesson="lesson_13_compact_tool_calling_agent",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("llm.compact_tool_calling_agent", log_file=paths.logs_dir / "train.log")
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.llm.lesson_13_compact_tool_calling_agent.train"
        )
