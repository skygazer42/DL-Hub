...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 17 (LLM): compact self-refine prompting with draft and critique context."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="llm",
        lesson="lesson_17_compact_self_refine_prompting",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("llm.compact_self_refine_prompting", log_file=paths.logs_dir / "train.log")
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.llm.lesson_17_compact_self_refine_prompting.train"
        )
