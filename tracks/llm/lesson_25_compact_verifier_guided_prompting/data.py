...
class Vocab:
    ...
    candidate_token_id: int = 2
    verify_token_id: int = 3
    pass_token_id: int = 4
    fail_token_id: int = 5
    guide_token_id: int = 6
    ...
    def to_dict(self) -> dict[str, int]:
        return {
            ...
            "verify_token_id": int(self.verify_token_id),
            "pass_token_id": int(self.pass_token_id),
            "fail_token_id": int(self.fail_token_id),
            "guide_token_id": int(self.guide_token_id),
...
def _build_example(*, prompt_id: int, vocab: Vocab, seq_length: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    tokens = [
        ...
        *verified_candidate,
        int(vocab.verify_token_id),
        int(vocab.pass_token_id),
        int(vocab.separator_token_id),
        int(vocab.candidate_token_id),
        *rejected_candidate,
        int(vocab.verify_token_id),
        int(vocab.fail_token_id),
        int(vocab.separator_token_id),
        int(vocab.guide_token_id),
        *guided_answer,
        int(vocab.eos_id),
    ]
    ...
    guide_pos = tokens.index(int(vocab.guide_token_id))
    targets = guided_answer + [int(vocab.eos_id)]
    for offset, token_id in enumerate(targets):
        pos = guide_pos + offset
...
class SyntheticVerifierGuidedPromptingDataset:
...
def get_dataloaders(cfg: DataConfig):
    ...
    if int(cfg.seq_length) < 20:
        raise ValueError("seq_length must be >= 20 for synthetic verifier-guided prompting template")

    vocab = Vocab(base_vocab_size=int(cfg.base_vocab_size))
    input_ids, labels = _make_dataset_arrays(cfg, vocab)
    dataset = SyntheticVerifierGuidedPromptingDataset(input_ids=input_ids, labels=labels, vocab=vocab)
...
__all__ = ["DataConfig", "SyntheticVerifierGuidedPromptingDataset", "Vocab", "get_dataloaders"]
