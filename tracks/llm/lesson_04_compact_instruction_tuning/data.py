...
class SyntheticInstructionTuningDataset:
...
def get_dataloaders(cfg: DataConfig):
    ...
    if int(cfg.seq_length) < 12:
        raise ValueError("seq_length must be >= 12 for the synthetic instruction template")

    vocab = Vocab(base_vocab_size=int(cfg.base_vocab_size))
    input_ids, labels = _make_dataset_arrays(cfg, vocab)
    ds = SyntheticInstructionTuningDataset(input_ids=input_ids, labels=labels, vocab=vocab)
...
__all__ = ["DataConfig", "SyntheticInstructionTuningDataset", "Vocab", "get_dataloaders"]
