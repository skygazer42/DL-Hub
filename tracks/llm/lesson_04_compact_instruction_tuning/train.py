...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 04 (LLM): compact single-turn instruction tuning."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="llm",
        lesson="lesson_04_compact_instruction_tuning",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("llm.compact_instruction_tuning", log_file=paths.logs_dir / "train.log")
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.llm.lesson_04_compact_instruction_tuning.train"
        )
