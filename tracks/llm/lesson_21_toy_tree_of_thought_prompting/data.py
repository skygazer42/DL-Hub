...
class ToyTreeOfThoughtPromptingDataset:
...
def get_dataloaders(cfg: DataConfig):
    ...
    if int(cfg.seq_length) < 18:
        raise ValueError("seq_length must be >= 18 for toy tree-of-thought prompting")

    vocab = Vocab(base_vocab_size=int(cfg.base_vocab_size))
    input_ids, labels = _make_dataset_arrays(cfg, vocab)
    dataset = ToyTreeOfThoughtPromptingDataset(input_ids=input_ids, labels=labels, vocab=vocab)
...
__all__ = ["DataConfig", "ToyTreeOfThoughtPromptingDataset", "Vocab", "get_dataloaders"]
