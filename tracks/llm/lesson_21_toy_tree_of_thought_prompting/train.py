...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 21 (LLM): toy tree-of-thought prompting with branch selection."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="llm",
        lesson="lesson_21_toy_tree_of_thought_prompting",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("llm.toy_tree_of_thought_prompting", log_file=paths.logs_dir / "train.log")
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.llm.lesson_21_toy_tree_of_thought_prompting.train"
        )
