...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(description="Lesson 36 (LLM): compact EBNF constrained prompting.")
...
def _sample_prompt(vocab: Vocab) -> list[int]:
    ...
    rule_id = int(vocab.content_start_id + 1)
    value_id = int(vocab.content_start_id + 2)
    alt_id = int(vocab.content_start_id + 3)
    return [
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="llm",
        lesson="lesson_36_compact_ebnf_constrained_prompting",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("llm.compact_ebnf_constrained_prompting", log_file=paths.logs_dir / "train.log")
