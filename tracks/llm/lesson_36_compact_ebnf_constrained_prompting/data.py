...
class Vocab:
    ...
    grammar_token_id: int = 2
    rule_token_id: int = 3
    assign_token_id: int = 4
    choice_token_id: int = 5
    ebnf_token_id: int = 6
    ...
    def to_dict(self) -> dict[str, int]:
        return {
            ...
            "grammar_token_id": int(self.grammar_token_id),
            "rule_token_id": int(self.rule_token_id),
            "assign_token_id": int(self.assign_token_id),
            "choice_token_id": int(self.choice_token_id),
            "ebnf_token_id": int(self.ebnf_token_id),
...
def _build_example(*, prompt_id: int, vocab: Vocab, seq_length: int) -> tuple[np.ndarray, np.ndarray]:
    rule_id = _offset_content_token(prompt_id, offset=1, vocab=vocab)
    value_id = _offset_content_token(prompt_id, offset=2, vocab=vocab)
    alt_id = _offset_content_token(prompt_id, offset=3, vocab=vocab)

    tokens = [
        int(vocab.prompt_token_id),
        int(prompt_id),
        int(vocab.separator_token_id),
        int(vocab.grammar_token_id),
        int(prompt_id),
        int(vocab.separator_token_id),
        int(vocab.rule_token_id),
        int(rule_id),
        int(vocab.separator_token_id),
        int(vocab.assign_token_id),
        int(value_id),
        int(vocab.separator_token_id),
        int(vocab.choice_token_id),
        int(alt_id),
        int(vocab.separator_token_id),
        int(vocab.ebnf_token_id),
        int(rule_id),
        int(vocab.assign_token_id),
        int(value_id),
        int(vocab.separator_token_id),
    ...
    ebnf_pos = tokens.index(int(vocab.ebnf_token_id))
    targets = [
        int(vocab.ebnf_token_id),
        int(rule_id),
        int(vocab.assign_token_id),
...
class SyntheticEbnfConstrainedPromptingDataset:
...
def get_dataloaders(cfg: DataConfig):
    ...
    if int(cfg.seq_length) < 24:
        raise ValueError("seq_length must be >= 24 for synthetic EBNF constrained prompting template")

    vocab = Vocab(base_vocab_size=int(cfg.base_vocab_size))
    input_ids, labels = _make_dataset_arrays(cfg, vocab)
    dataset = SyntheticEbnfConstrainedPromptingDataset(input_ids=input_ids, labels=labels, vocab=vocab)
...
__all__ = ["DataConfig", "SyntheticEbnfConstrainedPromptingDataset", "Vocab", "get_dataloaders"]
