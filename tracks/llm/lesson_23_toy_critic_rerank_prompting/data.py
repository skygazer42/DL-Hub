...
class ToyCriticRerankPromptingDataset:
...
def get_dataloaders(cfg: DataConfig):
    ...
    if int(cfg.seq_length) < 18:
        raise ValueError("seq_length must be >= 18 for toy critic-rerank prompting template")

    vocab = Vocab(base_vocab_size=int(cfg.base_vocab_size))
    input_ids, labels = _make_dataset_arrays(cfg, vocab)
    dataset = ToyCriticRerankPromptingDataset(input_ids=input_ids, labels=labels, vocab=vocab)
...
__all__ = ["DataConfig", "ToyCriticRerankPromptingDataset", "Vocab", "get_dataloaders"]
