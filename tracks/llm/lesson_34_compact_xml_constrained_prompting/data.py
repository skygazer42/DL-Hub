...
class Vocab:
    ...
    def to_dict(self) -> dict[str, int]:
        return {
            ...
            "xml_token_id": int(self.xml_token_id),
...
def _build_example(*, prompt_id: int, vocab: Vocab, seq_length: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    xml_pos = tokens.index(int(vocab.xml_token_id))
    targets = [
        int(vocab.xml_token_id),
        int(prompt_id),
        int(value_a),
...
class SyntheticXmlConstrainedPromptingDataset:
...
def get_dataloaders(cfg: DataConfig):
    ...
    if int(cfg.seq_length) < 18:
        raise ValueError("seq_length must be >= 18 for synthetic XML constrained prompting template")

    vocab = Vocab(base_vocab_size=int(cfg.base_vocab_size))
    input_ids, labels = _make_dataset_arrays(cfg, vocab)
    dataset = SyntheticXmlConstrainedPromptingDataset(input_ids=input_ids, labels=labels, vocab=vocab)
...
__all__ = ["DataConfig", "SyntheticXmlConstrainedPromptingDataset", "Vocab", "get_dataloaders"]
