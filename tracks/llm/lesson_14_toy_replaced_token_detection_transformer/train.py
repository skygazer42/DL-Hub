...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 14 (LLM): toy replaced-token detection transformer pretraining."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="llm",
        lesson="lesson_14_toy_replaced_token_detection_transformer",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("llm.toy_replaced_token_detection", log_file=paths.logs_dir / "train.log")
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.llm.lesson_14_toy_replaced_token_detection_transformer.train"
        )
