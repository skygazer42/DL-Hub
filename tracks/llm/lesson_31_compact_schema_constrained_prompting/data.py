...
class Vocab:
    ...
    schema_token_id: int = 2
    field_token_id: int = 3
    type_token_id: int = 4
    value_token_id: int = 5
    json_token_id: int = 6
    ...
    def to_dict(self) -> dict[str, int]:
        return {
            ...
            "schema_token_id": int(self.schema_token_id),
...
def _build_example(*, prompt_id: int, vocab: Vocab, seq_length: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    value_a = _offset_content_token(prompt_id, offset=5, vocab=vocab)
    value_b = _offset_content_token(prompt_id, offset=6, vocab=vocab)
    ...
    schema_pos = tokens.index(int(vocab.schema_token_id))
    targets = [
        int(vocab.json_token_id),
        int(field_a),
        int(vocab.value_token_id),
...
class SyntheticSchemaConstrainedPromptingDataset:
...
def get_dataloaders(cfg: DataConfig):
    ...
    if int(cfg.seq_length) < 24:
        raise ValueError("seq_length must be >= 24 for synthetic schema-constrained prompting template")

    vocab = Vocab(base_vocab_size=int(cfg.base_vocab_size))
    input_ids, labels = _make_dataset_arrays(cfg, vocab)
    dataset = SyntheticSchemaConstrainedPromptingDataset(input_ids=input_ids, labels=labels, vocab=vocab)
...
__all__ = ["DataConfig", "SyntheticSchemaConstrainedPromptingDataset", "Vocab", "get_dataloaders"]
