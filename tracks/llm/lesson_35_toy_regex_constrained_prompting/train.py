...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 35 (LLM): toy regex-constrained prompting with regex continuation targets."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="llm",
        lesson="lesson_35_toy_regex_constrained_prompting",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("llm.toy_regex_constrained_prompting", log_file=paths.logs_dir / "train.log")
