...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 25 (LLM): toy verifier-guided prompting with candidate checks."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="llm",
        lesson="lesson_25_toy_verifier_guided_prompting",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("llm.toy_verifier_guided_prompting", log_file=paths.logs_dir / "train.log")
