...
class Vocab:
    ...
    branch_token_id: int = 2
    score_high_token_id: int = 3
    score_low_token_id: int = 4
    choose_token_id: int = 5
    ...
    def to_dict(self) -> dict[str, int]:
        return {
            ...
            "branch_token_id": int(self.branch_token_id),
            "score_high_token_id": int(self.score_high_token_id),
            "score_low_token_id": int(self.score_low_token_id),
            "choose_token_id": int(self.choose_token_id),
...
def _build_example(*, prompt_id: int, vocab: Vocab, seq_length: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    tokens = [
        ...
        int(vocab.branch_token_id),
        *branch_a_ids,
        int(vocab.score_high_token_id),
        int(vocab.separator_token_id),
        int(vocab.branch_token_id),
        *branch_b_ids,
        int(vocab.score_low_token_id),
        int(vocab.separator_token_id),
        int(vocab.choose_token_id),
        *answer_ids,
        int(vocab.eos_id),
    ]
...
class SyntheticTreeOfThoughtPromptingDataset:
...
def get_dataloaders(cfg: DataConfig):
    ...
    if int(cfg.seq_length) < 18:
        raise ValueError("seq_length must be >= 18 for synthetic tree-of-thought prompting")

    vocab = Vocab(base_vocab_size=int(cfg.base_vocab_size))
    input_ids, labels = _make_dataset_arrays(cfg, vocab)
    dataset = SyntheticTreeOfThoughtPromptingDataset(input_ids=input_ids, labels=labels, vocab=vocab)
...
__all__ = ["DataConfig", "SyntheticTreeOfThoughtPromptingDataset", "Vocab", "get_dataloaders"]
