...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 27 (LLM): compact self-correction prompting with draft, critique, and fix."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="llm",
        lesson="lesson_27_compact_self_correction_prompting",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("llm.compact_self_correction_prompting", log_file=paths.logs_dir / "train.log")
