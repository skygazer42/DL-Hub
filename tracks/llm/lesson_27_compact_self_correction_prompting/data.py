...
class Vocab:
    ...
    def to_dict(self) -> dict[str, int]:
        return {
            ...
            "correct_token_id": int(self.correct_token_id),
...
def _build_example(*, prompt_id: int, vocab: Vocab, seq_length: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    correction_pos = tokens.index(int(vocab.correct_token_id))
    targets = corrected_answer + [int(vocab.eos_id)]
    for offset, token_id in enumerate(targets):
        pos = correction_pos + offset
...
class SyntheticSelfCorrectionPromptingDataset:
...
def get_dataloaders(cfg: DataConfig):
    ...
    if int(cfg.seq_length) < 16:
        raise ValueError("seq_length must be >= 16 for synthetic self-correction prompting template")

    vocab = Vocab(base_vocab_size=int(cfg.base_vocab_size))
    input_ids, labels = _make_dataset_arrays(cfg, vocab)
    dataset = SyntheticSelfCorrectionPromptingDataset(input_ids=input_ids, labels=labels, vocab=vocab)
...
__all__ = ["DataConfig", "SyntheticSelfCorrectionPromptingDataset", "Vocab", "get_dataloaders"]
