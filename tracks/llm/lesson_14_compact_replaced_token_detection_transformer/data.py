...
class SyntheticReplacedTokenDetectionDataset:
    def __init__(
        self,
        *,
...
def get_dataloaders(cfg: DataConfig):
    ...
    if int(cfg.seq_length) < 12:
        raise ValueError("seq_length must be >= 12 for synthetic replaced-token detection sequences")

    vocab = Vocab(base_vocab_size=int(cfg.base_vocab_size))
    arrays = _make_dataset_arrays(cfg, vocab)
    ds = SyntheticReplacedTokenDetectionDataset(
        input_ids=arrays[0],
        labels=arrays[1],
...
__all__ = ["DataConfig", "SyntheticReplacedTokenDetectionDataset", "Vocab", "get_dataloaders"]
