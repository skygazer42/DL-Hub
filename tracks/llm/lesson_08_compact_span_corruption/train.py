...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 08 (LLM): compact span corruption with decoder-only denoising LM."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="llm",
        lesson="lesson_08_compact_span_corruption",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("llm.compact_span_corruption", log_file=paths.logs_dir / "train.log")
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={"track": "llm", "lesson": "lesson_08_compact_span_corruption", "vocab_size": vocab.size},
    )
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.llm.lesson_08_compact_span_corruption.train"
        )
