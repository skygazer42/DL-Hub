...
class Vocab:
    ...
    claim_token_id: int = 2
    pro_token_id: int = 3
    con_token_id: int = 4
    judge_token_id: int = 5
    ...
    def to_dict(self) -> dict[str, int]:
        return {
            ...
            "claim_token_id": int(self.claim_token_id),
            "pro_token_id": int(self.pro_token_id),
            "con_token_id": int(self.con_token_id),
            "judge_token_id": int(self.judge_token_id),
...
def _build_example(*, prompt_id: int, vocab: Vocab, seq_length: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    verdict = (
        [
            int(vocab.pro_token_id),
            prompt_id,
            _offset_content_token(prompt_id, offset=5, vocab=vocab),
        ]
        if ((prompt_id - vocab.content_start_id) % 2 == 0)
        else [
            int(vocab.con_token_id),
            _offset_content_token(prompt_id, offset=6, vocab=vocab),
            _offset_content_token(prompt_id, offset=7, vocab=vocab),
        ]
    )

    tokens = [
        int(vocab.prompt_token_id),
        int(vocab.claim_token_id),
        int(prompt_id),
    ...
    judge_pos = tokens.index(int(vocab.judge_token_id))
    targets = verdict + [int(vocab.eos_id)]
    for offset, token_id in enumerate(targets):
        pos = judge_pos + offset
...
class SyntheticDebatePromptingDataset:
...
def get_dataloaders(cfg: DataConfig):
    ...
    if int(cfg.seq_length) < 16:
        raise ValueError("seq_length must be >= 16 for synthetic debate prompting template")

    vocab = Vocab(base_vocab_size=int(cfg.base_vocab_size))
    input_ids, labels = _make_dataset_arrays(cfg, vocab)
    dataset = SyntheticDebatePromptingDataset(input_ids=input_ids, labels=labels, vocab=vocab)
...
__all__ = ["DataConfig", "SyntheticDebatePromptingDataset", "Vocab", "get_dataloaders"]
