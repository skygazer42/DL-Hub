...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 24 (LLM): compact debate prompting with supervised verdict generation."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="llm",
        lesson="lesson_24_compact_debate_prompting",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("llm.compact_debate_prompting", log_file=paths.logs_dir / "train.log")
