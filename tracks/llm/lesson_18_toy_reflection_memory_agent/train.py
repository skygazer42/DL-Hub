...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 18 (LLM): toy reflection-memory agent for retrieval-guided answer revision."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="llm",
        lesson="lesson_18_toy_reflection_memory_agent",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("llm.toy_reflection_memory_agent", log_file=paths.logs_dir / "train.log")
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.llm.lesson_18_toy_reflection_memory_agent.train"
        )
