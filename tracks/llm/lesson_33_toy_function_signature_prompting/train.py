...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 33 (LLM): toy function-signature prompting with signature-compatible call targets."
    )
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="llm",
        lesson="lesson_33_toy_function_signature_prompting",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("llm.toy_function_signature_prompting", log_file=paths.logs_dir / "train.log")
