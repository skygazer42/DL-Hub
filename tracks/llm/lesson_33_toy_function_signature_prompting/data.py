...
class ToyFunctionSignaturePromptingDataset:
...
def get_dataloaders(cfg: DataConfig):
    ...
    if int(cfg.seq_length) < 26:
        raise ValueError("seq_length must be >= 26 for toy function-signature prompting template")

    vocab = Vocab(base_vocab_size=int(cfg.base_vocab_size))
    input_ids, labels = _make_dataset_arrays(cfg, vocab)
    dataset = ToyFunctionSignaturePromptingDataset(input_ids=input_ids, labels=labels, vocab=vocab)
...
__all__ = ["DataConfig", "ToyFunctionSignaturePromptingDataset", "Vocab", "get_dataloaders"]
