...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(description="Lesson 42 (LLM): compact INI constrained prompting.")
...
def generate_response(
    *,
    model: IniConstrainedPromptingTransformerLM,
    device: torch.device,
    prompt_ids: list[int],
    max_new_tokens: int,
    pad_id: int,
...
def _sample_prompt(vocab: Vocab) -> list[int]:
    ...
    section_id = int(vocab.content_start_id + 1)
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(track="llm", lesson="lesson_42_compact_ini_constrained_prompting", run_name=train_cfg.run_name)
    logger = get_logger("llm.compact_ini_constrained_prompting", log_file=paths.logs_dir / "train.log")
    ...
    save_checkpoint(
        ...
        extra={"track": "llm", "lesson": "lesson_42_compact_ini_constrained_prompting"},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.llm.lesson_42_compact_ini_constrained_prompting.train"
        )
