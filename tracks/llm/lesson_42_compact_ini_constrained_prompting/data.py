...
class Vocab:
    ...
    section_token_id: int = 2
    key_token_id: int = 3
    value_token_id: int = 4
    newline_token_id: int = 5
    ini_token_id: int = 6
    equals_token_id: int = 7
    bracket_open_token_id: int = 8
    bracket_close_token_id: int = 9
    eos_id: int = 10
    ...
    def to_dict(self) -> dict[str, int]:
        return {
            ...
            "section_token_id": int(self.section_token_id),
            "key_token_id": int(self.key_token_id),
            "value_token_id": int(self.value_token_id),
            "newline_token_id": int(self.newline_token_id),
            "ini_token_id": int(self.ini_token_id),
            "equals_token_id": int(self.equals_token_id),
            "bracket_open_token_id": int(self.bracket_open_token_id),
            "bracket_close_token_id": int(self.bracket_close_token_id),
...
def _build_example(*, prompt_id: int, vocab: Vocab, seq_length: int) -> tuple[np.ndarray, np.ndarray]:
    section_id = _offset_content_token(prompt_id, offset=1, vocab=vocab)
    first_key = _offset_content_token(prompt_id, offset=2, vocab=vocab)
    second_key = _offset_content_token(prompt_id, offset=3, vocab=vocab)
    first_value = _offset_content_token(prompt_id, offset=4, vocab=vocab)
    second_value = _offset_content_token(prompt_id, offset=5, vocab=vocab)

    tokens = [
        int(vocab.prompt_token_id),
        int(prompt_id),
        int(vocab.section_token_id),
        int(section_id),
        int(vocab.key_token_id),
        int(first_key),
        int(vocab.value_token_id),
        int(first_value),
        int(vocab.newline_token_id),
        int(vocab.ini_token_id),
        int(vocab.bracket_open_token_id),
        int(section_id),
        int(vocab.bracket_close_token_id),
        int(vocab.newline_token_id),
        int(first_key),
        int(vocab.equals_token_id),
        int(first_value),
        int(vocab.newline_token_id),
        int(second_key),
        int(vocab.equals_token_id),
    ...
    ini_pos = tokens.index(int(vocab.ini_token_id))
    targets = [
        int(vocab.ini_token_id),
        int(vocab.bracket_open_token_id),
        int(section_id),
...
class SyntheticIniConstrainedPromptingDataset:
...
def get_dataloaders(cfg: DataConfig):
    ...
    if int(cfg.seq_length) < 22:
        raise ValueError("seq_length must be >= 22 for synthetic INI constrained prompting")

    vocab = Vocab(base_vocab_size=int(cfg.base_vocab_size))
    input_ids, labels = _make_dataset_arrays(cfg, vocab)
    dataset = SyntheticIniConstrainedPromptingDataset(input_ids=input_ids, labels=labels, vocab=vocab)
...
__all__ = ["DataConfig", "SyntheticIniConstrainedPromptingDataset", "Vocab", "get_dataloaders"]
