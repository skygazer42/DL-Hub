...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(description="Lesson 40 (LLM): compact TOML constrained prompting.")
    parser.add_argument("--num-samples", type=int, default=4096)
    parser.add_argument("--batch-size", type=int, default=64)
    parser.add_argument("--seq-length", type=int, default=42)
    ...
    parser.add_argument("--generation-tokens", type=int, default=10)
...
def generate_response(
    *,
    model: TomlConstrainedPromptingTransformerLM,
    device: torch.device,
    prompt_ids: list[int],
    max_new_tokens: int,
    pad_id: int,
...
def _sample_prompt(vocab: Vocab) -> list[int]:
    prompt_id = int(vocab.content_start_id)
    topic_id = int(vocab.content_start_id + 1)
    first_key = int(vocab.content_start_id + 2)
    second_key = int(vocab.content_start_id + 3)
    first_value = int(vocab.content_start_id + 4)
    return [
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="llm",
        lesson="lesson_40_compact_toml_constrained_prompting",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("llm.compact_toml_constrained_prompting", log_file=paths.logs_dir / "train.log")
    ...
    for epoch in range(1, int(train_cfg.epochs) + 1):
        ...
        _write_samples(
            ...
            epoch=epoch,
            generation_tokens=train_cfg.generation_tokens,
        )
    ...
    save_checkpoint(
        ...
        extra={"track": "llm", "lesson": "lesson_40_compact_toml_constrained_prompting"},
    )
    return 0
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.llm.lesson_40_compact_toml_constrained_prompting.train"
        )
