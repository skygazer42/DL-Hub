...
def _build_example(*, prompt_id: int, vocab: Vocab, seq_length: int) -> tuple[np.ndarray, np.ndarray]:
    topic_id = _offset_content_token(prompt_id, offset=1, vocab=vocab)
    ...
    toml_pos = tokens.index(int(vocab.toml_token_id))
    targets = [
        int(vocab.toml_token_id),
        int(first_key),
        int(vocab.equals_token_id),
...
class SyntheticTomlConstrainedPromptingDataset:
...
def get_dataloaders(cfg: DataConfig):
    ...
    if int(cfg.seq_length) < 21:
        raise ValueError("seq_length must be >= 21 for synthetic TOML constrained prompting")

    vocab = Vocab(base_vocab_size=int(cfg.base_vocab_size))
    input_ids, labels = _make_dataset_arrays(cfg, vocab)
    dataset = SyntheticTomlConstrainedPromptingDataset(input_ids=input_ids, labels=labels, vocab=vocab)
...
__all__ = ["DataConfig", "SyntheticTomlConstrainedPromptingDataset", "Vocab", "get_dataloaders"]
