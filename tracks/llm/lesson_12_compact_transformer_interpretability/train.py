...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 12 (LLM): compact transformer interpretability with attention and saliency."
    )
...
def compute_attention_map(
    model: CompactInterpretabilityTransformerLM, inputs: dict[str, torch.Tensor]
) -> torch.Tensor:
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="llm",
        lesson="lesson_12_compact_transformer_interpretability",
        run_name=train_cfg.run_name,
    )
    logger = get_logger("llm.compact_transformer_interpretability", log_file=paths.logs_dir / "train.log")
...
def main() -> int:
    if __package__ is None:
        raise RuntimeError(
            ...
            "  python -m tracks.llm.lesson_12_compact_transformer_interpretability.train"
        )
