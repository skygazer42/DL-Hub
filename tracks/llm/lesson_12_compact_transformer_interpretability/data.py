...
class Vocab:
    ...
    def to_dict(self) -> dict[str, int]:
        return {
            ...
            "focus_token_id": int(self.focus_token_id),
...
def _build_sample(topic_id: int, vocab: Vocab, seq_length: int) -> tuple[np.ndarray, np.ndarray]:
    target = _next_content_token(topic_id, vocab)
    prompt = [
        int(vocab.prompt_token_id),
        int(topic_id),
        int(vocab.focus_token_id),
        int(topic_id),
        int(vocab.query_token_id),
        int(topic_id),
        int(vocab.separator_token_id),
    ]
    response = [int(target), int(vocab.eos_id)]
    ...
    # Predict the response conditioned on the prompt + query.
    targets = [int(target), int(vocab.eos_id)]
    for offset, token in enumerate(targets):
        pos = len(prompt) - 1 + offset
        if pos >= seq_length:
            break
...
def _make_dataset_arrays(cfg: DataConfig, vocab: Vocab) -> tuple[np.ndarray, np.ndarray]:
    ...
    for i in range(n):
        ...
        in_i, lbl_i = _build_sample(topic_id=topic_id, vocab=vocab, seq_length=t)
        inputs[i] = in_i
        labels[i] = lbl_i
    return inputs, labels


class SyntheticInterpretabilityDataset:
...
def get_dataloaders(cfg: DataConfig):
    ...
    if int(cfg.seq_length) < 10:
        raise ValueError("seq_length must be >= 10 for synthetic transformer interpretability sequences")

    vocab = Vocab(base_vocab_size=int(cfg.base_vocab_size))
    input_ids, labels = _make_dataset_arrays(cfg, vocab)
    ds = SyntheticInterpretabilityDataset(input_ids=input_ids, labels=labels, vocab=vocab)
...
__all__ = ["DataConfig", "SyntheticInterpretabilityDataset", "Vocab", "get_dataloaders"]
