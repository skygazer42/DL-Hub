...
def parse_args() -> tuple[TrainConfig, DataConfig]:
    parser = argparse.ArgumentParser(
        description="Lesson 03 (LLM): compact language model with a simplified selective state-space block."
    )
...
def _write_samples(*, model: CompactMambaLM, device: torch.device, vocab, out_path, epoch: int) -> None:
    prompt = [1, 2, 3, 4, 5]
...
def run_training(train_cfg: TrainConfig, data_cfg: DataConfig) -> int:
    ...
    paths = build_run_paths(
        track="llm", lesson="lesson_03_compact_mamba_language_model", run_name=train_cfg.run_name
    )
    logger = get_logger("llm.compact_mamba_lm", log_file=paths.logs_dir / "train.log")
    ...
    ckpt_path = save_checkpoint(
        ...
        extra={
            "track": "llm",
            "lesson": "lesson_03_compact_mamba_language_model",
            "vocab_size": vocab.size,
        },
    )
